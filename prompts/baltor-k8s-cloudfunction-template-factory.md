...
gpu_worker,sandbox_worker}/`. Code `src/baltor/templates/{template_renderer,template_registry,template_validator,
...
## 2. WorkerAppContract
`schemas/workers/{WorkerAppSpec,WorkerLifecycleSpec,WorkerRuntimeConfig,WorkerTelemetrySpec,WorkerLoggingSpec,
...
`src/baltor/workers/base_worker_app.py`: BaseWorkerApp + LifecycleRunner + mixins (Preflight · Bootstrap ·
...
`src/baltor/observability/{json_logger,telemetry_db,otel_exporter_candidate}.py`. JSON stdout fields: timestamp ·
...
`schemas/resources/{DataResourceSpec,DatasetSpec,TableSpec,TemporaryResourceSpec,PersistentResourceSpec,
...
`src/baltor/ports/resource_manager.py` + `src/baltor/resources/{resource_manager,local_sqlite_resource_manager,
...
`src/baltor/workers/{preflight,bootstrap,module_loader,shutdown,recovery}.py`. Preflight: config validates ·
...
`check_generate_worker_from_template`. `src/baltor/workers/generated_worker_gate.py` promotion states:
...
Docs `docs/workers/{worker-template-factory,base-worker-app-contract,k8s-worker-templates,cloud-function-templates,
