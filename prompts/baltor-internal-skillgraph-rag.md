...
> provenance / dedup** knowledge layer. `prompts/baltor-path-improvement-and-skill-intake-factory.md` keeps the
...
skill_source_allowlist,skill_dependency_policy}.json` + `src/baltor/skills/{skill_registry,skill_graph,
...
`src/baltor/skills/{skill_rag,skill_indexer,skill_recall}.py`. Index: manifest · SKILL.md · README · examples ·
...
## 5. Similarity + dedup (`src/baltor/skills/{skill_similarity,skill_dedup}.py`)
...
## 6. Intake pipeline (`src/baltor/skills/{skill_intake,skill_manifest_parser,skill_sandbox,skill_evaluator,skill_promotion}.py`)
...
## 8. Why-ingested ledger (`src/baltor/skills/skill_intake_reasons.py`)
