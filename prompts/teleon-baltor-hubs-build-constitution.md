...
2. **The canonical references this prompt compresses** (read when a section is load-bearing for your task):
   `docs/strategy/teleon-baltor-openharnesshub-portfolio.md` (portfolio architecture),
...
- Real homes already in-repo: `src/teleon/synthesis/` (intent→DAG, synthesis tree with backtracking),
  `src/teleon/economics/` (live cost model, routing, simulator), `src/teleon/inference/` (model lanes/ports),
  `src/teleon/components/` (uniform `Component.invoke`, `lower_to_dag`, `verify_buildable_dag`),
...
- This is the deliberately **under-built moat** — the Verification universe is where the durable advantage is. Bias
  new effort here. Real homes: `src/baltor/` (being incrementally extracted to `src/teleon/` for generic runtime
...
- Real homes: `src/teleon/observer/{router,capture,review,session_store}.py` (one router over a typed intervention
...
```
Baltor  →  Teleon  →  OpenHarnessHub        (imports flow this way only)
```

Baltor imports Teleon; Teleon imports OpenHarnessHub; **never the reverse.** Teleon must never import Baltor;
OpenHarnessHub imports neither. PurposeTask is **Teleon**, not a Baltor subsystem. These two laws are orthogonal and
...
interface Registry<T extends RegistryObject> {
  // BUILT today on src/teleon/registry/port.py (RegistryPort over the 7 catalogs):
  list()                       // enumerate
...
ladders), `src/teleon/economics/` (the cost/routing brain), and the synthesis tree's backtracking. "Make it work →
...
pruning carry the rest. Grounded in `src/teleon/registry/search.py` (federated search → the BUILD / TROUBLESHOOT /
IMPROVE builder flows) and `src/teleon/registry/compose.py` (the compiler: intent or vertical playbook → candidate
...
resumable cursor, **honest** rate-limit stop), `src/teleon/registry/populate.py` (signal → record → enrich →
...
accept/reject outcomes in `session_store.py`. The intervention engine is `src/teleon/observer/router.py`. Scale the
...
Grounded in `src/teleon/knowledge/` (dependency graph, product-distance, repo-similarity, **code-genome** AST
