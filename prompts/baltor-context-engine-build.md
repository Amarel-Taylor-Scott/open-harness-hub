...
1. **Warrant before change** (`docs/codex/change-verification-contract.md`): every change
...
2. **No magic values** (`docs/codex/no-magic-values.md`): never type a count/threshold/ID/
...
3. **Verify tools — do NOT trust lists.** Any tool/repo/library name (including ones in
   ...
   `data/backend-tools.yaml` (if present) and `research/backend-tool-verification.md` — use
...
5. **Real or labeled SEAM.** If a step needs network/credentials you don't have, implement
   ...
   (mirror `scripts/ingest/sanctions_feed_live.py`). Never fake a result; label the seam.
6. **No secrets, no real PII.** `.env.example` only. Public/synthetic data only.
7. **Every phase ends green:** run the phase's self-test + `python3 scripts/validate.py`
...
reinvent:
- `scripts/pipeline/verified_context_flow.py` — the e2e ingest→assure→serve flow (run its
  self-test: `python3 scripts/pipeline/verified_context_flow.py`).
- `scripts/ingest/sanctions_feed.py` + `scripts/ingest/sanctions_feed_live.py` — the live
...
- `scripts/sanctions/sanctions_freshness.py` — the freshness/contradiction scorer.
- `scripts/foundry/scrapers.py` — `HttpFetcher` / `CannedFetcher` / `detect_change` /
  `content_hash` (the scraping + CDC primitives — reuse these for any new source).
- `scripts/processors/assurance/{corpus_integrity_check,multi_source_corroborate,oracle_c2pa_attest}.py`
  — the verification + receipt processors.
- `scripts/enrichment/serve.py`, `tier_pipeline.py` — tiering + MCP serving.
- `schemas/` (JSON Schemas + `scripts/validate.py`), `data/source-registry.jsonl`,
  `web/baltor/` (the visual prototype), `.claude/skills/` (where skills live — NOT a
  top-level `skills/`).
- `docs/backend/architecture-overview.md` (storage model + verified stack), `docs/backend/
  document-decomposition.md` (the raw-document → recursive-object-tree contract), the verified
  tool catalog (`research/backend-tool-verification.md` + `data/backend-tools.yaml`), and free
...
- **Rename product/UI language** in `web/baltor/` (`oracle-hero.html`, `stages/*.html`):
...
- **Do NOT touch** the *technical* term "oracle source / oracle publisher" (a trusted
  external data feed) used in `docs/strategy/` and `scripts/processors/assurance/` — that is
  a different, legitimate meaning. If unsure on a specific occurrence, leave it and list it.
- Add `docs/product-language.md`: deprecated ("Baltor Oracle", product "Oracle") vs
...
Create `web/baltor/stages.json` as the single source for stage copy, and have the hero +
...
under `schemas/context/`, `schemas/pipeline/`, `schemas/queue/`, and wire them into
`scripts/validate.py`. Start with the **6 load-bearing ones**, each with one example fixture
under `fixtures/`:
...
`docs/backend/document-decomposition.md`; claims attach to LEAF nodes, expansion pulls one node).
Then `pipeline-object` + `queue-message` if time remains. Write `docs/standards/` with:
...
UN/OFAC/BIS lists. For each: a connector under `scripts/ingest/` with an offline
...
treasury.gov 302-redirects (urllib follows). Add a `scripts/demo_context_engine_proof.py`
...
This is the highest-value new capability. Build `scripts/ingest/context_rot.py`:
...
Add a `schemas`/fixtures validation step (extend `scripts/validate.py` or add a CI job),
run all new self-tests, run `python3 scripts/validate.py` on changed paths. Then report:
