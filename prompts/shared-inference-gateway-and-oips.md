...
> the core — `src/teleon/inference/oips.py` (preference inheritance · numeric provider selection · governed
> fallback · ModelInvocationReceipt · local deterministic stub); contracts
> `schemas/inference/{InferencePreference,ResolvedInferencePreference,ModelInvocationReceipt}.v1` (registered);
> numeric graph `architecture/model_{quality_tier,specialization,provider_status,provider_edge_type}_codes.json`
> + `model_provider_graph.json`; proof `scripts/check_inference_gateway.py`; docs
> `docs/inference/open-inference-preference-spec.md`. Placed in **Teleon** (model routing = runtime), consumed by
> Baltor + hubs via Baltor→Teleon; sits ABOVE the existing `scripts/llm_gateway` (reused, not reinvented).
...
Build a shared LLM/model routing module used by Teleon, Baltor, OpenSkillsHub, OpenToolsHub, OpenContextHub, and
OpenHarnessHub. **Preference requests intent. Router decides execution. Receipt records reality.**
