...
durable runner (`scripts/run_north_star_loop.sh`) will relaunch you and you resume from that state — never
from chat memory.
...
The final offline demo must be runnable via `PYTHONPATH=. python3 scripts/demo_offline_full_baltor.py
...
1. Read: `.agent/north-star-loop-state.json`, `.agent/baltor-goal-loop-log.md`,
   `architecture/section_maturity_matrix.json`, `architecture/{worker_bucket_registry,worker_registry,
   contract_registry,runtime_ownership,external_capability_catalog}.json`,
   `docs/status/{current-state,opportunities,risks-and-gaps}.md`.
...
3. **Baseline:** `PYTHONPATH=. python3 scripts/baltor_flywheel.py --once`.
...
Build `scripts/demo_offline_full_baltor.py` (offline, deterministic) proving the full motion: intake →
...
`scripts/check_offline_full_demo.py` and register it.
...
The real `scripts/baltor_flywheel.py --watch` loop must register a supervisor instance, heartbeat,
