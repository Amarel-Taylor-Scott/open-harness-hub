...
> (`CLAUDE.md`, `AGENTS.md`, `docs/NORTHSTAR.md`, `docs/BIBLE.md`, `DESIGN-BIBLE.md`), so keep its file references
...
en dashes). For the frontend-to-backend seam a page's `fetch` rides on, see `INTEGRATION-BIBLE.md` (do not
...
`text/babel` modules (the kit, then the brand main). Quoted from `web/teleon/index.html` (the shipped file is
...
`[[heading, [[label, href], ...]], ...]`. Modeled on `Landing` in `web/teleon/teleon-main.jsx`.
...
hash route, picks the page, and wraps it in the shell. Modeled on `App` and `APP_NAV` in `web/teleon/teleon-main.jsx`:
...
The kit ships its own router in `web/teleon/kit/oh-site.jsx`. There is no React Router. The contract is two functions
...
the chosen page (see `App` in `web/teleon/teleon-main.jsx`).
...
`OH_SEAM_*_BASE` env in the cloud. The full seam table and the FE-to-BE recipe live in `INTEGRATION-BIBLE.md`;
...
1. Copy an existing app folder: `cp -r web/teleon web/<newbrand>` (keep `index.html`, the `kit/` folder, the brand
...
3. Wrap it in the marketing chrome so it keeps the top nav and footer. The pattern is a small `MarketingShell`
   (`OhTopBar` + `children` + `OhFooter`); see `MarketingShell` in `web/teleon/teleon-main.jsx`.
...
1. Add the function to `web/teleon/kit/oh-site.jsx`, building only from `var(--token)` values and the `.oh-*` atoms
...
`INTEGRATION-BIBLE.md` (build the service, register its port, add one seam line, then `fetch('/api/<x>/...')`).
...
| Add a bespoke, token-built piece in `web/aidevobserver/aidevobserver.css` (a new card, a chart). | The copy rules in section 11 (no placeholders, no em or en dashes, no strategy leakage). |
