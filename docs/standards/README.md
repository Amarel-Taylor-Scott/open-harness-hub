...
|---|---|---|
| Standards | `architecture/standard_catalog.json` | Normative description of each canonical pattern (must-have / must-not-have / naming / location / contract / proof / doc / registry / security / observability rules) with REAL exemplar paths. |
| Templates | `architecture/template_catalog.json` | The catalog of generatable scaffolds; each maps to a standard. 7 are `active` (built under `templates/`); the rest are `candidate` (planned). |
| Generator | `scripts/generate_from_template.py` | Renders a template's `files/` tree (with `{{variable}}` substitution) into a new component + a content-addressed generation receipt + the next required commands. |

## Standard `status_enum`

`draft` -> `active` -> `enforced` -> `deprecated`. `enforced` standards (e.g. `standard.proof_script.v1`,
`standard.tenant_isolation.v1`) are non-negotiable for any new component; `active` ones are expected;
...
python3 scripts/generate_from_template.py --template proof.self_test.v1 \
  ...
  --owner scripts/runtime/consumption.py --subject_module scripts.runtime.consumption

# generate a new projection API route (a STUB whose proof FAILS until you wire serve())
python3 scripts/generate_from_template.py --template api.projection_route.v1 \
  --area corpus --route_path /api/corpus/serve --returns_contract CorpusResponse.v1 \
...
- **Implementation files** (`.py`, `.html`) start as **stubs with a failing TODO** — the generated proof
  ...
  as done.
- **Proof files** from `proof.self_test.v1` are a complete, **passing** deterministic scaffold (temp dir,
...
|---|---|---|
| `ingestion.source_adapter.v1` | `standard.source_adapter.v1` | a governed `SourceAdapter` + proof + docs |
| `worker.command_handler.v1` | `standard.durable_command.v1` | an idempotent durable command handler + proof + docs |
| `api.projection_route.v1` | `standard.api_projection.v1` | a projection-only HTTP handler + proof + docs |
| `ui.projection_page.v1` | `standard.ui_projection.v1` | a projection-only web page + proof + docs |
| `provider.adapter.v1` | `standard.provider_adapter.v1` | a provider adapter behind a port + proof + docs |
| `proof.self_test.v1` | `standard.proof_script.v1` | the canonical deterministic proof scaffold |
| `docs.section_page.v1` | `standard.docs_page.v1` | a section doc page with all required headings |
...
```bash
python3 scripts/check_standard_catalog.py --self-test     # standards well-formed + grounded in real paths
python3 scripts/check_template_catalog.py --self-test      # templates buildable; active paths exist
python3 scripts/check_template_generation.py --self-test   # the generator actually renders conformant scaffolds
```
...
- `docs/standards/template-authoring.md` — how to author a new template.
- `architecture/standard_catalog.json` / `architecture/template_catalog.json` — the catalogs themselves.
