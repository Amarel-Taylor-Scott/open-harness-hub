...
every config of a given type must satisfy, and `templates/configs/` ships starting templates that
...
`templates/configs/<config_type>/template.json` is the fill-in starting point; several types also
...
under `templates/configs/` parses and is **secret-clean**; and — as a negative test in a temp dir —
