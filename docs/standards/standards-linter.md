...
|---|---|---|
| `source_adapter_shape` | `src/baltor/adapters/source/*.py` | declares `source_type` + `parser_provider` + `def ingest(` |
| `proof_self_test` | `scripts/check_*.py` | supports `--self-test` |
| `doc_has_heading` | `docs/standards/*.md` | carries a top-level `# ` heading |
| `projection_safe` | `scripts/runtime/projections.py`, `src/baltor/api/projections/*.py` | no truth mutation, no embedded secrets |
| `ui_projection_only` | `web/baltor/*.html` | no durable writes (projection-only) |
| `config_no_raw_secret` | `templates/configs/**.json` | secret references only (`env://…` / `secret://…`) |
| `no_new_vague_filename` | `scripts/`, `src/baltor/` | no new `utils.py`/`helpers.py`/`common.py`/… outside the allowlist |
