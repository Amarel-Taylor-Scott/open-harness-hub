...
| **Login / registration / sessions** | one shared auth KIT, separate realms, no SSO (`docs/architecture/auth-identity-kit.md`) · S3 API conventions (`/api/identity/<realm>/*`, `X-AIDR-Request-Id`) | `src/openharnesshub/auth_kit` + `scripts/identity_local_service.py` (register→onboard→login→session, all 12 realms) · client `web/harness-hub/identity.js` | `check_auth_kit_realm_isolation` · `check_identity_local_service_runtime` · `check_harness_hub_auth_wiring` | **STANDARDIZED** |
...
| **Analytics / events / A-B** | `EVENTS.md` — one event shape (`site·event·name·experiment·variant·anon·props`) · `POST /api/events` · `/summary` | `scripts/events_local_service.py` + beacon `web/harness-hub/events.js` (PII-guarded, anon-only) | `check_local_events_plane` · `check_events_beacon_wiring` | **STANDARDIZED** (PostHog sink is an env-gated seam) |
