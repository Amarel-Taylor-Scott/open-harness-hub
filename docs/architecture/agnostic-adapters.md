...
|---|---|---|---|
| **LLM** | `src/teleon/llm_port.py` — `LLMPort`, `select_llm`, `as_callable` | model index (`architecture/model_index.json`) + lanes (`scripts/_llm_client.PROVIDERS`) | `register_llm_adapter(name, factory)`; a new model_id/lane is auto-selectable |
| **Browser** | `src/teleon/research/browser_port.py` — `BrowserPort`, `select_browser` | browsing registry (`architecture/web_browsing_stack_registry.json`) | `register_browser_adapter(name, factory, engine=?)`; a chromium browser reuses Playwright automatically |
...
- A `model_id` selects its **provider lane**; a registry browser maps to its **engine** adapter; a declared-but-unwired
  ...
  (`src/teleon/inference`) — the port never re-implements it. `serves_truth=false` throughout.
...
## Enforcement
`scripts/check_agnostic_adapters.py` (registered in the proof suite) proves: ports populate from the registries; a
