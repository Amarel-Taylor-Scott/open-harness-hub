...
The shared schema definitions live in `schemas/_common.schema.json`; the
operational fact shape lives in `schemas/fact-record.schema.json`. Other objects
...
See `baltor-queue-priority-orchestration.md` for the shared lanes, priority
signals, and default task-routing policy.
