...
Code: [`scripts/foundry/batch_inference.py`](../../scripts/foundry/batch_inference.py) (the lane
manager) + the wiring in [`scripts/foundry/worker.py`](../../scripts/foundry/worker.py)
...
[`scripts/foundry/model_route.py`](../../scripts/foundry/model_route.py)'s `from_env()` and
`scripts/model_routes.py`'s `ChatRoute` already use — and points the existing route at
