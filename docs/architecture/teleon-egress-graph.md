...
Route policies live in `architecture/egress_route_policy_taxonomy.json`.
Execution-surface defaults point at those policies from
`architecture/execution_environment_profiles.json`.
...
- `src/teleon/egress/ledger.py` stores append-only intent, decision, attempt,
  and payload-reference records.
- `src/teleon/egress/traffic_graph.py` projects ledgered observations into the
  searchable graph:
...
```bash
python3 scripts/check_teleon_egress_graph.py --self-test
