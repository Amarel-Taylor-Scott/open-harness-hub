...
- Google documents Gemma as a family with small deployable sizes and specialized
  variants, including Gemma 3/Gemma 3n, MedGemma, EmbeddingGemma, and
  FunctionGemma.
...
- `scripts/model_routes.py` for chat/model calls.
- `scripts/model_gateway.py` for policy-aware model route selection.
- `scripts/embeddings.py` for embeddings.
...
The route decision conforms to `schemas/model-route-record.schema.json` and
