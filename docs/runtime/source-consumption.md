...
`scripts/runtime/source_consumption.py` is the **domain-agnostic** orchestrator that takes any source
...
`architecture/multi_source_run_matrix.json` is the per-source-TYPE run matrix: each row declares a
`source_type`, a `payload`, a `scope`, and the `expected_consumption_status`. See
[`docs/status/multi-source-regression.md`](../status/multi-source-regression.md) for the live table.
