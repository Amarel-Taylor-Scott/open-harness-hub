...
**File:** `scripts/teleon_local_runtime.py` (the local Teleon capability runtime).
...
```
python3 scripts/teleon_local_runtime.py --self-test
```
...
```
for i in 1 2 3 4 5; do python3 scripts/teleon_local_runtime.py --self-test; done
```
...
```
python3 -m py_compile scripts/teleon_local_runtime.py   # PY_COMPILE_OK
```
...
`capabilities.json` mtimes unchanged; `git status` shows only `scripts/teleon_local_runtime.py`.
