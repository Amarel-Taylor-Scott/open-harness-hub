...
`scripts/artifact_graph/graph_builder.py` — `build_edges(...)`, `build_pack_edges(...)`. Every `edge_id` is a
...
(`architecture/runtime_ownership.json#GraphBuilder`).

## Proof scripts

`scripts/check_cfpb_deterministic_graph.py` (registered in the flywheel) — builds the CFPB graph twice and
asserts identical, rule-derived edges (no model in the loop, no RNG).

## Commands

```bash
PYTHONPATH=. python3 scripts/check_cfpb_deterministic_graph.py --self-test
```
