...
`scripts/context_events.py` — `EventBus` (`publish`, `subscribe`, `recent`, `clear`, `restore`,
`by_correlation`) and the `emit(...)` helper. Registered runtime owner `EventBus`
(`architecture/runtime_ownership.json#EventBus`).
...
`scripts/context_events.py` (own `--self-test`), `scripts/check_event_integration.py`,
`scripts/check_event_envelope.py` — all registered in the flywheel.

## Commands

```bash
PYTHONPATH=. python3 scripts/context_events.py --self-test
PYTHONPATH=. python3 scripts/check_event_integration.py --self-test
PYTHONPATH=. python3 scripts/check_event_envelope.py --self-test
```
