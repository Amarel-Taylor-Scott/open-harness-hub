...
`scripts/artifact_graph/reconciliation.py` — `Reconciler.reconcile(...)`. Every `reconciliation_id` is a
...
(`architecture/runtime_ownership.json#Reconciler`).

## Proof scripts

`scripts/check_cfpb_reconciliation.py` (registered in the flywheel) — asserts the Reg E winner, the FAQ-30
hold-out, and a stable reconciliation receipt across reruns.

## Commands

```bash
PYTHONPATH=. python3 scripts/check_cfpb_reconciliation.py --self-test
```
