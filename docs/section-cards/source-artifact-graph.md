...
documentation standard; the artifact-graph demo doc (`docs/architecture/cfpb-artifact-graph-demo.md`) gives
...
`scripts/pipeline_runtime/source_graph.py` — `SourceArtifact`, `build_cfpb_source_graph(...)`,
...
`scripts/check_cfpb_source_graph_diff.py` (registered in the flywheel) — builds the CFPB source graph and
asserts the content-hash diff is deterministic and scoped to the changed nodes.

## Commands

```bash
PYTHONPATH=. python3 scripts/check_cfpb_source_graph_diff.py --self-test
```
...
(`OPP-vector-graph-lineage`, `architecture/opportunities.json`).
