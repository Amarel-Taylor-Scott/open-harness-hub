...
| A3 | **Three product layers** | **Teleon** (`teleon.dev`) = runtime SaaS engine (efficiency/descent); **Baltor** (`baltor.ai`) = customer-facing governed context, *powered by Teleon* (a tenant); **Open\*Hubs** = the open registries/funnel both consume. | `CLAUDE.md` Portfolio; `docs/strategy/teleon-baltor-openharnesshub-portfolio.md`; `brand.json` `positioning` |
...
| A5 | **Architectural dependency law** | **Baltor → Teleon → OpenHarnessHub, never the reverse.** Teleon must not import Baltor; OpenHarnessHub imports neither. Enforced, not aspirational. | `architecture/portfolio_dependency_law.json`, enforced by `scripts/check_portfolio_dependency_law.py` |
| A6 | **One-site consolidation** | All Open\*Hubs + registries live under **ONE site, `OpenHubForAI.io`** (records are the unit; hubs/registries are facets). The Open\*Hubs are the open funnel/commons, not the parent identity. | owner 2026-06-25, `docs/BIBLE.md` ("UI/UX CONSOLIDATION"); `brand.json` `scope.open_hubs` |
| A7 | **Economic unit = usage meters, not seats** | Bill on `source monitored → facts verified → context package served → agent risk reduced` (monitored sources, pages, facts under management, verification jobs, served packages, premium feeds, frontier overage). **Never price by seat.** | master §4.1; `docs/strategy/baltor-cloud-cost-pricing-pro-forma.md` |
| A8 | **Open-core monetization boundary** | **Free** = a *freezable* verified snapshot pulled into an Open\*Hub harness. **Paid (Baltor)** = the *live, kept-fresh, governed* serving layer against a dynamic corpus. One governed object, two doors. | master §4.3; archived `brand-architecture.md` (monetization boundary) |
