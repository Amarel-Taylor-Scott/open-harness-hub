...
1. **Verified public-authority publisher → FREE.** Anything a **government entity /
   public authority publishes through a verified OpenHarnessHub account** is free
...
2. **Execution** — `code-executing` (custom tools, harnesses, adapters, pipelines)
   ...
   *(Freshness/dynamic is a separate **delivery/billing** axis — `scripts/foundry/access.py`,
   `docs/strategy/monetization-mechanisms.md` — not an openness axis: a gov-published
...
*Grounds: `docs/strategy/product-market-monetization-brief.md` (export-is-the-funnel /
