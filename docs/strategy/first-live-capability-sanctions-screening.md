...
**Date:** 2026-06-25. **Anchor:** `docs/strategy/yc-master-current-state-business-plan-and-pitch.md` (the
...
- **Always-green offline conformance:** a synthetic-fixture self-test fires on every run —
  `PYTHONPATH=. python3 scripts/ingest/sanctions_feed_live.py --self-test`.
- **A dated `--live` catch:** on **2026-06-14**, a real `--live` run fetched the production SDN list
  ...
  **planted, synthetic** would-be violation — an internal claim asserting entity `OFAC-36` is CLEAR while
  ...
  real. Receipt: `docs/strategy/evidence/ofac-live-run-2026-06-14.json`. This is a dated attestation, **not**
...
correct* with governance built in (`src/teleon/verticals/provider_directory.py`):
...
**Source descent (cheapest-first, governed)** — `src/teleon/verticals/provider_sources.py`: free public
...
`PYTHONPATH=. python3 scripts/run_proofs.py`. The full offline demo runs with
`PYTHONPATH=. python3 scripts/demo_offline_full_baltor.py --self-test` — no cloud, no network LLM, no pip.
...
self-test + the dated 2026-06-14 `--live` receipt (`docs/strategy/evidence/ofac-live-run-2026-06-14.json`),
the provider-directory modules (`src/teleon/verticals/provider_directory.py`, `provider_sources.py`), and the
deterministic proof gate (computed/dated, recompute `PYTHONPATH=. python3 scripts/run_proofs.py`). Pre-revenue;
