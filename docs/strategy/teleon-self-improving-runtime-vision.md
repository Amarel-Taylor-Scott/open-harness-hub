...
`.research-notes/`, `docs/{strategy,concepts,codex,architecture}/`, and the Teleon
source surface (`src/teleon/`, `scripts/teleon_local_runtime.py`,
`schemas/{workers,agents,runtime}/`). Every claim cites a path. This doc connects
...
(`src/teleon/purpose_tasks/purpose_task.py` module docstring: *"A PurposeTask is
...
self-adaptation loop** (`docs/architecture/purpose-task-self-adapting-execution.md`).
The category line already exists: *"Serverless runs code. Kubernetes runs workloads.
**Teleon runs purpose.**"* (`docs/strategy/teleon-baltor-openharnesshub-portfolio.md`).
...
- **Distillation is never replacement** — preserve raw + lineage + held-out +
  rollback (`docs/codex/lossless-distillation.md`).
- **Self-improve from VERIFIED outcomes only** — the Determinism Factory principle
  (`docs/architecture/purpose-task-self-adapting-execution.md`; memory
...
- **GAP (DESIGNED→MISSING):** there is **no running intake queue** that takes a fresh
  ...
  (`docs/architecture/capability-rubric-and-deep-dive-2026-06-11.md` step 4) — that
...
1. **The runtime promotion gate** (`scripts/teleon_local_runtime.py`): promotion
...
2. **The side-by-side promotion gate** for adaptation
   (`src/teleon/experiments/path_promotion.py::decide`): a candidate promotes ONLY if
...
- **Durability** (the second admission axis) is the capability-valleys theory:
  admit on lift AND structural durability (`docs/concepts/capability-valleys.md`;
  single source `scripts/eval/reason_codes.py`; sorter
  `scripts/eval/durable_gap_harness.py`). **GAP:** durability gating is enforced for
...
### Stage 5 — COMPILE to a runtime unit (the "new compiler") — **BUILT (pure compile + emit) — but UNTRACKED and UNDOCUMENTED**
This is the headline finding. **The compiler exists** and is faithful to every law:
- `src/teleon/compiler/compile.py::compile_capability` — PURE + DETERMINISTIC (same
  ...
  from `architecture/worker_resource_classes.json`; budgets from the SLA policy +
  OIPS `budget_policy` + the task's `max_attempts`; image from
  `architecture/deploy_topology.json`); **lossless** (`unit` carries `capability_id`,
  `capability_version`, the binding decision, `gate_evidence`, `receipt_refs`,
  `rollback_target`); **honest** (`is_truth: False`).
- `src/teleon/compiler/emit.py` renders ONE unit to **all three deployable shapes**
...
- **GAPS (the documentation/wiring debt):** (a) the module + schema are **untracked
  `??`** — not committed, not in `.research-notes` ledger, contradicting the rubric;
  (b) **no proof script** (`scripts/check_*capability_compil*` does not exist — only
  the in-module fixtures `src/teleon/compiler/fixtures.py`); (c) **no CLI** wired
  (`scripts/teleon_local_runtime.py --run-capability` is referenced as the unit's
...
- `runtime_binding.bind` / `bind_allowed` (`src/teleon/purpose_tasks/runtime_binding.py`)
  ...
  The vocabulary (`architecture/capability_runtime_classes.json`) maps **every K8s/
...
- The **durable FleetLedger** (`src/teleon/workers/durable_fleet_ledger.py`) is the
  ...
  scale-to-zero* (`docs/architecture/durable-runtime-plan.md`; memory
  [[flywheel-operating-model]]).
- The **deploy topology generator** is already a *working deterministic compiler* at
  the per-SERVICE plane (`scripts/deploy/generate_provider_configs.py`: declarative
  JSON → fly.toml/k8s/compose, drift-gated — verified TRUE in
  `docs/status/full-verification-sweep-2026-06-11.md` claim 1). The new
...
- **GAP:** nothing actually launches a CompiledRuntimeUnit onto a Fly Machine / K8s
  ...
  (`src/teleon/runtime/capability_binding.py::TARGETS`). The local-process path is the
...
- Every model call mints a **ModelInvocationReceipt** with `is_truth:false` and an
  **allowed-use ladder** (`blocked < draft < non_serving_explanation < candidate <
  ...
  (`src/teleon/inference/oips.py`, `ALLOWED_USE_ORDER`). Receipts persist to ONE
  durable JSONL sink, **hashes/metadata only**, raw prompts/keys scrubbed
  (`src/teleon/inference/receipts.py`, `_FORBIDDEN_RECEIPT_KEYS`).
- The runtime's per-example **receipts** carry input/output/expected hashes, the
  ...
  (`scripts/teleon_local_runtime.py::_suite`, persisted to `receipts.jsonl`).
...
- The **risk-tiered ADAPTATION LADDER** (`purpose_tasks/adaptation_ladder.py` over
  `architecture/capability_adaptation_ladder.json`) decides what may auto-promote.
...
- "Improve from VERIFIED outcomes only" is the Determinism-Factory principle stated in
  ...
  upheld by lossless distillation (`docs/codex/lossless-distillation.md`).
...
- `src/teleon/digestion/digester.py` parses a SKILL.md (parse-only, never executes),
...
- **GAP:** the broader Determinism-Factory **M0→M8 distillation lane** that converts
  *stable verified LLM decisions* into deterministic rules (the consumption path's
  reconciliation, per `docs/architecture/consumption-runtime.md` + memory
...
The loop (`docs/architecture/purpose-task-self-adapting-execution.md` §self-adaptation):
...
1. **Evals/benchmarks are the fitness function.** Promotion is decided by
   ...
   not sufficient — the gate checks the *warrant* (`docs/codex/change-verification-contract.md`).
...
(`scripts/primitives/base.py::Primitive`, contract `run(po)->po`):
**Input · Knowledge Corpus · If Statement · Action · Loop · Stop/End · Output**
(`docs/concepts/component-taxonomy-and-stages.md`). Product vocabulary is enforced:
...
- **Knowledge Corpus / context packs** — facts with a `retrieval` trigger (keyword/
  ...
  (`component-taxonomy-and-stages.md` §Knowledge Corpus; OpenContextHub supplies
  reference context, *"reference context is not served truth — Baltor governs it"* —
  `teleon-baltor-openharnesshub-portfolio.md`).
...
- **Imported workloads** (a provider-native cloud function, a cron) → PurposeTask +
  ...
  (`src/teleon/lift/pipeline.py::lift_workload`; `RuntimeProfile` built ONLY from
...
positioning has materially advanced and the narrative hasn't caught up. **Action:**
commit it with a proof script (`scripts/check_capability_runtime_compiler.py`), update
...
`src/teleon/workers/durable_fleet_ledger.py` line 1 still says
...
> 7-gate promotion, the train/holdout runtime gate, and (newly, untracked) the
...
`docs/architecture/{purpose-task-self-adapting-execution,cts-1-runtime-class-binding,consumption-runtime,durable-runtime-plan,flexible-inference-lanes,capability-rubric-and-deep-dive-2026-06-11}.md` ·
`docs/status/full-verification-sweep-2026-06-11.md` · `.research-notes/{autonomous-session-ledger,stream-3,stream-5,stream-7,stream-9}.md`.
