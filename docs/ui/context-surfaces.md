...
| `/consume` | `web/baltor/consume.html` | ingestion → consumption: the served `ContextResponse.v1` for the governed CFPB run (answer "10 business days", served facts + source handles, FAQ-30 held out, the three receipt ids, section maturity, candidate providers) | `/api/context/*`, `/api/runtime/*` | `scripts/check_consumption_ui.py` |
