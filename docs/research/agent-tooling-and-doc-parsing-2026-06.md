...
> `docs/architecture/numeric-preference-graph.md`) so a new tool drops in *between* existing ones without a
...
**Action (recommended, low-risk):** adopt a `DESIGN.md` (or `web/baltor/DESIGN.md`) seeded from
`docs/strategy/brand-architecture.md` (thesis, pillars Verified·Current·Efficient·Provable, lexicon,
