...
- `architecture/stateful_swarm_provider_catalog.json` — the swarm-runner catalog (Irys @ research_candidate + the offline local stub @ active).
- `architecture/blackboard_provider_catalog.json` — the BlackboardProviderPort candidates (local SQLite @ active + Graphiti/Letta/LangGraph-checkpoint/AutoGen-Mem0/seekdb @ candidate).
- `scripts/check_stateful_swarm_provider_catalog.py --self-test` — deterministic, offline, stdlib proof of the governance boundary (60 checks, exit 0).
...
- **OpenBenchmarkHub** — catalogs **Harvey LAB** and **Irys's reported outputs** as **EVIDENCE, never
  ...
  `benchmark.legal_context_governance` opportunity already exists in `architecture/open_benchmark_registry.json`
...
- **OpenHarnessHub** — **owns the harnesses** that grade a swarm run (this is our lane, not Irys's):
...
- **OpenContextHub** — **stateful context-pack examples**: a governed blackboard snapshot is a context pack with
  source handles + held-out + receipts (a stateful sibling of the existing context-pack examples).
- **OpenSkillsHub** — the **worker skills** (the per-role analyst prompts a self-defining swarm uses) cataloged
...
Teleon runs capabilities** (`architecture/open_hubs_bridge_graph.json`).
...
`scripts/check_stateful_swarm_provider_catalog.py`; cataloged in
`architecture/stateful_swarm_provider_catalog.json` + `architecture/blackboard_provider_catalog.json`.*
