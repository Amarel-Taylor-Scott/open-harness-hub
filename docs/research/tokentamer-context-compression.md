...
> `docs/research/context-compression-tools.md`. This doc is narrower and different in **shape**:
...
> `architecture/context_compression_provider_catalog.json` (checked by
> `scripts/check_context_compression_catalog.py`); it is deliberately separate from the chunk-compressor
> `compression` slot in `architecture/external_capability_catalog.json`.
...
Compression is a stage **between policy and the router** in the inference plane (`src/teleon/inference`):
...
The manifest is what `scripts/check_context_compression_catalog.py` governs in shape, and what the
...
- **Lossless law:** `docs/codex/lossless-distillation.md` (compression shrinks the text surface only if
  facts + handles + held-out warnings survive + rehydration exists).
- **Compression landscape + `compression` slot:** `docs/research/context-compression-tools.md` (Headroom /
...
- **Inference plane:** `src/teleon/inference` (OIPS preference resolution → numeric provider router →
  ModelInvocationReceipt). ContextTamer is the Context Optimizer stage in front of the router.
- **Catalog + checker (this pass):** `architecture/context_compression_provider_catalog.json` +
  `scripts/check_context_compression_catalog.py`.
...
- Internal — lossless-distillation law: `docs/codex/lossless-distillation.md`
- Internal — compression landscape & `compression` slot: `docs/research/context-compression-tools.md`
- Internal — inference plane: `src/teleon/inference/` (OIPS, router, receipts)
