...
(`architecture/external_capability_catalog.json`) · `CompressionProviderPort` · the Optimization stage.

> **Governing law — compression is never lossy of truth.** Per `docs/codex/lossless-distillation.md`,
...
- **Components:** `catalog/processors/compress/llmlingua-context.yaml`,
  `catalog/processors/compression/structural-compress.yaml`,
  `catalog/processors/retrieval/contextual-compressor.yaml`,
  `catalog/processors/retrieval/llmlingua-compress.yaml`,
  `catalog/processors/verify/compression-fidelity-check.yaml` (the fidelity gate),
  `catalog/patterns/input-token-compression.yaml`. Deterministic compression runs in
