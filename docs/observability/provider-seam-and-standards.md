...
- `src/baltor/observability/tracing/provider.py` — spans → `SpanTree` (nesting, deterministic order by `(occurred_at, span_id)`, held-out propagation). Spans are **frozen** dataclasses, so a provider physically cannot mutate canonical state through them.
- `src/baltor/observability/projections/lineage.py` — `LineageEvent → LineageFacet` (normalized record **plus** standards renderings + a content hash for CDC).
- `src/baltor/observability/projections/standards.py` — the four deterministic mappers.
...
2. **OpenLineage RunEvent** (spec 2-0-2) — `to_openlineage_run_event(event, event_type=…)` →
   ...
   reproduces). There is already a *template* emitter at `scripts/emit/openlineage.py` for pipeline
