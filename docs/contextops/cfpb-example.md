...
`scripts/artifact_graph/reconciliation.py`; this example REPRODUCES it (assert-equivalence), never replaces it.
...
`scripts/check_contextops_cfpb_reference.py` — asserts every stage above plus the invariant: NO agent/LLM served
...
python3 scripts/check_cfpb_reconciliation.py --self-test
```
