...
- Spec: `prompts/baltor-determinism-factory.md`.
- Engine: `src/baltor/determinism/` —
...
- This factory is a **META observe→mine→replay→shadow→promote ledger** over the EXISTING deterministic
  authorities (`scripts/artifact_graph/reconciliation.py` + `conflict_detector.py`, the optimizer, the
...
4. **Lossless** — a promoted deterministic rule NEVER deletes the LLM/consensus/adjudication traces it was
   ...
   `src/baltor/distillation` subsystem rehydrates them. Tenant-private traces cannot create global rules
...
- End-to-end driver: `scripts/check_determinism_full_stack.py` — walks M0→M7 on one CFPB fixture and asserts
...
1. MAIN registers the proofs in `scripts/baltor_flywheel.py`, the matrix section, the contract registry
