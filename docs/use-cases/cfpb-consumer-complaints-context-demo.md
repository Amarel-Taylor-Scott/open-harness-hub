...
```bash
python3 scripts/demo_cfpb_context_pack.py
```

Default mode is offline and uses:

```text
data/cfpb-demo/complaints-fixture.json
```
...
```bash
python3 scripts/demo_cfpb_context_pack.py --live --limit 5
```

Optional filters:

```bash
python3 scripts/demo_cfpb_context_pack.py --live --limit 10 --state CA
python3 scripts/demo_cfpb_context_pack.py --live --limit 10 --product "Mortgage"
```
