#!/usr/bin/env python3
"""scripts.check_{{source_family}}_adapter — proof of standard.source_adapter.v1 for {{title}}.

GENERATED STUB (standard.proof_script.v1). Asserts: the adapter declares source_type/parser_provider, ingest
...
def _self_test() -> int:
    ...
    check("TODO: implement {{source_family}} adapter proof (standard.source_adapter.v1)", False,
...
def _main(argv=None) -> int:
    p = argparse.ArgumentParser(description="Proof: {{source_family}} source adapter (standard.source_adapter.v1).")
