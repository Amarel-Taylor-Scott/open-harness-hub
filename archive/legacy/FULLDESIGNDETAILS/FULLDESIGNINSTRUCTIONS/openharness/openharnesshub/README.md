...
OHH shares the design system in `../shared/` **and now consumes the shared SITE KIT**
(`../shared/oh-site.*`, loaded in the HTML):
- **Chrome** is the kit. `proto-main.jsx` `AppShell` renders the kit's `OhAppShell` (OHH's
...
- OHH keeps its own **scheme switcher, command palette and tweaks** (text-size / density /
...
- Structural tie-ins to keep intact: `.pt-panel` must stay in the `.oh-card` alias group in
  ...
  adapts the kit's page-scroll shell to OHH's fixed-height root.
