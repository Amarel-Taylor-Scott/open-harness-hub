# CLAUDE.md — `aidoneright-yc-applications` agent operating manual
...
- **Name:** `aidoneright-yc-applications`
- **Kind:** CONTEXT repo — **no product code**. The YC / accelerator application artifacts, read by humans
  and agents for grounding, never imported as code.
- **North star (one line):** The Y Combinator (and other accelerator) **application artifacts** —
  application drafts, founder and video answers, readiness scoring, and the demo storyboard — kept separate
  from product code.
- **What this repo exposes (its published interface):** `yc-application`, `yc-readiness`, `demo-storyboard`,
  `founder-answers`. Keep these stable — a surface may reference them. **`yc-readiness` is COMPUTED** by
  `scripts/yc_readiness.py` (in `shared-backend-components`), never asserted here.
- **Depth before breadth:** the ONE thing proven here is a submittable, grounded application draft where
  every claim traces to a source and every readiness number is computed, not asserted. Serve that before
  broadening. An application padded with unverifiable claims is the failure mode.
...
4. `context/blackbox.md` — what this repo owns, its current contents, and the pointers to the code-coupled
   inputs (`yc_readiness.py`, the storyboard/deck JSON) that stay in `shared-backend-components`.
...
- Add or update one application answer / founder answer / storyboard note; validate only the changed paths;
  do not start with a full rebuild.
- **Reuse-first.** Before adding an answer, check it is not already recorded — a contradicting duplicate is
  worse than a gap. *"This already exists, don't rebuild it"* applies to the application too.
- **`yc-readiness` is computed, not asserted.** Do not hand-type a readiness score into prose; it is the
  output of `scripts/yc_readiness.py`. Read that output; never restate it as a fixed number (§6).
- When a new answer supersedes an old one, **update or archive the stale entry in the same change** (§4, §8)
  — no orphaned contradictions.

## Adding work

- **A new application answer / founder answer / storyboard note** = added under `context/`, dated, with its
  source; anything needing an owner decision (raise size, pricing, founder bio) is flagged and not invented.
- **A readiness claim** = a pointer to the computed output of `scripts/yc_readiness.py`, never a hand-typed
  number (§6).
...
- **A superseded answer** = updated in place or moved to `archive/legacy/` with a status label (§8).

## Safety and scope

- **No confidential financials or real investor PII.** Public / synthetic only. Never commit a real cap
  table, term sheet, bank figure, founder SSN/DOB, or an investor's private details into an application.
...
If one answer or source is blocked, switch paths — do not stop. Tighten a founder answer, reconcile a stale
claim, re-run `scripts/yc_readiness.py` to refresh the readiness picture, or add documentation that prevents
a repeated wrong path. Every serious turn improves at least one durable piece of the application.
