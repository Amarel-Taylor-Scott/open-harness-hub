...
> the AI Done Right portfolio. It is executed **INCREMENTALLY** by the autonomous loop (not as one
...
> (covering parts of PART 8/9). Boundary packages `_repos/teleon/backend/src/teleon/` + `_repos/openhubforai/backend/src/openhubforai/` exist. Infra doc:
...
> proofs. The big generic-runtime extraction (`_repos/baltor/backend/src/baltor/` → `_repos/teleon/backend/src/teleon/`) is tracked in the law file's
...
4. **OpenHubForAI** — open skill/harness/template/eval ecosystem.

**Do not** treat Teleon as a Baltor feature · treat Baltor as Teleon's only use case · treat OpenHubForAI as
...
**Core relationship:** OpenHubForAI supplies reusable capability parts → Teleon runs and evolves capabilities
...
Local topology: Teleon API/web · Baltor API/web · OpenHubForAI registry/web · Portfolio hub · shared local
telemetry viewer. Rules: each service its own port; Baltor runs with embedded Teleon if Teleon service
unavailable; OpenHubForAI runs as fixture/static registry; no service requires paid cloud; local env vars
documented.
...
Baltor stores source artifacts/context/facts/reconciliation/receipts/customer data; OpenHubForAI stores
...
`_repos/baltor/backend/src/baltor/ports/purpose_task_provider.py` · `_repos/baltor/backend/src/baltor/adapters/teleon_embedded_local.py` ·
`teleon_local_http.py` · `teleon_saas_candidate.py`. Teleon side if present:
`companies/teleon/_repos/teleon/backend/src/teleon/sdk/` · `.../api/`. Rules: Baltor calls Teleon only through PurposeTaskProviderPort;
...
OpenHubForAI ≠ hosted runtime; AI Done Right is portfolio-level.
...
AI Done Right conflict checks, Teleon conflict notes, required registrar/WHOIS/RDAP/trademark checks,
...
Teleon internals · OpenHubForAI skill becomes active without eval · holding company contains runtime code ·
shared production DB configured · website messaging blurs Teleon/Baltor · Baltor demo requires Teleon SaaS ·
Teleon requires Baltor · OpenHubForAI requires private customer data · AI Done Right domain conflict
ignored.
...
distinct · AI Done Right brand risk recorded · redteam fails safely · Baltor demo green · flywheel green.
