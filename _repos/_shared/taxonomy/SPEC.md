...
component-definition fields used by the OpenHubForAI. Every entry in `catalog/`
must conform to one of the component schemas in `schemas/`.

> **Vocabulary note (v0.1.0 legacy schema spec — read first).** This spec predates the canonical
> **seven-primitive model** and still uses the older "pack / manifest" naming throughout. In all **new
> prose and user-facing docs** use the canonical product vocabulary (root `CLAUDE.md`; `../GLOSSARY.md`;
> `../../shared-backend-components/docs/concepts/component-taxonomy-and-stages.md` — on any vocabulary
> conflict, those win):
> - **knowledge pack → Knowledge Corpus** (the fact store),
> - **rule pack / logic pack → If Statement** (the IF condition, kept separate from the THEN — do **not**
>   call it a "Conditional"),
> - **persona · tool · processor · harness · adapter · rubric · benchmark → Action** (the THEN),
> - a component's YAML definition → a **component** / **component definition** ("manifest" is avoided in new
>   prose).
>
> The schema `type` keys, YAML field names, and filenames below (e.g. `knowledge-pack`, `rule-pack`,
> `manifest.yaml`, `schemas/*.schema.json`) are **retained verbatim** because they name real files and
> schemas kept for back-compat storage — the `CLAUDE.md` exemption for "an existing schema, filename, or
> legacy phrase" applies. They are never shown to users.
...
Layer 2 — Harnesses (named workflows around a model or trust boundary)
              ▲
Layer 1 — Knowledge Corpora · If Statements · Tools · Personas · Adapters · Schemas
          (canonical: Knowledge Corpus = knowledge pack; If Statement = rule/logic pack;
           Tools · Personas · Adapters · Rubrics are all Actions)
```

Cross-cutting: **Datasets** can live at any layer; **Rubrics** are Actions (in
the legacy schema, a kind of logic pack) but get their own top-level catalog
because they are central to evaluation; **Adapters** are how a harness talks to a model.
