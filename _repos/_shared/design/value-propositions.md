...
1. **Measured lift, never asserted.** Every pipeline shows `▲ +Δ` capability lift over a bare
   ...
   benchmark — same metric: paired with- vs without-skill. See `_repos/shared-backend-components/context/strategy/skillsbench-alignment.md`.)
...
- **vs. fine-tuning / LoRA / cheap inference-time methods** (e.g. closed-form controllers,
  reasoning-by-sampling — see `_repos/_shared/strategy/inference-time-capability-watch.md`): when capability
