...
1. **`_repos/shared-backend-components/docs/DESIGN-BIBLE.md`** (the GUIDE): tokens, type scale, every component, the two layouts (marketing top-nav
...
5. **`_repos/shared-backend-components/docs/INTEGRATION-BIBLE.md`**: how a frontend talks to a backend (same-origin seams), local and cloud.
...
`_repos/shared-backend-components/docs/DESIGN-BIBLE.md` now carries concrete, copy-paste HTML and JSX so you can build a page without inventing an API.
Read these three, in order:

1. **`_repos/shared-backend-components/docs/DESIGN-BIBLE.md` sections 7, 8, 9.** Section 7 (Page skeletons) has the exact boot HTML, a full marketing
...
3. **`_repos/shared-backend-components/docs/INTEGRATION-BIBLE.md`.** The frontend-to-backend seam: a page calls a same-origin path
   (`/api/<service>/...` or `/registry/...`) and the showcase routes it to the backend, so the page code is the same
   locally and in the cloud.

**The canonical build stack (per `_repos/shared-backend-components/docs/DESIGN-BIBLE.md` section 1).** The shipped product is the set of full apps
...
|---|---|---|
| **AI Done Right** (hub) | `#5a6b87` | (local showcase; tunnel ephemeral) |
| **Teleon** | `#6d5ef0` | (local showcase; tunnel ephemeral) |
| **Baltor** | `#0e7c86` | (local showcase; tunnel ephemeral) |
| **AIDevObserver** | `#b25fd6` | (local showcase; tunnel ephemeral) |
| **OpenHubForAI** (carries `/browse`) | `#3b6fd4` | (local showcase; tunnel ephemeral) |
...
Full detail for every line above: **`_repos/shared-backend-components/docs/DESIGN-BIBLE.md`** §12 (the SAFE-vs-LOCKED handoff). `serves_truth = false` (candidate output, not
