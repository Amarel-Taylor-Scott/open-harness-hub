...
> **Read `_repos/shared-backend-components/docs/DESIGN-BIBLE.md` FIRST** — the single, designer-ready reference for the LIVE design system
...
edit `_CSS_TEMPLATE` once and it propagates to all 5. Full SAFE-vs-LOCKED map: `_repos/shared-backend-components/docs/DESIGN-BIBLE.md` §11.
...
|---|---|---|---|
| **AI Done Right** | `#5a6b87` | parent / holding brand — the portfolio **HUB / index** | (local showcase; tunnel ephemeral) |
| **Teleon** | `#6d5ef0` | purpose-driven, eval-gated, self-adaptive compute runtime | (local showcase; tunnel ephemeral) |
| **Baltor** | `#0e7c86` | managed, verified, provable context — governs TRUTH | (local showcase; tunnel ephemeral) |
| **AIDevObserver** | `#b25fd6` | watches AI USAGE — reviews the SESSION (post) + intra-session | (local showcase; tunnel ephemeral) |
| **OpenHubForAI** | `#3b6fd4` | the open registry Teleon, Baltor, and AIDevObserver consume — carries the faceted **`/browse`** | (local showcase; tunnel ephemeral) |
...
> `openhubforai.io`). See `_repos/shared-backend-components/docs/DESIGN-BIBLE.md` §10.

## Read order (Claude design / Claude Code Max)

1. **`_repos/shared-backend-components/docs/DESIGN-BIBLE.md`** — the design system (tokens, type, components, routes, governance, SAFE-vs-LOCKED).
