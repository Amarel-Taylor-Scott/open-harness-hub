...
> Companion to `AIDevObserver-BRIEF.md` (the feature spec) and `_repos/shared-backend-components/docs/DESIGN-BIBLE.md` (the design system). This gives the
...
- **AI Done Right** is the parent (holding) brand. It owns three product layers plus an open ecosystem, shown as **5
  ...
  - **Teleon** (the runtime), **Baltor** (the governed context product, powered by Teleon), **AIDevObserver** (AI
    coding usage review), **OpenHubForAI** (the open registry Teleon, Baltor, and AIDevObserver consume), and **AI Done Right** (the parent
    hub / portfolio).
  - Dependency law (do not cross it): **Baltor depends on Teleon depends on OpenHubForAI**, never the reverse.
...
- **Backends** are the service-plane (identity, registry, events, Teleon runtime, AIDevObserver review, mailbox),
  ...
  runs locally and in cloud; only `OH_SEAM_*_BASE` differs. See `_repos/shared-backend-components/docs/INTEGRATION-BIBLE.md`.
...
`AIDevObserver-BRIEF.md`. The design system and the verbatim source are in `_repos/shared-backend-components/docs/DESIGN-BIBLE.md` + `DESIGN-ASSETS.md`.*
