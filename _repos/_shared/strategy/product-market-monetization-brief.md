...
> **Detailed source doc — consolidated elsewhere (trimmed 2026-07-03).** The consolidated portfolio thesis
> lives in **`../PRODUCT-MARKET-FIT.md`** (wedge, moat hierarchy, ICP, monetization) and the Baltor-side
> competition/moat in **`competitive-landscape-2026.md`** — on any conflict, those win. This brief is kept
> for the OpenHubForAI-product detail they only summarize: the OHH-specific competitive landscape (prompt
> hubs / agent builders / model registries — complementary to, not the same map as, competitive-landscape's
> Baltor competitors), monetization channels, the pricing table, cost advantage, the MVP, and risks.
...
and where reuse compounds. See [`_repos/_shared/codex/master-goal.md`](../codex/master-goal.md).
...
> **Reconciled ranking lives in `../PRODUCT-MARKET-FIT.md` §2** (governed data > lift bar > determinism
> factory). The three layers below are retained as the detailed breakdown, but their *ordering* is superseded
> by that hierarchy: layer 2 (the lift gate) and layer 3 (the experience database) sit *beneath* the
> governed-data lead, not co-equal with it.
...
- **Build-on-demand** — when a user needs a component the catalog lacks, that wall
  ...
  [`component-taxonomy-and-stages.md`](../../shared-backend-components/docs/concepts/component-taxonomy-and-stages.md)).
...
[`_repos/shared-backend-components/context/codex/handoff-2026-05-28-capability-requests.md`](../../shared-backend-components/context/codex/handoff-2026-05-28-capability-requests.md).
