...
- `check_inference_gateway`, `check_inference_gateway_redteam`, `check_context_audit_in_pipeline`,
  `check_durable_restart_survival` → all `KeyError: 'node_id'` from `_repos/teleon/backend/src/teleon/inference/oips.py`
...
`gateway_status`); the monolith should split per the allowlist target (`_repos/baltor/backend/src/baltor/api/...`); and `RUNS`
