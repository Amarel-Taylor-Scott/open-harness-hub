...
| `_repos/openhubforai/frontend/styles/admin-demo.css` | All layout, cards, sources, processing, outputs, and download styles in one file. | Split complete: `styles/admin-demo.css` is now an import shell for `styles/admin-demo/*.css`. |
...
| `_repos/openhubforai/frontend/admin-demo-assets/app.js` | Acceptable now, but will grow as routes deepen. | Split into `state.js`, `routes.js`, `events.js`, and route-specific view modules. |
...
```text
_repos/openhubforai/frontend/styles/admin-demo/
  tokens.css
  layout.css
  cards.css
  sources.css
...
- `_repos/openhubforai/frontend/styles/admin-demo.css` imports the split module files.
