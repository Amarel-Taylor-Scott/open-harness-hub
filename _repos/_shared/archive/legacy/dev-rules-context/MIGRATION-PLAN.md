...
19 repos. Every repo installs the shared **`aidoneright-devkit`** (standards + contracts + gates) instead of copying them. Splitting is history-preserving (`git subtree split`) so the working monorepo never breaks — do it repo-by-repo, in order.
