...
Each app is its own entry HTML + brand main file (for example `_repos/teleon/frontend/index.html` + `_repos/teleon/frontend/teleon-main.jsx`);
...
```bash
OH_PRODUCT=baltor python3 -m scripts.showcase --port 8001     # one surface (the full _repos/baltor/frontend app)
...
```bash
OH_BIND_HOST=0.0.0.0 OH_PRODUCT=baltor python3 -m scripts.showcase --port "$PORT"   # serves _repos/baltor/frontend
```
...
- **Render / Cloud Run / Fly / Railway:** one web service per surface. Set `OH_PRODUCT` (+ `OH_BIND_HOST=0.0.0.0`), let
  ...
  Canonical runbook: `docs/INTEGRATION-BIBLE.md` (section 6) + `_repos/shared-backend-components/context/architecture/fly-deploy-runbook.md`.
