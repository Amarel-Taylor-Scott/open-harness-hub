...
> and the "Cairn" sub-brand (both retired). See `_repos/shared-backend-components/docs/strategy/teleon-naming-and-domain.md` +
> `_repos/shared-backend-components/docs/strategy/teleon-company-strategy.md`.
...
- **Part C → `_repos/shared-backend-components/docs/teleon-spec.md`** (the full design Claude reads on demand).
...
surface for "PurposeTasks." Read CLAUDE.md and _repos/shared-backend-components/docs/teleon-spec.md IN FULL before doing anything.

Operate in PLAN MODE first: produce a written plan and the Phase 0 task list, then STOP and wait
for my approval before writing code. Use TaskCreate to track the phases in _repos/shared-backend-components/docs/teleon-spec.md
...
- add an external dependency/service, or expand scope beyond _repos/shared-backend-components/docs/teleon-spec.md.
...
for **PurposeTasks**. Full design: `_repos/shared-backend-components/docs/teleon-spec.md` (read it).
...
- Work in phases (`_repos/shared-backend-components/docs/teleon-spec.md` §12). After each: tests pass, redteam green, commit, STOP.
...
# PART C — `_repos/shared-backend-components/docs/teleon-spec.md` (full design)
...
for path_re, bad_re in FORBIDDEN:
    if re.search(path_re, path) and re.search(bad_re, content):
        print(f"BLOCKED by Teleon guard: '{bad_re}' in {path}. "
              ...
              f"See _repos/shared-backend-components/docs/teleon-spec.md §0/§2.", file=sys.stderr)
