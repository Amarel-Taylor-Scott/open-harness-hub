...
the runtime gate is DESIGNED-not-applied — see "Built vs designed"). **Code:** `_repos/teleon/backend/src/teleon/exploration/` ·
...
1. **Forbidden / ENDS change → T4 (human), always.** If `task.change_type` is an ENDS change or a
   ...
   `_repos/teleon/backend/src/teleon/purpose_tasks/adaptation_ladder.py`, L0–L5), the task goes straight to a human. An open-ended
...
- **delegates** to the existing `_repos/teleon/backend/src/teleon/agents/agent_runtime_provider.dispatch_agent_request`, which selects
...
- **defaults the runtime to the offline `local_emulator@v1` invariant.** A caller may instead name a catalog
  candidate (`clawless_openclaw@candidate`, `hermes@candidate` in `_repos/shared-backend-components/architecture/agent_runtime_catalog.json`),
...
`_repos/baltor/backend/src/baltor/ports/research_agent_provider.py`; the Teleon-side dispatch produces the runtime-native
...
(`_repos/teleon/backend/src/teleon/ports/stateful_swarm_provider.py`) is the same story behind the same bucket: bounded workers on a
...
open-ended task ──escalation_decision──▶ T3 ──dispatch_exploration──▶ ExplorationProposal
   (serves_truth=False, receipt, provenance)
        │
        └──re-enters──▶ runtime gate (_repos/shared-backend-components/scripts/teleon_local_runtime: train+holdout, receipts)
                                   │  promoted?
                                   ├─ yes ─▶ _repos/teleon/backend/src/teleon/compiler.compile_capability  ──▶ CompiledRuntimeUnit
...
- Proof that the package imports **no** OpenClaw/Hermes/OpenHands/Open SWE/agent SDK (source scan) and obeys the
  portfolio dependency law (`_repos/shared-backend-components/scripts/check_portfolio_dependency_law.py --self-test` passes with this package
  present).
...
- **The runtime-gate failure-path wire-in is documented, not applied** (see next section). It is a one-call
  hook; applying it edits `_repos/shared-backend-components/scripts/teleon_local_runtime.py`, which other agents are live-editing, so it is left
...
or `rolled-back` (not `promoted`) in `_repos/shared-backend-components/scripts/teleon_local_runtime.py::Runtime._run_to_completion`, right after
