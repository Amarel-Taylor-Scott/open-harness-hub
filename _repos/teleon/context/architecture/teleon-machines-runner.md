...
**Code:** `_repos/shared-backend-components/scripts/deploy/teleon_machines_runner.py` ·
**Consumes:** `_repos/teleon/backend/src/teleon/compiler/` (`CompiledRuntimeUnit`) + `_repos/shared-backend-components/schemas/runtime/CompiledRuntimeUnit.schema.json` ·
**Reuses:** `_repos/shared-backend-components/scripts/deploy/fly_worker_controller.py` (`MachinesAPI`) ·
...
```
python -m py_compile _repos/shared-backend-components/scripts/deploy/teleon_machines_runner.py
python -m scripts.deploy.teleon_machines_runner --self-test     # 41/41 (run 3×, deterministic)
python _repos/shared-backend-components/scripts/deploy/preflight.py                              # GO — deploy artifacts unaffected
```
