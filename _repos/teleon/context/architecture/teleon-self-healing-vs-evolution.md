...
| | `_repos/teleon/backend/src/teleon/self_healing/` | `_repos/teleon/backend/src/teleon/evolution/` |
|---|---|---|
...
1. **The mechanism** — both reuse `_repos/teleon/backend/src/teleon/purpose_tasks` (`provision` / `run_current` / `adapt` /
...
- `_repos/shared-backend-components/scripts/check_teleon_capability_evolution_graph.py` — the graph, documented forks, descent path,
  `most_deterministic_runner`, the lossless failures, and the `record_heal` bridge.
- `_repos/shared-backend-components/scripts/check_teleon_objective_gated_descent.py` — the objective decides whether a descent fires or is held.
- `_repos/shared-backend-components/scripts/check_purpose_task_self_heal_e2e.py` + the `self_healing` proofs — the reactive restore path.
