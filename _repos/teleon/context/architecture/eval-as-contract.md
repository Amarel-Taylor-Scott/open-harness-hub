...
`_repos/teleon/context/strategy/teleon-self-improving-runtime-vision.md` — *"There is no `eval_suite` / `benchmark_ref` /
...
- `_repos/shared-backend-components/schemas/purpose_tasks/PurposeTaskSpec.schema.json`
...
frontier_judge | human` — the **same vocabulary** as `_repos/shared-backend-components/schemas/benchmark.schema.json`), and the
...
- `_repos/teleon/backend/src/teleon/purpose_tasks/eval_suite.py` — the validated field + helpers (pure, deterministic, no I/O):
...
- `_repos/teleon/backend/src/teleon/purpose_tasks/purpose_task.py`:
...
- `_repos/teleon/backend/src/teleon/purpose_tasks/projections.py` — the customer view summarizes the eval_suite
...
- Re-exported through `_repos/baltor/backend/src/baltor/purpose_tasks/*` shims (lossless Baltor→Teleon seam; Teleon never imports
  Baltor — `_repos/shared-backend-components/scripts/check_portfolio_dependency_law.py` stays green).

## How the runtime gate consumes it (the wire-in)

Today the live gate (`_repos/shared-backend-components/scripts/teleon_local_runtime.py`) iterates a **hardcoded** per-capability suite:
...
`_repos/teleon/backend/src/teleon/compiler/` compiles only **promoted** capabilities ("Run the eval gate to promotion first, then
...
(`_repos/shared-backend-components/docs/standards/open-capability-task-specification.md`). The compiler never bypasses the suite — agents
...
server. `_repos/shared-backend-components/scripts/` is tooling, not a brand layer, so this `src/teleon` → `scripts` read does **not** violate
...
**Real now (proven by `_repos/shared-backend-components/scripts/check_eval_suite_contract.py --self-test`, 38 checks):** the field on both
...
1. **The registry-of-named-suites.** `resolve_benchmark_ref` is the real, honest seam (hand it a registry and
   ...
   `_repos/shared-backend-components/schemas/benchmarks/BenchmarkArtifact` / `BenchmarkResult` and the `benchmark.schema.json` manifest)
   is the next increment so `benchmark_ref` resolves from a store, not just an in-memory dict.
2. **Flipping the live runtime gate** (`_repos/shared-backend-components/scripts/teleon_local_runtime.py`, other-owned) to read
...
3. **Wiring the Stage-2 confirm scorer** (`_repos/shared-backend-components/scripts/eval/measured_lift_headtohead.py`) to the declared
...
`_repos/shared-backend-components/architecture/capability_adaptation_ladder.json` — a task may never autonomously drop its benchmark. Making the
