...
1. **The final unit runs externally, forever.** Teleon COMPILES the unit (`_repos/teleon/backend/src/teleon/compiler/{compile,emit}.py`)
...
2. **Even the improvement LLMs can run on Cloudflare.** Cloudflare Workers AI is a model lane
   (`_repos/shared-backend-components/architecture/lowcost_llm_endpoint_registry.json`) — the cheap brain that proposes conversions can run there, not
   on our compute.
3. **Teleon need not run the improvement runs at all.** The descent brain
   (`_repos/teleon/backend/src/teleon/evolution/descent_attempt_store.py`) is the canonical, append-only, lossless record of every attempt.
   `_repos/teleon/backend/src/teleon/evolution/external_outcomes.py` records an EXTERNAL run's result + metadata (axes from its receipt) into
...
`ExecutionProviderPort` + `_repos/shared-backend-components/architecture/execution_backend_policy_matrix.json` (`compute_ownership`) ·
`_repos/teleon/backend/src/teleon/runtime/execution_providers/cloudflare_workers.py` · the Cloudflare Workers AI lane ·
`_repos/teleon/backend/src/teleon/evolution/descent_attempt_store.py` + `external_outcomes.py` · `_repos/shared-backend-components/scripts/check_teleon_control_plane.py`.
See also `_repos/shared-backend-components/context/architecture/cloud-architecture.md`, `_repos/shared-backend-components/architecture/surface_map.json`.
