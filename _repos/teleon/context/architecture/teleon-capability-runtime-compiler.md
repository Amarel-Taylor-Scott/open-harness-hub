...
**Code:** `_repos/teleon/backend/src/teleon/compiler/` · **Schema:** `_repos/shared-backend-components/schemas/runtime/CompiledRuntimeUnit.schema.json` ·
...
per-capability analog of `_repos/shared-backend-components/architecture/deploy_topology.json` (which is the per-**service** plane), built on the
**same proven generator pattern** as `_repos/shared-backend-components/scripts/deploy/generate_provider_configs.py` (one declarative source →
...
```
  ...
  EVAL GATE (teleon_local_runtime)      ← train+holdout pass-rates must BOTH clear PROMOTE_AT
        ...
        ▼
  ┌─────────────────────────────────────────────────────────┐
  │  THE COMPILER  (_repos/teleon/backend/src/teleon/compiler)                     │
...
| `runtime_class` | the CTS-portable class the CapabilityTask declared (`_repos/shared-backend-components/architecture/capability_runtime_classes.json`) |
| `backend` | **`_repos/teleon/backend/src/teleon/purpose_tasks/runtime_binding.bind_allowed`** — the SAME class→backend authority PurposeTask uses |
...
| `resources.{cpu,memory_mb,gpu_required}` | `_repos/shared-backend-components/architecture/worker_resource_classes.json` keyed by the task's `required_resource_class` |
...
- **`_repos/shared-backend-components/architecture/*.json` policy files** are the single sources for resources, SLA timeouts, the execution
...
- **OIPS (`_repos/teleon/backend/src/teleon/inference/oips`)** owns model preference/budget; the compiler reads the resolved
  preference's `budget_policy` for the token ceiling. It does not invent a budget engine.
- **Receipts (`_repos/teleon/backend/src/teleon/inference/receipts`)** are the provenance envelope; the compiler attaches receipt
  **refs**, mirroring the receipt sink's metadata-and-hashes-only, no-raw-secret discipline.
- **The eval gate (`_repos/shared-backend-components/scripts/teleon_local_runtime.py`)** owns promotion; the compiler **consumes** its
...
drift-gate property applied to capabilities: the committed example units (`_repos/teleon/backend/src/teleon/compiler/examples/`) are
...
- **No magic values** (`_repos/shared-backend-components/docs/codex/no-magic-values.md`): every field is joined from one source; the one numeric
...
- **Lossless distillation** (`_repos/shared-backend-components/docs/codex/lossless-distillation.md`): the unit carries lineage back to the
...
- **Change verification** (`_repos/shared-backend-components/docs/codex/change-verification-contract.md`): the warrant is **clear user intent**
...
- **Dependency law** (`_repos/shared-backend-components/architecture/portfolio_dependency_law.json`): Teleon-layer code; the package imports only
  stdlib + `src.teleon` siblings + `scripts` tooling. `check_portfolio_dependency_law.py --self-test` scans it
  (86 teleon files) and confirms no `teleon→baltor` / `teleon→openhubforai` edge.
