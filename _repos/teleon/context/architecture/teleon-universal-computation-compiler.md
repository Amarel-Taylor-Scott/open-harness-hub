...
exactly the **descent** (`_repos/teleon/backend/src/teleon/evolution/descent.py`): make it work, then make it efficient.
...
| **Component agnostic** | python · docker · OpenAPI · MCP · CLI · SQL · browser · remote inference | **built** — `_repos/teleon/backend/src/teleon/components/` (uniform `invoke`) |
...
| 3 | Universal Adapter | **built (v1)** | `_repos/teleon/backend/src/teleon/components/` (`invoke`/`lower`/`conformance`) + agnostic ports + `runtime/execution_providers/` |
...
**Purpose.** Erase bespoke integrations: everything is `invoke(inputs) → outputs`. **Have.** `_repos/teleon/backend/src/teleon/components/`
