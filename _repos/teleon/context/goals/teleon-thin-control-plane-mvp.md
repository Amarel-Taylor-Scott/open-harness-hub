...
positioning: `_repos/teleon/context/architecture/teleon-control-plane-vs-compute.md`.
...
hardwired to it. ALL compute goes through `ExecutionProviderPort` (`_repos/shared-backend-components/architecture/execution_backend_policy_matrix.json`
...
1. End-to-end thin-control-plane path: a CapabilityTask → resolve the PER-FUNCTION medium
   (`_repos/teleon/backend/src/teleon/config/medium_resolver.py` over `_repos/shared-backend-components/architecture/medium_config.json` — Cloudflare-preferred when a token
   is present, else the configured/fallback lane) → COMPILE the unit (`_repos/teleon/backend/src/teleon/compiler`) → dispatch to the chosen
...
```bash
PYTHONPATH=. python3 _repos/shared-backend-components/scripts/check_teleon_control_plane.py --self-test
PYTHONPATH=. python3 _repos/shared-backend-components/scripts/check_medium_config.py --self-test
PYTHONPATH=. python3 _repos/shared-backend-components/scripts/check_execution_provider_factory.py --self-test
PYTHONPATH=. python3 _repos/shared-backend-components/scripts/build_config_ui.py --self-test
PYTHONPATH=. python3 _repos/shared-backend-components/scripts/config_ui_server.py --self-test
PYTHONPATH=. python3 _repos/shared-backend-components/scripts/check_byo_compute.py --self-test
PYTHONPATH=. python3 _repos/shared-backend-components/scripts/check_surface_map.py --self-test
PYTHONPATH=. python3 _repos/shared-backend-components/scripts/check_teleon_example_descents.py --self-test
PYTHONPATH=. python3 _repos/shared-backend-components/scripts/check_dag_pipeline.py --self-test
PYTHONPATH=. python3 _repos/shared-backend-components/scripts/run_teleon_demos_live.py --self-test
PYTHONPATH=. python3 _repos/shared-backend-components/scripts/check_portfolio_dependency_law.py --self-test
python3 _repos/shared-backend-components/scripts/check_ai_done_right_surface_family.py --self-test
```
...
When all boxes are green, write a short status note to `_repos/shared-backend-components/docs/status/` and stop.
