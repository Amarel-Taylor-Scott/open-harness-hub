...
> sources: `_repos/shared-backend-components/docs/strategy/teleon-naming-and-domain.md` + `_repos/shared-backend-components/docs/strategy/teleon-baltor-openhubforai-portfolio.md`:
...
>   spec). Dependency law: Baltor → Teleon → OpenHubForAI, never the reverse
...
>   `_repos/dev-rules-context/prompts/teleon-build-kit.md` as canonical where they differ. Generic runtime concepts below are **Teleon**
>   (extracting from `_repos/baltor/backend/src/baltor/` into `_repos/teleon/backend/src/teleon/`), not Baltor internals.
...
`_repos/dev-rules-context/prompts/baltor-{purpose-driven-cloud-task-runtime,open-capability-task-standard,purpose-task-dashboard-and-standard,
path-improvement-and-skill-intake-factory,k8s-cloudfunction-template-factory}.md`; doctrine
`_repos/teleon/context/standards/capability-task-architecture-doctrine.md`; vision `_repos/teleon/context/architecture/purpose-task-self-adapting-execution.md`.
...
1. **Adaptation ladder as config + proof** — `_repos/shared-backend-components/architecture/capability_adaptation_ladder.json` (L0–L5 + auto-vs-
...
5. **The dual-audience dashboard** — remaining projections + `/api/purpose-tasks/*` + staff/customer UI (the
   redaction core is built) — `_repos/baltor/context/baltor-purpose-task-dashboard-and-standard.md`.
...
rebuild), now extracting into `_repos/teleon/backend/src/teleon/`. Standard stays neutral (CTS, stewarded by OpenHubForAI); Teleon =
