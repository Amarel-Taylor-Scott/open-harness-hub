...
analog of the per-service ``_repos/shared-backend-components/architecture/deploy_topology.json`` plane, built on the SAME proven generator pattern
as ``_repos/shared-backend-components/scripts/deploy/generate_provider_configs.py`` (declarative source → provider configs, drift-gated,
self-tested), but for capabilities instead of services.
  ...
  * ``CompiledUnitRegistry`` (``registry.py``) — the durable, append-only REGISTRY of compiled units: the
    per-capability analog of the per-service ``_repos/shared-backend-components/architecture/deploy_topology.json``. It tracks which
...
Doc: ``_repos/teleon/context/architecture/teleon-capability-runtime-compiler.md``.
"""
