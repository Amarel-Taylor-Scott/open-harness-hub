...
THE GAP THIS CLOSES (_repos/teleon/context/strategy/teleon-self-improving-runtime-vision.md §Stage 5/6 GAP (d);
...
of the per-SERVICE ``_repos/shared-backend-components/architecture/deploy_topology.json``: no way to ask "what is the latest compiled unit for
...
LAWS:
  * LOSSLESS (_repos/shared-backend-components/docs/codex/lossless-distillation.md): superseded units are DEMOTED, never deleted — full history is
...
``_repos/shared-backend-components/scripts/check_portfolio_dependency_law.py``).
"""
...
#: uses (_repos/shared-backend-components/scripts/teleon_local_runtime.py STATE_DIR = dist/local-services-state/teleon-runtime). The registry is
...
DEFAULT_STATE_DIR = _resource("dist") / "local-services-state" / "teleon-compiler"
