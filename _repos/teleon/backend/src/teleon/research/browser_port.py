...
the registry (_repos/shared-backend-components/architecture/web_browsing_stack_registry.json) — a browser added there is selectable here; a chromium-
...
_RENDER_MJS = _resource("e2e") / "scrape_url.mjs"
_REGISTRY = _resource("architecture") / "web_browsing_stack_registry.json"
...
class PlaywrightBrowser:
    ...
    def render(self, url: str, *, timeout: int = 40, mode: str = "headless") -> dict:
        """mode = headless (default) | headed — a rung on _repos/shared-backend-components/architecture/browser_escalation_ladder.json."""
        try:
...
class UndetectedBrowser:
    """Rung 5 of the browser ladder (_repos/shared-backend-components/architecture/browser_escalation_ladder.json): a stealth/undetected driver
