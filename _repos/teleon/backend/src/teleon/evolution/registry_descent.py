...
Registries the converter now spans:
  - _repos/shared-backend-components/architecture/modality_capability_catalog.json   (pipelines: deterministic-spine vs model-core)   [via catalog_descent]
  - _repos/shared-backend-components/architecture/capability_implementation_registry.json  (each capability's impl options: cost_tier + determinism)
  - _repos/shared-backend-components/architecture/tunable_task_catalog.json           (each task's cost-ranked tiers, cheapest-that-meets)
...
fresh model from _repos/shared-backend-components/architecture/model_index.json (via substrate_selector, reusing the model-index selector) for every
...
_IMPL = _resource("architecture") / "capability_implementation_registry.json"
_TUNABLE = _resource("architecture") / "tunable_task_catalog.json"
