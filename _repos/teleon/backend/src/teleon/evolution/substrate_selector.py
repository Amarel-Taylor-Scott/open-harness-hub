...
#: fresh model qualifies. This is the ONLY estimate; the real path is sourced from _repos/shared-backend-components/architecture/model_index.json.
...
def model_downgrade(*, quality_floor_rank: int = 0, entries: list[dict] | None = None) -> dict:
    ...
    expensive (frontier) FRESH model, both from _repos/shared-backend-components/architecture/model_index.json. Returns the registry-derived factor
