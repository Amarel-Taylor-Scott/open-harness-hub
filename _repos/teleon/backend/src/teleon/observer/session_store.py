...
def _cid(*parts: str) -> str:
    ...
    return hashlib.sha256("␟".join(parts).encode()).hexdigest()[:SESSION_STORE_SETTINGS.content_id_digest_chars]
...
def event_record(session_id: str, t: int, kind: str, payload: str) -> dict:
    ...
    return {"record": "event", "session_id": session_id, "t": t, "kind": kind,
            "id": _cid(session_id, str(t), kind), "payload": payload[:SESSION_STORE_SETTINGS.event_payload_chars],
            "serves_truth": False}
...
def outcome_record(
    ...
    outcome: str,
    *,
    source_ref_keys: list[str] | None = None,
    ...
    record = {"record": "outcome", "session_id": session_id, "intervention_id": intervention_id,
              "outcome": outcome, "serves_truth": False}
    if source_ref_keys:
        record["source_ref_keys"] = sorted({key for key in source_ref_keys if isinstance(key, str) and key})
    if finding_type:
        record["type"] = finding_type
    return record
...
def load(session_id: str) -> list[dict]:
    ...
    for ln in p.read_text().splitlines():
        ...
        try:
            ...
            # append-only store: tolerate a torn/corrupt line (e.g. a crash mid-write) — skip it,
            # never let one bad line crash the whole session read.
            continue
    return out


def load_recent(*, limit_files: int = SESSION_STORE_SETTINGS.recent_files) -> list[dict]:
    """Load recent append-only session/outcome records across local sessions.

    This is the local-file fork's memory reader. It keeps the outcome loop useful
    across sessions without reading transcript text from outside the normalized
    observer-session store.
    """
    if not _DIR.exists():
        return []
    files = sorted((p for p in _DIR.glob("*.jsonl") if p.is_file()), key=lambda p: p.stat().st_mtime, reverse=True)
    records: list[dict] = []
    for path in files[:max(0, limit_files)]:
        try:
            records.extend(json.loads(ln) for ln in path.read_text().splitlines() if ln.strip())
...
def _self_test() -> int:
    ...
    keyed = outcome_record(sid, iv_id, "reused", source_ref_keys=["src_b", "src_a", "src_a"], finding_type="reinvention")
    ck("outcome can carry deduped source-ref memory keys", keyed.get("source_ref_keys") == ["src_a", "src_b"])
    ck("outcome can carry finding type metadata", keyed.get("type") == "reinvention")
