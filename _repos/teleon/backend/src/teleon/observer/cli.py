...
an honest message when no session is found. Same data the MCP server (_repos/shared-backend-components/scripts/aidevobserver_mcp_server.py)
...
from .registry_search import enrich_report_with_local_registry
...
def review_path(
    ...
    *,
    ...
    """Governed post-session report for one transcript file.

    ``registry_cwd`` is optional and still candidate-only. When local registry
    search is explicitly enabled, it enriches findings with repo-relative
    source refs and reuse cards through the shared registry adapter.
    """

    report = review_session(from_transcript(path))
    return enrich_report_with_local_registry(
        report,
        root_value=registry_cwd,
...
def cmd_review(args: argparse.Namespace) -> int:
    ...
    report = review_path(
        path,
        registry_cwd=args.registry_cwd,
...
def _add_target(p: argparse.ArgumentParser) -> None:
    ...
    p.add_argument("--registry-cwd", help="optional repo root for opt-in local registry/source-ref enrichment")
    p.add_argument("--requested-input", help="optional requested input edge/contract for registry enrichment")
    p.add_argument("--requested-output", help="optional requested output edge/contract for registry enrichment")
