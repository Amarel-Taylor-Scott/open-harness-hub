...
# ══════════════════════════════════════════════════════════════════════════════════════════════════
# POSTGRES backend — the cloud twin of DurableFleetLedger (added ALONGSIDE; SQLite stays the default).
# Same method shape, so the live supervisor + N worker PROCESSES claim against ONE ledger. The atomic
# claim is a single FOR UPDATE SKIP LOCKED + LIMIT 1 statement: concurrent workers each lock a DIFFERENT
# ready row and claim WITHOUT serializing — the whole point of the cloud backend (the SQLite
# BEGIN IMMEDIATE analog serializes writers). ISO timestamps stored as TEXT → identical lexical ordering
# to the SQLite ledger. The driver is imported lazily; `connect` is injectable for the offline proof.
# ══════════════════════════════════════════════════════════════════════════════════════════════════

# Column order (single source for dict-row mapping; mirrors the SQLite schema exactly so a task/worker
# dict has identical keys on either backend — no caller change across the swap).
_TASK_COLUMNS = ("task_id", "tenant_id", "capability_id", "idempotency_key", "status", "priority_class",
                 "queue_name", "deadline_at", "not_before", "max_attempts", "attempt", "lease_owner",
                 "lease_until", "created_at", "claimed_at", "started_at", "finished_at", "failed_at",
                 "error_json", "result_ids_json", "depends_on_json", "payload_json")
_TASK_COLS = ", ".join(_TASK_COLUMNS)
_WORKER_COLUMNS = ("worker_id", "capability_ids_json", "status", "max_concurrency", "active_task_count",
                   "started_at", "heartbeat_at", "last_claim_at", "total_tasks_processed", "total_failures")
_WORKER_COLS = ", ".join(_WORKER_COLUMNS)

# The Postgres atomic claim: ONE statement — FOR UPDATE SKIP LOCKED + LIMIT 1 — so N workers each lock a
# DIFFERENT ready row and claim CONCURRENTLY without serializing. The dependency gate (claimable only when
# every depended-on task has SUCCEEDED) is evaluated IN SQL via jsonb, so the row lock and the readiness
# test are atomic in the same statement. Every value is a %s placeholder (no string interpolation).
_PG_CLAIM_TASK_SQL = (
    "UPDATE capability_tasks SET status=%s, lease_owner=%s, lease_until=%s, claimed_at=%s "
    "WHERE task_id = ("
    "  SELECT t.task_id FROM capability_tasks t "
    "  WHERE t.status=%s AND t.capability_id=%s AND (t.not_before='' OR t.not_before<=%s) "
    "    AND NOT EXISTS ("
    "      SELECT 1 FROM jsonb_array_elements_text(t.depends_on_json::jsonb) AS dep(id) "
    "      LEFT JOIN capability_tasks d ON d.task_id = dep.id "
    "      WHERE d.task_id IS NULL OR d.status <> %s) "
...
class PostgresFleetLedger:
    ...
    the live dispatch path needs, with a NON-serializing concurrent claim. serves_truth=false."""

    backend = "postgres"

    def __init__(self, *, dsn: str | None = None, connect=None) -> None:
        from scripts._config import OH_PG_DSN_ENV
        from src.teleon.storage.record_store import (  # single source for the Postgres connection plumbing
            StorageError, connect_postgres, psycopg_available, resolve_postgres_dsn)
        self.dsn = resolve_postgres_dsn() if dsn is None else dsn
        if connect is None:                            # production path: require a real, reachable backend
            if self.dsn is None:
                raise StorageError(
                    "the postgres fleet ledger is selected but no DSN is configured — set "
                    f"{OH_PG_DSN_ENV} (or DATABASE_URL). It refuses rather than silently using the local "
                    "SQLite ledger.")
            if not psycopg_available():
                raise StorageError("the postgres fleet ledger is configured but no Postgres driver "
                                   "(psycopg / psycopg2) is importable.")
        self.conn = (connect or connect_postgres)(self.dsn or "")
        self._init_schema()
    ...
    def _init_schema(self) -> None:
        self._exec("""CREATE TABLE IF NOT EXISTS capability_workers(
            worker_id TEXT PRIMARY KEY, capability_ids_json TEXT, status TEXT, max_concurrency INTEGER,
            active_task_count INTEGER, started_at TEXT, heartbeat_at TEXT, last_claim_at TEXT,
            total_tasks_processed INTEGER, total_failures INTEGER)""")
        self._exec("""CREATE TABLE IF NOT EXISTS capability_tasks(
            task_id TEXT PRIMARY KEY, tenant_id TEXT, capability_id TEXT, idempotency_key TEXT UNIQUE,
            status TEXT, priority_class TEXT, queue_name TEXT, deadline_at TEXT, not_before TEXT,
            max_attempts INTEGER, attempt INTEGER, lease_owner TEXT, lease_until TEXT, created_at TEXT,
            claimed_at TEXT, started_at TEXT, finished_at TEXT, failed_at TEXT, error_json TEXT,
    ...
    def register_worker(self, *, worker_id: str, capability_ids: list, max_concurrency: int = 1,
                        now: str | None = None) -> dict:
        t = _iso(now)
        self._exec("""INSERT INTO capability_workers(worker_id,capability_ids_json,status,max_concurrency,
            active_task_count,started_at,heartbeat_at,last_claim_at,total_tasks_processed,total_failures)
    ...
    def set_worker_status(self, worker_id: str, status: str, *, now: str | None = None) -> None:
        self._exec("UPDATE capability_workers SET status=%s,heartbeat_at=%s WHERE worker_id=%s",
                   (status, _iso(now), worker_id))
        self.conn.commit()

    def worker(self, worker_id: str) -> dict | None:
        row = self._exec(f"SELECT {_WORKER_COLS} FROM capability_workers WHERE worker_id=%s",
                         (worker_id,), fetchone=True)
        return dict(zip(_WORKER_COLUMNS, row)) if row else None

    def live_workers(self, capability_id: str | None = None, *, now: str | None = None) -> list[dict]:
        rows = self._exec(f"SELECT {_WORKER_COLS} FROM capability_workers "
                          f"WHERE status NOT IN ('stopped','failed')", fetch=True) or []
        out = []
        for r in rows:
            d = dict(zip(_WORKER_COLUMNS, r)); caps = json.loads(d["capability_ids_json"])
            d["capability_ids"] = caps
            if capability_id is None or capability_id in caps:
                out.append(d)
        return out
    ...
    def enqueue_task(self, *, tenant_id: str, capability_id: str, idempotency_key: str, now: str | None = None,
                     ...
                     payload: dict | None = None) -> dict:
        t = _iso(now)
        existing = self._exec("SELECT task_id FROM capability_tasks WHERE idempotency_key=%s",
                              (idempotency_key,), fetchone=True)
        if existing:
            return self.task(existing[0])               # idempotent: no duplicate
        tid = _hid("task-", tenant_id, capability_id, idempotency_key)
        # ON CONFLICT(idempotency_key) DO NOTHING makes the enqueue concurrency-safe (two processes that
        # compute the SAME deterministic tid for one key cannot fork a duplicate); re-resolve by key after.
        self._exec("""INSERT INTO capability_tasks(task_id,tenant_id,capability_id,idempotency_key,status,
            priority_class,queue_name,deadline_at,not_before,max_attempts,attempt,lease_owner,lease_until,
            created_at,claimed_at,started_at,finished_at,failed_at,error_json,result_ids_json,depends_on_json,payload_json)
            VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)
            ON CONFLICT(idempotency_key) DO NOTHING""",
            (tid, tenant_id, capability_id, idempotency_key, QUEUED, priority_class, queue_name, deadline_at,
             not_before, max_attempts, 0, "", "", t, "", "", "", "", "{}", "[]",
             json.dumps(list(depends_on or [])), json.dumps(payload or {})))
        self.conn.commit()
        row = self._exec("SELECT task_id FROM capability_tasks WHERE idempotency_key=%s",
                         (idempotency_key,), fetchone=True)
    ...
    def claim_task(self, *, worker_id: str, capability_id: str, now: str | None = None,
        ...
        ready row (no serialization); a row is handed to EXACTLY ONE worker."""
        t = _iso(now)
        try:
            row = self._exec(_PG_CLAIM_TASK_SQL,
                             (CLAIMED, worker_id, _plus_s(t, lease_seconds), t,
                              QUEUED, capability_id, t, SUCCEEDED), fetchone=True)
            if row is None:
                self.conn.commit()
                return None                             # nothing ready (queue empty or all dep/lock-blocked)
            tid = row[0]
            self._exec("UPDATE capability_workers SET last_claim_at=%s WHERE worker_id=%s", (t, worker_id))
            self.conn.commit()
            return self.task(tid)
        except Exception:
    ...
    def start_task(self, task_id: str, worker_id: str, now: str | None = None) -> dict:
        self._require_owner(task_id, worker_id)
        self._exec("UPDATE capability_tasks SET status=%s,started_at=%s WHERE task_id=%s",
                   (RUNNING, _iso(now), task_id))
        self.conn.commit()
        return self.task(task_id)

    def ack_task(self, task_id: str, worker_id: str, result_ids: list | None = None,
                 now: str | None = None) -> dict:
        self._require_owner(task_id, worker_id)
        self._exec("UPDATE capability_tasks SET status=%s,finished_at=%s,lease_until='',result_ids_json=%s "
                   "WHERE task_id=%s", (SUCCEEDED, _iso(now), json.dumps(result_ids or []), task_id))
        self._exec("UPDATE capability_workers SET total_tasks_processed=total_tasks_processed+1 "
                   "WHERE worker_id=%s", (worker_id,))
        self.conn.commit()
        return self.task(task_id)

    def nack_task(self, task_id: str, worker_id: str, error: dict, *, retryable: bool,
                  now: str | None = None) -> dict:
        self._require_owner(task_id, worker_id)
        t = _iso(now); task = self.task(task_id); attempt = task["attempt"] + 1
        dead = (not retryable) or attempt >= task["max_attempts"]
        self._exec("UPDATE capability_tasks SET status=%s,failed_at=%s,error_json=%s,attempt=%s,"
                   "lease_owner='',lease_until='' WHERE task_id=%s",
                   (DEAD if dead else QUEUED, t, json.dumps(error), attempt, task_id))
        self._exec("UPDATE capability_workers SET total_failures=total_failures+1 WHERE worker_id=%s",
                   (worker_id,))
        self.conn.commit()
        return self.task(task_id)
    ...
    def reclaim_expired_leases(self, now: str | None = None) -> list[str]:
        t = _iso(now)
        rows = self._exec("SELECT task_id FROM capability_tasks WHERE status IN (%s,%s) "
                          "AND lease_until<>'' AND lease_until<%s", (CLAIMED, RUNNING, t), fetch=True) or []
        ids = [r[0] for r in rows]
        for tid in ids:
            self._exec("UPDATE capability_tasks SET status=%s,lease_owner='',lease_until='' WHERE task_id=%s",
                       (QUEUED, tid))
        self.conn.commit()
        return ids

    def _require_owner(self, task_id: str, worker_id: str) -> None:
        row = self._exec("SELECT lease_owner FROM capability_tasks WHERE task_id=%s",
    ...
    # ── reads ──────────────────────────────────────────────────────────────
    def task(self, task_id: str) -> dict | None:
        row = self._exec(f"SELECT {_TASK_COLS} FROM capability_tasks WHERE task_id=%s",
                         (task_id,), fetchone=True)
        return dict(zip(_TASK_COLUMNS, row)) if row else None

    def queued_tasks(self, capability_id: str | None = None) -> list[dict]:
        if capability_id:
            rows = self._exec(f"SELECT {_TASK_COLS} FROM capability_tasks WHERE status=%s AND capability_id=%s "
                              f"ORDER BY created_at,task_id", (QUEUED, capability_id), fetch=True) or []
        else:
            rows = self._exec(f"SELECT {_TASK_COLS} FROM capability_tasks WHERE status=%s "
                              f"ORDER BY created_at,task_id", (QUEUED,), fetch=True) or []
        return [dict(zip(_TASK_COLUMNS, r)) for r in rows]

    def tasks_by_status(self, status: str) -> list[dict]:
        rows = self._exec(f"SELECT {_TASK_COLS} FROM capability_tasks WHERE status=%s ORDER BY task_id",
...
def open_fleet_ledger(path: str | Path = DEFAULT_DB, *, dsn: str | None = None, connect=None):
    ...
    seam is injected) AND a driver is importable; otherwise the SQLite ``DurableFleetLedger`` (the default /
    local fallback). The cloud swap is config, not a caller change — both uphold the same contract."""
    if connect is not None:
        return PostgresFleetLedger(dsn=dsn, connect=connect)
    from src.teleon.storage.record_store import psycopg_available, resolve_postgres_dsn
    resolved = resolve_postgres_dsn() if dsn is None else dsn
    if resolved is not None and psycopg_available():
        return PostgresFleetLedger(dsn=resolved)
    return DurableFleetLedger(path)


__all__ = ["DurableFleetLedger", "PostgresFleetLedger", "open_fleet_ledger", "DEFAULT_DB"]
