...
engine. Teleon layer — never imports src.baltor.
  ...
  python3 _repos/teleon/backend/src/teleon/examples/product_pipelines.py --self-test
"""
...
REPO = next((_ar for _ar in Path(__file__).resolve().parents if (_ar / ".aidoneright-root").exists()), Path(__file__).resolve().parents[3])   # _repos/teleon/backend/src/teleon/examples/ -> repo root
...
def openhubs_compose_pipeline() -> Pipeline:
    ...
    from src.teleon.registry.search import py_function_src_teleon_registry_search__build_capability
    from src.teleon.registry.compose import py_function_src_teleon_registry_compose__compile_universal

    def s_search(state):
        return {"ingredients": py_function_src_teleon_registry_search__build_capability(state["intent"])}

    def s_compose(state):
        ing = state["ingredients"]["ingredients_by_registry"]
        plan = py_function_src_teleon_registry_compose__compile_universal(state["intent"])
        return {"output": {
...
def openhubs_populate_enrich_pipeline() -> Pipeline:
    ...
    from src.teleon.registry.populate import py_function_src_teleon_registry_populate__repo_to_record
    from src.teleon.registry.enrich import py_function_src_teleon_registry_enrich__enrich_record

    def s_record(state):
        return {"record": py_function_src_teleon_registry_populate__repo_to_record(state["slug"], source="examples", desc=state["desc"])}

    def s_enrich(state):
        enriched = py_function_src_teleon_registry_enrich__enrich_record(state["record"])
