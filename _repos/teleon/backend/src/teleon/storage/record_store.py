...
(_repos/shared-backend-components/architecture/storage_tier_policy.json), not a code rewrite. Callers write through this port; the
...
_POLICY_PATH = _resource("architecture") / "storage_tier_policy.json"

# Single source for the operational Postgres DSN env-var names (No Magic Values, _repos/shared-backend-components/docs/codex/no-magic-values.md).
...
def tier_policy() -> dict:
    """The whole storage tier policy (single source: _repos/shared-backend-components/architecture/storage_tier_policy.json)."""
...
def open_record_store(stream: str, *, shard: str = "000", mode: str | None = None) -> RecordStore:
    ...
    if backend == "sqlite_wal":
        jsonl = _resource(spec["jsonl"].replace("{shard}", shard))
        return cls(jsonl)
