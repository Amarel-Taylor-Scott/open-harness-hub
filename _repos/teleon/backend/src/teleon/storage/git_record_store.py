...
record is evidence, never a truth claim); Teleon-layer — never imports src.baltor.
    ...
    PYTHONPATH=. python3 _repos/teleon/backend/src/teleon/storage/git_record_store.py --self-test
"""
...
from src.teleon.registry.enrich import py_var_src_teleon_registry_enrich___EMBED_DIM as EMBED_DIM  # noqa: E402  deterministic-embed dim (single source)
from src.teleon.registry.enrich import py_function_src_teleon_registry_enrich__embedding  # noqa: E402  deterministic lexical embedder (a learned one drops in)
...
class GitRecordStore(RecordStore):
    ...
    def promote(self, record_hash: str, *, losers: list[str] | None = None, now: str | None = None) -> dict:
        ...
        self.mirror.upsert(record_hash, record, py_function_src_teleon_registry_enrich__embedding(record))  # mirror the operational projection
    ...
    def search(self, query: dict | list[float], k: int = 5) -> list[tuple[str, float, dict]]:
        ...
        vector = query if isinstance(query, list) else py_function_src_teleon_registry_enrich__embedding(query)
...
def self_test() -> int:
    ...
    pg.upsert(h_alpha, r_alpha, py_function_src_teleon_registry_enrich__embedding(r_alpha))
    pg.upsert(h_beta, r_beta, py_function_src_teleon_registry_enrich__embedding(r_beta))
    assert pg.count() == 2
    assert any(pgvector_type(EMBED_DIM) in sql for sql in fake.executed), "DDL must use pgvector_type (no literal dim)"
    pg_hits = pg.search(py_function_src_teleon_registry_enrich__embedding(r_alpha), k=1)
