...
Guarantees:
  ...
  - NO CYCLES: stdlib-only and imports nothing from the rest of src/teleon, so oips, adapters, and
    repo tooling (_repos/shared-backend-components/scripts/) may all import it freely.
"""
...
_REPO_ROOT = next((_ar for _ar in Path(__file__).resolve().parents if (_ar / ".aidoneright-root").exists()), Path(__file__).resolve().parents[3])
#: the ONE durable receipt sink shared by every model-call plane (append-only JSONL, one receipt per line)
RECEIPTS_JSONL_PATH = _resource("dist") / "local-services-state" / "model-receipts" / "receipts.jsonl"
