...
# architecture/ moved under _repos/shared-backend-components/ in the _repos/ migration; resolve it through the
# single-source migration-aware resolver rather than a repo-root join (which pointed at the now-gone root architecture/).
...
def select_provider(resolved: dict, *, available_secrets: set | None = None,
    ...
    that can never violate policy. See _repos/teleon/context/architecture/flexible-inference-lanes.md
    (the 'Model-efficiency routing' section, merged there 2026-07-03)."""
