...
# the open layer (allowed: teleon may_depend_on openhubforai)
...
def plan(intent: str, *, hubs: list[str], kinds: dict | None = None, available: set | None = None,
    ...
    t = (intent or "").lower()
    steps = [
        PlanStep("research_select", "openhubforai.research_catalog.select_component",
                 f"descend the research catalog → cheapest component for '{cap}' (selected: {rd.get('selected')})"),
        PlanStep("discover", "openhubforai.discovery.OpenClaw/Hermes",
                 "run the hub's finders across the (allowed) tool repository — continuous append"),
        PlanStep("digest", "openhubforai.hub_engine.HubEngine.digest", "normalize + enrich each candidate"),
        PlanStep("dedupe", "openhubforai.component_store (content-hash)", "collapse duplicates (idempotent, lossless versions)"),
    ]
    if any(h in t for h in _IMPROVE_HINTS):
        steps.append(PlanStep("improve", "openhubforai.hub_engine.HubEngine.improve",
                              "ask the model for one concrete improvement → a new lossless version"))
    steps += [
        PlanStep("verify", "openhubforai.hub_engine.HubEngine.verify", "the hub's verify gate (discovery≠trust)"),
        PlanStep("version", "openhubforai.component_store.put_version", "append a new governed version"),
    ]
    if any(h in t for h in _CONTRIBUTE_HINTS):
        steps.append(PlanStep("contribute", "openhubforai.hub_engine.HubEngine.contribute",
