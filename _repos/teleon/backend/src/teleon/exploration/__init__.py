...
ARCHITECTURAL LAW (_repos/shared-backend-components/architecture/portfolio_dependency_law.json): Teleon-layer code. Imports only the stdlib +
``src.teleon`` siblings — NEVER ``src.baltor`` / ``src.openhubforai``. Deterministic when ``now`` is injected
(content-addressed ids; no RNG / no wall-clock). Doc: _repos/teleon/context/architecture/open-ended-exploration-ladder.md.
"""
from .ladder import (
    escalation_decision,
    EscalationDecision,
