...
real services:
  ...
  - real_search     real GROUNDED sources from FREE, no-key APIs: Wikipedia REST + the US Federal Register (GPO) — both
                    return citable source URLs (provenance). Registered in _repos/shared-backend-components/architecture/search_provider_registry.json.
...
def _get_json(url: str, *, timeout: int = 25) -> dict:
    req = urllib.request.Request(url, headers={"User-Agent": "OpenHubForAI-Teleon/0.1 (real_steps)"})
...
_SEARCH_REGISTRY = _resource("architecture") / "search_provider_registry.json"
...
def real_search(query: str, *, provider: str | None = None, limit: int = 3) -> list[dict]:
    """Dispatch to a real grounded provider SELECTED from _repos/shared-backend-components/architecture/search_provider_registry.json. provider=None ->
