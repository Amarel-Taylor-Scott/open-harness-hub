...
_repos/shared-backend-components/architecture/semantic_field_ontology.json and resolves any surface name -> its canonical id deterministically
...
_REPO = next((_ar for _ar in Path(__file__).resolve().parents if (_ar / ".aidoneright-root").exists()), Path(__file__).resolve().parents[3])          # _repos/teleon/backend/src/teleon/resolution/ -> repo root
_ONTOLOGY = _resource("architecture") / "semantic_field_ontology.json"
