...
// landing body sections (named so the shipped section order composes them)
function SecHow() {
  return (
...
// landing section orderings (TLN_LAYOUTS.default is shipped; the A/B re-sequencing was retired 2026-06-27)
...
function Landing({ theme, onToggle }) {
  // Kept only for analytics tracking. The teleon_hero / teleon_landing A/B experiments, the
  // in-hero A/B pill, and the floating OhExperimentsPanel were retired 2026-06-27 — Teleon is a
  // real product, not an experiment surface; the hero line + section order are fixed below.
  const [, hero] = useExperiment('teleon_hero', ['A', 'B', 'C', 'D']);
  const order = TLN_LAYOUTS.default;
  const heroLine = HERO_LINES.A;
  ...
  return (
    <div className={'oh dir-s theme-' + theme + ' oh-site tln'} style={{ '--accent': ACCENT }}>
      ...
      <OhFooter brand={BRAND} tagline={`${T.kind} · part of AI Done Right`} cols={[
        ...
        ['Group', [['AI Done Right ↗', '../context-is-everything/Context is Everything.html'], ['Baltor.ai ↗', '../context-enrichment/Context Enrichment.html'], ['OpenHubForAI ↗', '../openhubforai/OpenHubForAI.html']]],
      ]} />
    </div>
  );
}
