<!-- GENERATED by scripts/deploy/generate_provider_configs.py — edit architecture/deploy_topology.json, then re-run; do not hand-edit -->

# Fly.io deploy — generated plan

Region: `iad` (fallback `ord`). One Fly app per service; internal services have NO public proxy and talk over the private 6PN network (`<app>.internal:<port>`). Full owner/agent runbook: `_repos/shared-backend-components/context/architecture/fly-deploy-runbook.md`.

Deploy order (dependencies first):

## 1. aidr-identity

```bash
fly apps create aidr-identity
fly volumes create identity_state -a aidr-identity -r iad -s 1
fly secrets set -a aidr-identity --stage OH_LLM_BASE_URL=<value> OH_LLM_MODEL=<value> OH_LLM_API_KEY=<value> OH_EMBED_MODE=<value> OH_EMBED_MODEL=<value> OH_EMBED_BASE_URL=<value> OH_INFERENCE_ALLOW_NETWORK=<value> RESEND_API_KEY=<value> POSTMARK_API_KEY=<value> EMAIL_FROM=<value> EMAIL_ADAPTER=<value> AIDR_PUBLIC_BASE=<value>
(cd _repos && fly deploy -c shared-backend-components/fly/aidr-identity.fly.toml --dockerfile shared-backend-components/deploy/Dockerfile --ha=false)
```

## 2. aidr-registry

```bash
fly apps create aidr-registry
fly volumes create registry_state -a aidr-registry -r iad -s 1
fly secrets set -a aidr-registry --stage AIDR_REGISTRY_ADMINS=<value>
(cd _repos && fly deploy -c shared-backend-components/fly/aidr-registry.fly.toml --dockerfile shared-backend-components/deploy/Dockerfile --ha=false)
```

## 3. aidr-events

```bash
fly apps create aidr-events
fly volumes create events_state -a aidr-events -r iad -s 1
(cd _repos && fly deploy -c shared-backend-components/fly/aidr-events.fly.toml --dockerfile shared-backend-components/deploy/Dockerfile --ha=false)
```

## 4. aidr-receipt

```bash
fly apps create aidr-receipt
fly volumes create receipt_state -a aidr-receipt -r iad -s 1
(cd _repos && fly deploy -c shared-backend-components/fly/aidr-receipt.fly.toml --dockerfile shared-backend-components/deploy/Dockerfile --ha=false)
```

## 5. aidr-state

```bash
fly apps create aidr-state
fly volumes create state_state -a aidr-state -r iad -s 1
(cd _repos && fly deploy -c shared-backend-components/fly/aidr-state.fly.toml --dockerfile shared-backend-components/deploy/Dockerfile --ha=false)
```

## 6. aidr-mailbox

```bash
fly apps create aidr-mailbox
fly volumes create mailbox_state -a aidr-mailbox -r iad -s 1
(cd _repos && fly deploy -c shared-backend-components/fly/aidr-mailbox.fly.toml --dockerfile shared-backend-components/deploy/Dockerfile --ha=false)
```

## 7. aidr-teleon-runtime

```bash
fly apps create aidr-teleon-runtime
fly volumes create teleon_runtime_state -a aidr-teleon-runtime -r iad -s 1
fly secrets set -a aidr-teleon-runtime --stage OH_LLM_BASE_URL=<value> OH_LLM_MODEL=<value> OH_LLM_API_KEY=<value> OH_EMBED_MODE=<value> OH_EMBED_MODEL=<value> OH_EMBED_BASE_URL=<value> OH_INFERENCE_ALLOW_NETWORK=<value> FLY_API_TOKEN=<value>
(cd _repos && fly deploy -c shared-backend-components/fly/aidr-teleon-runtime.fly.toml --dockerfile shared-backend-components/deploy/Dockerfile --ha=false)
```

## 8. aidr-baltor-backend

```bash
fly apps create aidr-baltor-backend
fly volumes create baltor_backend_state -a aidr-baltor-backend -r iad -s 1
fly secrets set -a aidr-baltor-backend --stage OH_LLM_BASE_URL=<value> OH_LLM_MODEL=<value> OH_LLM_API_KEY=<value> OH_EMBED_MODE=<value> OH_EMBED_MODEL=<value> OH_EMBED_BASE_URL=<value> OH_INFERENCE_ALLOW_NETWORK=<value> REDIS_URL=<value>
(cd _repos && fly deploy -c shared-backend-components/fly/aidr-baltor-backend.fly.toml --dockerfile shared-backend-components/deploy/Dockerfile --ha=false)
```

## 9. aidr-observer

```bash
fly apps create aidr-observer
fly volumes create observer_state -a aidr-observer -r iad -s 1
(cd _repos && fly deploy -c shared-backend-components/fly/aidr-observer.fly.toml --dockerfile shared-backend-components/deploy/Dockerfile --ha=false)
```

## 10. aidr-web-openhubforai

```bash
fly apps create aidr-web-openhubforai
fly secrets set -a aidr-web-openhubforai --stage OH_LLM_BASE_URL=<value> OH_LLM_MODEL=<value> OH_LLM_API_KEY=<value> OH_EMBED_MODE=<value> OH_EMBED_MODEL=<value> OH_EMBED_BASE_URL=<value> OH_INFERENCE_ALLOW_NETWORK=<value> OH_SHOWCASE_TOKEN=<value>
(cd _repos && fly deploy -c shared-backend-components/fly/aidr-web-openhubforai.fly.toml --dockerfile shared-backend-components/deploy/Dockerfile --ha=false)
```

## 11. aidr-web-baltor

```bash
fly apps create aidr-web-baltor
fly secrets set -a aidr-web-baltor --stage OH_LLM_BASE_URL=<value> OH_LLM_MODEL=<value> OH_LLM_API_KEY=<value> OH_EMBED_MODE=<value> OH_EMBED_MODEL=<value> OH_EMBED_BASE_URL=<value> OH_INFERENCE_ALLOW_NETWORK=<value> OH_SHOWCASE_TOKEN=<value>
(cd _repos && fly deploy -c shared-backend-components/fly/aidr-web-baltor.fly.toml --dockerfile shared-backend-components/deploy/Dockerfile --ha=false)
```

## 12. aidr-web-context

```bash
fly apps create aidr-web-context
fly secrets set -a aidr-web-context --stage OH_LLM_BASE_URL=<value> OH_LLM_MODEL=<value> OH_LLM_API_KEY=<value> OH_EMBED_MODE=<value> OH_EMBED_MODEL=<value> OH_EMBED_BASE_URL=<value> OH_INFERENCE_ALLOW_NETWORK=<value> OH_SHOWCASE_TOKEN=<value>
(cd _repos && fly deploy -c shared-backend-components/fly/aidr-web-context.fly.toml --dockerfile shared-backend-components/deploy/Dockerfile --ha=false)
```

## 13. aidr-web-teleon

```bash
fly apps create aidr-web-teleon
fly secrets set -a aidr-web-teleon --stage OH_LLM_BASE_URL=<value> OH_LLM_MODEL=<value> OH_LLM_API_KEY=<value> OH_EMBED_MODE=<value> OH_EMBED_MODEL=<value> OH_EMBED_BASE_URL=<value> OH_INFERENCE_ALLOW_NETWORK=<value> OH_SHOWCASE_TOKEN=<value>
(cd _repos && fly deploy -c shared-backend-components/fly/aidr-web-teleon.fly.toml --dockerfile shared-backend-components/deploy/Dockerfile --ha=false)
```

## 14. aidr-web-aidevobserver

```bash
fly apps create aidr-web-aidevobserver
fly secrets set -a aidr-web-aidevobserver --stage OH_LLM_BASE_URL=<value> OH_LLM_MODEL=<value> OH_LLM_API_KEY=<value> OH_EMBED_MODE=<value> OH_EMBED_MODEL=<value> OH_EMBED_BASE_URL=<value> OH_INFERENCE_ALLOW_NETWORK=<value> OH_SHOWCASE_TOKEN=<value>
(cd _repos && fly deploy -c shared-backend-components/fly/aidr-web-aidevobserver.fly.toml --dockerfile shared-backend-components/deploy/Dockerfile --ha=false)
```

## 15. aidr-redis

```bash
fly apps create aidr-redis
fly volumes create redis_state -a aidr-redis -r iad -s 1
(cd _repos && fly deploy -c shared-backend-components/fly/aidr-redis.fly.toml --dockerfile shared-backend-components/deploy/Dockerfile --ha=false)
```

## 16. aidr-postgres — phase-2: the demo plane is file/SQLite-backed; Postgres+pgvector is the staged-row load target (scripts.db.*). Provision when load plans go live.

```bash
fly apps create aidr-postgres
fly volumes create postgres_state -a aidr-postgres -r iad -s 10
fly secrets set -a aidr-postgres --stage POSTGRES_PASSWORD=<value>
(cd _repos && fly deploy -c shared-backend-components/fly/aidr-postgres.fly.toml --dockerfile shared-backend-components/deploy/Dockerfile --ha=false)
```

## 17. aidr-worker

```bash
fly apps create aidr-worker
fly secrets set -a aidr-worker --stage OH_LLM_BASE_URL=<value> OH_LLM_MODEL=<value> OH_LLM_API_KEY=<value> OH_EMBED_MODE=<value> OH_EMBED_MODEL=<value> OH_EMBED_BASE_URL=<value> OH_INFERENCE_ALLOW_NETWORK=<value> OLLAMA_API_KEY=<value> OLLAMA_HOST=<value> MISTRAL_API_KEY=<value> OPENROUTER_API_KEY=<value> OH_CHAT_MODEL=<value> OH_EMBED_MODEL=<value> REDIS_URL=<value> DATABASE_URL=<value>
(cd _repos && fly deploy -c shared-backend-components/fly/aidr-worker.fly.toml --dockerfile shared-backend-components/deploy/Dockerfile --ha=false)
fly machine list -a aidr-worker -q | xargs -r -n1 fly machine stop -a aidr-worker  # controller owns the fleet from here
```

## 18. aidr-worker-controller

```bash
fly apps create aidr-worker-controller
fly secrets set -a aidr-worker-controller --stage FLY_API_TOKEN=<value> REDIS_URL=<value> DATABASE_URL=<value>
(cd _repos && fly deploy -c shared-backend-components/fly/aidr-worker-controller.fly.toml --dockerfile shared-backend-components/deploy/Dockerfile --ha=false)
```

After deploys: point Cloudflare DNS at the public apps (CNAME → <app>.fly.dev, proxied), then run the public gates (e2e/ohh_public_gate.mjs, e2e/teleon_gate.mjs) against the domains.

## Scheduled feeder (keep the worker queue fed)

The worker fleet drains the `ohh:foundry:jobs` queue; feed it on a schedule with Fly scheduled machines on the worker app (run from the repo root — `.` builds `shared-backend-components/deploy/Dockerfile`; or pass the app's currently deployed image ref instead):

```bash
fly machine run . --schedule daily -a aidr-worker --command "python -m scripts.foundry.seeds --enqueue"
fly machine run . --schedule daily -a aidr-worker --command "python -m scripts.ingest.freshness --check --enqueue"
```

`seeds --enqueue` puts one partition job per research-queue area on the broker (add `--with-fixture` for a guaranteed-promoting smoke partition); `freshness --check --enqueue` polls every registered source (CDC) and enqueues a `reingest` job per new/changed source — the worker re-feeds it through `scripts.ingest.feed`.

## Single-machine law (stateful apps)

Every app below owns SINGLE-WRITER state (JSON/JSONL/SQLite on its volume). Never `fly scale count >1` on any of them — a second machine gets a second empty volume and forks the state; `--ha=false` at deploy time is mandatory, not an optimization. Scale these UP (bigger VM), never OUT:

- `aidr-identity` — single-writer state at `/app/dist/identity` (volume `identity_state`)
- `aidr-registry` — single-writer state at `/app/dist/registry` (volume `registry_state`)
- `aidr-events` — single-writer state at `/app/dist/analytics` (volume `events_state`)
- `aidr-receipt` — single-writer state at `/app/dist/local-services-state` (volume `receipt_state`)
- `aidr-state` — single-writer state at `/app/dist/local-services-state` (volume `state_state`)
- `aidr-mailbox` — single-writer state at `/app/dist/email-outbox` (volume `mailbox_state`)
- `aidr-teleon-runtime` — single-writer state at `/app/dist/local-services-state` (volume `teleon_runtime_state`)
- `aidr-baltor-backend` — single-writer state at `/app/dist/local-services-state` (volume `baltor_backend_state`)
- `aidr-observer` — single-writer state at `/app/dist/local-services-state` (volume `observer_state`)
- `aidr-redis` — single-writer state at `/data` (volume `redis_state`)
- `aidr-postgres` — single-writer state at `/data` (volume `postgres_state`)

## Volume snapshots (+ off-provider backup)

Fly bills snapshot storage — pin retention to 5 days on every volume (`fly volumes list -a <app>` shows the volume id):

```bash
fly volumes update <identity_state-id> --snapshot-retention 5  # app aidr-identity, volume identity_state
fly volumes update <registry_state-id> --snapshot-retention 5  # app aidr-registry, volume registry_state
fly volumes update <events_state-id> --snapshot-retention 5  # app aidr-events, volume events_state
fly volumes update <receipt_state-id> --snapshot-retention 5  # app aidr-receipt, volume receipt_state
fly volumes update <state_state-id> --snapshot-retention 5  # app aidr-state, volume state_state
fly volumes update <mailbox_state-id> --snapshot-retention 5  # app aidr-mailbox, volume mailbox_state
fly volumes update <teleon_runtime_state-id> --snapshot-retention 5  # app aidr-teleon-runtime, volume teleon_runtime_state
fly volumes update <baltor_backend_state-id> --snapshot-retention 5  # app aidr-baltor-backend, volume baltor_backend_state
fly volumes update <observer_state-id> --snapshot-retention 5  # app aidr-observer, volume observer_state
fly volumes update <redis_state-id> --snapshot-retention 5  # app aidr-redis, volume redis_state
fly volumes update <postgres_state-id> --snapshot-retention 5  # app aidr-postgres, volume postgres_state
```

Snapshots live on the same provider as the volumes — also run a nightly off-provider backup (e.g. a scheduled machine running restic from each state mount to Cloudflare R2 or Backblaze B2) so a provider/account-level incident can't take the state and its only copies together.
