...
CLI: PYTHONPATH=. python3 _repos/shared-backend-components/scripts/check_parallel_path_contracts.py --self-test
"""
...
_SCHEMA_DIR = _resource("schemas") / "experiments"
_EX_DIR = _SCHEMA_DIR / "examples"
_REGISTRY = _resource("architecture") / "contract_registry.json"
...
def _self_test() -> int:
    ...
    if pcost_valid and pcost_sc:
        ...
        pb = json.loads((_resource("architecture") / "execution_backend_pricebook.json").read_text())
    ...
    missing = [item["schema"] for item in reg.get("artifact_types", [])
               ...
               and not (_resource(item["schema"])).exists()]
