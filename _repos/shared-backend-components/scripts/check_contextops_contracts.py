...
CLI: PYTHONPATH=. python3 _repos/shared-backend-components/scripts/check_contextops_contracts.py --self-test
"""
...
_SCHEMA_DIR = _resource("schemas") / "contextops"
...
def _self_test() -> int:
    ...
    # ── every schema parses, declares $id contextops/<Stem>, is an object with additionalProperties:false,
    ...
    for stem, sc in schemas.items():
        check(f"{stem} $id is contextops/{stem}",
              sc.get("$id") == f"contextops/{stem}", str(sc.get("$id")))
        ...
        check(f"{stem} uses only stdlib-validator keywords (no false-sense pattern/minimum/anyOf)", _keys_ok(sc))
    ...
    # ── ContextTriageResult: the 12 lanes + needs_action + a triage never serves truth. ──
    tr_sc = schemas.get("ContextTriageResult", {})
    check("ContextTriageResult lane enum is EXACTLY the 12 canonical triage lanes",
          set(_enum_of(tr_sc, "lane")) == _TRIAGE_LANES,
          str(sorted(_TRIAGE_LANES ^ set(_enum_of(tr_sc, "lane")))))
    check("ContextTriageResult requires needs_action + lane + fact_key",
          {"needs_action", "lane", "fact_key"} <= _req(tr_sc))
    check("ContextTriageResult pins serves_truth == [false] (a triage is a routing signal, never a fact)",
    ...
    if tr_valid and tr_sc:
        ...
        check("ContextTriageResult rejects an out-of-enum lane", _validate(bad_lane, tr_sc) != [])
        serves = dict(tr_valid); serves["serves_truth"] = True
        check("ContextTriageResult rejects serves_truth=true (red-team: a triage cannot be served as truth)",
              _validate(serves, tr_sc) != [])

    # ── ResearchTask: agent is bounded, produces only a discovery report, never serves truth, no secrets. ──
    rt_sc = schemas.get("ResearchTask", {})
    check("ResearchTask produces pinned [source_discovery_report] (research yields candidate sources, NEVER a fact)",
          _enum_of(rt_sc, "produces") == ["source_discovery_report"])
    check("ResearchTask pins agent_may_serve_truth == [false] (THE INVARIANT — agents discover/propose, never serve)",
          _enum_of(rt_sc, "agent_may_serve_truth") == [False])
    check("ResearchTask requires bounds + question + authority_bar (a bounded research assignment)",
          {"bounds", "question", "authority_bar"} <= _req(rt_sc))
    check("ResearchTask bounds.secrets_allowed pinned [false] (a task never grants secrets)",
    ...
    if rt_valid and rt_sc:
        ...
        check("ResearchTask rejects agent_may_serve_truth=true (red-team: agent cannot be told to serve truth)",
              _validate(serves, rt_sc) != [])
        prod = json.loads(json.dumps(rt_valid)); prod["produces"] = "canonical_fact"
        check("ResearchTask rejects produces='canonical_fact' (research never produces a fact)",
              _validate(prod, rt_sc) != [])
        sec = json.loads(json.dumps(rt_valid)); sec["bounds"]["secrets_allowed"] = True
        check("ResearchTask rejects bounds.secrets_allowed=true", _validate(sec, rt_sc) != [])

    # ── ResearchPlan: a plan's execution yields a discovery report, its steps never serve a fact. ──
    rp_sc = schemas.get("ResearchPlan", {})
    check("ResearchPlan produces pinned [source_discovery_report]",
          _enum_of(rp_sc, "produces") == ["source_discovery_report"])
    check("ResearchPlan requires steps + step_budget + stop_conditions",
    ...
    check("ResearchPlan step actions are discovery-only (no 'serve_fact'/'promote')",
          step_actions and "serve_fact" not in step_actions and "promote" not in step_actions, str(sorted(step_actions)))

    # ── SourceDiscoveryReport: attributable discoverer, never serves truth, every candidate has a handle. ──
    sdr_sc = schemas.get("SourceDiscoveryReport", {})
    check("SourceDiscoveryReport requires discovered_by (an attributable, non-serving agent)",
          "discovered_by" in _req(sdr_sc))
    check("SourceDiscoveryReport pins serves_truth == [false] (a discovery report can NEVER be served as truth)",
          _enum_of(sdr_sc, "serves_truth") == [False])
    cand_item = sdr_sc.get("properties", {}).get("candidates", {}).get("items", {})
    check("SourceDiscoveryReport every candidate REQUIRES a source_handle",
    ...
    if sdr_valid and sdr_sc:
        ...
        check("SourceDiscoveryReport rejects serves_truth=true (red-team)", _validate(serves, sdr_sc) != [])
        nohandle = json.loads(json.dumps(sdr_valid))
        nohandle["candidates"] = [{"candidate_id": "c1", "authority_note": "no handle"}]
        check("SourceDiscoveryReport rejects a candidate that dropped its source_handle",
              _validate(nohandle, sdr_sc) != [])

    # ── SourceCandidate: requires source_handle + authority_rank (handle inadmissible-without; authority comparable). ──
    sc_sc = schemas.get("SourceCandidate", {})
    check("SourceCandidate requires source_handle + authority_rank (no handle => inadmissible; authority comparable)",
    ...
    if sc_valid and sc_sc:
        ...
        check("SourceCandidate rejects a candidate missing source_handle", _validate(nohandle, sc_sc) != [])
        norank = {k: v for k, v in sc_valid.items() if k != "authority_rank"}
        check("SourceCandidate rejects a candidate missing authority_rank", _validate(norank, sc_sc) != [])

    # ── SourceReliabilityScore / FactReliabilityScore: a score ranks, it never serves truth. ──
    for stem, key in (("SourceReliabilityScore", "candidate"), ("FactReliabilityScore", "fact")):
        rs_sc = schemas.get(stem, {})
        check(f"{stem} requires authority_rank and an enumerated factors block",
              ("authority_rank" in _req(rs_sc) or "winning_authority_rank" in _req(rs_sc)) and "factors" in _req(rs_sc))
        check(f"{stem} pins served_as_truth == [false] (a reliability score is never itself a served fact)",
        ...
        check(f"{stem} factors block is closed (additionalProperties:false)",
              rs_sc.get("properties", {}).get("factors", {}).get("additionalProperties") is False)

    # ── SourceRecipe: authority + cross_source policy + watch triggers (M2 rung is encoded, not re-derived). ──
    sr_sc = schemas.get("SourceRecipe", {})
    check("SourceRecipe requires authority + cross_source + watch_triggers",
          {"authority", "cross_source", "watch_triggers"} <= _req(sr_sc))
    check("SourceRecipe authority block requires authority_rank + source_type (reconciliation precedence encoded)",
          {"authority_rank", "source_type"} <= set(sr_sc.get("properties", {}).get("authority", {}).get("required", [])))
    check("SourceRecipe cross_source policy enum includes the official/two-source/tenant-signoff policies",
    ...
    if sr_valid and sr_sc:
        for f in ("cross_source", "watch_triggers", "authority"):
            ...
            check(f"SourceRecipe rejects a recipe missing '{f}'", _validate(broken, sr_sc) != [])

    # ── VerificationRecipe: authority + cross_source + freshness + tenant_scope; produces a run, not a fact;
    #    a FAQ can never be the winning source. ──
    vr_sc = schemas.get("VerificationRecipe", {})
    check("VerificationRecipe requires authority + cross_source + freshness + tenant_scope + validators",
          {"authority", "cross_source", "freshness", "tenant_scope", "validators"} <= _req(vr_sc))
    check("VerificationRecipe produces pinned [fact_verification_run] (a recipe verifies; it never promotes a fact)",
          _enum_of(vr_sc, "produces") == ["fact_verification_run"])
    winning = set(_nested_enum(vr_sc, "authority", "winning_source_type"))
    check("VerificationRecipe winning_source_type can NEVER be a FAQ/secondary (only high-authority sources win)",
    ...
    if vr_valid and vr_sc:
        ...
        check("VerificationRecipe rejects produces='canonical_fact'", _validate(prod, vr_sc) != [])
        faq = json.loads(json.dumps(vr_valid)); faq["authority"]["winning_source_type"] = "agency_faq"
        check("VerificationRecipe rejects winning_source_type='agency_faq' (red-team: FAQ cannot win)",
              _validate(faq, vr_sc) != [])

    # ── ExtractorSnippet: THE load-bearing invariant — output is ALWAYS a candidate, sandboxed + unit-tested. ──
    es_sc = schemas.get("ExtractorSnippet", {})
    check("ExtractorSnippet produces pinned [fact_assertion_candidate] (NEVER served/canonical truth)",
          _enum_of(es_sc, "produces") == ["fact_assertion_candidate"])
    check("ExtractorSnippet claim_status pinned [candidate]", _enum_of(es_sc, "claim_status") == ["candidate"])
    check("ExtractorSnippet has_unit_test pinned [true] (no untested extractor)",
          _enum_of(es_sc, "has_unit_test") == [True])
    check("ExtractorSnippet sandbox_required pinned [true] (runs in a sandbox before any use)",
          _enum_of(es_sc, "sandbox_required") == [True])
    check("ExtractorSnippet requires source_handle (the candidate is traceable to a source)",
    ...
    if es_valid and es_sc:
        ...
        check("ExtractorSnippet REJECTS produces='canonical_fact' (red-team: extractor can't claim canonical truth)",
              _validate(canon, es_sc) != [])
        served = dict(es_valid); served["produces"] = "served_fact"
        check("ExtractorSnippet REJECTS produces='served_fact'", _validate(served, es_sc) != [])
        notest = dict(es_valid); notest["has_unit_test"] = False
        check("ExtractorSnippet REJECTS has_unit_test=false (red-team: untested extractor)", _validate(notest, es_sc) != [])
        nohandle = {k: v for k, v in es_valid.items() if k != "source_handle"}
        check("ExtractorSnippet REJECTS an extractor that dropped its source_handle", _validate(nohandle, es_sc) != [])
        nosandbox = dict(es_valid); nosandbox["sandbox_required"] = False
        check("ExtractorSnippet REJECTS sandbox_required=false", _validate(nosandbox, es_sc) != [])

    # ── GeneratedWorkerSpec: proofs + idempotency + retry/DLQ; rides existing framework; never auto-active. ──
    gw_sc = schemas.get("GeneratedWorkerSpec", {})
    check("GeneratedWorkerSpec requires proof_scripts + idempotency_key_template + retry_policy",
          {"proof_scripts", "idempotency_key_template", "retry_policy"} <= _req(gw_sc))
    check("GeneratedWorkerSpec retry_policy requires max_attempts + backoff + dlq (retry/DLQ discipline)",
          {"max_attempts", "backoff", "dlq"} <= set(gw_sc.get("properties", {}).get("retry_policy", {}).get("required", [])))
    check("GeneratedWorkerSpec rides_existing_worker_framework pinned [true] (no 2nd runtime/worker framework)",
          _enum_of(gw_sc, "rides_existing_worker_framework") == [True])
    check("GeneratedWorkerSpec registration_status pinned [proposed] (nothing auto-registers/executes this pass)",
    ...
    if gw_valid and gw_sc:
        ...
        check("GeneratedWorkerSpec REJECTS registration_status='active' (red-team: unverified worker auto-registered)",
              _validate(active, gw_sc) != [])
        runtime = json.loads(json.dumps(gw_valid)); runtime["rides_existing_worker_framework"] = False
        check("GeneratedWorkerSpec REJECTS rides_existing_worker_framework=false (red-team: 2nd runtime)",
              _validate(runtime, gw_sc) != [])
        noretry = {k: v for k, v in gw_valid.items() if k != "retry_policy"}
        check("GeneratedWorkerSpec REJECTS a worker spec missing retry_policy", _validate(noretry, gw_sc) != [])

    # ── VerifierProofResult: sandbox_passed + proof_passed + gate_decision; no secrets in the sandbox. ──
    vp_sc = schemas.get("VerifierProofResult", {})
    check("VerifierProofResult requires sandbox_passed + proof_passed + gate_decision",
          {"sandbox_passed", "proof_passed", "gate_decision"} <= _req(vp_sc))
    check("VerifierProofResult gate_decision enum ⊆ {approved, rejected, needs_human_approval}",
          set(_enum_of(vp_sc, "gate_decision")) == {"approved", "rejected", "needs_human_approval"})
    check("VerifierProofResult sandbox_report.secrets_present pinned [false] (a sandbox run carries no secrets)",
          vp_sc.get("properties", {}).get("sandbox_report", {}).get("properties", {})
          .get("secrets_present", {}).get("enum") == [False])

    # ── FactRefreshPlan: watch mode + injected last_verified_at + computed next_check_at; refresh != fact. ──
    fr_sc = schemas.get("FactRefreshPlan", {})
    check("FactRefreshPlan requires watch_mode + last_verified_at + next_check_at + refresh_interval_seconds",
          {"watch_mode", "last_verified_at", "next_check_at", "refresh_interval_seconds"} <= _req(fr_sc))
    check("FactRefreshPlan produces pinned [fact_verification_run] (a refresh runs verification, not a publish)",
          _enum_of(fr_sc, "produces") == ["fact_verification_run"])
    check("FactRefreshPlan last_verified_at is an integer (injected epoch seconds — no wall-clock)",
    ...
    if fr_valid and fr_sc:
        ...
        check("FactRefreshPlan valid example next_check_at == last_verified_at + refresh_interval_seconds (deterministic)",
              fr_valid.get("next_check_at") == fr_valid.get("last_verified_at") + fr_valid.get("refresh_interval_seconds"))

    # ── FactVerificationRun: a run is a candidate + receipt, never canonical; carries the cost-ladder moat. ──
    fv_sc = schemas.get("FactVerificationRun", {})
    check("FactVerificationRun claim_status pinned [candidate] (a run yields a candidate, not canonical truth)",
          _enum_of(fv_sc, "claim_status") == ["candidate"])
    check("FactVerificationRun promoted_to_canonical pinned [false] (a run never self-promotes; reconciliation decides)",
          _enum_of(fv_sc, "promoted_to_canonical") == [False])
    check("FactVerificationRun requires source_handles + receipt_ref + cost_tracking",
          {"source_handles", "receipt_ref", "cost_tracking"} <= _req(fv_sc))
    ct_req = set(fv_sc.get("properties", {}).get("cost_tracking", {}).get("required", []))
    check("FactVerificationRun cost_tracking requires the 6 cost-ladder moat metrics",
          _COST_METRICS <= ct_req, str(sorted(_COST_METRICS - ct_req)))
    check("FactVerificationRun cost_tracking block is closed (additionalProperties:false — the metric set can't drift)",
    ...
    if fv_valid and fv_sc:
        ...
        check("FactVerificationRun REJECTS promoted_to_canonical=true (red-team: a run can't self-promote)",
              _validate(promo, fv_sc) != [])
        served = json.loads(json.dumps(fv_valid)); served["claim_status"] = "served"
        check("FactVerificationRun REJECTS claim_status='served' (red-team: run output served as truth)",
              _validate(served, fv_sc) != [])
        for m in sorted(_COST_METRICS):
            broken = json.loads(json.dumps(fv_valid)); broken["cost_tracking"].pop(m, None)
            check(f"FactVerificationRun REJECTS a run missing cost metric '{m}'", _validate(broken, fv_sc) != [])
