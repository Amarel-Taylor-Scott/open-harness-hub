#!/usr/bin/env python3
"""scripts.check_standard_catalog — proof: _repos/shared-backend-components/architecture/standard_catalog.json is well-formed + grounded.
...
CLI: python3 _repos/shared-backend-components/scripts/check_standard_catalog.py --self-test
"""
...
def _self_test() -> int:
    ...
    for s in cat.get("standards", []):
        ...
        for ex in s.get("examples", []):
            check(f"{sid}: example path exists -> {ex}", (_resource(ex)).exists())
