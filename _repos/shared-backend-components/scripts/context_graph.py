...
CLI / self-test (proves interrogation on the Acme Billing demo graph; no model, no network):
    python3 _repos/shared-backend-components/scripts/context_graph.py --self-test
    python3 _repos/shared-backend-components/scripts/context_graph.py --ask "what is the retry ceiling?"
    python3 _repos/shared-backend-components/scripts/context_graph.py --neighbors obj-adr-014
"""
...
DEMO_SEED = _resource("demo-data") / "acme-billing" / "seed-graph.json"
