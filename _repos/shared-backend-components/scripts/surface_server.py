...
from config. Nothing here is hand-typed that lives elsewhere (No-Magic-Values):
  ...
  * surface ids + brand + role + capabilities  ← _repos/shared-backend-components/architecture/surface_capability_spec.json (pillars[])
...
serves_truth=false everywhere (candidate output; only Baltor's governed source answers serve truth).
  ...
  python3 _repos/shared-backend-components/scripts/surface_server.py <surface-id> [--port N]      # surface-id ∈ the 5 ids
"""
...
from src.teleon.registry.browse import py_const_src_teleon_registry_browse__FACET_DIMS, py_function_src_teleon_registry_browse__browse  # noqa: E402  (reuse the faceted browse + counts)

SPEC = _resource("architecture") / "surface_capability_spec.json"
SURFACE_URLS_FILE = _resource("dist") / "surface-urls.json"
...
def render_browse(surface_id: str, query: str = "", filters: dict | None = None) -> str:
    filters = filters or {}
    p = pillar(surface_id)
    b = py_function_src_teleon_registry_browse__browse(query, filters)

    facet_html = ""
    for dim in py_const_src_teleon_registry_browse__FACET_DIMS:
    ...
    active = "".join(
        ...
        for d in py_const_src_teleon_registry_browse__FACET_DIMS if filters.get(d)
    )
    ...
    hidden = "".join(
        f'<input type="hidden" name="{esc(d)}" value="{esc(filters[d])}">' for d in py_const_src_teleon_registry_browse__FACET_DIMS if filters.get(d)
    )
    search = (
