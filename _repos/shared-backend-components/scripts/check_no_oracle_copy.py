...
"Oracle" is retired as product language (_repos/shared-backend-components/docs/product-language.md). This fails if "oracle"
...
(_repos/baltor/frontend/oracle-hero.html), and the product-language doc itself. The *technical* term
"oracle source / oracle publisher" lives in _repos/shared-backend-components/docs/strategy + _repos/shared-backend-components/scripts/processors/assurance and is
NOT scanned here (this check targets user-facing UI + alias copy only).

CLI:
    python3 _repos/shared-backend-components/scripts/check_no_oracle_copy.py --self-test
    python3 _repos/shared-backend-components/scripts/check_no_oracle_copy.py            # scan; exit 1 on violations
"""
...
SCAN_GLOBS = ["_repos/baltor/frontend/**/*.html", "_repos/baltor/frontend/**/*.js", "data/alias-packs/*.yaml"]

#: Allowlisted files where "oracle" may legitimately appear.
ALLOWLIST = {
    "_repos/baltor/frontend/oracle-hero.html",                    # legacy redirect stub (deprecation comment)
