#!/usr/bin/env python3
"""scripts.check_fragile_context_audit_offers — PROOF + GENERATOR: _repos/shared-backend-components/docs/sales/fragile-context-audit-offers.md is
GENERATED from _repos/shared-backend-components/architecture/fragile_context_atlas.json (every distinct pack.sales_audit_offer becomes a catalogued
audit), connects to the existing sales subsystem (schemas/sales/* + _repos/shared-backend-components/docs/sales/public-claim-and-engagement-policy.md),
...
Asserts:
  ...
  C. POLICY: the doc cites _repos/shared-backend-components/docs/sales/public-claim-and-engagement-policy.md + the single-source policy files.
...
_TAXONOMY = _resource("architecture") / "fragile_context_taxonomy.json"
_ATLAS = _resource("architecture") / "fragile_context_atlas.json"
_DOC = _resource("docs") / "sales" / "fragile-context-audit-offers.md"
...
def render_offers_doc(atlas: dict, tax: dict) -> str:
    ...
    w(f"Everything passes through the safety gate in `{_POLICY}` "
      ...
      "guard `_repos/baltor/backend/src/baltor/sales/claim_guard.py`, redteam `scripts/check_sales_guardrails.py`).")
    w("")
