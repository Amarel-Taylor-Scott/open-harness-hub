#!/usr/bin/env python3
"""scripts.check_scripts_are_entrypoints — proof: top-level _repos/shared-backend-components/scripts/ files stay entrypoints/proofs/wrappers
...
mini-framework from being dropped into _repos/shared-backend-components/scripts/.

CLI: python3 _repos/shared-backend-components/scripts/check_scripts_are_entrypoints.py --self-test
"""
...
def _self_test() -> int:
    ...
    owners = json.loads((_resource("architecture") / "runtime_ownership.json").read_text())["owners"]
    declared_owner_files = {p for p in owners.values()}

    # rule 1: no top-level _repos/shared-backend-components/scripts/*.py defines a framework class unless it's that class's declared owner
    offenders = []
    for p in sorted((_resource("scripts")).glob("*.py")):
        rel = str(p.relative_to(_resource("scripts").parent))
    ...
    for p in sorted((_resource("scripts")).glob("*.py")):
        ...
        if "__main__" not in text and "argparse" not in text and "def _main" not in text:
            not_entry.append(str(p.relative_to(_resource("scripts").parent)))
    check("every check_/demo_/baltor_ script is a runnable entrypoint", not_entry == [], str(not_entry[:6]))

    # the declared owners that live in _repos/shared-backend-components/scripts/ root are the ONLY framework-class scripts (legacy, pending migration)
    ...
    check("declared owner files in scripts/ root exist (durable_store + context_events)",
          all((_resource(p)).exists() for p in legacy_owner_scripts) and len(legacy_owner_scripts) >= 2, str(legacy_owner_scripts))
