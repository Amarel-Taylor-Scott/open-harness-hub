...
_SCHEMA_PATH = _resource("schemas") / "agents" / "AgentBoundaryExpansionRequest.schema.json"
...
def _self_test() -> int:
    ...
    check("B: runtime status == 'pending_human_approval' (== module constant)",
          obj.get("status") == py_const_src_teleon_agent_gateway_gateway__BOUNDARY_STATUS_PENDING == "pending_human_approval", repr(obj.get("status")))
