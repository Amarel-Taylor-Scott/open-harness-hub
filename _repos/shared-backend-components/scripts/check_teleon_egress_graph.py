...
def _obs(**overrides):
    base = {
        "tenant_id": py_const_src_teleon_runtime_tenancy__INTERNAL_TENANT_ID,
...
def _self_test() -> int:
    ...
    blob = json.dumps(store.graph_for_query(tenant_id=py_const_src_teleon_runtime_tenancy__INTERNAL_TENANT_ID, text="Island Recruiters"), sort_keys=True)
    ...
    by_query = store.search_events(tenant_id=py_const_src_teleon_runtime_tenancy__INTERNAL_TENANT_ID, text="Island Recruiters")
    ...
    check("destination-host search works",
          len(store.search_events(tenant_id=py_const_src_teleon_runtime_tenancy__INTERNAL_TENANT_ID, destination_host="onlineservices.dmw.gov.ph")) == 1)
    check("worker search works",
          store.search_events(tenant_id=py_const_src_teleon_runtime_tenancy__INTERNAL_TENANT_ID, worker_id="worker-dole-advisory-search")[0]["status"]
          == "source_unavailable_requires_browser")

    graph = store.graph_for_query(tenant_id=py_const_src_teleon_runtime_tenancy__INTERNAL_TENANT_ID, text="Island Recruiters")
