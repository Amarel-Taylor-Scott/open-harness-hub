...
Stage-1 (_repos/shared-backend-components/scripts/check_parallel_path_contracts.py) proved the 6 contract SCHEMAS enforce the
non-negotiable parallel-path semantics. This proof exercises the Stage-2 RUNTIME under
``_repos/baltor/backend/src/baltor/experiments/`` and proves it actually does the right thing — not that files exist:
...
CLI: PYTHONPATH=. python3 _repos/shared-backend-components/scripts/check_parallel_path_implementation.py --self-test
"""
