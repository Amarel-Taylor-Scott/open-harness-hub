...
descent until promoted (the boundary). serves_truth=false; discovery≠trust. Dev-plane script.
  ...
  python3 _repos/shared-backend-components/scripts/harvest_tools.py --run 20          # harvest up to 20 queries this batch (rate-limited; resumable)
  python3 _repos/shared-backend-components/scripts/harvest_tools.py --stats           # computed counts over the staged layer
  python3 _repos/shared-backend-components/scripts/harvest_tools.py --self-test        # hermetic
"""
...
STAGING = _resource("data") / "dev-intel" / "tool_registry_staging.jsonl"
CURSOR = _resource("data") / "dev-intel" / "harvest_cursor.json"
...
def generate_queries() -> list[str]:
    ...
    tasks = json.loads((_resource("architecture") / "ml_model_registry.json").read_text())["ml_tasks"]
    qs = []
    for p in planes:
        name = p.replace("_", " ")
...
def _curated_ids() -> set[str]:
    reg = json.loads((_resource("architecture") / "tool_registry.json").read_text())["tools"]
