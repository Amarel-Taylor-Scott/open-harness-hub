"""check_registry_ontology — grounding + ANTI-FRAGMENTATION proof for _repos/shared-backend-components/architecture/registry_ontology.json.
...
_ONT = _resource("architecture") / "registry_ontology.json"
_PROFILES = _resource("architecture") / "hub_profiles.json"
...
def main() -> int:
    ...
    ck("universal_interface.contract points at a real file (the menu is realized)",
       bool(ui_contract) and (_resource(ui_contract)).exists(), str(ui_contract))
    ...
    ck("universal_object.schema points at a real file", bool(obj_schema_path) and (_resource(obj_schema_path)).exists(),
       str(obj_schema_path))
    entry_schema_path = doc.get("entry_schema", "")
    ck("entry_schema points at a real file", bool(entry_schema_path) and (_resource(entry_schema_path)).exists(),
       str(entry_schema_path))
    entry_schema = json.loads((_resource(entry_schema_path)).read_text()) if entry_schema_path and (_resource(entry_schema_path)).exists() else {}
    ...
    for r in regs:
        ...
        if status in {"live", "partial"}:
            ...
            for p in backing:
                ck(f"{rid}: backing path exists '{p}'", (_resource(p)).exists())
        elif status == "gap":
