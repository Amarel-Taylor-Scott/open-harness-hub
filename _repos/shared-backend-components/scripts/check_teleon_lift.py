...
Asserts:
  ...
  M. DEPENDENCY LAW (local): no module under _repos/teleon/backend/src/teleon/lift imports src.baltor.
...
_SCHEMA_DIR = _resource("schemas") / "teleon" / "lift"
...
def _self_test() -> int:
    ...
    # M. dependency law (local) — no src.baltor import under _repos/teleon/backend/src/teleon/lift
    bad = []
    for py in sorted((_resource("src/teleon/lift")).rglob("*.py")):
        for line in py.read_text().splitlines():
            s = line.strip()
            if (s.startswith("import src.baltor") or s.startswith("from src.baltor")):
                bad.append(str(py.relative_to(_REPO)))
    check("M: no _repos/teleon/backend/src/teleon/lift module imports src.baltor (dependency law)", not bad, "; ".join(bad))
