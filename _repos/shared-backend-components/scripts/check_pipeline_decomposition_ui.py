#!/usr/bin/env python3
"""scripts.check_pipeline_decomposition_ui — proof (C-PIPE-DECOMP-1): _repos/baltor/frontend/pipeline/decomposition.html
is the "Processing & Decomposition" stage page and is a PROJECTION ONLY. It fetches ONLY its contracted
/api/pipeline/decomposition (+ /api/pipeline/overview); it performs NO durable writes; it stores NO truth
in localStorage/sessionStorage/indexedDB; it leaks NO secret and renders NO private memory. The required
panels are present (INPUT structured record, OUTPUT atomic_facts with claim_status, the LOSSLESS held-out
narrative-allegations panel, source-handle/lineage), it handles loading/empty/error/degraded, and the
7-stage stepper links to every sibling with this stage highlighted + prev/next. Static source scan —
deterministic, offline. Models on _repos/shared-backend-components/scripts/check_consumption_ui.py + _repos/shared-backend-components/scripts/check_memory_page_projection_only.py.

CLI: python3 _repos/shared-backend-components/scripts/check_pipeline_decomposition_ui.py --self-test
"""
...
_PAGE = _resource("web/baltor/pipeline/decomposition.html")
...
_REQUIRED_ENDPOINTS = ("/api/pipeline/decomposition", "/api/pipeline/overview")
...
#: required panel headings (INPUT, OUTPUT atomic facts, the lossless panel, source-handle/lineage).
_REQUIRED_PANELS = ("INPUT", "OUTPUT", "atomic facts", "LOSSLESS", "Source handles", "lineage", "Run overview")
#: lossless vocabulary that must be visible (held-out narrative allegations + source handle + claim_status).
_REQUIRED_LOSSLESS = ("held-out", "allegation", "claim_status", "source handle")
...
def _self_test() -> int:
    ...
    check("_repos/baltor/frontend/pipeline/decomposition.html exists", _PAGE.exists())
    if not _PAGE.exists():
    ...
    check("page fetches its contracted /api/pipeline/decomposition (+ overview)", missing_eps == [], str(missing_eps))
    ...
    # 4) required panels present (INPUT, OUTPUT atomic facts, the lossless panel, source-handle/lineage)
    missing_panels = [p for p in _REQUIRED_PANELS if p not in text]
    check("required panels present (INPUT / OUTPUT atomic facts / LOSSLESS / source-handles / lineage / overview)",
          missing_panels == [], str(missing_panels))

    # 5) the LOSSLESS panel actually shows held-out narrative allegations + claim_status + source handle
    missing_loss = [l for l in _REQUIRED_LOSSLESS if l not in text]
    check("lossless panel shows held-out allegations with claim_status + source handle (nothing erased)",
          missing_loss == [], str(missing_loss))
    check("the held-out allegation is explicitly NOT served as fact",
          "never served" in text or "served as fact" in text)
    ...
    check("this stage (decomposition) is the highlighted/current step",
          'class="step current" href="./decomposition.html"' in text and 'aria-current="step"' in text)
    check("prev/next pager present (prev → upload, next → reconciliation)",
          'id="prev"' in text and 'href="./upload.html"' in text and 'href="./reconciliation.html"' in text)
    ...
    print(f"\n{'PASS — check_pipeline_decomposition_ui: decomposition.html fetches ONLY /api/pipeline/decomposition (+ overview), is projection-only (no durable writes / secrets / private memory), shows INPUT + OUTPUT atomic_facts(claim_status) + the LOSSLESS held-out narrative-allegations panel (with ctx:// handles, never served) + source-handle/lineage, handles loading/empty/error/degraded, and the 7-stage stepper links all siblings with this stage highlighted + prev/next.' if not fails else f'{len(fails)} FAILURES: {fails}'}")
    return 0 if not fails else 1


def _main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(description="Proof: pipeline decomposition stage page is projection-only.")
