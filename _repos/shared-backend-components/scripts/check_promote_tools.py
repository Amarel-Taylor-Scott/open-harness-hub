...
plane, marked promoted + vetted=false (not the human-vetted core), unique, disjoint from the core. serves_truth=false.
  ...
  python3 _repos/shared-backend-components/scripts/check_promote_tools.py --self-test
"""
...
def _self_test() -> int:
    ...
    declared = {p["plane"] for p in json.loads((_resource("architecture") / "tool_planes.json").read_text())["planes"]}
    core = P._core_ids()
