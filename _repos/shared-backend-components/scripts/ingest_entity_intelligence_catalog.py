...
_repos/shared-backend-components/architecture/fragile_context_taxonomy.json — and `adoptable` is computed by cross-checking the license/package
against the org guardrail's AVOID ledger (_repos/shared-backend-components/architecture/org_guardrail_policies.json), so a copyleft/proprietary/
...
CLI: PYTHONPATH=. python3 _repos/shared-backend-components/scripts/ingest_entity_intelligence_catalog.py --build | --self-test
"""
...
_FEED = _resource("data") / "capability-candidates" / "discovered-feed-entity-intelligence-2026-06-20.json"
...
_CATALOG: list[tuple] = [
    ...
    ("ftm-normalize", "Normalize any governed entity record → FollowTheMoney EntityProxy (Aleph-loadable).",
     ...
     "MIT", 1.0, "green", (), "schema adopted via our _repos/baltor/backend/src/baltor/native/ftm_adapter (PyICU-free); lib optional"),
...
def _denied() -> tuple[set, set]:
    pol = json.loads((_resource("architecture") / "org_guardrail_policies.json").read_text())
...
def _self_test() -> int:
    ...
    taxonomy = json.loads((_resource("architecture") / "fragile_context_taxonomy.json").read_text())
