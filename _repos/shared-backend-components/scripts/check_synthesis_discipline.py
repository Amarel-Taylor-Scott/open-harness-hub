...
can't + the registry-driven defaults produce real expanded plans. serves_truth=false.
  ...
  python3 _repos/shared-backend-components/scripts/check_synthesis_discipline.py --self-test
"""
...
from src.teleon.synthesis.strategist import py_function_src_teleon_synthesis_strategist__default_strategies, py_function_src_teleon_synthesis_strategist__resilient_synthesize
from src.teleon.synthesis.synthesis_trace import py_class_src_teleon_synthesis_synthesis_trace__SynthesisTrace
from src.teleon.synthesis.synthesis_tree import py_class_src_teleon_synthesis_synthesis_tree__Attempt, py_class_src_teleon_synthesis_synthesis_tree__SynthesisTree
...
def _self_test() -> int:
    tmpl = json.loads((_resource("architecture") / "synthesis_prompt_templates.json").read_text(encoding="utf-8"))
    fails = []
    def ck(n, ok, d=""):
    ...
    fp = I.py_function_src_teleon_synthesis_intent_to_dag__frontier_prompts("extract fields from these invoices into our schema")
    ...
    def tester(path, partial):
        ...
        if partial:
            return py_class_src_teleon_synthesis_synthesis_tree__Attempt("A=a1" not in ids, "broke" if "A=a1" in ids else "ok", cost=0.001, confidence=0.9)
        return py_class_src_teleon_synthesis_synthesis_tree__Attempt(ids == ["A=a2", "B=b2"], "assembly", cost=0.01, confidence=0.8)
    tree = py_class_src_teleon_synthesis_synthesis_tree__SynthesisTree(); tree.synthesize(pts, tester)
    tr = py_class_src_teleon_synthesis_synthesis_trace__SynthesisTrace.from_tree(tree, intent="toy")
    s = tr.summary()
    ...
    def t2(path, partial):
        ids = [f"{d.id}={d.choice}" for d in path]
        return py_class_src_teleon_synthesis_synthesis_tree__Attempt(True) if partial else py_class_src_teleon_synthesis_synthesis_tree__Attempt(ids == ["A=a1", "B=b2"])   # needs b2, NOT in base -> must sprout
    sol, strat, attempts = py_function_src_teleon_synthesis_strategist__resilient_synthesize(base, t2, intent="toy", strategies=[("sprout_b2", lambda h: [("A", ["a1"]), ("B", ["b1", "b2"])])])
    ...
    sol2, strat2, att2 = py_function_src_teleon_synthesis_strategist__resilient_synthesize(base, t2, intent="toy", strategies=[("noop", lambda h: None)])
    ck("honest no-solution when no strategy escapes (never fabricated)", sol2 is None and strat2 is None)

    names = [n for n, _ in py_function_src_teleon_synthesis_strategist__default_strategies("find public statements by X and flag contradictions")]
    ...
    strat_fns = dict(py_function_src_teleon_synthesis_strategist__default_strategies("extract fields from these invoices into our schema"))
