#!/usr/bin/env python3
"""scripts.check_standardized_examples — proof: _repos/shared-backend-components/docs/examples/standardized-patterns.md is grounded.
...
CLI: python3 _repos/shared-backend-components/scripts/check_standardized_examples.py --self-test
"""
...
_DOC = _resource("docs") / "examples" / "standardized-patterns.md"
...
#: a path-like token (e.g. scripts/foo.py, _repos/baltor/frontend/x.html, architecture/y.json) inside backticks.
...
def _self_test() -> int:
    ...
    missing_examples = [p for p in example_paths if not (_resource(p)).exists()]
    ...
    missing_proofs = [p for p in proof_paths if not (_resource(p)).exists()]
    ...
    missing_refs = [f for f in _CROSS_REF_FILES if not (_resource(f)).exists()]
