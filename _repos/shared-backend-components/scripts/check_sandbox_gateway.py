...
Asserts:
  ...
  H. DEPENDENCY LAW: no module under _repos/teleon/backend/src/teleon/sandbox imports src.baltor.
...
def _self_test() -> int:
    ...
    # H dependency law
    bad = [str(p.relative_to(_REPO)) for p in (_resource("src/teleon/sandbox")).rglob("*.py")
           for line in p.read_text().splitlines() if line.strip().startswith(("import src.baltor", "from src.baltor"))]
    check("H: no _repos/teleon/backend/src/teleon/sandbox module imports src.baltor", not bad, "; ".join(bad))
