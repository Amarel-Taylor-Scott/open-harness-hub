...
COMPUTES (never hand-types) a dependency graph from _repos/shared-backend-components/architecture/registry_ontology.json: nodes = every registry
...
real cross-references so it can't drift.
  ...
  python3 _repos/shared-backend-components/scripts/build_registry_dependency_graph.py            # (re)write _repos/shared-backend-components/architecture/registry_dependency_graph.json
  python3 _repos/shared-backend-components/scripts/build_registry_dependency_graph.py --self-test # regenerate + validate consistency + freshness
...
_OUT = _resource("architecture") / "registry_dependency_graph.json"
