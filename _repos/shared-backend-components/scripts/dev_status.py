...
CLI:
    python3 _repos/shared-backend-components/scripts/dev_status.py             # print the assembled dev status (JSON)
    python3 _repos/shared-backend-components/scripts/dev_status.py --self-test  # offline deterministic proof of the parsers
"""
...
BACKLOG = _resource("research") / "future-ideas.md"
REVIEW_LATEST = _resource("e2e") / "artifacts" / "review-pack" / "REVIEW-latest.md"
...
def _mcp() -> dict:
    ...
    return {"servers": servers, "count": len(servers),
            "scanner": "scripts/scan_mcp_manifests.py" if (_resource("scripts/scan_mcp_manifests.py")).exists() else None}
