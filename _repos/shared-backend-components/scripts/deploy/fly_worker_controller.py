...
Every scaling number comes from `_repos/shared-backend-components/architecture/deploy_topology.json` (the same block the KEDA
...
Run modes:
  python3 _repos/shared-backend-components/scripts/deploy/fly_worker_controller.py              # reconcile loop (cloud role)
  python3 _repos/shared-backend-components/scripts/deploy/fly_worker_controller.py --once       # single tick (cron-shaped)
  python3 _repos/shared-backend-components/scripts/deploy/fly_worker_controller.py --self-test  # offline: fake API + fake Redis
"""
