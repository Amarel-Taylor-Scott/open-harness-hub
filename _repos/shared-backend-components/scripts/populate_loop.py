...
from scripts._time import now_iso  # the ONE UTC, Z-suffixed timestamp (see scripts/_time.py)
if str(REPO) not in sys.path:
    sys.path.insert(0, str(REPO))
CANDIDATES = _resource("data") / "dev-intel" / "swarm_candidates.jsonl"
RECORDS = _resource("data") / "dev-intel" / "registry_records.jsonl"
GRAPH = _resource("data") / "dev-intel" / "record_graph.jsonl"
STATE = _resource("data") / "dev-intel" / "populate_cursor.json"
...
def _now() -> str:
    return now_iso()  # UTC + Z via scripts._time (was time.strftime local time, no timezone/Z — unorderable across zones)
