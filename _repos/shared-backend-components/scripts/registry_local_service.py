...
``local_openhubforai_projection_api`` service declared in _repos/shared-backend-components/architecture/local_service_registry.json).
...
read the public catalog from here. Two cleanly separated surfaces:
  ...
  hub's ``entries`` array from the design bundle (single source; no hand-retyped catalog):
    GET  /api/openhubforai/<realm>/search?q=     entries for a hub (optional substring filter)
    GET  /api/openhubforai/<realm>/entry/<id>    one entry
    GET  /api/openhubforai/catalog               {realm: entry_count} across the bundle
  ...
  (cross-service auth, never trusting a client-supplied account id):
    POST /api/openhubforai/<realm>/install       {session_id, entry}     record an install
    POST /api/openhubforai/<realm>/uninstall     {session_id, entry_id}  record an uninstall
    POST /api/openhubforai/<realm>/submit        {session_id, entry}      publish -> REVIEW QUEUE
    GET  /api/openhubforai/<realm>/workspace?session_id=   the dashboard summary (real)
...
writes, rotation); the full service/HTTP proof remains ``_repos/shared-backend-components/scripts/check_registry_backend.py``.

Offline, stdlib-only. Port + identity port come from the registries (single source, drift-gated by
_repos/shared-backend-components/scripts/check_registry_backend.py).
"""
...
from src.teleon.registry.browse import py_function_src_teleon_registry_browse__browse as _spine_browse  # noqa: E402
from src.teleon.registry.index import py_function_src_teleon_registry_index__records as _spine_records  # noqa: E402

SERVICE_ID = "local_openhubforai_projection_api"
REGISTRY_PATH = _resource("architecture") / "local_service_registry.json"
IDENTITY_REGISTRY_PATH = _resource("architecture") / "identity_realm_registry.json"
BUNDLE_DIR = _resource("dist") / "sites" / "aidoneright-design"
VERSION = "1.0"
...
class RegistryStore:
    def __init__(self, state_dir: Path | None = None, bundle_dir: Path | None = None,
        ...
        self.state_dir = Path(state_dir) if state_dir else (_resource("dist") / "registry")
...
class _Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if path == "/api/openhubforai/catalog":
        ...
        if path == "/api/openhubforai/registries":
        ...
        if path.startswith("/api/openhubforai/registries/") and path.endswith("/records"):
            rid = path[len("/api/openhubforai/registries/"):-len("/records")]
            return self._send(200, {"registry": rid, "records": _spine_records(rid, limit=50),
                                    "truth_authority": False})

        seg = path.strip("/").split("/")     # api/openhubforai/<realm>/<action>...
        if len(seg) >= 4 and seg[0] == "api" and seg[1] == "openhubforai":
    ...
    def _handle_decide(self, realm: str, path: str) -> None:
        """POST /api/openhubforai/<realm>/review/decide — the promotion gate (reviewer-only)."""
        try:
    ...
    def _handle_admin_reviewer(self, realm: str, op: str, path: str) -> None:
        """POST /api/openhubforai/<realm>/admin/reviewers/<grant|revoke> — admin-only reviewer management.
...
def _self_test() -> int:
    ...
    stays in _repos/shared-backend-components/scripts/check_registry_backend.py): db-served reads reflect writes immediately, a
