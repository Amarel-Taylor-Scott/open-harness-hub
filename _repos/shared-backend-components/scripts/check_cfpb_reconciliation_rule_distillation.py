...
What this proof asserts (the non-negotiable semantics):
  ...
  * The rule is mined ONLY from a VERIFIED outcome — the EXISTING deterministic authority
    ``_repos/shared-backend-components/scripts/artifact_graph/reconciliation.py`` already decided "Reg E (10 business days)" beats the
...
CLI: PYTHONPATH=. python3 _repos/shared-backend-components/scripts/check_cfpb_reconciliation_rule_distillation.py --self-test
"""
