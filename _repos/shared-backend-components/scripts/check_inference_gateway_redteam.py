#!/usr/bin/env python3
"""scripts.check_inference_gateway_redteam — REDTEAM PROOF over the EXISTING shared LLM plane (_repos/teleon/backend/src/teleon/inference).
...
def _self_test() -> int:
    ...
    pol = json.loads((_resource("architecture") / "free_limited_llm_endpoint_policy.json").read_text())
