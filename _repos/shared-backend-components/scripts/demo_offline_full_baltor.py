...
CLI: PYTHONPATH=. python3 _repos/shared-backend-components/scripts/demo_offline_full_baltor.py --self-test
"""
...
def run_offline_full_demo() -> dict:
    ...
    sec("temporal_fact_graph", "m6_candidate" if Path("_repos/baltor/backend/src/baltor/graph/temporal").exists() else "absent",
    ...
    sec("native_format_sidecars", "m8_ui" if Path("_repos/baltor/backend/src/baltor/native").exists() else "absent",
