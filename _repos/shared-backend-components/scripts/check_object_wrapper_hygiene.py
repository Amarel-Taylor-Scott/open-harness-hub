...
`_repos/teleon/backend/src/teleon/io/governed_record.mint_record` (a governed envelope + a flexible payload, with the version carried in
...
def self_test() -> int:
    ...
    shell = json.loads((_resource("schemas/shared/ObjectShell.schema.json")).read_text(encoding="utf-8"))
    sp = shell["properties"]
    ...
    ro = json.loads((_resource("schemas/registry/RegistryObject.schema.json")).read_text(encoding="utf-8"))
    rp = ro["properties"]
