...
# The synthetic CFPB corpus moved under `_repos/` with the rest of `demo-data/`; the loader defaults its base to
# the module's own repo-root walk-up (now the monorepo root, where `demo-data/` no longer lives). Point its
# injectable `repo` at the dir that actually contains `demo-data/cfpb-sample/` via the universal resolver, so the
# 6 docs load (an empty corpus is what left `crossover_n` None and broke the boundary test).
_CORPUS_REPO = _resource("demo-data").parent
...
_SOURCE_FILES = [
    _resource("src/teleon/blackboard/token_economics.py"),
    _resource("scripts/check_stateful_vs_stateless_token_economics.py"),
]
...
def _self_test() -> int:
    ...
    docs = py_function_src_teleon_blackboard_token_economics__load_cfpb_sample_docs(
        py_arg_src_teleon_blackboard_token_economics__load_cfpb_sample_docs__repo=_CORPUS_REPO)
    check("setup: CFPB-sample docs loaded (the synthetic corpus to 'read')", len(docs) >= 5,
          f"loaded {len(docs)} docs")

    rep = py_function_src_teleon_blackboard_token_economics__estimate_token_economics(docs, _QUESTIONS, now=_NOW)
    ...
    rep1 = py_function_src_teleon_blackboard_token_economics__estimate_token_economics(docs, _QUESTIONS[:1], now=_NOW)
    ...
    rep_star = py_function_src_teleon_blackboard_token_economics__estimate_token_economics(docs, _QUESTIONS[:1] * n_star, now=_NOW)
    ...
    rep_again = py_function_src_teleon_blackboard_token_economics__estimate_token_economics(docs, _QUESTIONS, now=_NOW)
    ...
    check("C: estimate_tokens is pure (same text -> same estimate)",
          py_function_src_teleon_blackboard_token_economics__estimate_tokens("the sanction took effect on 2026-04-15")
          == py_function_src_teleon_blackboard_token_economics__estimate_tokens("the sanction took effect on 2026-04-15"))
    check("C: estimate_tokens('') == 0 (empty text costs nothing)", py_function_src_teleon_blackboard_token_economics__estimate_tokens("") == 0)
    ...
    check("C: digest max-chars is exported as OBSERVATION_DIGEST_MAX_CHARS and is positive",
          isinstance(py_const_src_teleon_blackboard_token_economics__OBSERVATION_DIGEST_MAX_CHARS, int) and py_const_src_teleon_blackboard_token_economics__OBSERVATION_DIGEST_MAX_CHARS > 0,
          f"OBSERVATION_DIGEST_MAX_CHARS={py_const_src_teleon_blackboard_token_economics__OBSERVATION_DIGEST_MAX_CHARS}")
    ...
    check("E: estimate_method names it an ESTIMATE / not a real tokenizer (no overclaim)",
          rep["estimate_method"] == py_const_src_teleon_blackboard_token_economics__ESTIMATE_METHOD
