...
`Draft202012Validator` `_repos/shared-backend-components/scripts/validate.py` uses. Self-test also includes a NEGATIVE case (an
...
CLI:
    python3 _repos/shared-backend-components/scripts/validate_flywheel_schemas.py --self-test
    python3 _repos/shared-backend-components/scripts/validate_flywheel_schemas.py            # validate + print results
"""
...
def _self_test() -> int:
    ...
    bad = json.loads((_resource("fixtures/flywheel/agent-decision-context.example.json")).read_text(encoding="utf-8"))
    ...
    badf = json.loads((_resource("fixtures/flywheel/context-rot-flywheel.example.json")).read_text(encoding="utf-8"))
    badf["triggers"] = []
