...
CLI / self-test:
    python3 _repos/shared-backend-components/scripts/foundry/eval_scorers.py
    ...
    def score(self, task: dict, answer: str | None) -> float | None:
        ...
        if self.llm_judge is None:
            raise RuntimeError(
                ...
                "(wire it to scripts.foundry.model_route behind _repos/teleon/backend/src/teleon/inference); "
...
def _raise_seam(pkg: str, klass: str) -> None:
    raise NotImplementedError(
        ...
        f"calls with scripts.foundry.model_route via _repos/teleon/backend/src/teleon/inference so model "
