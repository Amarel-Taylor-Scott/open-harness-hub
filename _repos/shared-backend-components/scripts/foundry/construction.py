...
of grounded entries, a IfStatement/rule pack, an Action/tool wrapper, …). One rich
...
class DeterministicAuthor:
    ...
    def _conditional(gap: dict, source: dict, payload: dict, rules: list) -> dict:
        """Mine declarative IF→THEN rules into a IfStatement (rule-pack)."""
        name = (str(payload.get("name") or gap.get("summary") or "IfStatement") + " rules")[:128]
        url = source.get("source_url", "")
        ...
        body = {
            ...
            "name": name,
            "description": (
                f"{gap.get('summary', 'IfStatement rules')} "
...
def _self_test() -> int:
    ...
    check("two drafts built (Knowledge Corpus + IfStatement)",
    ...
    check("IfStatement (rule-pack) built", rp is not None)
    ...
    check("IfStatement standardizes + schema-validates", rp.alive and rp.component_id, str(rp.reasons))
