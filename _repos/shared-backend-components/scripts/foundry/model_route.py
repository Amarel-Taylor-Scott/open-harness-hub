...
def from_env() -> ModelRoute | None:
    ...
    from scripts._config import (DEFAULT_MISTRAL_EMBED_MODEL, DEFAULT_OLLAMA_EMBED_MODEL, DEFAULT_OPENAI_EMBED_MODEL,
                                 NVIDIA_BUILD_API_KEY_ENV, NVIDIA_BUILD_BASE_URL, NVIDIA_BUILD_DEFAULT_MODEL)
    model = os.environ.get("OH_CHAT_MODEL", _DEFAULT_CHAT_MODEL)

    # NVIDIA Build — OpenAI-compatible hosted lane (owner key); serves GLM 5.2 (z-ai/glm-5.2) among others.
    # FIRST in the chain: setting NVIDIA_API_KEY is the owner explicitly choosing the credited endpoint (the
    # Ollama Cloud key is rate-limited / uncredited). Chat-only (GLM 5.2 is a chat model); the embedder stays
    # on Ollama/local, exactly like the Anthropic lane below. Model pin: OH_CHAT_MODEL, else z-ai/glm-5.2.
    if os.environ.get(NVIDIA_BUILD_API_KEY_ENV):
        base = os.environ.get("NVIDIA_BASE_URL", NVIDIA_BUILD_BASE_URL)
        # NVIDIA's model IDs are provider-specific (z-ai/glm-5.2), so use a NVIDIA-SCOPED override, NOT the
        # generic OH_CHAT_MODEL (which is the Ollama tag 'glm-5.2' — a different id on this endpoint).
        mdl = os.environ.get("OH_NVIDIA_CHAT_MODEL") or NVIDIA_BUILD_DEFAULT_MODEL
        comp, _ = _openai_compatible(base, os.environ[NVIDIA_BUILD_API_KEY_ENV], mdl, DEFAULT_OPENAI_EMBED_MODEL)
        return ModelRoute(comp, name=f"nvidia-build:{mdl}")
...
def _self_test() -> int:
    ...
    saved = {k: os.environ.pop(k, None) for k in ("NVIDIA_API_KEY", "OLLAMA_API_KEY", "MISTRAL_API_KEY",
