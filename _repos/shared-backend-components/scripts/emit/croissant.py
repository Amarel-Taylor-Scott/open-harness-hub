...
# Allow `python _repos/shared-backend-components/scripts/emit/croissant.py` and `python -m scripts.emit.croissant`.
...
def render_one(manifest_path: Path, manifest: dict) -> dict[str, Any]:
    ...
    out: dict[str, Any] = {
        "@context":     CONTEXT,
        "@type":        "sc:Dataset",
        "@id":          f"https://openhubforai.io/{manifest['type']}/{slug}",
        "conformsTo":   CONFORMS_TO,
        "name":         manifest.get("name", slug),
        "description":  manifest.get("description", "").strip(),
        "version":      manifest.get("version", "0.1.0"),
        "license":      spdx_license_url(manifest.get("license")),
        "url":          f"https://openhubforai.io/{manifest['type']}/{slug}",
        "creator":      creators,
