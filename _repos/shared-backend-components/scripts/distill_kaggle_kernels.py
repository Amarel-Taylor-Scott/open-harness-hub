...
candidate registry entries:
  - DETERMINISTIC FLOOR (always): map each detected pattern -> a candidate entry (_repos/shared-backend-components/architecture/kaggle_mining_questions.json
...
insight is a CANDIDATE; the promotion boundary + verification apply before adoption).
  ...
  python3 _repos/shared-backend-components/scripts/distill_kaggle_kernels.py            # write the candidate feed (floor)
  python3 _repos/shared-backend-components/scripts/distill_kaggle_kernels.py --live     # + LLM distill a sample (needs OH_LLM_API_KEY)
  python3 _repos/shared-backend-components/scripts/distill_kaggle_kernels.py --self-test
"""
...
_QB = _resource("architecture") / "kaggle_mining_questions.json"
_OUT = _resource("data") / "dev-intel" / "kaggle_distilled_candidates.json"
...
def _load_findings() -> dict[str, list[str]]:
    ...
    for f in sorted(glob.glob(str(_resource(_FINDINGS_GLOB)))):
...
def distill_floor(findings: dict[str, list[str]], pattern_map: dict[str, str],
    ...
    ``enrichment`` (_repos/shared-backend-components/architecture/kaggle_mining_questions.json -> pattern_enrichment) attaches MODEL-AUTHORED
...
def _llm_distill_sample(findings: dict[str, list[str]], qb: dict, *, llm_limit: int = 0) -> list[dict]:
    """OPTIONAL richer distillation via the ollama lane. Honest-offline: returns [] (with reason) when no lane."""
    if not os.environ.get("OH_LLM_API_KEY") and os.environ.get("OLLAMA_API_KEY"):
        os.environ["OH_LLM_API_KEY"] = os.environ["OLLAMA_API_KEY"]
    if not os.environ.get("OH_LLM_API_KEY"):
        return []
    try:
        from scripts._llm_client import chat, resolve_provider  # noqa: PLC0415
    ...
    if llm_limit > 0:
        items = items[:llm_limit]
    for kernel, pats in items:
...
def _build(live: bool = False, *, llm_limit: int = 0) -> dict:
    findings = _load_findings()
    qb = json.loads(_QB.read_text())
    floor = distill_floor(findings, qb.get("pattern_to_registry", {}), qb.get("pattern_enrichment", {}))
    llm = _llm_distill_sample(findings, qb, llm_limit=llm_limit) if live else []
    return {
        "version": "0.1.0",
...
def main() -> int:
    ...
    ap.add_argument("--llm-limit", type=int, default=0, help="0 means all mined kernels")
    ap.add_argument("--self-test", action="store_true")
    args = ap.parse_args()

    doc = _build(live=args.live and not args.self_test, llm_limit=args.llm_limit)
