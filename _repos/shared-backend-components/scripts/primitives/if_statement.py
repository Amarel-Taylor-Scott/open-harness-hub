"""If Statement primitive — the decision layer (the IF), kept separate from the THEN.
...
The boolean it produces decides which Action (the THEN) runs. (An If Statement holds operators and custom logic, not
just a single if.)
"""
...
class IfStatement(Primitive):
    ...
    # label-suffix map and stays empty so label_for(rule-pack) == "IfStatement").
