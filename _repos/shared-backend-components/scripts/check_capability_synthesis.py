...
from the ladders + emits 5W1H + alternatives + the 5 frontier prompts; module_bundles reference real components. serves_truth=false.
  ...
  python3 _repos/shared-backend-components/scripts/check_capability_synthesis.py --self-test
"""
...
def _self_test() -> int:
    ...
    def tester(path, partial):
        ...
        if partial:
            return py_class_src_teleon_synthesis_synthesis_tree__Attempt("A=a1" not in ids, "a1 broken" if "A=a1" in ids else "ok")
        return py_class_src_teleon_synthesis_synthesis_tree__Attempt(ids == ["A=a2", "B=b2"], "assembly")
    ...
    t2 = py_class_src_teleon_synthesis_synthesis_tree__SynthesisTree()
    sol2 = t2.synthesize(points, lambda path, partial: py_class_src_teleon_synthesis_synthesis_tree__Attempt(partial))   # every component ok, no assembly ok
    ...
    dag = I.py_function_src_teleon_synthesis_intent_to_dag__build_dag("extract fields from these invoices into our schema")
    ...
    pts = I.py_function_src_teleon_synthesis_intent_to_dag__decision_points("extract fields from these invoices into our schema")
    ck("decision points feed the synthesis tree (every point has >=1 option)", pts and all(o for _, o in pts))
    vq = I.py_function_src_teleon_synthesis_intent_to_dag__verification_ladder(dag["nodes"][0])
    ...
    ck("alternatives ladder emits same-plane alternatives + critique prompts",
       "alternatives_same_plane" in I.py_function_src_teleon_synthesis_intent_to_dag__alternatives_ladder(dag["nodes"][0]))
    fp = I.py_function_src_teleon_synthesis_intent_to_dag__frontier_prompts("find public statements by X and flag contradictions")
