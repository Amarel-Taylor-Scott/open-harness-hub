...
stamps the git rev so staleness is visible. Bounded + secrets-excluded. serves_truth=false; offline; stdlib only.
  ...
  --emit        also write _repos/shared-backend-components/docs/context/global-context.generated.md (the regenerable non-fragile global doc)
...
GLOBAL_DOC = _resource("docs") / "context" / "global-context.generated.md"
...
REGIONS = {
    "teleon.evolution (the descent brain + converters)": ["_repos/teleon/backend/src/teleon/evolution"],
    "teleon.storage (tiered append-only record stores)": ["_repos/teleon/backend/src/teleon/storage", "scripts/_jsonl_store.py"],
    "teleon.inference (model index + gateway + OIPS)": ["_repos/teleon/backend/src/teleon/inference"],
    "teleon.objectives (objective-based selection)": ["_repos/teleon/backend/src/teleon/objectives"],
    "teleon.extraction (document->schema cascade — example demo 1)": ["_repos/teleon/backend/src/teleon/extraction"],
    "teleon.enrichment (search+LLM enrichment — example demo 2)": ["_repos/teleon/backend/src/teleon/enrichment"],
}
#: default LOCAL focus — the descent reader/executor + the two flagship example demos (so the board reviews them).
DEFAULT_FOCUS = [
    "_repos/teleon/backend/src/teleon/evolution/descent_attempt_store.py",
    "_repos/teleon/backend/src/teleon/evolution/catalog_descent.py",
    "_repos/teleon/backend/src/teleon/evolution/substrate_selector.py",
    "_repos/teleon/backend/src/teleon/storage/record_store.py",
    "_repos/teleon/backend/src/teleon/extraction/document_extraction_cascade.py",
    "_repos/teleon/backend/src/teleon/enrichment/search_enrich.py",
]
...
def _iter_py(spec: str):
    ...
    p = resource(spec)   # resolves a moved dir (architecture/schemas/vocabularies) to its _repos home; root otherwise
...
def global_context() -> str:
    ...
    if law.exists():
        try:
            ...
            parts.append(f"Dependency law: {d.get('law') or d.get('allowed_direction') or 'Baltor->Teleon->OpenHubForAI'}\n")
        except Exception:
            pass
    try:
    ...
    try:
        ...
        pass
    sm = _resource("architecture") / "surface_map.json"
    if sm.exists():
        try:
...
#: PMF / product knowledge categories (real prose docs; _repos/shared-backend-components/docs/catalog is generated pages, excluded).
...
def knowledge_section(*, head_chars: int = 220, per_cat: int = 30) -> str:
    ...
    grouped by category — so the model understands the PRODUCT, not just the code. Excludes _repos/shared-backend-components/docs/catalog (generated)."""
...
def _self_test() -> int:
    ...
    ck("full pack includes the COMPLETE folder structure (every file map)", "FULL FOLDER STRUCTURE" in fpack and "_repos/teleon/backend/src/teleon/dag/real_steps.py" in fpack)
