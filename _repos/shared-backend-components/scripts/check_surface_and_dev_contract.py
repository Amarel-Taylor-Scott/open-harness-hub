...
Guards the exact failure modes from the 2026-06-25 incident (_repos/shared-backend-components/docs/codex/surface-and-development-contract.md):
  ...
  2. Reports _repos/shared-backend-components/scripts/*.py that define a self_test but are NOT registered in flywheel_proof_modules (orphaned seams).
...
def surface_violations(spec: dict | None = None) -> list[str]:
    ...
    for p in spec.get("pillars", []):
        cs = _resource(p["canonical_surface"])
        if not cs.exists():
...
def unregistered_seams() -> list[str]:
    ...
    out: list[str] = []
    for f in sorted((_resource("scripts")).glob("*.py")):
        try:
