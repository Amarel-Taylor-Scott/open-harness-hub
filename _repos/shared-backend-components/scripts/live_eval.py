...
the live LLM arm (honest offline). serves_truth=false.
  ...
  python3 _repos/shared-backend-components/scripts/live_eval.py            # live A/B on the refund-policy eval
  python3 _repos/shared-backend-components/scripts/live_eval.py --self-test
"""
...
EVAL = _resource("data") / "eval" / "refund_policy_eval.json"
...
def run_live() -> dict:
    ...
    (_resource("data") / "dev-intel" / "live-eval-smoke.json").write_text(json.dumps(rec, indent=2) + "\n")
    return rec
