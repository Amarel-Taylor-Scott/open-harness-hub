...
def main() -> int:
    ...
    for p in sorted(_resource("data/gxp-samples").glob("*.json")):
    ...
    for p in sorted(_resource("data/climate-samples").glob("*.json")):
    ...
    for p in sorted(_resource("data/threat-intel-samples").glob("*.json")):
