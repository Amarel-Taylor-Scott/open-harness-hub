#!/usr/bin/env python3
"""Create a load and promotion audit plan for public-source row families."""
...
def create_public_source_load_plan(
    *,
    row_dir: str | Path,
    ...
    out = Path(output_dir) if output_dir else Path(tempfile.mkdtemp(prefix="ohh-public-source-load-plan-"))
    ...
    manifest = {
        ...
        "safety_notes": [
            "Bulk load plan contains public-source blueprint and replay metadata only.",
            "Raw source bodies, emails, private documents, and proprietary source dumps are not represented in the load plan.",
            "Promotion decisions are advisory; high-risk public facts and consumer-impacting objects remain review-gated.",
            "Insurance scope remains excluded for this factory path.",
        ],
    }
    (out / "load-plan-manifest.json").write_text(json.dumps(manifest, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    return manifest
...
def _self_test() -> int:
    row_dir = _resource("dist/source-surface-scan-partitions/seed/rows")
    if not row_dir.exists():
        raise FileNotFoundError("dist/source-surface-scan-partitions/seed/rows is required for this self-test")
    with tempfile.TemporaryDirectory() as tmp:
        result = create_public_source_load_plan(row_dir=row_dir, output_dir=tmp)
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Create a bulk-load and promotion audit plan for public-source row families.")
    ...
    result = create_public_source_load_plan(
