...
#
#   Context is Everything   (parent / mission landing)  → :8002 → _repos/aidoneright/frontend
#   Baltor                  (verified-context SaaS)      → :8001 → _repos/baltor/frontend
#   OpenHubForAI        (open builder funnel)        → :8000 → _repos/openhubforai/frontend
#
...
#
# Brand/identity: _repos/aidoneright/context/strategy/brand-architecture.md (LOCKED 2026-05-29).
set -u
cd "$(dirname "$0")/.."
mkdir -p dist
say(){ printf '%s\n' "$*"; }
