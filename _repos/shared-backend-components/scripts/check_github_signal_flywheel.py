...
def _self_test() -> int:
    ...
    for c in ("RepoSnapshot", "RepoTrendSignal", "RepoIntakeDecision"):
        check(f"A: {c} schema present + registered",
              (_resource("schemas") / "repo_intel" / f"{c}.schema.json").exists() and f"repo_intel/{c}.schema.json" in contracts)
    ...
    cases = {"microsoft/markitdown": {"opentoolshub", "opencontexthub"},
             "zereight/headroom": {"opentoolshub", "openhubforai", "teleon"},
    ...
    # K reuse
    check("K: existing GitHub harvester referenced (not reinvented)", (_resource("scripts") / "acquisition" / "github_repo_harvester.py").exists())
