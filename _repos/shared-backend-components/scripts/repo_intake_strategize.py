...
_repos/teleon/backend/src/teleon/repo_strategy (pure, offline-testable); this is the operator surface (live fetch + feed write).

  PYTHONPATH=. python3 _repos/shared-backend-components/scripts/repo_intake_strategize.py --run                 # the owner's default repos, live
  PYTHONPATH=. python3 _repos/shared-backend-components/scripts/repo_intake_strategize.py --run --urls "<url> <url>"
  PYTHONPATH=. python3 _repos/shared-backend-components/scripts/repo_intake_strategize.py --run --file repos.txt --offline
  PYTHONPATH=. python3 _repos/shared-backend-components/scripts/repo_intake_strategize.py --self-test
"""
...
_FEED = _resource("data") / "capability-candidates" / "discovered-feed-repo-strategize.json"
...
def fetch_repo_meta(owner: str, repo: str, *, timeout: int = 12):
    ...
    hdr = {"User-Agent": "OpenHubForAI-repo-intake/1.0", "Accept": "application/vnd.github+json"}
    try:
...
def _self_test() -> int:
    ...
    ck("permissive (MIT) → ADOPT-CANDIDATE + adoptable + maps to a hub",
       ...
       and by["permissive"]["strategy"]["feeds_hub"] in ("OpenHubForAI", "OpenAgentHub"))
