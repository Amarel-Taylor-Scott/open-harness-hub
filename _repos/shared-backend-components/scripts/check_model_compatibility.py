...
def _self_test() -> int:
    ...
    check("H: composes the Inference Gateway provider graph", (_resource("architecture") / "model_provider_graph.json").exists() and all(r["tested_via"].startswith("inference_gateway") for r in res["reports"]))
    ...
    check("I: no _repos/teleon/backend/src/teleon/digestion module imports src.baltor", not bad_imp, "; ".join(bad_imp))
