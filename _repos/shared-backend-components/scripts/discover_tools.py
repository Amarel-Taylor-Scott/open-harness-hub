...
tools, then governs them: classify the license from the single source (_repos/openhubforai/backend/src/openhubforai/licenses), dedupe against the
...
never the reverse).
  ...
  python3 _repos/shared-backend-components/scripts/discover_tools.py "reranker" github --plane reranker
  python3 _repos/shared-backend-components/scripts/discover_tools.py --sweep                 # one query per under-target plane
  python3 _repos/shared-backend-components/scripts/discover_tools.py --self-test             # hermetic (no network)
"""
...
FEED = _resource("data") / "dev-intel" / "discovered_tools.jsonl"
REGISTRY = _resource("architecture") / "tool_registry.json"
