...
_R = next((_ar for _ar in Path(__file__).resolve().parents if (_ar / ".aidoneright-root").exists()), Path(__file__).resolve().parents[1]); _S = _resource("schemas") / "workers"; _A = _resource("architecture")
