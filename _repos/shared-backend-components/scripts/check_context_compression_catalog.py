...
context-compression provider catalog (_repos/shared-backend-components/architecture/context_compression_provider_catalog.json) is
...
Asserts the governance shape that keeps TokenTamer a fork-target rather than a deployed dependency:
  ...
  * governance_law references the lossless law (_repos/shared-backend-components/docs/codex/lossless-distillation.md) and states
...
CLI: PYTHONPATH=. python3 _repos/shared-backend-components/scripts/check_context_compression_catalog.py --self-test
"""
...
_CATALOG = _resource("architecture") / "context_compression_provider_catalog.json"
