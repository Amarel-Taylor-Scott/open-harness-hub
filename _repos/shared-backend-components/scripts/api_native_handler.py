...
(C-NATIVE-1). Model: _repos/shared-backend-components/scripts/api_context_handler.py, _repos/shared-backend-components/scripts/api_memory_handler.py.
...
unverified value as a confirmed fact:
  ...
  GET  /api/native/export/<source_id>?mode=...  — projection-only same-shape export per output_mode
                                        (_repos/shared-backend-components/architecture/native_output_modes.json). Calls the Lane B
...
LOSSLESS-DISTILLATION + GOVERNANCE (the whole point):
  ...
  - The Lane B service (_repos/baltor/backend/src/baltor/native/native_export_service) is imported LAZILY; if it (or the requested
