...
boundary); run it yourself:
    ...
    python3 _repos/shared-backend-components/scripts/operator_add_allow_rules.py
...
SETTINGS = next((_ar for _ar in Path(__file__).resolve().parents if (_ar / ".aidoneright-root").exists()), Path(__file__).resolve().parents[1]) / ".claude" / "settings.local.json"
RULES = [
    "Bash(docker compose *)",
    "Bash(node *)",
    "Bash(npm install *)",
