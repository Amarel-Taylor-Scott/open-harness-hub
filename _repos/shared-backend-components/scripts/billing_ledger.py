...
CLASS_RATES_USD_PER_MTOK: dict[str, tuple[float, float]] = {
    ...
    "capability_tool_call": (0.00, 0.00),  # capability_saas_gateway invocations — v1 unpriced (the plan base is
                                           # the money); per-call pricing later = editing this ONE row
}
_MILLION = 1_000_000
...
def main(argv: list[str]) -> int:
    if "--demo" in argv:
        path = _resource("dist") / "analytics" / "llm-receipts.jsonl"
