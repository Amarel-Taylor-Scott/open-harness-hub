...
# REPO_ROOT above stays the MONOREPO root (used for relative_to below). `from scripts.*` needs the SUBSTRATE root
# on sys.path — resolve it via the scripts/_repo_paths.py sentinel, then install() every code root.
...
DEFAULT_FIXTURE = _resource("data") / "cfpb-demo" / "complaints-fixture.json"
DEFAULT_OUT_DIR = _resource("site") / "baltor-demos" / "cfpb-complaints"
