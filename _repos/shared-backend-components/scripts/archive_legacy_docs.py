...
move OUT of the live tree so they stop misleading agents — but per the lossless-distillation law (_repos/shared-backend-components/docs/codex/
...
Excludes _reference/ (never touched/republished), archive/ itself, and anything under .git/.
  ...
  --scan [paths]  list archive candidates (default: _repos/shared-backend-components/docs/)  [dry-run]
  --apply [paths] actually move candidates to archive/legacy/ + write the manifest (lossless)
CLI: python3 _repos/shared-backend-components/scripts/archive_legacy_docs.py --scan
"""
...
ARCHIVE_ROOT = _resource("archive") / "legacy"
MANIFEST = _resource("_manifest.jsonl")
DEFAULT_SCAN = ["docs"]
...
def apply_archive(candidates: list[dict], *, now: str = "") -> list[dict]:
    ...
    moved = []
    for c in candidates:
        src = _resource(c["path"])
        if not src.exists():
            continue
        dst = _resource(c["path"])
        if dst.exists():
...
def reconcile_references(*, now: str = "") -> list[dict]:
    ...
    tombstoned = []
    for r in recs:
        orig = _resource(r["original_path"])
        if orig.exists():
        ...
        archived = _resource(r["archived_path"])
        successor = ""
        try:
...
def _self_test() -> int:
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        try:
            REPO = Path(d); ARCHIVE_ROOT = _resource("archive") / "legacy"; MANIFEST = _resource("_manifest.jsonl")
            (_resource("docs")).mkdir(parents=True)
            f = _resource("docs") / "stale.md"; f.write_text("---\nstatus: archived\n---\n# Stale\nbody\n")
            content = f.read_text()
            ...
            archived = _resource(moved[0]["archived_path"])
