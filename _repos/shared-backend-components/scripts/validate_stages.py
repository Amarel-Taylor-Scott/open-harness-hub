...
Includes negative cases so enforcement is proven. Stdlib + jsonschema. CLI:
    python3 _repos/shared-backend-components/scripts/validate_stages.py --self-test
"""
...
STAGES = _resource("data") / "stages.json"
FORMAT_SCHEMA = _resource("schemas") / "presentation" / "format-profile.schema.json"
...
def _self_test() -> int:
    ...
    for rel in FORMAT_FIXTURES:
        fmt_errs += [f"{rel}: {e.message}" for e in v.iter_errors(json.loads((_resource(rel)).read_text(encoding="utf-8")))]
