...
LLM proposes, the registry + governance dispose. The ladder is a fallback when no LLM. Network-gated. serves_truth=false.
  ...
  python3 _repos/shared-backend-components/scripts/compile_capability_live.py "extract fields from these invoices into our schema"
  python3 _repos/shared-backend-components/scripts/compile_capability_live.py --self-test
"""
...
RECEIPT = _resource("data") / "dev-intel" / "live-compile-smoke.json"
...
def candidate_pool(intent: str, *, k: int = 28) -> dict:
    ...
    tools = {t["id"]: t for t in json.loads((_resource("architecture") / "tool_registry.json").read_text())["tools"]}
    pool: dict = {}
    def add(cid: str):
    ...
    for h in CS.py_function_src_teleon_synthesis_component_search__search(intent, k=k):
        add(h["id"])
    for ids in CS.py_function_src_teleon_synthesis_component_search__compose(I.py_function_src_teleon_synthesis_intent_to_dag__outline(intent).get("capability") or "", k=4).values():
        for cid in ids:
            add(cid)
...
def _io_hint(plane) -> str:
    ...
    io = py_function_src_teleon_synthesis_io_contracts__plane_io(plane)
...
def _validate(spec: dict, pool: dict) -> dict:
    ...
    type_warnings = [[a, b] for a, b in edges if not py_function_src_teleon_synthesis_io_contracts__edge_compatible(plane_of.get(a), plane_of.get(b))]
...
def intelligent_compile(intent: str, *, llm=None) -> dict:
    ...
    def _attempt(prompt):
        try:
            ...
            return None, f"LLM unavailable or did not return parseable JSON: {type(exc).__name__}: {str(exc)[:160]}"

    base = _prompt(intent, pool)
    tmpl = T.py_function_src_teleon_synthesis_templates__retrieve_template(intent)                       # RETRIEVE-AND-MUTATE: prefer a proven skeleton over scratch
    if tmpl:
    ...
    build = py_function_src_teleon_synthesis_dag_contract__verify_buildable_dag(v["nodes"], v["edges"])
    ...
    return {"intent": intent, "capability": I.py_function_src_teleon_synthesis_intent_to_dag__outline(intent).get("capability"), "live": True, "pool_size": len(pool),
...
def run_live(intent: str) -> dict:
    ...
    if art.get("accepted"):
        try:
            from src.teleon.synthesis.variation_store import py_function_src_teleon_synthesis_variation_store__record_variations
            path = "/".join(n["component"] for n in art["composed_dag"]["nodes"])
            py_function_src_teleon_synthesis_variation_store__record_variations(art.get("capability") or "adhoc", [{"decision_path": path, "status": "compiled_live", "strategy": "llm_orchestrated", "note": intent[:80]}])
...
def _self_test() -> int:
    ...
    ck("type-aware edge check: ocr(text)->field_parsing(text) compatible; tts(audio)->field_parsing(text) NOT",
       py_function_src_teleon_synthesis_io_contracts__edge_compatible("ocr", "field_parsing") and not py_function_src_teleon_synthesis_io_contracts__edge_compatible("tts", "field_parsing"))
    ...
    _o0, _ollm = REG.py_var_src_teleon_components_registry___PLANE_INVOKERS.get(plane0), REG.py_var_src_teleon_components_registry___PLANE_INVOKERS.get("llm")
    REG.py_function_src_teleon_components_registry__register_component_invoker(plane0, lambda inp: {"text": "stub", **({} if plane0 == "llm" else {})})
    REG.py_function_src_teleon_components_registry__register_component_invoker("llm", lambda inp: {"text": "stub-llm"})
    try:
        ...
        for _pl, _fn in ((plane0, _o0), ("llm", _ollm)):
            REG.py_var_src_teleon_components_registry___PLANE_INVOKERS.pop(_pl, None)
            if _fn:
                REG.py_function_src_teleon_components_registry__register_component_invoker(_pl, _fn)
