...
CLI: PYTHONPATH=. python3 _repos/shared-backend-components/scripts/flywheel_orchestrator.py --forever
"""
...
STATE = _resource("data") / "dev-intel" / "flywheel-state.json"
STATUS = _resource("data") / "dev-intel" / "flywheel-status.md"
PID = _resource("data") / "dev-intel" / "flywheel.pid"      # liveness: the --forever process writes its pid here
LOG = _resource("data") / "dev-intel" / "flywheel.log"      # detached-run log (for --supervise)
...
AUTO_APPLIED = _resource("data") / "dev-intel" / "auto-applied.jsonl"


def _findings_count() -> int:
    f = _resource("data") / "dev-intel" / "findings.jsonl"
...
def _fw_status(state: dict) -> dict:
    ...
    radar = _resource("data") / "dev-intel" / "research-radar.jsonl"
...
def _fw_adapters() -> dict:
    ...
    try:
        ...
        layers = _json.loads((_resource("architecture") / "adapter_layers.json").read_text(encoding="utf-8"))["layers"]
        ...
        planes = _json.loads((_resource("architecture") / "tool_planes.json").read_text(encoding="utf-8"))["planes"]
        ...
        reg = _json.loads((_resource("architecture") / "tool_registry.json").read_text(encoding="utf-8"))["tools"]
        n_tools = len(reg)
        ...
        laddered = {L.get("plane") for L in _json.loads((_resource("architecture") / "capability_ladders.json").read_text(encoding="utf-8"))["ladders"]}
