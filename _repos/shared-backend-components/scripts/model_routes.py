...
The text-generation sibling of ``_repos/shared-backend-components/scripts/embeddings.py``: ONE resolver, env-
...
# DEPENDENCY-LAW direction (verified before wiring): _repos/shared-backend-components/architecture/portfolio_dependency_law.json
# forbids import edges only AMONG the three brand package roots (_repos/openhubforai/backend/src/openhubforai must not import
# src/teleon, etc.). _repos/shared-backend-components/scripts/ is repo TOOLING, not a brand layer — the law file itself says so and
# _repos/shared-backend-components/scripts/check_portfolio_dependency_law.py scans only the src/* roots; ~46 scripts already import
...
if str(_REPO) not in sys.path:  # lets `python3 _repos/shared-backend-components/scripts/model_routes.py` work like `-m scripts...`
...
#: provider-graph node ids this route binds to (ids defined in _repos/shared-backend-components/architecture/model_provider_graph.json)
...
# _repos/shared-backend-components/architecture/model_provider_graph.json (never a second literal here), so route and graph cannot drift.
