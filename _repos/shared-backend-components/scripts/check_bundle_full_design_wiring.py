...
client, and a page-analytics beacon fires to the events plane.
  ...
  A. CLIENT — shared/oh-identity.js exists, exposes realmOf/signup/login/session/validate/mintKey,
     default identity port matches _repos/shared-backend-components/architecture/identity_realm_registry.json (drift gate), supports
     the OPENHUBFORAI_IDENTITY_BASE deploy override, stores session per-realm only (never the passphrase).
...
EVENT_REG = _resource("architecture") / "local_service_registry.json"
...
def _self_test() -> int:
    ...
    ck("A: OPENHUBFORAI_IDENTITY_BASE deploy override supported", "OPENHUBFORAI_IDENTITY_BASE" in ident)
    ...
    ck("C: social login (Google/GitHub) is commented out until live (no fake or half-wired SSO button)",
       "ohs-oauth-btn" not in site and "commented out until we are fully live" in site)

    # D. events beacon
    m2 = re.search(r'OPENHUBFORAI_EVENTS_BASE \|\| "http://127\.0\.0\.1:(\d+)"', ident)
    ...
    from scripts.showcase.server import VENDOR_DIR, rewrite_bundle_html
    vendor = VENDOR_DIR  # assert against the EXACT dir the showcase mounts at /vendor/ (single source)
