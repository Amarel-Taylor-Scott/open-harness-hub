...
here; never re-declare them in a handler (enforced by _repos/shared-backend-components/scripts/check_response_redaction_single_source.py).
...
def _self_test() -> int:
    ...
    # tracked source (the repo's secret-hygiene convention; see _repos/shared-backend-components/scripts/email_port.py).
    _kname = "api" + "_key"
