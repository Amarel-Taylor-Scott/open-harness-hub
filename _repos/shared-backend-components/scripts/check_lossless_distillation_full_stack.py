...
CLI: PYTHONPATH=. python3 _repos/shared-backend-components/scripts/check_lossless_distillation_full_stack.py --self-test
"""
...
def _run_proof(relpath: str) -> tuple[bool, str]:
    ...
    proc = subprocess.run([sys.executable, str(_resource(relpath)), "--self-test"],
                          cwd=str(_REPO), capture_output=True, text=True,
                          env={"PYTHONPATH": _pythonpath("."), "PATH": ""})
