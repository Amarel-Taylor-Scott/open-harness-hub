...
Complements _repos/shared-backend-components/scripts/code_graph.py (which is file/module import-level): this adds the finer nodes + edges a review
...
edge points at the right symbol, and the load-bearing ranking is trustworthy. Offline, stdlib only. serves_truth=false.
  ...
  --self-test   prove nodes+edges are extracted (classes/functions/methods + calls/inherits) over _repos/teleon/backend/src/teleon
  --emit        write _repos/shared-backend-components/docs/context/symbol-graph.generated.json + .md
  --print       print the compact graph
CLI: PYTHONPATH=. python3 _repos/shared-backend-components/scripts/symbol_graph.py --print
"""
...
# Single source for module naming + file discovery, shared with the file layer so the two graphs agree on module ids
# after the _repos/ migration (a relocated backend is `src.<x>.…`, substrate stays `scripts.…`). code_graph imports
# only _repo_paths + stdlib, so this import introduces no cycle (codegraph loads code_graph before symbol_graph).
from scripts.code_graph import _canonical_rel as _cg_canonical_rel, _module_of as _cg_module_of
...
GRAPH_JSON = _resource("docs") / "context" / "symbol-graph.generated.json"
GRAPH_MD = _resource("docs") / "context" / "symbol-graph.generated.md"
...
def _modname(path: Path) -> str:
    return _cg_module_of(path)          # single source: canonical, import-matching module name (src.<x>.… / scripts.…)


def _iter_py(roots: list[str]):
    # A logical root maps to its real post-migration home via _resource; additionally the logical `src` root now
    # spans EVERY relocated product backend `_repos/<x>/backend/src` (the src.<x> code). Without this expansion
    # `_resource("src")` alone points at a non-existent top-level `src/` and the backend is never scanned — so the
    # symbol layer would miss the same src.<x> code the file layer scans, and the fused graph would disagree. Deduped.
    seen: set[str] = set()
    for spec in roots:
        bases = [_resource(spec)]
        if str(spec).replace("\\", "/").strip("/") == "src":
            bases += sorted((REPO / "_repos").glob("*/backend/src"))
        for base in bases:
            if str(base) in seen:
                continue
            seen.add(str(base))
...
def build_graph(roots: list[str] | None = None) -> dict:
    ...
    for f in _iter_py(roots):
        ...
        nodes.append({"id": mod, "kind": "module", "file": _cg_canonical_rel(f)})
...
def _self_test() -> int:
    ...
    g = build_graph(["_repos/teleon/backend/src/teleon"])
