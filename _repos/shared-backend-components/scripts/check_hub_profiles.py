"""check_hub_profiles — anti-drift proof for _repos/shared-backend-components/architecture/hub_profiles.json.
...
This check enforces that invariant bidirectionally against the single sources of truth:
  - the 9 live hubs + the active candidates come from _repos/shared-backend-components/architecture/candidate_open_hubs.json (existing_hubs + candidates);
...
_POLICY = _resource("architecture") / "candidate_open_hubs.json"
