...
CLI: python3 _repos/shared-backend-components/scripts/check_no_direct_provider_bypass.py --self-test
"""
...
_LLM_GATEWAY_DIRS = ("scripts/llm_gateway", "_repos/baltor/backend/src/baltor/llm_gateway")
_GOVERNED = ["scripts/runtime", "scripts/artifact_graph", "scripts/security", "_repos/baltor/backend/src/baltor"]
...
def _self_test() -> int:
    ...
    check("the LLM gateway seam exists (router)", (_resource("scripts/llm_gateway/router.py")).exists())
