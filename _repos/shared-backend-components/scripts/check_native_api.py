...
(_repos/shared-backend-components/scripts/api_native_handler.py) is projection-safe. Tests the PURE request handler (no socket), so the
...
Deterministic + offline. CLI: python3 _repos/shared-backend-components/scripts/check_native_api.py --self-test
"""
