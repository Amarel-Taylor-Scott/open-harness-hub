...
The single source `_repos/shared-backend-components/architecture/adapter_layers.json` tracks which swappable LAYERS sit behind an agnostic port/wrapper
...
The 'adapters' flywheel consumes this to file gap proposals. serves_truth=false.
  ...
  python3 _repos/shared-backend-components/scripts/check_adapter_layers.py --self-test
"""
...
REGISTRY = _resource("architecture") / "adapter_layers.json"
...
def _self_test() -> int:
    ...
    missing = [L["layer"] for L in layers if L.get("status") == "wired"
               and not (L.get("port_module") and (_resource(L["port_module"])).exists())]
    ...
    ck("reranker layer is now WIRED (loop adapter-gap closed with a real port)", "reranker" not in cov["gaps"])
    ck("remaining gaps are surfaced honestly (e.g. tts-stt — for the agent to wrap)", "tts_stt_generation" in cov["gaps"])
