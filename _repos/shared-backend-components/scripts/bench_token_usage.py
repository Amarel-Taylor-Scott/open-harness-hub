...
_llm_client's usage field), cost (from _repos/shared-backend-components/architecture/model_index.json), and reliability (deterministic pass-check
over N trials):
...
Builds on _repos/shared-backend-components/scripts/eval/measured_lift_headtohead.py (the with/without A/B) + economics/cost_model.
...
MODEL_INDEX = _resource("architecture") / "model_index.json"
OUT = _resource("data") / "dev-intel" / "benchmarks.jsonl"
