...
report backs _repos/shared-backend-components/docs/strategy/teleon-go-live-readiness-and-sprints-2026-06.md.

CLI: python3 _repos/shared-backend-components/scripts/check_teleon_go_live_readiness.py --self-test | --report
"""
...
def _built() -> list[dict]:
    ...
    verify("governed candidate corpus (discovered feeds present)",
           lambda: len(list((_resource("data") / "capability-candidates").glob("discovered-feed-*.json"))) >= 10)
    verify("deploy topology config (Fly/compose generator inputs present)",
           lambda: (_resource("architecture") / "deploy_topology.json").exists())
    return out
...
def _live_llm_verified() -> bool:
    ...
    import json
    p = _resource("data") / "dev-intel" / "live-llm-smoke.json"
    try:
...
def _live_eval_verified() -> bool:
    ...
    import json
    p = _resource("data") / "dev-intel" / "live-eval-smoke.json"
    try:
        return bool(json.loads(p.read_text(encoding="utf-8")).get("verified"))
    except Exception:  # noqa: BLE001
        return False
...
def _live_distillation_verified() -> bool:
    ...
    import json
    p = _resource("data") / "dev-intel" / "live-distillation-smoke.json"
    try:
        return bool(json.loads(p.read_text(encoding="utf-8")).get("verified"))
    except Exception:  # noqa: BLE001
        return False
