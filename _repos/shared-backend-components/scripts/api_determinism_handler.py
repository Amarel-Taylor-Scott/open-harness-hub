...
the admin server just delegates to it (model: _repos/shared-backend-components/scripts/api_context_handler.py, _repos/shared-backend-components/scripts/api_memory_handler.py).
...
if __name__ == "__main__":  # tiny manual smoke (not the proof — see _repos/shared-backend-components/scripts/check_determinism_api.py)
    for r in ROUTES:
