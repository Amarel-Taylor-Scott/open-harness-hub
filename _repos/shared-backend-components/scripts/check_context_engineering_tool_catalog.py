...
context-engineering tool landscape (_repos/shared-backend-components/architecture/context_engineering_tool_catalog.json) is GOVERNED
...
Asserts the governance shape that keeps these tools a due-diligence landscape rather than an adopted runtime:
  ...
  * governance_law references the lossless law (_repos/shared-backend-components/docs/codex/lossless-distillation.md) AND the word "candidate".
...
CLI:    PYTHONPATH=. python3 _repos/shared-backend-components/scripts/check_context_engineering_tool_catalog.py --self-test
...
_CATALOG = _resource("architecture") / "context_engineering_tool_catalog.json"
