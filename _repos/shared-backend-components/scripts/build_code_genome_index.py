"""build_code_genome_index — DOGFOOD the Code Genome (#§9) on our own source: fingerprint every _repos/teleon/backend/src/teleon module
...
data/dev-intel (not git-tracked prose). serves_truth=false.
  ...
  python3 _repos/shared-backend-components/scripts/build_code_genome_index.py            # write the index + report
  python3 _repos/shared-backend-components/scripts/build_code_genome_index.py --self-test
"""
...
from src.teleon.knowledge.code_genome import py_function_src_teleon_knowledge_code_genome__ast_fingerprint, py_function_src_teleon_knowledge_code_genome__decompose_ast, py_function_src_teleon_knowledge_code_genome__genome_similarity  # noqa: E402

_SRC = _resource("src/teleon")
_OUT = _resource("data") / "dev-intel" / "code_genome_index.json"
...
def _fingerprint_modules() -> list[dict]:
    ...
    for p in sorted(_SRC.rglob("*.py")):
        ...
        try:
            ...
            continue
        prims = py_function_src_teleon_knowledge_code_genome__decompose_ast(src)
        if len(prims) >= _MIN_PRIMITIVES:
            mods.append({"path": str(p.relative_to(_REPO)), "primitives": prims, "fingerprint": py_function_src_teleon_knowledge_code_genome__ast_fingerprint(src)})
    return mods
...
def _near_duplicates(mods: list[dict]) -> list[dict]:
    ...
    for a, b in combinations(mods, 2):
        sim = round(py_function_src_teleon_knowledge_code_genome__genome_similarity(a["fingerprint"], b["fingerprint"]), 4)
        if sim >= _DUP:
...
def main() -> int:
    ...
    if fails:
        ...
        return 1
    print(f"PASS - build_code_genome_index: dogfooded the Code Genome on {idx['modules_fingerprinted']} _repos/teleon/backend/src/teleon "
