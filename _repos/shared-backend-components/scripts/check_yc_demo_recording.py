...
CLI: PYTHONPATH=. python3 _repos/shared-backend-components/scripts/check_yc_demo_recording.py --self-test
"""
...
STORYBOARD = _resource("architecture") / "yc_demo_storyboard.json"
RECORDER = _resource("e2e") / "record_yc_demo.mjs"
DOCKERFILE = _resource("deploy") / "demo-web.Dockerfile"
...
def validate(sb: dict) -> list[str]:
    ...
    for i, s in enumerate(scenes):
        ...
        tgt = s.get("target")
        if tgt and not (_resource(tgt)).exists():
...
def tool_status() -> dict:
    ...
    pw = (_resource("node_modules") / ".bin" / "playwright").exists() or bool(shutil.which("playwright"))
