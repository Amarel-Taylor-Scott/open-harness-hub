...
CLI: python3 _repos/shared-backend-components/scripts/check_examples_video_verifier.py [--self-test]
"""
...
VERIFIER = _resource("e2e") / "verify_examples_gallery_videos.mjs"
RECORDER = _resource("e2e") / "record_examples_gallery.mjs"
EXPLORER = _resource("e2e") / "confused_user_explore.mjs"  # confused-user UX explorer (same toolchain)
STRESS = _resource("e2e") / "qa_stress_test.mjs"           # hostile QA stress harness (same toolchain)
RESPONSIVE = _resource("e2e") / "responsive_audit.mjs"     # device-matrix responsive audit (same toolchain)
...
def _self_test() -> int:
    ...
    for f in (VERIFIER, RECORDER, EXPLORER, STRESS, RESPONSIVE):
        if not f.exists():
