...
Usage:
  python _repos/shared-backend-components/scripts/demo_esg_pipeline.py            # all 3 samples (full catalog load — interactive)
  python _repos/shared-backend-components/scripts/demo_esg_pipeline.py T3         # just the Mae Sot one
  python _repos/shared-backend-components/scripts/demo_esg_pipeline.py --self-test  # BOUNDED scoped run (one pipeline file, simulate, offline)
"""
...
from scripts.run_pipeline import run_pipeline, load_catalog

_ROOT = Path(__file__).resolve().parent.parent
_PIPELINE_FILE = _resource("catalog/pipelines/esg-supplier-grading/supplier-policy-grading.yaml")
...
def pick_samples(filter_term: str | None) -> list[Path]:
    base = _resource("data/supplier-disclosure-samples")
