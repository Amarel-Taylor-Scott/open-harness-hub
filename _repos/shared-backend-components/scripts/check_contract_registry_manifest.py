...
CLI: python3 _repos/shared-backend-components/scripts/check_contract_registry_manifest.py --self-test
"""
...
def _self_test() -> int:
    ...
    for r in _REGISTRIES:
        for item in reg[r]:
            ...
            if "schema" in item and not (_resource(item["schema"])).exists():
