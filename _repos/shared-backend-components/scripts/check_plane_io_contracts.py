...
an un-typed plane is treated as compatible (never blocks). serves_truth=false.
  ...
  python3 _repos/shared-backend-components/scripts/check_plane_io_contracts.py --self-test
"""
...
from src.teleon.synthesis.io_contracts import py_function_src_teleon_synthesis_io_contracts__edge_compatible, py_function_src_teleon_synthesis_io_contracts__load_contracts, py_function_src_teleon_synthesis_io_contracts__plane_io
...
def _self_test() -> int:
    ...
    c = py_function_src_teleon_synthesis_io_contracts__load_contracts()
    ...
    real = {x["plane"] for x in json.loads((_resource("architecture") / "tool_planes.json").read_text())["planes"]}
    ...
    ck("edge_compatible: a type-matching edge is accepted (ocr text -> field_parsing text)",
       py_function_src_teleon_synthesis_io_contracts__edge_compatible("ocr", "field_parsing"))
    ck("edge_compatible: a mismatch is FLAGGED (tts audio -> field_parsing text)",
       not py_function_src_teleon_synthesis_io_contracts__edge_compatible("tts", "field_parsing"))
    ck("edge_compatible: chain ocr->embedding->vector_store is type-valid",
       py_function_src_teleon_synthesis_io_contracts__edge_compatible("ocr", "embedding") and py_function_src_teleon_synthesis_io_contracts__edge_compatible("embedding", "vector_store"))
    ck("edge_compatible: an UN-TYPED plane never blocks (treated compatible)",
       py_function_src_teleon_synthesis_io_contracts__edge_compatible("ocr", "a_plane_not_in_the_contract") and py_function_src_teleon_synthesis_io_contracts__edge_compatible(None, "llm"))
    ck("plane_io returns the contract for a typed plane and None for an un-typed one",
       py_function_src_teleon_synthesis_io_contracts__plane_io("embedding") is not None and py_function_src_teleon_synthesis_io_contracts__plane_io("nope") is None)
