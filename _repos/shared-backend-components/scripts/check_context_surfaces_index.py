...
Model: _repos/shared-backend-components/scripts/check_memory_page_projection_only.py + _repos/shared-backend-components/scripts/check_native_ui.py + _repos/shared-backend-components/scripts/check_determinism_ui.py.
...
The served context surfaces (route → page) are the canonical six:
  /pipeline     -> _repos/baltor/frontend/pipeline/index.html  (the seven-stage pipeline journey)
  /memory       -> _repos/baltor/frontend/memory.html           (the memory projection)
  /consume      -> _repos/baltor/frontend/consume.html          (ingestion -> consumption ContextResponse)
  /standards    -> _repos/baltor/frontend/standards.html        (configuration standards + maturity)
  /determinism  -> _repos/baltor/frontend/determinism.html      (the Determinism Factory projection)
  /native       -> _repos/baltor/frontend/native.html           (native-output preview + lossless sidecar/diff)
...
CLI: python3 _repos/shared-backend-components/scripts/check_context_surfaces_index.py --self-test
"""
...
#: Keep this in sync with _repos/shared-backend-components/docs/ui/context-surfaces.md and the _repos/baltor/frontend/hub.html cards (MAIN integrates the cards).
SURFACES: tuple[tuple[str, str, str], ...] = (
    ("/pipeline",    "_repos/baltor/frontend/pipeline/index.html", "the seven-stage pipeline journey (upload -> consumption)"),
    ("/memory",      "_repos/baltor/frontend/memory.html",         "the memory projection (artifacts, claim_status, traces, held-out)"),
    ("/consume",     "_repos/baltor/frontend/consume.html",        "ingestion -> consumption: the served ContextResponse for CFPB"),
    ("/standards",   "_repos/baltor/frontend/standards.html",      "configuration standards, patterns, routines, maturity, waivers"),
    ("/determinism", "_repos/baltor/frontend/determinism.html",    "the Determinism Factory (traces -> consensus -> rules -> promotions)"),
    ("/native",      "_repos/baltor/frontend/native.html",         "native-output preview + lossless sidecar/diff (original never overwritten)"),
)
...
def _self_test() -> int:
    ...
    doc = _resource("docs") / "ui" / "context-surfaces.md"
