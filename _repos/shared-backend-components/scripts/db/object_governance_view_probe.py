#!/usr/bin/env python3
"""Probe object-governance operational views.

This is a side-effect-free readiness probe. Without a database URL it verifies
that the canonical schema declares the expected views. With DATABASE_URL or
--database-url it also checks live Postgres view presence and row counts.
"""
...
from scripts._config import OBJECT_GOVERNANCE_OPERATIONAL_VIEWS

DEFAULT_SCHEMA_SQL = _resource("db") / "postgres" / "schema.sql"
...
def _schema_view_presence(schema_sql: str | Path) -> list[dict[str, Any]]:
    ...
    return [
        ...
        for view in OBJECT_GOVERNANCE_OPERATIONAL_VIEWS
    ]
...
def probe_object_governance_views(
    *,
    ...
    database_probe = (
        _database_view_probe(database_url, OBJECT_GOVERNANCE_OPERATIONAL_VIEWS)
        if database_url
        else {
            "mode": "not_configured",
            "configured": False,
    ...
    return {
        ...
        "expected_views": list(OBJECT_GOVERNANCE_OPERATIONAL_VIEWS),
...
def _self_test() -> int:
    result = probe_object_governance_views()
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Probe object-governance operational views.")
