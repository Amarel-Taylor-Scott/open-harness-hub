#!/usr/bin/env python3
"""Emit a side-effect-free local Postgres smoke plan for theory component rows and vectors."""
...
from scripts._config import (
    ...
    LOCAL_DOCKER_PGVECTOR_CANDIDATE_TARGET,
...
def build_theory_local_postgres_smoke_plan(
    *,
    ...
    theory_summary = _read_json(theory_batch_summary)
    load_audit = _read_json(theory_load_audit_summary)
    vector_load = _read_json(pgvector_load_plan_summary)
    out = Path(output_dir) if output_dir else Path(tempfile.mkdtemp(prefix="ohh-theory-postgres-smoke-"))
    ...
    if not candidate_load_sql:
        raise ValueError("theory load audit summary does not include bulk_manifest.load_sql")
    ...
    if not load_plan_manifest:
        raise ValueError("theory load audit summary does not include load_plan_manifest")
    ...
    _require_path(load_plan_manifest, "load_plan")
    ...
    plan_output = out / "theory-local-postgres-smoke-plan.json"

    candidate_rows = int((theory_summary.get("row_counts") or {}).get("normalized_object", 0) or 0)
    candidate_unique_total = int((load_audit.get("merge_report") or {}).get("unique_total", 0) or 0)
    ...
    commands = [
        ...
        {
            "step": "apply_theory_candidate_rows",
            "command": f"psql {psql_target} -f {candidate_load_sql}",
        },
        {
            "step": "apply_theory_embedding_vectors",
    ...
    result = {
        ...
        "target": LOCAL_DOCKER_PGVECTOR_CANDIDATE_TARGET,
        "inputs": {
            "theory_batch_summary": str(theory_batch_summary),
            "theory_load_audit_summary": str(theory_load_audit_summary),
        ...
        "readiness": {
            ...
            "candidate_unique_staged_rows": candidate_unique_total,
        ...
        "safety_notes": [
            ...
            "Run the commands only after reviewing candidate row SQL, vector SQL, and the destination DATABASE_URL.",
            "Active promotion remains blocked until review queues and committed-count audits are resolved.",
        ],
    }
...
def _self_test() -> int:
    with tempfile.TemporaryDirectory() as tmp:
        result = build_theory_local_postgres_smoke_plan(
            theory_batch_summary="dist/theory-component-batch/2026-05-26/theory-component-batch-summary.json",
            theory_load_audit_summary="dist/theory-component-batch-load-audit/2026-05-26/summary.json",
            pgvector_load_plan_summary=dated_artifact_path(
                THEORY_PGVECTOR_LOAD_PLAN_DIST_DIR,
                SELF_TEST_DATE,
                PGVECTOR_LOAD_PLAN_SUMMARY_FILENAME,
            ),
            embedding_execution_plan="dist/theory-batch-governance-bridge/2026-05-26/embedding/embedding-plan/embedding-execution-plan.json",
            vector_readiness_summary="dist/theory-local-hash-embedding-worker/2026-05-26/vector-readiness/vector-readiness-summary.json",
            output_dir=tmp,
            run_id="self-test",
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--theory-load-audit-summary")
    ...
    parser.add_argument("--run-id", default="theory-local-postgres-smoke")
    ...
    if args.self_test:
        return _self_test()
    required = [
        args.theory_batch_summary,
        args.theory_load_audit_summary,
        args.pgvector_load_plan_summary,
        args.embedding_execution_plan,
        args.vector_readiness_summary,
    ]
    if not all(required):
        parser.error("theory batch, load audit, pgvector load, embedding plan, and vector readiness inputs are required unless --self-test is used")
    result = build_theory_local_postgres_smoke_plan(
        theory_batch_summary=args.theory_batch_summary,
        theory_load_audit_summary=args.theory_load_audit_summary,
