...
DEFAULT_OUTPUT = _resource("dist") / "catalog-migration-smoke" / "catalog-yaml-reader-audit.json"
...
def default_paths() -> list[Path]:
    return [
        *list((_resource("scripts")).rglob("*.py")),
        *list((_resource("docs") / "architecture").rglob("*.md")),
    ]
...
def main(argv: list[str] | None = None) -> int:
    ...
    paths = [path if path.is_absolute() else _resource(path) for path in args.path] if args.path else default_paths()
