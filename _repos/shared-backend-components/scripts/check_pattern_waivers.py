#!/usr/bin/env python3
"""scripts.check_pattern_waivers — proof: _repos/shared-backend-components/architecture/pattern_waivers.json is well-formed and honest.
...
CLI: python3 _repos/shared-backend-components/scripts/check_pattern_waivers.py --self-test
"""
...
def _self_test() -> int:
    ...
    missing_paths = [w.get("waiver_id") for w in data.get("waivers", [])
                     if w.get("file_path") and not (_resource(w["file_path"])).exists()]
