#!/usr/bin/env python3
"""scripts.check_current_state_freshness — PROOF: the orientation doc _repos/shared-backend-components/docs/CURRENT-STATE.md cannot silently ROT.
...
Asserts:
  A. EXISTS: _repos/shared-backend-components/docs/CURRENT-STATE.md is present and non-trivial.
  ...
  D. NO DANGLING HANDLES (anti-rot): every concrete repo path the doc cites in backticks (_repos/shared-backend-components/scripts/…, _repos/shared-backend-components/docs/…,
     _repos/shared-backend-components/architecture/…, web/…, src/…, schemas/…, prompts/…, .agent/…, _repos/shared-backend-components/catalog/…, dist/…, fixtures/…, rubrics/…) resolves
...
from scripts._repo_paths import resource as _resource, repo_root as _repo_root
...
_DOC = _resource("docs") / "CURRENT-STATE.md"


def _on_disk(rel: str) -> bool:
    """True if a cited repo-relative path exists — via the universal resolver OR the repo-root fallback.
    `_resource` strips a leading dot (`.agent/...` -> `agent/...`), so dotfile dirs like `.agent`
    (a root symlink to the canonical generated `.agent`) must resolve through the true repo root."""
    return _resource(rel).exists() or (_repo_root() / rel).exists()
...
def _self_test() -> int:
    ...
    missing = [p for p in cited if not _on_disk(p)]
