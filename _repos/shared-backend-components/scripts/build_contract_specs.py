#!/usr/bin/env python3
"""scripts.build_contract_specs — GENERATE OpenAPI + AsyncAPI specs from _repos/shared-backend-components/architecture/contract_registry.json.

This is the machine-checked projection of the contract registry — NOT a hand-edited source. The curated narrative
specs (_repos/shared-backend-components/docs/contracts/{openapi,asyncapi}.runtime.yaml) are preserved; these GENERATED docs are an additive derived
layer that must cover every registered route/channel and is regenerated, never hand-edited.
  ...
  - OpenAPI 3.1 from contract_registry.api_routes (one path+method per registered route; bodies/returns reference the
    ...
    routes carry x-projection-only). Includes the _repos/teleon/backend/src/teleon/inference/api_projection /api/inference/* read routes.
...
_REG = _resource("architecture") / "contract_registry.json"
_OUT = _resource("docs") / "contracts"
_OPENAPI = _OUT / "openapi.generated.json"
_ASYNCAPI = _OUT / "asyncapi.generated.json"
# $ref paths are relative to _repos/shared-backend-components/docs/contracts/ (mirrors the hand-written *.runtime.yaml convention)
_REF_PREFIX = "../../schemas/"
...
def _schema_index() -> dict:
    ...
    idx: dict[str, str] = {}
    for f in sorted((_resource("schemas")).rglob("*.schema.json")):
        rel = f.relative_to(_resource("schemas")).as_posix()
