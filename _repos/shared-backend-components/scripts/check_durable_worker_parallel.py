...
CLI:
    python3 _repos/shared-backend-components/scripts/check_durable_worker_parallel.py --self-test
"""
...
def _self_test() -> int:
    ...
    t0 = time.time()
    procs = [subprocess.Popen([sys.executable, str(_resource("scripts/flywheel_worker.py")), "--db", db, "--queue", queue,
