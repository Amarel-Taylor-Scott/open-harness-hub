...
port (src.teleon.storage.record_store) governed by a single-source tier policy (_repos/shared-backend-components/architecture/storage_tier_policy.json).
This proves:
...
CLI: PYTHONPATH=. python3 _repos/shared-backend-components/scripts/check_scalable_record_store.py --self-test
"""
...
def _self_test() -> int:
    ...
    ck("every config stream points at an existing JSON file", all((_resource(s["json"])).exists() for s in cfg), str([s["stream"] for s in cfg if not (_resource(s.get("json", ""))).exists()]))
    ...
    #     silently local; full proof in _repos/shared-backend-components/scripts/check_postgres_record_store.py); history stays the named
