...
EXISTING ``MarkdownFolderAdapter`` (``_repos/baltor/backend/src/baltor/adapters/source/markdown_folder.py``) on its own demo vault —
...
CLI: PYTHONPATH=. python3 _repos/shared-backend-components/scripts/check_source_classification_rule_distillation.py --self-test
"""
...
class RuleCandidate:
    ...
    asserts_equivalence_to: str = "_repos/baltor/backend/src/baltor/adapters/source/markdown_folder.py"
    active: bool = False
...
def _self_test() -> int:
    ...
    check("the rule asserts equivalence to the existing markdown adapter",
          rule.asserts_equivalence_to == "_repos/baltor/backend/src/baltor/adapters/source/markdown_folder.py")
