...
_repos/shared-backend-components/architecture/standards_interop_manifest.json lists every adopted/declined standard with its direction, status,
...
_MANIFEST = _resource("architecture") / "standards_interop_manifest.json"
#: the generated public surface (opencontexthub site). Generated — do not hand-edit.
_PAGE = _resource("dist") / "sites" / "opencontexthub" / "interop.html"
...
def _self_test() -> int:
    ...
    bad_proof = [s["id"] for s in stds if s.get("proof") and not (_resource(s["proof"])).exists()]
    ...
    bad_map = [s["id"] for s in stds if s["status"] == "mapped" and not (_resource(s["module"])).exists()]
