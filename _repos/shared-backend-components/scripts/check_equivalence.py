...
separates a disagreeing one; every verdict is flagged proven=False / benchmark_relative=True. serves_truth=false.
  ...
  python3 _repos/shared-backend-components/scripts/check_equivalence.py --self-test
"""
...
def _self_test() -> int:
    ...
    ck("agreement = fraction of matching probe outputs", EQ.py_function_src_teleon_synthesis_equivalence__agreement(paddle, tess) == 1.0 and EQ.py_function_src_teleon_synthesis_equivalence__agreement(paddle, weak) == 0.5)
    eqv = EQ.py_function_src_teleon_synthesis_equivalence__equivalent(paddle, tess)
    ...
    ck("two runners that disagree are NOT equivalent at threshold", EQ.py_function_src_teleon_synthesis_equivalence__equivalent(paddle, weak)["equivalent"] is False)
    ck("length mismatch / empty -> 0 agreement (no false equivalence)", EQ.py_function_src_teleon_synthesis_equivalence__agreement(paddle, ["acme inc"]) == 0.0 and EQ.py_function_src_teleon_synthesis_equivalence__agreement([], []) == 0.0)

    classes = EQ.py_function_src_teleon_synthesis_equivalence__equivalence_classes({"paddleocr": paddle, "tesseract": tess, "weak_ocr": weak})
