...
serves_truth=false.
  ...
  python3 _repos/shared-backend-components/scripts/check_variation_store.py --self-test
"""
...
def _self_test() -> int:
    ...
    r1 = V.py_function_src_teleon_synthesis_variation_store__record_variations(cap, variations)
    ck("variations persist to version_history (winners + losers, lossless)", r1["total"] == 3)
    r2 = V.py_function_src_teleon_synthesis_variation_store__record_variations(cap, variations)
    ck("re-record is idempotent (version_hash dedupe — no dup rows)", r2["stored"] == 0)
    got = {v["status"] for v in V.py_function_src_teleon_synthesis_variation_store__list_variations(cap)}
    ...
    g = V.py_function_src_teleon_synthesis_variation_store__commit_to_git(cap, variations, platform="internal", now="2026-06-22T00:00:00Z")
    ...
    gh = V.py_function_src_teleon_synthesis_variation_store__commit_to_git(cap, variations, platform="github", now="2026-06-22T00:00:00Z", token_present=False)
    ...
    gh2 = V.py_function_src_teleon_synthesis_variation_store__commit_to_git(cap, variations, platform="github", now="2026-06-22T00:00:00Z", token_present=True)
    ck("github with a token -> pushes (winner main + loser branches)", gh2.get("pushed") is True)
    ck("unsupported platform -> honest error", "error" in V.py_function_src_teleon_synthesis_variation_store__commit_to_git(cap, variations, platform="dropbox", now="x"))

    # the other/hosting credential KINDS
    ck("credential kinds modeled (git_token + deploy_token beyond api_key)",
       C.py_function_src_teleon_runtime_credentials__credential_kind("github") == "git_token" and C.py_function_src_teleon_runtime_credentials__credential_kind("fly") == "deploy_token")
    ck("gitlab git_token + fly/cloudflare deploy (hosting) keys registered",
       "gitlab" in C.py_function_src_teleon_runtime_credentials__services_by_kind("git_token") and {"fly", "cloudflare_ai"} <= set(C.py_function_src_teleon_runtime_credentials__services_by_kind("deploy_token")))
    ck("default kind is api_key", C.py_function_src_teleon_runtime_credentials__credential_kind("tavily") == "api_key")
