...
falling back honestly to the last stage when nothing clears the bar. serves_truth=false.
  ...
  python3 _repos/shared-backend-components/scripts/check_cascade.py --self-test
"""
...
def _self_test() -> int:
    ...
    ck("agreement_score = majority answer + its fraction", C.py_function_src_teleon_synthesis_cascade__agreement_score(["A", "A", "A", "B"]) == {"majority": "A", "agreement": 0.75, "n": 4})
    ck("unanimous cheap ensemble -> STOP with that answer", C.py_function_src_teleon_synthesis_cascade__should_escalate(["A", "A", "A"])["decision"] == "stop")
    split = C.py_function_src_teleon_synthesis_cascade__should_escalate(["A", "B", "C"], threshold=0.7)
    ...
    ck("agreement is the training-free confidence signal (2/3 majority clears a 0.6 bar, escalates at 0.7)",
       C.py_function_src_teleon_synthesis_cascade__should_escalate(["A", "A", "B"], threshold=0.6)["decision"] == "stop" and C.py_function_src_teleon_synthesis_cascade__should_escalate(["A", "A", "B"], threshold=0.7)["decision"] == "escalate")

    # full cascade: cheap stage is split (escalate) -> strong stage agrees (stop), counting one escalation
    res = C.py_function_src_teleon_synthesis_cascade__run_cascade([
    ...
    ck("cascade stops EARLY (no escalation) when the cheap stage is already confident",
       C.py_function_src_teleon_synthesis_cascade__run_cascade([{"name": "cheap", "responses": ["A", "A", "A"]}, {"name": "frontier", "responses": ["Z"]}])["escalations"] == 0)

    # honest exhaustion: nothing clears the bar -> last stage's majority, flagged exhausted
    ex = C.py_function_src_teleon_synthesis_cascade__run_cascade([{"name": "a", "responses": ["A", "B"]}, {"name": "b", "responses": ["C", "D"]}], threshold=0.9)
