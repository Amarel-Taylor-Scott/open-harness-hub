...
catalog (_repos/shared-backend-components/architecture/stateful_swarm_provider_catalog.json) and the blackboard provider catalog
(_repos/shared-backend-components/architecture/blackboard_provider_catalog.json) are well-formed and encode the governance boundary for the
...
CLI: PYTHONPATH=. python3 _repos/shared-backend-components/scripts/check_stateful_swarm_provider_catalog.py --self-test
"""
...
_BLACKBOARD_CATALOG = _resource("architecture") / "blackboard_provider_catalog.json"
