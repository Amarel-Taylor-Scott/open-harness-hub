...
The registry SPINE (`_repos/teleon/backend/src/teleon/registry/index.reconciled_index`) unions the ontology TYPES + the live CATALOGS into
...
def self_test() -> int:
    from src.teleon.registry.index import py_function_src_teleon_registry_index__records, py_function_src_teleon_registry_index__reconciled_index, py_function_src_teleon_registry_index__to_registry_object
    ...
    o = py_function_src_teleon_registry_index__to_registry_object({"id": "x", "name": "X", "foo": 1}, "demo")
    ...
    objs = py_function_src_teleon_registry_index__records("component", limit=3) or py_function_src_teleon_registry_index__records("capability", limit=3)
    assert objs and all({"id", "type", "component"} <= set(x) for x in objs), "records are RegistryObject wrappers"
    assert isinstance(py_function_src_teleon_registry_index__records("agent_qa"), list)   # a definition-only type → honest [] (not an error)
