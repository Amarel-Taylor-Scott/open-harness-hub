...
Examples:
  _repos/shared-backend-components/scripts/oh_hub.py list pipeline
  _repos/shared-backend-components/scripts/oh_hub.py describe persona/esg-auditor
  _repos/shared-backend-components/scripts/oh_hub.py search forced-labor
  _repos/shared-backend-components/scripts/oh_hub.py industries esg
  _repos/shared-backend-components/scripts/oh_hub.py depends pipeline/supplier-policy-grading
"""
...
def cmd_validate(args, catalog):
    import subprocess
    proc = subprocess.run(["python3", str(_resource("scripts") / "validate.py")], capture_output=True, text=True)
...
def cmd_run(args, catalog):
    import subprocess
    cmd = ["python3", str(_resource("scripts") / "run_pipeline.py"), args.pipeline_id]
...
def cmd_emit(args, catalog):
    import subprocess
    emit_dir = _resource("scripts") / "emit"
