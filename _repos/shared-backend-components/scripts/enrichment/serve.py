...
(``_repos/baltor/context/case-studies/ceaas-context-for-claude-code.md`` §3) and the standards
audit's drift-#1 remediation (``_repos/shared-backend-components/context/strategy/standards-conformance-audit.md`` §3
...
first cut"*), this module ships the two **freezable / contract** surfaces:
  ...
  1. ``emit_llms_txt(corpus)``  — the no-integration surface. Renders the governed
     ...
     (``_repos/shared-backend-components/catalog/processors/deliver/emit-llms-txt.yaml``) declared but never
...
``_repos/shared-backend-components/docs/codex/change-verification-contract.md`` — *"real vs spec; no invented
...
CLI / self-test:
    python3 _repos/shared-backend-components/scripts/enrichment/serve.py
...
# (``python3 _repos/shared-backend-components/scripts/enrichment/serve.py``). The repo intentionally has no
# top-level ``_repos/shared-backend-components/scripts/__init__.py`` (namespace package run via ``-m`` from the
...
# names the real MCP emitter (``_repos/shared-backend-components/scripts/emit/mcp_server.py``) produces. One
...
#: real emitter uses (``_repos/shared-backend-components/scripts/emit/mcp_server.py`` docstring) so the contract
...
def _corpus_provenance(corpus: dict[str, Any]) -> dict[str, Any]:
    ...
    Provenance is the moat (``_repos/_shared/design/value-propositions.md``); we always
...
def _tool_descriptor(tool: Any) -> dict[str, Any]:
    ...
    ``_repos/shared-backend-components/scripts/emit/mcp_server.py`` produces, so this descriptor's tools are
...
_SAMPLE_CORPUS: dict[str, Any] = {
    ...
    "license": "MIT",
    "provenance": {
        "signer": "openhubforai.com (demo oracle)",
    ...
    "documents": [
        {
            ...
            "url": "https://openhubforai.com/docs/tier-planner",
            "section": "Reference",
        ...
        },
        {
            "doc_id": "serve-router",
            "title": "Serve router",
            "url": "https://openhubforai.com/docs/serve-router",
            "section": "Reference",
        ...
        },
        {
            ...
            "url": "https://openhubforai.com/docs/overview",
        ...
        },
        {
            "doc_id": "changelog",
            "title": "Changelog",
            "url": "https://openhubforai.com/docs/changelog",
            "section": "Optional",
...
def _selftest() -> None:
    ...
    assert "- [Tier budget planner](https://openhubforai.com/docs/tier-planner): " in text, \
    ...
    assert res0["_meta"]["ohh:sourceUrl"] == "https://openhubforai.com/docs/tier-planner", \
    ...
    assert desc["provenance"]["signer"] == "openhubforai.com (demo oracle)", \
