...
The composed motion (every stage uses the real module, no stub of a stub):
  ...
  8. RECONCILE — the EXISTING deterministic reconciliation authority (_repos/shared-backend-components/scripts/artifact_graph/reconciliation.py)
...
ASSERTIONS the invariant rides on:
  ...
  * the reconciliation outcome is byte-equivalent to the existing _repos/shared-backend-components/scripts/artifact_graph reconciliation reference.

Deterministic, stdlib-only, offline (no network, no RNG, injected time, hashlib ids).
CLI: PYTHONPATH=. python3 _repos/shared-backend-components/scripts/check_contextops_cfpb_reference.py --self-test
"""
