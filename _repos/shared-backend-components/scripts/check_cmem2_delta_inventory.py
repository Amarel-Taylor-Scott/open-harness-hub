...
CLI: PYTHONPATH=. python3 _repos/shared-backend-components/scripts/check_cmem2_delta_inventory.py --self-test
"""
...
def _self_test() -> int:
    ...
    missing = [p for p in d.get("already_present", []) if not (_resource(p)).exists()]
    ...
    missing2 = [p for p in d.get("added_this_pass", []) if not (_resource(p)).exists()]
