#!/usr/bin/env python3
"""scripts.check_openhubforai_auth_wiring — PROOF that the openhubforai web app is wired to the LOCAL
Identity & Access service (realm: openhubforai) honestly.

The web app is the kit-based SPA emitted by _repos/shared-backend-components/scripts/port_full_design_to_web.py (DESIGN-CONTRACT,
proven byte-for-byte by that script's --check). The realm-aware identity CLIENT now lives in
_repos/openhubforai/frontend/kit/oh-identity.js (exposes window.OHIdentity); the auth UI + API-key console are
React components in _repos/openhubforai/frontend/kit/oh-site.jsx (OhAuth / OhApiKeys). The same-origin deploy
seam (window.OPENHUBFORAI_IDENTITY_BASE) is injected by the generator, not hand-typed — so we single-source
...
Asserts (static contract — the service side is proven live by check_identity_local_service_runtime):
  ...
  B. CLIENT CONTRACT — oh-identity.js exposes window.OHIdentity, derives THIS product's realm from
     the brand registry (realmOf), the openhubforai realm exists in
     _repos/shared-backend-components/architecture/identity_realm_registry.json, the client's default port matches the registry's
     defaults.port (drift gate — the one allowed mirror), the window.OPENHUBFORAI_IDENTITY_BASE deploy
...
WEB = _resource("web/openhubforai")
...
IDENTITY_BASE_VAR = "OPENHUBFORAI_IDENTITY_BASE"
...
def _self_test() -> int:
    ...
    ck("B: this product's realm (openhubforai) exists in the identity realm registry",
       "openhubforai" in realm_ids, str(sorted(realm_ids)))
    ...
    ck("D: social login (Google/GitHub) is commented out until live (no fake or half-wired SSO button)",
       re.search(r"Continue with Google[^<]*</button>", site) is None
       and "commented out until we are fully live" in site)
    ...
    print("\n" + ("PASS — check_openhubforai_auth_wiring: the openhubforai kit SPA loads the realm-aware identity "
                  "client (window.OHIdentity) before the app, derives the openhubforai realm from the "
...
if __name__ == "__main__":
    ...
    print("usage: python3 scripts/check_openhubforai_auth_wiring.py --self-test")
    raise SystemExit(0)
