...
design-bundle tunnel + _repos/shared-backend-components/architecture/surface_capability_spec.json, and renders a single page with links to every
...
correct when ephemeral TryCloudflare tunnels restart. serves_truth=false.
  ...
  python3 _repos/shared-backend-components/scripts/landing_server.py [--port 8099]    then tunnel it for a public URL
  --self-test
"""
...
SVC = _resource("dist") / "service-plane-tunnel-urls.json"
OHD_LOG = _resource("data") / "dev-intel" / "ohd-tunnel.log"
...
PILLAR_APP = {"ai-done-right": "context_is_everything_app", "teleon": "teleon_app", "aidevobserver": "aidevobserver_demo",
              "baltor": "baltor_app", "open-star-hubs": "openhubforai_app"}
