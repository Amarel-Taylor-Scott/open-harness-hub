...
Pairs with ``_repos/shared-backend-components/docs/review/REVIEW-CHECKLIST.md`` (the human checklist) + the ``e2e`` recorders.

CLI:
    python3 _repos/shared-backend-components/scripts/make_review_pack.py             # run battery, write the review pack, print summary
    python3 _repos/shared-backend-components/scripts/make_review_pack.py --self-test  # offline deterministic proof of the report builder
"""
...
_PY = sys.executable
_OUT_DIR = _resource("e2e") / "artifacts" / "review-pack"
_RECEIPTS = (_REPO / ".agent") / "baltor-goal-loop-log.md"
_SHOT_DIR = _resource("e2e") / "artifacts"
...
CHECKS: list[dict] = [
    ...
    {"id": "tokens", "cat": "C · Real-data output (not just self-tests)",
     "title": "Design-token drift reporter runs on the actual _repos/baltor/frontend CSS",
...
def run_check(check: dict) -> dict:
    ...
    try:
        p = subprocess.run(cmd, cwd=str(_REPO), capture_output=True, text=True,
                           timeout=_CHECK_TIMEOUT_S, env={"PYTHONPATH": _pythonpath("."), "PATH": _env_path()})
