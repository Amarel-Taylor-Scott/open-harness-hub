#!/usr/bin/env python3
"""scripts.check_opportunity_map — proof: _repos/shared-backend-components/architecture/opportunities.json is complete + actionable. Every
...
CLI: python3 _repos/shared-backend-components/scripts/check_opportunity_map.py --self-test
"""
...
def _self_test() -> int:
    ...
    opps = json.loads((_resource("architecture") / "opportunities.json").read_text())["opportunities"]
    matrix = json.loads((_resource("architecture") / "section_maturity_matrix.json").read_text())["sections"]
