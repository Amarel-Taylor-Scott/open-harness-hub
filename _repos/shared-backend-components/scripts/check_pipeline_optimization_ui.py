#!/usr/bin/env python3
"""scripts.check_pipeline_optimization_ui — proof (D-PIPE-OPT-1): _repos/baltor/frontend/pipeline/optimization.html is a
...
Static source scan — deterministic, offline. CLI: python3 _repos/shared-backend-components/scripts/check_pipeline_optimization_ui.py --self-test
"""
...
_PAGE = _resource("web/baltor/pipeline/optimization.html")
...
def _run(page: Path, stage: str, allowed: set[str], required_eps, sibling_html, prose: str) -> int:
    ...
    check(f"_repos/baltor/frontend/pipeline/{stage}.html exists", page.exists())
    if not page.exists():
