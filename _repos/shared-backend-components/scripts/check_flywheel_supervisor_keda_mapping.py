...
CLI: PYTHONPATH=. python3 _repos/shared-backend-components/scripts/check_flywheel_supervisor_keda_mapping.py --self-test
"""
...
_DOC = _resource("docs") / "workers" / "flywheel-supervisor-scaling.md"
_SCHEMA = _resource("schemas") / "workers" / "SupervisorScaling.schema.json"
...
def _self_test() -> int:
    ...
    reg = json.loads((_resource("architecture") / "contract_registry.json").read_text())
