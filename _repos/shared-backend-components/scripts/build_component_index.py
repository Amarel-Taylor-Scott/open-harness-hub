...
components by meaning (_repos/teleon/backend/src/teleon/synthesis/component_search.py). serves_truth=false. Dev-plane script.

  python3 _repos/shared-backend-components/scripts/build_component_index.py            # rebuild the index
  python3 _repos/shared-backend-components/scripts/build_component_index.py --self-test
"""
...
from src.teleon.synthesis.component_search import py_const_src_teleon_synthesis_component_search__INDEX_PATH, py_function_src_teleon_synthesis_component_search__index_entry
...
def collect() -> list[dict]:
    ...
    def add(id, kind, text, keywords):
        ...
        if id and key not in seen:
            seen.add(key); rows.append(py_function_src_teleon_synthesis_component_search__index_entry(key, kind, text, keywords))
...
def build() -> int:
    rows = collect()
    py_const_src_teleon_synthesis_component_search__INDEX_PATH.parent.mkdir(parents=True, exist_ok=True)
    py_const_src_teleon_synthesis_component_search__INDEX_PATH.write_text("\n".join(json.dumps(r) for r in rows) + "\n", encoding="utf-8")
    return len(rows)
...
if __name__ == "__main__":
    ...
    print(f"indexed {build()} components -> {py_const_src_teleon_synthesis_component_search__INDEX_PATH.relative_to(REPO)}")
