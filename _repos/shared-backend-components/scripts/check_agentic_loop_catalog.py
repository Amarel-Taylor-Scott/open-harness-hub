...
strictly lower (the descent actually saves). serves_truth=false.
  ...
  python3 _repos/shared-backend-components/scripts/check_agentic_loop_catalog.py --self-test
"""
...
def _self_test() -> int:
    ...
    loops = cat["loops"]
    plane_ids = {p["plane"] for p in json.loads((_resource("architecture") / "tool_planes.json").read_text(encoding="utf-8"))["planes"]}
    domains = {d["domain"] for d in json.loads((_resource("architecture") / "capability_taxonomy.json").read_text(encoding="utf-8"))["domains"]}
    fails = []
    def ck(n, ok, d=""):
