...
Reads _repos/shared-backend-components/architecture/template_catalog.json, takes --template <id> --name <n> plus template variables, and
...
CLI:
    ...
    python3 _repos/shared-backend-components/scripts/generate_from_template.py --template proof.self_test --proof_name foo \\
        --title "Foo proof" --owner scripts/foo.py --subject_module scripts.foo
    python3 _repos/shared-backend-components/scripts/generate_from_template.py --self-test
"""
...
# _REPO above stays the MONOREPO root (used for the receipt dir + default repo arg). `from scripts.*` needs the
...
_CATALOG = _resource("architecture") / "template_catalog.json"
...
def _self_test() -> int:
    ...
    try:
        ...
        shutil.copytree(_resource("templates"), tmp / "templates")
        ...
        try:
            shutil.copytree(_resource("templates"), tmp2 / "templates")
