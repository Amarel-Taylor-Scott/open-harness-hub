#!/usr/bin/env python3
"""build_pitch_deck — generate the Teleon pitch/demo deck (HTML slides) from _repos/shared-backend-components/architecture/teleon_pitch_deck.json,
...
CLI: PYTHONPATH=. python3 _repos/shared-backend-components/scripts/build_pitch_deck.py --build | --self-test
"""
...
_DECK = _resource("architecture") / "teleon_pitch_deck.json"
_OUT = _resource("dist/decks/teleon-pitch.html")
...
def facts() -> dict:
    ...
    cat = json.loads((_resource("architecture") / "descent_method_catalog.json").read_text())
    methods = [m for d in cat["dimensions"] for m in d["methods"]]
    modules = [x for m in methods for x in m["modules"]]
    std = json.loads((_resource("architecture") / "standards_interop_manifest.json").read_text())
