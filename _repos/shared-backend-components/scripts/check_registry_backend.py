...
full-design dashboards real (not mock):
  ...
  A. SERVICE   — _repos/shared-backend-components/architecture/local_service_registry.json declares local_openhubforai_projection_api as
...
REG_PATH = _resource("architecture") / "local_service_registry.json"
REALM_REG = _resource("architecture") / "identity_realm_registry.json"
...
def _self_test() -> int:
    ...
    try:
        ...
        try:
            ...
            sc, body = _get(b + "/api/openhubforai/opencontexthub/search")
            ...
            sc, br = _get(b + "/api/openhubforai/registries")
            ...
            sc, brf = _get(b + "/api/openhubforai/registries?kind=static")
            ck("D: a facet filter narrows the public browse", sc == 200 and 0 < brf.get("count", 0) < br["count"])
            sc, recs = _get(b + "/api/openhubforai/registries/access_policy/records")
            ...
            sc, _ = _get(b + "/api/openhubforai/opencontexthub/workspace")          # no session
            ck("D: workspace FAILS CLOSED without a session", sc == 401)
            sc, _ = _get(b + "/api/openhubforai/opencontexthub/workspace?session_id=bogus")
            ck("D: workspace FAILS CLOSED on an invalid session", sc == 401)
            sc, out = _post(b + "/api/openhubforai/opencontexthub/install",
                            {"session_id": "sess_demo", "entry": {"id": "ilo", "name": "ILO", "score": "4.9"}})
            ck("D: install with a valid session → 202", sc == 202 and out.get("ok") is True)
            sc, ws_live = _get(b + "/api/openhubforai/opencontexthub/workspace?session_id=sess_demo")
            ck("D: workspace now reflects the real install", sc == 200 and dict(ws_live["stats"])["Installed"] >= 1)
            sc, _ = _get(b + "/api/openhubforai/opencontexthub/audit")                      # no session
            ck("D: audit FAILS CLOSED without a session", sc == 401)
            sc, au = _get(b + "/api/openhubforai/opencontexthub/audit?session_id=sess_demo")
            ...
            sc, sub = _post(b + "/api/openhubforai/opencontexthub/submit",
            ...
            sc, _ = _get(b + "/api/openhubforai/opencontexthub/submissions")                      # no session
            ck("D: submissions FAILS CLOSED without a session", sc == 401)
            sc, subs_body = _get(b + "/api/openhubforai/opencontexthub/submissions?session_id=sess_demo")
            ...
            sc, st_body = _get(b + "/api/openhubforai/opencontexthub/review/status?session_id=sess_demo")
            ck("D: review/status → not a reviewer by default", sc == 200 and st_body.get("reviewer") is False)
            sc, _ = _get(b + "/api/openhubforai/opencontexthub/review/queue?session_id=sess_demo")
            ck("D: review/queue FORBIDDEN for non-reviewers (403)", sc == 403)
            _store.grant_reviewer("opencontexthub", "acct_rev", by="proof", now=0)
            sc, st2 = _get(b + "/api/openhubforai/opencontexthub/review/status?session_id=sess_rev")
            ck("D: a granted account sees reviewer=true", sc == 200 and st2.get("reviewer") is True)
            sc, qb = _get(b + "/api/openhubforai/opencontexthub/review/queue?session_id=sess_rev")
            ...
            sc, _ = _post(b + "/api/openhubforai/opencontexthub/review/decide",
                          {"session_id": "sess_demo", "entry_id": "p1", "decision": "approve"})
            ck("D: a reviewer CANNOT approve their own submission (403)", sc == 403)
            sc, dr = _post(b + "/api/openhubforai/opencontexthub/review/decide",
            ...
            sc, sb = _get(b + "/api/openhubforai/opencontexthub/search?q=Pub%201")
            ...
            sc, ad0 = _get(b + "/api/openhubforai/opencontexthub/admin/status?session_id=sess_demo")
            ck("D: admin/status → not an admin by default", sc == 200 and ad0.get("admin") is False)
            sc, _ = _get(b + "/api/openhubforai/opencontexthub/admin/reviewers?session_id=sess_demo")
            ...
            sc, ad1 = _get(b + "/api/openhubforai/opencontexthub/admin/status?session_id=sess_demo")
            ck("D: an operator-bootstrapped account sees admin=true", sc == 200 and ad1.get("admin") is True)
            sc, rv = _post(b + "/api/openhubforai/opencontexthub/admin/reviewers/grant",
            ...
            sc, _ = _post(b + "/api/openhubforai/opencontexthub/admin/reviewers/grant",
    ...
    ck("E: deploy override supported", "OPENHUBFORAI_REGISTRY_BASE" in client)
