...
Reuses: _repos/shared-backend-components/architecture/demo_surface_registry.json (the honest surface list) + _repos/shared-backend-components/scripts/portfolio_lib.py (titles/
...
_CT_TUNNEL = _resource("dist/demo-control-tower-url.txt")   # the control tower's own public URL (written post-launch)
...
def _extra_tunnels() -> dict:
    ...
    f = _resource("dist/cloudflare-extra-tunnels.json")
...
def build() -> dict:
    ...
    (_resource("dist")).mkdir(exist_ok=True)
    (_resource("dist/demo-all-urls.json")).write_text(json.dumps(
    ...
    for grp in _GROUP_ORDER:
        ...
        md.append("")
    (_resource("dist/demo-all-urls.md")).write_text("\n".join(md) + "\n")
    (_resource("dist/demo-all-urls.txt")).write_text("\n".join(txt) + "\n")
    # ── start-here page ──
...
def _self_test() -> int:
    ...
    md = (_resource("dist/demo-all-urls.md")).read_text()
    check("start-here page built", _PAGE.exists() and "Demo Control Tower" in page)
    check("consolidated manifests written (json/md/txt)", all((_resource("dist") / f"demo-all-urls.{e}").exists() for e in ("json", "md", "txt")))
