#!/usr/bin/env python3
"""scripts.check_pipeline_reconciliation_ui — proof (C-PIPE-RECON-UI): _repos/baltor/frontend/pipeline/reconciliation.html
is a PROJECTION-ONLY stage page for the "Conflict & Reconciliation" stage. It fetches ONLY its contracted
/api/pipeline/reconciliation (+ /api/pipeline/overview), renders the required panels INCLUDING the lossless
held-out panel (the losing value preserved with the authority rule + receipt), handles loading/empty/error
states, carries the shared 7-stage stepper that links to its siblings, and performs NO durable write / no
secret / no private-memory leak. Static source scan — deterministic, offline.

CLI: python3 _repos/shared-backend-components/scripts/check_pipeline_reconciliation_ui.py --self-test
"""
...
_SIBLINGS = ("upload", "decomposition", "enhancement", "optimization", "verification", "consumption")
...
#: this stage's lossless content: winner served, loser held out with the rule + receipt (preserved, not deleted).
_REQUIRED_LOSSLESS = ("winner", "held_out_value", "rule", "receipt", "≠ deleted")
...
def _self_test() -> int:
    ...
    check("_repos/baltor/frontend/pipeline/reconciliation.html exists", _PAGE.exists())
    if not _PAGE.exists():
    ...
    # 3) the lossless story: winner served, loser HELD OUT with the authority rule + receipt (preserved)
    missing_loss = [s for s in _REQUIRED_LOSSLESS if s not in text]
    check("lossless held-out story present (winner + held_out_value + rule + receipt + preserved)", missing_loss == [], str(missing_loss))

    # 4) loading / empty / error states handled
    ...
    # 5) shared 7-stage stepper links to every sibling stage page.
    ...
    # 6) no durable writes / truth storage / secrets / private memory
    ...
    # 7) governance copy: the page states it computes no truth
    check("page declares projection-only (computes no truth; reads the API)",
          "projection-only" in text and "computes no truth" in text)

    print(f"\n{'PASS — check_pipeline_reconciliation_ui: reconciliation.html fetches ONLY /api/pipeline/reconciliation (+ overview), renders input/output/lossless-held-out/source-handle panels (winner served, loser held out with rule + receipt), handles loading/empty/error, the 7-stage stepper links to all siblings, and it computes/stores/leaks no truth or secret.' if not fails else f'{len(fails)} FAILURES: {fails}'}")
    return 0 if not fails else 1


def _main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(description="Proof: pipeline reconciliation stage page is projection-only.")
