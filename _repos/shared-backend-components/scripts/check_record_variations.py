"""check_record_variations — proof for record variation generation (_repos/teleon/backend/src/teleon/registry/variations.py).
...
from src.teleon.registry.variations import py_const_src_teleon_registry_variations__AXES, py_function_src_teleon_registry_variations__expand_record, py_function_src_teleon_registry_variations__fork_questions, py_function_src_teleon_registry_variations__variations_of  # noqa: E402
...
def main() -> int:
    ...
    ck("axes include industry/region/scale/approach", {"industry", "region", "scale", "approach"} <= set(py_const_src_teleon_registry_variations__AXES))

    ind = py_function_src_teleon_registry_variations__variations_of(_REC, "industry")
    ck("industry generates one variation per industry value", len(ind) == len(py_const_src_teleon_registry_variations__AXES["industry"]), str(len(ind)))
    ...
    one = py_function_src_teleon_registry_variations__variations_of(_REC, "industry", ["healthcare"])
    ...
    qs = py_function_src_teleon_registry_variations__fork_questions(_REC)
    ...
    full = py_function_src_teleon_registry_variations__expand_record(_REC)
    ...
    print(f"PASS - check_record_variations: any record -> governed candidate variations across "
          f"{len(py_const_src_teleon_registry_variations__AXES)} axes (industry/region/scale/approach) + the fork questions; parent-linked; {checks} assertions; serves_truth=false.")
    return 0
