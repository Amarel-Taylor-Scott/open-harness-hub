...
is the pure, deterministic SCORER (``_repos/_shared/codex/north-star.md`` — *"we verify your docs
...
CLI / self-test:
    python3 _repos/shared-backend-components/scripts/ingest/sanctions_feed.py
    python3 -m scripts.ingest.sanctions_feed
    python3 _repos/shared-backend-components/scripts/ingest/sanctions_feed.py --demo   # JSON: fetched records + a mapped row
"""
...
# _repos/shared-backend-components/scripts/__init__.py; it is a namespace package run via -m from the root). This
...
# ``python3 _repos/shared-backend-components/scripts/ingest/sanctions_feed.py`` would otherwise fail with
