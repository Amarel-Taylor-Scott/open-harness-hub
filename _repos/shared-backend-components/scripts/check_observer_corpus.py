"""check_observer_corpus — proof wrapper for the consented-corpus pipeline (_repos/teleon/backend/src/teleon/observer/corpus.py).

The corpus pipeline (consented retain + redact -> mine -> standardize) uses package-relative imports, so
it can't run as a bare script path; this top-level wrapper bootstraps sys.path and delegates to its
self-test. Run with --self-test (gate convention).
"""
...
from src.teleon.observer.corpus import _self_test  # noqa: E402
