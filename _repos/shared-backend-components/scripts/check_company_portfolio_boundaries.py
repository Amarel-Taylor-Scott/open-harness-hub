...
`contextiseverything`; tagline "Context. Capability. Proof."; canonical _repos/shared-backend-components/architecture/brand.json) owns Teleon (runtime
SaaS), Baltor (applied product, a tenant of Teleon), and OpenHubForAI (open ecosystem). Sources:
_repos/shared-backend-components/architecture/company_portfolio_map.json (this model) + _repos/shared-backend-components/architecture/portfolio_dependency_law.json (import law)
+ _repos/shared-backend-components/architecture/domain_brand_risk_register.json (brand risk).
...
def _self_test() -> int:
    ...
    check("A: tracked company-boundary entities present",
          set(companies) == {"contextiseverything", "teleon", "baltor", "openhubforai",
    ...
    layer_of = {"teleon": "teleon", "baltor": "baltor", "openhubforai": "openhubforai"}
