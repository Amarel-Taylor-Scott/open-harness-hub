...
Two parts, so the self-test is non-vacuous regardless of integration order:
  ...
  2. REAL CATALOG (asserted only IF present — MAIN adds the entries during integration): if
     _repos/shared-backend-components/architecture/external_capability_catalog.json already contains the two memory slots, assert THOSE
...
CLI: python3 _repos/shared-backend-components/scripts/check_supermemory_candidate_catalog.py --self-test
"""
...
_REAL_CATALOG = _resource("architecture") / "external_capability_catalog.json"
