...
Asserts (deterministic + offline; rebuilds the page/manifest, does not require live servers/tunnels):
  ...
  E. CFPB E2E INVARIANT cited (answer 10 business days · FAQ-30 held out · receipt + source handles) — the live
     evidence is _repos/shared-backend-components/scripts/demo_offline_full_baltor.py (the offline e2e proof), referenced on the page.
...
def _self_test() -> int:
    ...
    reg = json.loads((_resource("architecture") / "demo_surface_registry.json").read_text())["surfaces"]
    ...
    allj = json.loads((_resource("dist/demo-all-urls.json")).read_text())
    md = (_resource("dist/demo-all-urls.md")).read_text()
    check("B: start-here page + 3 manifests emitted",
          B._PAGE.exists() and all((_resource("dist") / f"demo-all-urls.{e}").exists() for e in ("json", "md", "txt")))
    ...
    check("E: the offline e2e proof exists", (_resource("scripts/demo_offline_full_baltor.py")).exists())
    ...
    svc_reg = json.loads((_resource("architecture") / "local_service_registry.json").read_text())
