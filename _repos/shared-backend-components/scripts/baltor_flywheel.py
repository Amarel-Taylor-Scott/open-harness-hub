...
CLI:
    python3 _repos/shared-backend-components/scripts/baltor_flywheel.py --self-test                 # one offline tick + assertions
    python3 _repos/shared-backend-components/scripts/baltor_flywheel.py --once                      # one tick, print + log
    python3 _repos/shared-backend-components/scripts/baltor_flywheel.py --watch --interval 600      # loop forever (offline ticks)
    python3 _repos/shared-backend-components/scripts/baltor_flywheel.py --watch --interval 900 --live  # also CDC-watch live sources
"""
...
def _proof_env() -> dict:
    ...
    # repo root FIRST (so scripts.* is never shadowed), then the full post-migration path (every
    # _repos/*/backend) so a spawned proof can import scripts.* AND src.* even when launched bare.
    env["PYTHONPATH"] = _REPO_ROOT + os.pathsep + _pythonpath(".") + (os.pathsep + prior if prior else "")
    return env
