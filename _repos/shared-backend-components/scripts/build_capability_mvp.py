...
serves_truth=false (the showcase grounds 'what exists', never asserts truth).
  ...
  python3 _repos/shared-backend-components/scripts/build_capability_mvp.py            # build dist/capability-mvp/index.html
  python3 _repos/shared-backend-components/scripts/build_capability_mvp.py --self-test
"""
...
from src.teleon.registry.reinvention_guard import py_function_src_teleon_registry_reinvention_guard__check as guard_check  # noqa: E402
from src.teleon.registry.search import py_function_src_teleon_registry_search__build_capability  # noqa: E402

_ONT = _resource("architecture") / "registry_ontology.json"
_OUT = _resource("dist/capability-mvp/index.html")
...
def _compute() -> dict:
    ...
    build = {reg: items[:4] for reg, items in py_function_src_teleon_registry_search__build_capability(_BUILD_INTENT)["ingredients_by_registry"].items() if items}
    return {
