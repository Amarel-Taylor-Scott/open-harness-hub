...
SEAM to run in CI when present (_repos/shared-backend-components/docs/security/mcp-and-skill-security.md). The shared detector here
is reused by scan_agent_skills.py (No-Magic-Values: one pattern set).

CLI:
    python3 _repos/shared-backend-components/scripts/scan_mcp_manifests.py --self-test
    python3 _repos/shared-backend-components/scripts/scan_mcp_manifests.py path/to/manifest.json
"""
...
def _self_test() -> int:
    ...
    clean = scan_file(_resource("fixtures/security/clean-tool.json"))
    poisoned = scan_file(_resource("fixtures/security/poisoned-tool.json"))
