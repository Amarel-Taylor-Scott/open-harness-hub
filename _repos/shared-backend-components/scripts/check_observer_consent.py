"""check_observer_consent — proof wrapper for GATE #0, the consent model (_repos/teleon/backend/src/teleon/observer/consent.py).

The consent gate uses package-relative imports, so it can't run as a bare script path; this top-level
wrapper bootstraps sys.path and delegates to its self-test. Run with --self-test (gate convention).
"""
...
from src.teleon.observer.consent import _self_test  # noqa: E402
