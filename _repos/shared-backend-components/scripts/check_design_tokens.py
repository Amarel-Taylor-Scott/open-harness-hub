...
(``_repos/shared-backend-components/docs/codex/no-magic-values.md``): a value typed twice is a value that drifts. The drift this
surfaces is documented in ``_repos/shared-backend-components/docs/standards/DESIGN.md`` §Known-drift.
...
``_repos/baltor/frontend/tokens.css``.
...
CLI:
    python3 _repos/shared-backend-components/scripts/check_design_tokens.py             # scan web/baltor, print drift report (exit 0)
    python3 _repos/shared-backend-components/scripts/check_design_tokens.py --strict    # exit 1 if any token has >1 distinct value
    python3 _repos/shared-backend-components/scripts/check_design_tokens.py --self-test  # offline, deterministic proof of the detector
"""
...
_REPO_ROOT = next((_ar for _ar in Path(__file__).resolve().parents if (_ar / ".aidoneright-root").exists()), Path(__file__).resolve().parents[1])

# ── Constants (single source; No-Magic-Values) ───────────────────────────────
#: Where the shipped front-end design tokens live.
WEB_DIR = _resource("web/baltor")
...
def _self_test() -> int:
    ...
    check("real scan reads _repos/baltor/frontend file(s)", nfiles >= 1, str(nfiles))
    rd = drift(real)
