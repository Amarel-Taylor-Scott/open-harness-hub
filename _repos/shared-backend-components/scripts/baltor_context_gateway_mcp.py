...
demo HTTP API. It speaks JSON-RPC over stdio with newline-delimited framing
(the MCP stdio spec) via the shared ``scripts._mcp_stdio`` transport.
"""
...
# Find the SUBSTRATE root (the dir holding the `scripts` package) via the scripts/_repo_paths.py sentinel —
# NOT the `.aidoneright-root` marker, which sits at the MONOREPO root (one level up from _repos/) and has no
# `scripts/` package, so `from scripts.*` below would fail when this server is launched from the repo root
# (as .mcp.json does). The sentinel resolves to shared-backend-components both locally and in-container.
REPO_ROOT = next((_p for _p in Path(__file__).resolve().parents
                  if (_p / "scripts" / "_repo_paths.py").exists()), Path(__file__).resolve().parents[1])
if str(REPO_ROOT) not in sys.path:
    sys.path.insert(0, str(REPO_ROOT))

from scripts import _mcp_stdio  # the ONE newline-delimited (spec) MCP stdio transport, shared by every server
...
# MCP stdio spec). This server owns only PROTOCOL/dispatch (handle_rpc, which closes over base_url).
...
def main(argv: list[str] | None = None) -> int:
    ...
    return _mcp_stdio.serve(lambda request: handle_rpc(request, base_url=args.base_url))
