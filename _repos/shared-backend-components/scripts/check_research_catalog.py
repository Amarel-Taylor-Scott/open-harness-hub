...
resources (browser_runtime + llm) are available and within budget.
  ...
  python3 _repos/shared-backend-components/scripts/check_research_catalog.py --self-test
"""
...
def _self_test() -> int:
    ...
    raw = _json.loads((_resource("architecture") / "research_component_catalog.json").read_text(encoding="utf-8"))["components"]
