...
(no-magic single source — no drifting OH_* key); and the reachability helper (_repos/teleon/backend/src/teleon/runtime/credentials) computes
present/reachable/missing_for correctly on injected env. serves_truth=false.

  python3 _repos/shared-backend-components/scripts/check_credential_registry.py --self-test
"""
...
def _keys_read_in_product() -> set[str]:
    ...
    found = set()
    for p in (_resource("src/teleon")).rglob("*.py"):
...
def _self_test() -> int:
    ...
    env_none = {}
    ck("keyless service reachable with no keys (github)", C.py_function_src_teleon_runtime_credentials__is_present("github", env_none))
    ck("keyed service blocked with no keys (libraries_io)", not C.py_function_src_teleon_runtime_credentials__is_present("libraries_io", env_none))
    ck("missing_for names the needed var", C.py_function_src_teleon_runtime_credentials__missing_for("libraries_io", env_none) == ["OH_LIBRARIESIO_KEY"])
    env_have = {"OH_LLM_API_KEY": "x", "OH_LLM_BASE_URL": "y", "COHERE_API_KEY": "z"}
    ck("present when all env vars set (llm_gateway)", C.py_function_src_teleon_runtime_credentials__is_present("llm_gateway", env_have))
    ck("planes unlocked reflects present keys (llm + reranker)",
       {"llm", "reranker"} <= C.py_function_src_teleon_runtime_credentials__reachable_planes(env_have))
    ck("services_for_tool maps a tool to its key (cohere_rerank -> cohere)", "cohere" in C.py_function_src_teleon_runtime_credentials__services_for_tool("cohere_rerank"))
    ck("status() exposes no secret values", all(not _VALUE_LOOKING.search(x) for x in C.py_function_src_teleon_runtime_credentials__status(env_have)["reachable"]))
    ...
    ck("key_mode: tenant byo key preferred", C.py_function_src_teleon_runtime_credentials__key_mode("openai", {}, byo={"openai"}) == "byo")
    ck("key_mode: platform key within limits when no byo", C.py_function_src_teleon_runtime_credentials__key_mode("anthropic", {"ANTHROPIC_API_KEY": "x"}) == "platform")
    ck("key_mode: byo-only blocked on platform with no key", C.py_function_src_teleon_runtime_credentials__key_mode("serpapi", {}) is None)
    ck("status() splits byo_only vs platform_capable", "byo_only" in C.py_function_src_teleon_runtime_credentials__status(env_have) and "platform_capable" in C.py_function_src_teleon_runtime_credentials__status(env_have))
