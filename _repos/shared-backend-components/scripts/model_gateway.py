...
CANONICAL LAYER: `_repos/teleon/backend/src/teleon/inference` (OIPS) owns provider-ordering truth (the numeric preference
graph). This Baltor-era gateway + `_repos/shared-backend-components/scripts/llm_gateway` are the legacy route-selection path during the
