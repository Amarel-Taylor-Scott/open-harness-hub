...
CLI: python3 _repos/shared-backend-components/scripts/check_llm_secret_hygiene.py --self-test
"""
...
# rel is computed relative to the monorepo root, so post-_repos/-migration these carry their real
# _repos/<owner>/… prefix (the scan still fails on a REAL/unknown key in any of these fixture files).
_REDACTION_FIXTURE_FILES = {"_repos/shared-backend-components/scripts/check_prelaunch.py",
                            "_repos/shared-backend-components/scripts/foundry/interactions.py",
                            "_repos/shared-backend-components/scripts/email_port.py",  # send() secret-shaped-prop guard
                            "_repos/teleon/backend/src/teleon/observer/corpus.py",       # consented-retain redaction self-test
                            "_repos/shared-backend-components/scripts/observer_corpus_harvest.py",  # harvester redaction self-test
                            "_repos/dev-rules-context/tests/unit/test_teleon_primitive_groups.py"}  # tenant-settings redaction test
_SYNTHETIC_KEYS = {"sk-ABCDEFGHIJKLMNOP12345", "sk-ABCDEF1234567890"}
#: known-synthetic INLINE api_key fixtures (redaction tests need a raw-shaped VALUE to prove masking);
#: any other inline api_key — even in an allowlisted fixture file — still fails.
_SYNTHETIC_INLINE = {'"api_key": "default-secret"', '"api_key": "tenant-secret"'}
