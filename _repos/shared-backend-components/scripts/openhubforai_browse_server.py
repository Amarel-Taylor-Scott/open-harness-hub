#!/usr/bin/env python3
"""scripts.openhubforai_browse_server — the FACETED BROWSE UI for OpenHubForAI.io.
...
Dark family theme (Hanken Grotesk), accent #3b6fd4. serves_truth=false.
  ...
  python3 _repos/shared-backend-components/scripts/openhubforai_browse_server.py [--port 8130]    then tunnel it
  --self-test
"""
...
from src.teleon.registry.browse import py_const_src_teleon_registry_browse__FACET_DIMS, py_function_src_teleon_registry_browse__browse, py_function_src_teleon_registry_browse__facets  # noqa: E402
...
def _facet_groups() -> str:
    ...
    return "".join(
        ...
        for d in py_const_src_teleon_registry_browse__FACET_DIMS)
...
def _index() -> str:
    ...
    boot = py_function_src_teleon_registry_browse__browse("", {})
    js = _JS.replace("__DIMS__", _safe_json(list(py_const_src_teleon_registry_browse__FACET_DIMS))).replace("__BOOT__", _safe_json(boot))
    return (
...
def _browse_response(raw_query: str) -> dict:
    ...
    filters = {d: qs[d][0] for d in py_const_src_teleon_registry_browse__FACET_DIMS if qs.get(d) and qs[d][0]}
    return py_function_src_teleon_registry_browse__browse(query, filters)
...
def self_test() -> int:
    ...
    assert all(d.capitalize() in idx for d in py_const_src_teleon_registry_browse__FACET_DIMS), f"all 5 facet group labels present: {py_const_src_teleon_registry_browse__FACET_DIMS}"

    # the facets() API exposes exactly the five browsable dimensions
    f = py_function_src_teleon_registry_browse__facets()
    assert set(f) == set(py_const_src_teleon_registry_browse__FACET_DIMS) and len(f["type"]) >= 100, sorted(f)
    ...
    print(f"openhubforai_browse_server self-test: OK (index renders 'OpenHubForAI' + search + "
          f"{len(py_const_src_teleon_registry_browse__FACET_DIMS)} facet groups; browse handler -> {full['count']} records, "
