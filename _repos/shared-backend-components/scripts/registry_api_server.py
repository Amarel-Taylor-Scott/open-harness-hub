...
The API is read-only and CORS-open so the browse UI can call it from a tunnel. serves_truth=false.
  ...
  python3 _repos/shared-backend-components/scripts/registry_api_server.py [--port 8140]    then tunnel it
  --self-test
"""
...
from src.teleon.registry.index import py_function_src_teleon_registry_index__get, py_function_src_teleon_registry_index__reconciled_index, py_function_src_teleon_registry_index__records  # noqa: E402
from src.teleon.registry.search import py_function_src_teleon_registry_search__search_all  # noqa: E402
from src.teleon.registry.browse import py_function_src_teleon_registry_browse__browse  # noqa: E402  the rich faceted browse (filtered items + facet counts)
...
def _api_index() -> dict:
    ...
    idx = py_function_src_teleon_registry_index__reconciled_index()
    return {
...
def route(path: str, query: str = "") -> tuple[int, dict]:
    ...
    if path == "/reconciliation":
        return 200, py_function_src_teleon_registry_index__reconciled_index()["reconciliation"]
    if path == "/search":
        q = params.get("q", "")
        return 200, {"query": q, "hits": py_function_src_teleon_registry_search__search_all(q) if q else [], "serves_truth": False}
    if path == "/registries":
        return 200, _filtered_registries(params)
    if path == "/browse":
        flt = {k: params[k] for k in ("category", "type", "kind", "layer", "status") if params.get(k)}
        return 200, {**py_function_src_teleon_registry_browse__browse(params.get("q", ""), flt), "service": "registry_api_server"}

    parts = [urllib.parse.unquote(p) for p in path.split("/") if p]
    if parts and parts[0] == "registries":
        if len(parts) == 2:
            reg = py_function_src_teleon_registry_index__get(parts[1])
            if reg is None:
                return 404, {"error": "unknown registry", "id": parts[1], "serves_truth": False}
            return 200, reg
        if len(parts) == 3 and parts[2] == "records":
            rid = parts[1]
            if py_function_src_teleon_registry_index__get(rid) is None:
            ...
            return 200, {"registry": rid, "limit": limit,
                         "records": py_function_src_teleon_registry_index__records(rid, limit=limit), "serves_truth": False}
