...
number `_repos/_shared/strategy/measured-lift-head-to-head.md` specifies and the Contextual
...
It is the M4 milestone of `_repos/_shared/codex/north-star.md` — "the credibility + the
...
WHAT IS REAL HERE vs WHAT IS A SEAM (honesty first — change-verification-contract):
  ...
  SEAM (NOT faked — explicitly unwired here, wired the moment a route exists):
    * the live model route for the answer arms and a *different-family* judge route
      (`_repos/shared-backend-components/scripts/foundry/model_route.py`; today it shares one route across
...
CLI / self-test (proves the protocol on a bundled fixture, no model, no network):
    python3 _repos/shared-backend-components/scripts/eval/measured_lift_headtohead.py
