#!/usr/bin/env python3
"""scripts.check_pipeline_upload_ui — proof (C-PIPE-UPLOAD-1): _repos/baltor/frontend/pipeline/upload.html is the
...
_repos/shared-backend-components/scripts/check_consumption_ui.py + _repos/shared-backend-components/scripts/check_memory_page_projection_only.py.

CLI: python3 _repos/shared-backend-components/scripts/check_pipeline_upload_ui.py --self-test
"""
...
_PAGE = _resource("web/baltor/pipeline/upload.html")
...
def _self_test() -> int:
    ...
    check("_repos/baltor/frontend/pipeline/upload.html exists", _PAGE.exists())
    if not _PAGE.exists():
