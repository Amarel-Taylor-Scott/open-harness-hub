...
CLI: PYTHONPATH=. python3 _repos/shared-backend-components/scripts/ingest_github_signal_intake.py --build | --self-test
"""
...
_FEED = _resource("data") / "capability-candidates" / "discovered-feed-github-signal-2026-06-20.json"
...
def _self_test() -> int:
    ...
    denied = {x.lower() for x in json.loads((_resource("architecture") / "org_guardrail_policies.json").read_text())
