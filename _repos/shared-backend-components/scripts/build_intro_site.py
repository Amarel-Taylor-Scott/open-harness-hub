...
FOUR self-contained, cross-linked pages (no external resources; branded Hanken Grotesk + IBM Plex Mono; CSS animations):
  ...
  hubs.html   — OpenHubForAI: the open component registries Teleon, Baltor, and AIDevObserver consume.
Every NUMBER is COMPUTED from the registries + the live cascade (no-magic-values). serves_truth=false.

  PYTHONPATH=. python3 _repos/shared-backend-components/scripts/build_intro_site.py [--self-test]
"""
...
OUTDIR = _resource("dist/intro")
PAGES = ("index", "baltor", "teleon", "hubs")


def counts() -> dict:
    c = {}
    c["hubs"] = len(json.loads((_resource("architecture") / "portfolio_connection_map.json").read_text())["hubs"])
    c["research"] = len(json.loads((_resource("architecture") / "research_component_catalog.json").read_text())["components"])
    bs = json.loads((_resource("architecture") / "web_browsing_stack_registry.json").read_text())
...
def _capability_types() -> str:
    ...
    try:
        tasks = json.loads((_resource("architecture") / "tunable_task_catalog.json").read_text(encoding="utf-8"))["tasks"]
    except Exception:  # noqa: BLE001
        return ""
    return "".join(
...
<section><div class="wrap reveal">
  ...
  <div class="grid g3">
    ...
    <a class=card href="hubs.html"><h4>{c['hubs']} OpenHubForAI hubs</h4><p>Open component registries — tools, skills,
      models, methods — that Teleon, Baltor, and AIDevObserver consume.</p><span class=more>See the hubs →</span></a>
  </div>
  <p style="margin-top:18px" class=mono>Architecture law: Baltor &rarr; Teleon &rarr; OpenHubForAI. Never the reverse.</p>
</div></section>"""
