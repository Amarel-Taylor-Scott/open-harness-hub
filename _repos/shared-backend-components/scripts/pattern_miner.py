...
optimizer, parser, or consumption service. It only READS the tree (_repos/shared-backend-components/scripts/, _repos/baltor/backend/src/baltor/, _repos/shared-backend-components/architecture/,
schemas/, _repos/baltor/frontend/, _repos/shared-backend-components/docs/) and reports where the same shape recurs, where a one-off should become a
...
CLI:
    python3 _repos/shared-backend-components/scripts/pattern_miner.py --scan              # scan the real repo, write the report
    python3 _repos/shared-backend-components/scripts/pattern_miner.py --scan --root DIR   # scan an arbitrary fixture tree (used by the proof)
    python3 _repos/shared-backend-components/scripts/pattern_miner.py --self-test         # tiny deterministic fixture, assert 7 keys + detections
"""
...
SCAN_DIRS = ("scripts", "_repos/baltor/backend/src/baltor", "architecture", "schemas", "_repos/baltor/frontend", "docs")

# A shape must recur at least this many times to be a "pattern" rather than a "one-off".
MIN_REPEAT = 2  # "build it twice -> it is a pattern" (see _repos/shared-backend-components/docs/standards/pattern-system.md)
...
_PROVIDER_ALLOWED = ("_repos/baltor/backend/src/baltor/adapters", "_repos/baltor/backend/src/baltor/llm_gateway", "scripts/llm_gateway", "scripts/ingest")
...
def scan(root: Path) -> dict[str, Any]:
    ...
    for p in files:
        ...
        # --- processor harness ---
        if rel.startswith("_repos/baltor/backend/src/baltor/processors/") and name not in ("__init__.py", "README.md"):
        ...
        if rel.startswith("_repos/baltor/backend/src/baltor/contracts/artifacts/") and name.endswith(".py") and name != "__init__.py":
        ...
        if rel.startswith("_repos/baltor/backend/src/baltor/llm_gateway/providers/") and name.endswith(".py") and name != "__init__.py":
            hits["provider_adapter"].append(rel)
        if rel.startswith("_repos/baltor/backend/src/baltor/adapters/") and name.endswith(".py") and name not in ("__init__.py",) \
        ...
        # --- JSON registries (_repos/shared-backend-components/architecture/*.json with version+note, the matrix/registry shape) ---
        ...
        if name.endswith(".py"):
            ...
            if rel.startswith("_repos/baltor/backend/src/baltor/processors/") and _SQLITE_RE.search(txt):
...
def _load_registry(root: Path) -> dict[str, Any]:
    ...
    if not reg_path.exists():
        ...
        reg_path = _resource(REGISTRY_REL)
    if not reg_path.exists():
        return {"patterns": []}
    try:
...
def _build_fixture(root: Path) -> None:
    ...
    (root / "_repos/baltor/backend/src/baltor/adapters/source").mkdir(parents=True)
    (root / "_repos/baltor/backend/src/baltor/processors/decompose").mkdir(parents=True)
    (root / "architecture").mkdir(parents=True)
    (root / "schemas").mkdir(parents=True)
    (root / "_repos/baltor/frontend").mkdir(parents=True)
    ...
    (root / "_repos/baltor/frontend/page_one.html").write_text(
        '<script>fetch("/api/health")</script>\n', encoding="utf-8")
    (root / "_repos/baltor/frontend/page_two.html").write_text(
        '<script>fetch("/api/events")</script>\n', encoding="utf-8")
    # two source adapters (-> source_adapter_pattern)
    (root / "_repos/baltor/backend/src/baltor/adapters/source/alpha_source.py").write_text(
        "class AlphaSource:\n    adapter_id = 'alpha@v1'\n", encoding="utf-8")
    (root / "_repos/baltor/backend/src/baltor/adapters/source/beta_source.py").write_text(
        "class BetaSource:\n    adapter_id = 'beta@v1'\n", encoding="utf-8")
    ...
    (root / "_repos/baltor/backend/src/baltor/processors/decompose/bad_processor.py").write_text(
