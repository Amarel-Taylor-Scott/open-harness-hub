...
ENGINE (_repos/baltor/backend/src/baltor/experiments/) FAILS SAFELY. The engine must never serve a candidate as truth, never
...
(_repos/shared-backend-components/scripts/check_contextops_cfpb_reference.py + _repos/shared-backend-components/scripts/demo_offline_full_baltor.py): the agency FAQ says
...
CLI: PYTHONPATH=. python3 _repos/shared-backend-components/scripts/check_parallel_path_redteam.py --self-test
"""
