...
CLI: PYTHONPATH=. python3 _repos/shared-backend-components/scripts/check_stateful_swarm_local_stub.py --self-test
"""
...
from src.teleon.blackboard.local_sqlite_blackboard import py_class_src_teleon_blackboard_local_sqlite_blackboard__LocalSqliteBlackboard  # noqa: E402
...
_TELEON_FILES = [
    ...
    _resource("src/teleon/stateful_swarms/__init__.py"),
...
def _run_on_temp(now: str = _NOW) -> tuple[py_class_src_teleon_blackboard_local_sqlite_blackboard__LocalSqliteBlackboard, str, dict, LocalStatefulSwarm]:
    """Create a fresh in-memory blackboard, run the swarm on it, return (bb, blackboard_id, envelope, swarm)."""
    bb = py_class_src_teleon_blackboard_local_sqlite_blackboard__LocalSqliteBlackboard(db_path=":memory:")
