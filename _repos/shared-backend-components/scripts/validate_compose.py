...
data-safety hazard (_repos/shared-backend-components/docs/standards/table-shape-guidelines.md: canonical state is durable). Self-test
runs over the real compose files (they pass — images are pinned) + a negative inline case.

CLI:
    python3 _repos/shared-backend-components/scripts/validate_compose.py --self-test
    python3 _repos/shared-backend-components/scripts/validate_compose.py            # scan repo compose files; exit 1 on violations
"""
...
def _compose_files() -> list[Path]:
    # infra/ and deploy/ moved under _repos/shared-backend-components — resolve them, don't glob the old root
    files = [p for p in _resource("infra").glob("**/*.yml") if "compose" in p.name.lower()]
    files += [p for p in _resource("deploy").glob("**/*.yml") if "compose" in p.name.lower()]
...
def scan() -> tuple[list[str], int]:
    ...
    # docker-compose extension tags (e.g. !override, !reset) are valid compose syntax but unknown to
    # SafeLoader; map any '!'-tag to its underlying value so real compose files still parse + get guarded.
    ...
    for p in files:
        try:
            doc = yaml.load(p.read_text(encoding="utf-8"), Loader=_ComposeLoader) or {}
