...
_TELEON_FILES = [
    _resource("src/teleon/ports/blackboard_provider.py"),
    _resource("src/teleon/blackboard/__init__.py"),
    _resource("src/teleon/blackboard/local_sqlite_blackboard.py"),
]
...
def _self_test() -> int:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        bb = py_class_src_teleon_blackboard_local_sqlite_blackboard__LocalSqliteBlackboard(db_path=db_path)  # TEMP db — never the real .agent db
        ...
        check("A: provider exposes provider_id + create/append/query/get_receipts",
              bb.provider_id == py_const_src_teleon_blackboard_local_sqlite_blackboard__LOCAL_SQLITE_BLACKBOARD_PROVIDER_ID
        ...
        check("A: card is local-first (append_only, no network) + is its own local_equivalent",
              ...
              and card.get("local_equivalent") == py_const_src_teleon_blackboard_local_sqlite_blackboard__LOCAL_SQLITE_BLACKBOARD_PROVIDER_ID)
        ...
        with tempfile.TemporaryDirectory() as tmp2, tempfile.TemporaryDirectory() as tmp3:
            seeded_a = py_function_src_teleon_blackboard_local_sqlite_blackboard__seed_demo_blackboard(db_path=str(Path(tmp2) / "seed.db"), now=_NOW, tenant_scope=_TENANT)
            seeded_b = py_function_src_teleon_blackboard_local_sqlite_blackboard__seed_demo_blackboard(db_path=str(Path(tmp3) / "seed.db"), now=_NOW, tenant_scope=_TENANT)
