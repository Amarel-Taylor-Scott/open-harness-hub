...
# folder (default openhubforai). See _repos/_shared/strategy/two-services-shared-infrastructure.md.
OH_PRODUCT = os.environ.get("OH_PRODUCT", "").strip() or "openhubforai"
_REPO_DIR = next((_ar for _ar in Path(__file__).resolve().parents if (_ar / ".aidoneright-root").exists()), Path(__file__).resolve().parents[2])
# NB: resolve WITH the tail — `_resource("web/<brand>")` fires the web/<brand> -> _repos/<repo>/frontend
# remap; `_resource("web") / <brand>` skips it and resolves to a nonexistent monorepo-root /web/<brand>.
WEB_DIR = _resource(f"web/{OH_PRODUCT}")
ADMIN_DEMO_WEB_DIR = _resource("web/openhubforai")
...
DESIGN_BUNDLE_DIR = _resource("dist") / "sites" / "aidoneright-design"
# Shared pinned runtime (React/Babel UMD) for the full-design front-ends — ONE copy, every product,
# co-located with the design bundle whose <script> pins it must match. (Was web/vendor/ pre-migration;
# resolved from DESIGN_BUNDLE_DIR so there is no root-level web/ dir and the path stays single-sourced.)
VENDOR_DIR = DESIGN_BUNDLE_DIR / "vendor"
...
# (e.g. '../teleon/Teleon.html' in shared/products.js); resolved from '/', those links
...
_ARCH_DIR = _resource("architecture")
...
#: The shared pinned runtime files this server mounts at /vendor/ (VENDOR_DIR, beside the design
#: bundle). The design-bundle prototypes load these from unpkg's CDN; we repoint them here so they
#: load OFFLINE.
...
def _local_service_port(service_id: str) -> int | None:
    """Port for a local service from _repos/shared-backend-components/architecture/local_service_registry.json (the single source
...
def _identity_service_port() -> int | None:
    """Identity port from _repos/shared-backend-components/architecture/identity_realm_registry.json defaults (its single source)."""
    try:
...
#: Every OH_SEAM_*_BASE override key. generate_provider_configs.py sets one per co-deployed peer, so
#: their presence is THE "multi-host deploy" signal (see _deploy_mode). Single source — no drift.
_SEAM_ENV_KEYS = (
    "OH_SEAM_IDENTITY_BASE", "OH_SEAM_REGISTRY_BASE", "OH_SEAM_ANALYTICS_BASE",
    "OH_SEAM_MAILBOX_BASE", "OH_SEAM_TELEON_RUNTIME_BASE", "OH_SEAM_OBSERVER_BASE",
    "OH_SEAM_LIVEOPS_BASE",
)


def _deploy_mode(env: dict | None = None) -> bool:
    """True when running as a multi-host deploy — signalled by ANY OH_SEAM_*_BASE override being set
    (generate_provider_configs.py sets one per co-deployed peer). In deploy mode a 127.0.0.1 fallback is
    never a real peer (localhost is the container itself), so a seam with no explicit base is a backend
    that was NOT co-deployed — skip it (a clean 404) rather than 502 to dead localhost."""
    src = env if env is not None else os.environ
    return any((src.get(k) or "").strip() for k in _SEAM_ENV_KEYS)


def _seam_base(env_key: str, port: int) -> str | None:
    """Seam target base URL, or None when the seam must NOT be registered. A non-empty OH_SEAM_*_BASE env
    overrides for cloud deploys where peers live on other hosts (http://identity:9410 on compose/K8s,
    http://aidr-identity.internal:9410 on Fly) — THE provider switch, same image everywhere, only these
    bases change (set by scripts/deploy/generate_provider_configs.py, never by hand). With no override:
    in dev, the host-local plane (http://127.0.0.1:<port>); in deploy mode, None — the peer is not
    co-deployed, so registering a 127.0.0.1 seam would only 502 (the observer-not-in-stack bug)."""
    override = (os.environ.get(env_key) or "").strip().rstrip("/")
    if override:
        return override
    return None if _deploy_mode() else f"http://127.0.0.1:{port}"
...
def _seam_proxies() -> list[tuple[str, str, str]]:
    ...
    cloud deploys override the host via OH_SEAM_*_BASE (see _seam_base). A seam is registered ONLY
    when its base resolves — in deploy mode a peer with no explicit base is not co-deployed and is
    skipped (a clean 404), never dangled at dead localhost (the observer-502 bug).
    """
    table: list[tuple[str, str, str]] = []

    def seam(env_key: str, port: int | None, *routes: tuple[str, str]) -> None:
        """Register (prefix, strip) routes for one service iff it resolves to a base (port known AND
        _seam_base not None). Collapses the per-service `if port:` + append boilerplate into one place."""
        if not port:
            return
        base = _seam_base(env_key, port)
        if not base:
            return
        for prefix, strip in routes:
            table.append((prefix, strip, base))

    seam("OH_SEAM_IDENTITY_BASE", _identity_service_port(), ("/api/identity/", ""))
    seam("OH_SEAM_REGISTRY_BASE", _local_service_port("local_openhubforai_projection_api"),
         ("/registry/", "/registry"))
    seam("OH_SEAM_ANALYTICS_BASE", _local_service_port("local_event_tracking_service"),
         ("/analytics/", "/analytics"))
    # mailbox = the visible transactional-email inbox (registration → verify-email click, same-origin)
    seam("OH_SEAM_MAILBOX_BASE", _local_service_port("mailbox_local_service"),
         ("/mailbox", ""), ("/api/mailbox/", ""))
    seam("OH_SEAM_TELEON_RUNTIME_BASE", _local_service_port("teleon_local_runtime"), ("/api/teleon/", ""))
    # observer = AIDevObserver session-review backend. It IS co-deployed in the current deploy stack:
    # deploy/docker-compose.deploy.yml runs the `observer` service and sets
    # OH_SEAM_OBSERVER_BASE=http://observer:9431 on every web service, so this seam registers and
    # /api/observer/* resolves live (200). The seam is registered ONLY when its base resolves — so if
    # observer is ever NOT co-deployed (no OH_SEAM_OBSERVER_BASE in deploy mode) it is skipped and
    # /api/observer/* becomes a clean 404, never a 502 to dead localhost (the original observer-502 bug).
    seam("OH_SEAM_OBSERVER_BASE", _local_service_port("observer_runtime"), ("/api/observer/", ""))
    # live-ops (baltor admin/demo): deep-dive 2026-06-11 — memory/pipeline/determinism/runtime were served
    # by the backend but missing here, so four shipped page families 404'd through the public origin.
    live_ops_routes = [(p, "") for p in (
        "/api/demo/", "/api/context/", "/api/dev/", "/api/fleet", "/api/admin-dashboard/",
        "/api/inference/", "/api/native/", "/api/standards/", "/api/graph/", "/api/context-gateway/",
        "/api/events", "/api/memory/", "/api/pipeline/", "/api/determinism/", "/api/runtime/")]
    seam("OH_SEAM_LIVEOPS_BASE", _local_service_port("baltor_admin_demo_server"), *live_ops_routes)
    return table
...
def _governance_metadata() -> dict[str, dict]:
    ...
    with _GOVERNANCE_LOCK:
        ...
        for path in (_resource("catalog")).rglob("*.yaml"):
            try:
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _serve_spa_index(self, path: str) -> bool:
        """Serve the product shell for clean client-side routes like /ide and /dashboard.

        Static assets and API/seam paths are handled earlier. If the last path segment has a
        file extension, keep returning 404 so missing assets do not silently become HTML.
        """
        name = Path(path).name
        if "." in name:
            return False
    ...
    def _serve_admin_demo_static(self, rel: str) -> bool:
        ...
        The demo currently lives under _repos/openhubforai/frontend because it was first built
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if parsed.path in ("/", "/index.html"):
            ...
            # Honest, cheap liveness probe — NOT the SPA. Without this branch '/healthz' is
            # extensionless, falls through the final else to _serve_spa_index, and returns 200 +
            # index.html for ANY value: a false-green signal a monitor/LB can't tell from a live
            # server. This proves THIS process is up and the component index is loaded, with NO
            # external/LLM network call (that reachability lives in /api/health). Never token-gated
            # (a liveness probe must not need a secret); serves_truth=false — liveness, not truth.
            self._send(200, json.dumps({
                "ok": True, "service": "showcase", "product": OH_PRODUCT,
                "components": len(self.index.items), "serves_truth": False,
            }).encode(), "application/json")
            ...
            def _subs(c) -> list[str]:
                ns = getattr(c, "node_subtypes", None)  # display-only node subtypes (e.g. IfStatement)
                if ns:
                    return list(ns)
            ...
            if not parsed.path.startswith("/api/") and self._serve_static(unquote(parsed.path)):
                return
            if not parsed.path.startswith("/api/") and self._serve_spa_index(parsed.path):
                return
            self._send(404, b"not found", "text/plain")
