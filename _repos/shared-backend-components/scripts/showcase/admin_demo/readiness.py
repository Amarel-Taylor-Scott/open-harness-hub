...
ROOT = next((_ar for _ar in Path(__file__).resolve().parents if (_ar / ".aidoneright-root").exists()), Path(__file__).resolve().parents[3])


def _file_check(label: str, path: str) -> dict[str, Any]:
    target = _resource(path)
    return {
        "id": label,
...
def admin_demo_readiness() -> dict[str, Any]:
    ...
    checks = [
        _file_check("six_tile_ui", "_repos/openhubforai/frontend/admin-demo.html"),
        _file_check("admin_demo_assets", "_repos/openhubforai/frontend/admin-demo-assets/app.js"),
        _file_check("admin_demo_styles", "_repos/openhubforai/frontend/styles/admin-demo/base.css"),
