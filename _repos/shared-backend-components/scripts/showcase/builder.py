...
(_repos/shared-backend-components/scripts/primitives/) via STAGE_ORDER / stage_for_type / label_for_type — single
...
def harness_recipe(task: str, kept: list[dict]) -> list[dict]:
    ...
    # `options` with a recommended `default`. Full menu + bundles: _repos/shared-backend-components/docs/concepts/retrieval-and-prompt-taxonomy.md.
    ...
    recipe += [
        ...
        step("polish", "default", 1, "action", "Render the model-query template",
             ...
             "Template spec: _repos/shared-backend-components/context/concepts/model-query-template.md", None,
    ...
    # exists (single source: _repos/shared-backend-components/catalog/processors/{retrieval,deliver,platform}/). See
    # _repos/shared-backend-components/context/concepts/recipe-component-map.md.
    canon = {
...
def flowchart(task: str, kept: list[dict]) -> list[dict]:
    ...
    for stage in STAGE_ORDER:
        ...
        if stage == IfStatement.stage and sum(1 for x in comps if x["branch"] == "main") >= 2:
            ...
            # IfStatement subtype), default OR — if ANY matches, the THEN action runs.
