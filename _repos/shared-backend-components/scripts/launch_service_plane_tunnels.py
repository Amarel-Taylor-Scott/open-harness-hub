...
cleanup only, ports come from `_repos/shared-backend-components/architecture/local_service_registry.json` (single source —
...
CLI:
    python3 _repos/shared-backend-components/scripts/launch_service_plane_tunnels.py --start|--stop|--status|--self-test
"""
...
_SHARE_JSON = _resource("dist") / "service-plane-tunnel-urls.json"
_SHARE_MD = _resource("dist") / "service-plane-tunnel-urls.md"
_SEAM_ENV = _resource("dist") / "service-plane-tunnel-seams.env"
...
TARGETS: dict[str, dict] = {
    "openhubforai_app": {"share": "showcase-share-url-openhubforai.txt", "seam": None, "token_query": True},
    ...
    "local_openhubforai_projection_api": {"share": None, "seam": "OH_SEAM_REGISTRY_BASE"},
    ...
    "ops_console": {"share": "ops-console-url.txt", "seam": None},
}
...
def _token() -> str:
    f = _resource("dist") / "showcase-token.txt"
...
def _write_manifests(records: dict) -> None:
    ...
    for sid, rec in records.items():
        ...
        if url and spec.get("share"):
            ...
            (_resource("dist") / spec["share"]).write_text(share + "\n")
