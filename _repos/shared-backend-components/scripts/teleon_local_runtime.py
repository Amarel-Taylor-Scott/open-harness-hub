...
_repos/teleon/frontend/teleon-live.js client depends on exactly that contract.
...
_repos/shared-backend-components/scripts/registry_local_service.py: the account is resolved server-side; the client never asserts
who it is. Reads are open (local demo plane). Nothing here is "truth" beyond what it really did:
deterministic code ran, receipts recorded. Port lives in _repos/shared-backend-components/architecture/local_service_registry.json.
...
sys.path.insert(0, str(next((_ar for _ar in Path(__file__).resolve().parents if (_ar / ".aidoneright-root").exists()), Path(__file__).resolve().parents[1])))  # direct-file run → repo importable
...
STATE_DIR = _resource("dist") / "local-services-state" / "teleon-runtime"
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        if body.get("async"):
            ...
            return self._send(202, {"run_id": pending["run_id"], "status": pending["status"],
                                    "run": pending})
        # SYNC default (backward compatible — the generated _repos/teleon/frontend/teleon-live.js contract):
...
def _self_test() -> int:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # the generated _repos/teleon/frontend/teleon-live.js (sync, expects 201 + finished {run}) and async
