...
CLI / self-test (swarms the demo's stale runbook claim; no model, no network):
    python3 _repos/shared-backend-components/scripts/context_swarm.py --self-test
    python3 _repos/shared-backend-components/scripts/context_swarm.py --swarm obj-runbook --purpose verify
"""
...
# Make the repo root importable when run *directly* (the repo has no top-level _repos/shared-backend-components/scripts/__init__.py;
...
def _self_test() -> int:
    ...
    try:
        ...
        schema = _j.loads((_resource("schemas/governance/steward-review-request.schema.json")).read_text())
