...
Usage:
    python3 _repos/shared-backend-components/scripts/build_readme_stats.py            # rewrite the block in README
    python3 _repos/shared-backend-components/scripts/build_readme_stats.py --check     # exit 1 if README is stale

CI / validate can run --check so a stale count breaks the build instead of
rotting quietly. See _repos/shared-backend-components/docs/codex/no-magic-values.md.
"""
...
# _repos/shared-backend-components/scripts/_config.py is the shared helper for module-invoked _repos/shared-backend-components/scripts/db/*.
REPO = Path(__file__).resolve().parent.parent
CATALOG = _resource("catalog")
SQLITE = _resource("dist") / "catalog.sqlite"
EMIT_DIR = _resource("scripts") / "emit"
SCRIPTS = _resource("scripts")
CODE_TEMPLATES = _resource("code-templates")
