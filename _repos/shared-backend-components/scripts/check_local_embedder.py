...
from src.teleon.retrieval.embedding_port import (  # noqa: E402
    py_class_src_teleon_retrieval_embedding_port__LexicalEmbedder, py_class_src_teleon_retrieval_embedding_port__LocalOllamaEmbedder, py_function_src_teleon_retrieval_embedding_port__best_embedder, py_function_src_teleon_retrieval_embedding_port__select_embedder,
)
...
def main() -> int:
    ...
    emb = py_function_src_teleon_retrieval_embedding_port__select_embedder("nomic_embed")
    ck("nomic_embed resolves to the LOCAL embedder", isinstance(emb, py_class_src_teleon_retrieval_embedding_port__LocalOllamaEmbedder))

    # local-first: it targets localhost, no API key / env var anywhere in the local path
    src = (_resource("src/teleon/retrieval/embedding_port.py")).read_text()
    local_block = inspect.getsource(py_class_src_teleon_retrieval_embedding_port__LocalOllamaEmbedder)
    ...
    be = py_function_src_teleon_retrieval_embedding_port__best_embedder()
    ...
    ck("best_embedder is local-semantic OR lexical baseline (never cloud)",
       isinstance(be, (py_class_src_teleon_retrieval_embedding_port__LocalOllamaEmbedder, py_class_src_teleon_retrieval_embedding_port__LexicalEmbedder)), be.name)

    # if the local daemon is up, prove it's genuinely SEMANTIC (pdf closer than an unrelated tool)
    if isinstance(be, py_class_src_teleon_retrieval_embedding_port__LocalOllamaEmbedder):
        import math
        def cos(a, b):
