...
unavailable offline (no fabricated prices). serves_truth=false.
  ...
  python3 _repos/shared-backend-components/scripts/check_economic_layer.py --self-test
"""
...
def _self_test() -> int:
    ...
    a_node = json.loads((_resource("architecture") / "model_provider_graph.json").read_text())["nodes"][0]["node_id"]
