...
_repos/shared-backend-components/architecture/deploy_topology.json (postgres, identity, teleon-runtime), brought up by the deploy compose + the
...
CLI: PYTHONPATH=. python3 _repos/shared-backend-components/scripts/check_teleon_local_emulation.py --self-test | --report
"""
...
def _topology_service_names() -> set[str]:
    d = json.loads((_resource("architecture") / "deploy_topology.json").read_text())
...
def _self_test() -> int:
    ...
    ck("the emulators overlay compose declares BOTH local emulator services",
       all(s in (_resource(_EMU_COMPOSE)).read_text() for s in ("model-emulator", "source-emulator")))
    ck("the Tiltfile brings up the stack + runs the local-emulation proof",
       all(t in (_resource(_TILTFILE)).read_text() for t in (_EMU_COMPOSE, "check_teleon_local_emulation")))
