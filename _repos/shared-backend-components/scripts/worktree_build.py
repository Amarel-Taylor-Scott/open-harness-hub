...
def run_in_worktree(extra: list[str], *, one: bool) -> int:
    ...
    cmd = [sys.executable, str(_resource("scripts/build_loop.py")), "--run" if one else "--supervise", *extra]
...
def self_test() -> int:
    import tempfile
    # build_loop in a worktree resolves REPO to the worktree (parents[1] of _repos/shared-backend-components/scripts/build_loop.py)
    ...
    cmd = [sys.executable, str(_resource("scripts/build_loop.py")), "--supervise", *extra]
