...
Asserts:
  ...
  I. NO-REINVENTION: the existing templates/ tree + _repos/shared-backend-components/scripts/{check_template_catalog,generate_from_template} are
     intact (not replaced).
  J. DEPENDENCY LAW (local): no module under _repos/teleon/backend/src/teleon/templates imports src.baltor.
...
def _self_test() -> int:
    ...
    for c in ("TemplateArtifact", "SchemaObjectTemplate"):
        p = _resource("schemas") / "templates" / f"{c}.schema.json"
    ...
    # I. no-reinvention
    check("I: existing templates/ tree intact", (_resource("templates/worker")).exists() and (_resource("templates/api")).exists())
    check("I: existing template scripts intact", (_resource("scripts/check_template_catalog.py")).exists() and (_resource("scripts/generate_from_template.py")).exists())

    # J. dependency law (local)
    bad = [str(p.relative_to(_REPO)) for p in (_resource("src/teleon/templates")).rglob("*.py")
           for line in p.read_text().splitlines() if line.strip().startswith(("import src.baltor", "from src.baltor"))]
    check("J: no _repos/teleon/backend/src/teleon/templates module imports src.baltor", not bad, "; ".join(bad))
