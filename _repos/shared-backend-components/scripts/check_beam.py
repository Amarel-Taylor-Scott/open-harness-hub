...
chosen), respects beam_width, and is honest (chosen=None) when nothing verifies. serves_truth=false.
  ...
  python3 _repos/shared-backend-components/scripts/check_beam.py --self-test
"""
...
def _self_test() -> int:
    ...
    res = beam.py_function_src_teleon_synthesis_beam__beam_compose([cand_pricey, cand_cheap, cand_broken], profile=cost_first(), beam_width=2, shard=_SHARD)
    ...
    empty = beam.py_function_src_teleon_synthesis_beam__beam_compose([cand_broken], profile=cost_first(), shard=_SHARD)
