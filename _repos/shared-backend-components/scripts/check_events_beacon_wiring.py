...
(not the legacy vanilla app.js/events.js, which the kit entry no longer loads):
  ...
  B. CLIENT CONTRACT — oh-identity.js exposes window.OHEvents (page/variant/exposure/conversion),
     ...
     OPENHUBFORAI_EVENTS_BASE deploy override, sends via sendBeacon, uses an anon id (never an email), and
...
KIT = WEB / "kit"
PROBE = _resource("e2e") / "oh_experiments_beacon_probe.mjs"
...
def _self_test() -> int:
    ...
    registry = json.loads((_resource("architecture") / "local_service_registry.json").read_text())
    ...
    base_match = re.search(r'OPENHUBFORAI_EVENTS_BASE \|\| "http://127\.0\.0\.1:(\d+)"', ident)
    ...
    ck("B: OPENHUBFORAI_EVENTS_BASE deploy override supported", "OPENHUBFORAI_EVENTS_BASE" in ident)
    ...
    if not shutil.which("node"):
        ...
        # Pass the resolved shipped-kit dir (web/openhubforai/kit → _repos/openhubforai/frontend/kit after
        # the _repos/ migration) so the Node probe loads the EXACT files sections A/B assert against — the
        # check's `_resource` is the single source of the kit path; the probe carries no layout assumption.
        proc = subprocess.run(["node", str(PROBE)], cwd=str(REPO_ROOT),
                              env={**os.environ, "OH_KIT_DIR": str(KIT)},
        ...
        ck("C: OHExp.exposure → an exposure beacon carrying experiment+variant (via the bridge)",
           bool(probe.get("exposure")) and probe["exposure"].get("experiment") == "openhubforai_hero"
    ...
    if probe.get("ok") and probe.get("exposure") and probe.get("conversion"):
        ...
        try:
            ...
            slot = summary["experiments"].get(f"openhubforai_hero:{variant}", {})
            ck(f"C: /summary shows the REAL A/B readout (openhubforai_hero:{variant} exposure=1, conversion=1, rate=1.0)",
