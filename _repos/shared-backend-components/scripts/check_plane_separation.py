#!/usr/bin/env python3
"""check_plane_separation — enforce the DEVELOPMENT vs PRODUCT boundary (_repos/shared-backend-components/architecture/plane_separation.json).

Dev research/tools/loops/data must stay separate from product modules/actions/data. This proves: (1) PRODUCT modules
(src/teleon, _repos/baltor/backend/src/baltor) import NO development tool (they may import shared_infra); (2) dev tools live under _repos/shared-backend-components/scripts/,
...
CLI: PYTHONPATH=. python3 _repos/shared-backend-components/scripts/check_plane_separation.py --self-test
"""
...
_SPEC = _resource("architecture") / "plane_separation.json"
...
def _self_test() -> int:
    ...
    # (2) dev tools live under _repos/shared-backend-components/scripts/, not src/
    ...
    ck("every development tool file exists", all((_resource(t)).exists() for t in dev_tools), str([t for t in dev_tools if not (_resource(t)).exists()]))

    # (3) dev data is routed to dev paths (the loop + radar constants point under data/dev-intel/)
    loop = (_resource("scripts") / "multi_model_improvement_loop.py").read_text()
    radar = (_resource("scripts") / "research_radar.py").read_text()
    ...
    streams = {s["stream"]: s for s in json.loads((_resource("architecture") / "storage_tier_policy.json").read_text())["streams"]}
    ...
    ck("research_radar is NOT inside the product package (_repos/teleon/backend/src/teleon)", not (REPO / "_repos/teleon/backend/src/teleon/intel/research_radar.py").exists())
    ck("shared_infra modules exist (the seam both planes use)", all((_resource(m)).exists() for m in d["shared_infra"]["modules"]))
