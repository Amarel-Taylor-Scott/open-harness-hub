...
DEVELOPMENT plane (the operator surface); the engines + store are PRODUCT (_repos/openhubforai/backend/src/openhubforai).
...
CLI: PYTHONPATH=. python3 _repos/shared-backend-components/scripts/hub_engine_runner.py --ingest OpenSkillsHub --okf my_skill.md --improve
"""
...
def _js_scrape(q: str) -> list:
    ...
    if not q.startswith("http"):
        return []
    try:
        r = subprocess.run(["node", str(_resource("e2e") / "scrape_url.mjs"), q], cwd=str(REPO),
...
def hub_query(hub: str) -> str:
    """Build a discovery query for a hub from _repos/shared-backend-components/architecture/hub_population_strategy.json (the per-hub sources)."""
    try:
        import json
        strat = json.loads((_resource("architecture") / "hub_population_strategy.json").read_text()).get("hubs", {})
