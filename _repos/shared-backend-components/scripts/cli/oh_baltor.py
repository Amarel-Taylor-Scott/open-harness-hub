...
north-star (``_repos/_shared/codex/north-star.md`` §"The execution sequence", M5 done bar:
...
``_repos/shared-backend-components/docs/codex/change-verification-contract.md`` — *real vs. seam; no invented
metrics*):
...
CLI / self-test:
    python3 _repos/shared-backend-components/scripts/cli/oh_baltor.py --selftest        # proves all three subcommands
...
# (``python3 _repos/shared-backend-components/scripts/cli/oh_baltor.py``). The CLI's whole job is to COMPOSE the
...
# in ``_repos/shared-backend-components/scripts/enrichment/serve.py`` and ``tier_pipeline.py``.
...
_SELFTEST_CORPUS: dict[str, Any] = {
    ...
    "license": "MIT",
    "provenance": {
        "signer": "openhubforai.com (selftest oracle)",
    ...
    "documents": [
        {
            ...
            "url": "https://openhubforai.com/docs/budget",
            "section": "Reference",
...
def _selftest() -> int:
    ...
    try:
        ...
        assert "- [Token budget planner](https://openhubforai.com/docs/budget): " in llms, \
