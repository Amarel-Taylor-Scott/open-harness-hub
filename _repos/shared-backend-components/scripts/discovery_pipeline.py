...
Composes: the KEY HOLDER (BYO keys, _repos/teleon/backend/src/teleon/runtime/key_holder), source_search (GitHub), the external-API registry
...
Dev-plane script (imports product; never the reverse).
  ...
  python3 _repos/shared-backend-components/scripts/discovery_pipeline.py "ocr" "reranker" "pdf table extraction"
  python3 _repos/shared-backend-components/scripts/discovery_pipeline.py --self-test
"""
...
FEED = _resource("data") / "dev-intel" / "discovered_pipeline.jsonl"
...
def _thin_planes(target: int = 12) -> set[str]:
    ...
    per = Counter(t["plane"] for t in json.loads((_resource("architecture") / "tool_registry.json").read_text())["tools"])
    laddered = {l.get("plane") for l in json.loads((_resource("architecture") / "capability_ladders.json").read_text())["ladders"]}
...
def security_screen(cand: dict) -> dict:
    ...
    _repos/shared-backend-components/docs/security/mcp-and-skill-security.md."""
...
def discover_sources(query: str, *, limit: int = 8, env: dict | None = None) -> tuple[list, dict]:
    ...
    if py_const_src_teleon_runtime_key_holder__HOLDER.resolve("rapidapi", env):
...
def _self_test() -> int:
    ...
    st = py_const_src_teleon_runtime_key_holder__HOLDER.status({})
    ...
    ck("rapidapi NOT held without a key (BYO gate)", "rapidapi" not in py_const_src_teleon_runtime_key_holder__HOLDER.held({}))
    ...
    ck("auth_headers empty when no key held", py_const_src_teleon_runtime_key_holder__HOLDER.auth_headers("rapidapi", {}) == {})
    ck("auth_headers shaped when key present (X-RapidAPI-Key)",
       py_const_src_teleon_runtime_key_holder__HOLDER.auth_headers("rapidapi", {"RAPIDAPI_KEY": "k"}) == {"X-RapidAPI-Key": "k"})
