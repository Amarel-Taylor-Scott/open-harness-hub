...
Asserts _repos/shared-backend-components/architecture/pattern_registry.json parses; all 25 canonical pattern ids are present exactly
...
CLI: python3 _repos/shared-backend-components/scripts/check_pattern_registry.py --self-test
"""
...
def _self_test() -> int:
    ...
    for p in patterns:
        if p.get("maturity") in ("standard", "enforced"):
            has_example = any((_resource(e)).exists() for e in p.get("detected_examples", []))
            has_real_proof = any((_resource(pr.split("#")[0])).exists() for pr in p.get("required_proofs", []))
