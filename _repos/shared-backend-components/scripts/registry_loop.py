...
hold a multi-hour daemon honestly) — e.g. `watch -n 900 python3 _repos/shared-backend-components/scripts/registry_loop.py --run`, or wire it as a
flywheel. serves_truth=false; all outputs are governed candidates.

  python3 _repos/shared-backend-components/scripts/registry_loop.py            # show the loop plan + how to schedule it
  python3 _repos/shared-backend-components/scripts/registry_loop.py --run      # execute one full refresh pass
  python3 _repos/shared-backend-components/scripts/registry_loop.py --self-test
"""
...
def main() -> int:
    ...
    if args.self_test:
        ...
        for label, script, _ in STEPS + POPULATION_STEPS:
            checks += 1
            if not (_resource(script)).exists():
