...
Each unit in _repos/shared-backend-components/architecture/teleon_capability_seed.json is a pure PurposeTaskSpec (declared by intent +
capability_slot + stable input->output contract, NOT by per-task code). This module:
  ...
  - classifies each unit's escalation tier with the REAL ladder (_repos/teleon/backend/src/teleon/exploration/ladder.py) and confirms
...
CLI: python3 _repos/shared-backend-components/scripts/teleon_preseed_capabilities.py --self-test | --list | --json
"""
...
_SEED_PATH = _resource("architecture") / "teleon_capability_seed.json"
_SCHEMA_PATH = _resource("schemas") / "purpose_tasks" / "PurposeTaskSpec.schema.json"
...
def _arch(name: str) -> dict:
    """Read a SHARED data registry (_repos/shared-backend-components/architecture/<name>). Data only — never a src.baltor import (Teleon is the
...
def resolve_resources(unit: dict) -> dict:
    ...
    (_repos/shared-backend-components/architecture/*.json) — never imports src.baltor (dependency-law-safe). Per unit returns: the runtime PROFILE
