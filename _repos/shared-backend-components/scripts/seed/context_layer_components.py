#!/usr/bin/env python3
"""Seed the context-layer GAP families the PMF analysis surfaced (_repos/_shared/strategy/context-layer-pmf.md):
...
builder; spec-level (lifecycle=experimental).
    ...
    python3 _repos/shared-backend-components/scripts/validate.py _repos/shared-backend-components/catalog/processors/memory/*.yaml _repos/shared-backend-components/catalog/processors/cache/*.yaml _repos/shared-backend-components/catalog/processors/connectors/*.yaml
"""
from __future__ import annotations

from scripts.seed.component_seed import processor, write_batch

DOC = "_repos/_shared/strategy/context-layer-pmf.md"
LABEL = "OpenHubForAI context-layer taxonomy"

# MEMORY — persist what was LEARNED across sessions (not raw history). (_repos/shared-backend-components/catalog/processors/memory/)
MEMORY = [
...
# CACHING — don't pay twice for the same tokens. (_repos/shared-backend-components/catalog/processors/cache/)
CACHE = [
...
# RETRIEVAL additions — round out the retrieval layer. (_repos/shared-backend-components/catalog/processors/retrieval/)
RETRIEVAL = [
...
# MCP CONNECTORS — bring the team's doc sources in as governed corpora. (_repos/shared-backend-components/catalog/processors/connectors/)
CONNECTORS = [
