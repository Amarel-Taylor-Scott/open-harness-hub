...
MANIFEST = _resource("architecture") / "local_dev_tunnel_auth_runtime.json"
DOC = _resource("docs") / "architecture" / "local-dev-tunnels-and-auth.md"
SERVICE_AUTH = _resource("architecture") / "service_auth_consumption_model.json"
SERVICE_REGISTRY = _resource("services") / "registry.yaml"
LAUNCHER = _resource("scripts") / "launch_portfolio_trycloudflare.py"
SERVER = _resource("scripts") / "serve_portfolio_sites.py"
...
def _self_test() -> int:
    ...
    compose_files = [_resource(item["file"]) for item in data.get("container_stacks", [])]
