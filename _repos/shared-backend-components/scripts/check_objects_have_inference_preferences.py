...
def _self_test() -> int:
    ...
    shell = json.loads((_resource("schemas") / "shared" / "ObjectShell.schema.json").read_text())
    ...
    cov = json.loads((_resource("architecture") / "inference_preference_coverage.json").read_text())
