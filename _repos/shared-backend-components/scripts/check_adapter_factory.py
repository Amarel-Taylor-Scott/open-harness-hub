...
inferred from the schema); the HTTP invoke is network-gated + honest offline (no fabricated response). serves_truth=false.
  ...
  python3 _repos/shared-backend-components/scripts/check_adapter_factory.py --self-test
"""
...
def _self_test() -> int:
    ...
    comps = AF.py_function_src_teleon_components_adapter_factory__from_openapi(_SPEC)
    ...
    res = AF.py_function_src_teleon_components_adapter_factory__http_invoke(by_id["enrich_company"], {"domain": "acme.com"}, network_allowed=False)
