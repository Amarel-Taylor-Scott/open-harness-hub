...
Usage:
  python _repos/shared-backend-components/scripts/run_pipeline.py pipeline/research-entity --inputs inputs.json
  python _repos/shared-backend-components/scripts/run_pipeline.py pipeline/brand-safe-product-photo --simulate
"""
...
def _invoke_tool(tool_id: str, target: Any, params: dict) -> dict:
    ...
    if tool_id == "tool/json-schema-validator":
        ...
        try:
            schema = json.loads((_resource(schema_path)).read_text())
