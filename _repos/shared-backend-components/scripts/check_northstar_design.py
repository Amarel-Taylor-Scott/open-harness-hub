...
def _surface_roots() -> list[Path]:
    ...
    roots = [_resource(p["canonical_surface"]) for p in spec.get("pillars", []) if (_resource(p["canonical_surface"])).is_dir()]
    design = _resource("dist") / "sites" / "aidoneright-design"
    if design.is_dir():
        roots.append(design)
    return roots
...
def check() -> int:
    ...
    if not (_resource("dist/sites/aidoneright-design/aidevobserver")).is_dir():
