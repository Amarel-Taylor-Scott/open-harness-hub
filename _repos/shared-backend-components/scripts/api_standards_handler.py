...
and the admin server just delegates to it (model: _repos/shared-backend-components/scripts/api_context_handler.py). This layer is a
PROJECTION over the standards-system truth files produced by sibling lanes:

  GET  /api/standards/patterns            <- _repos/shared-backend-components/architecture/pattern_registry.json
  GET  /api/standards/templates           <- _repos/shared-backend-components/architecture/template_catalog.json
  GET  /api/standards/routines            <- _repos/shared-backend-components/architecture/routine_library.json
  GET  /api/standards/waivers             <- _repos/shared-backend-components/architecture/pattern_waivers.json
  GET  /api/standards/maturity            <- _repos/shared-backend-components/architecture/pattern_maturity_matrix.json
...
#: routes this handler owns (registered in _repos/shared-backend-components/architecture/contract_registry.json#api_routes by MAIN).
ROUTES = (
...
def generate_preview(body: dict) -> tuple:
    ...
    patterns_list = [p for p in _as_list(pdata, "patterns") if isinstance(p, dict)]
    templates_list = [t for t in _as_list(tdata, "templates") if isinstance(t, dict)]
    # A supplied id must resolve; a PARAM-LESS call (the GET "convenience read-only projection" the
    # dispatcher documents — the UI's on-load default-plan read) picks the first registered pattern +
    # template as a representative default, so it returns a usable 200 plan instead of a spurious 400.
    pat = (next((p for p in patterns_list if (p.get("pattern_id") or p.get("id")) == pattern_id), None)
           if pattern_id else (patterns_list[0] if patterns_list else None))
    tmpl = (next((t for t in templates_list if (t.get("template_id") or t.get("id")) == template_id), None)
            if template_id else (templates_list[0] if templates_list else None))
    if pat is None or tmpl is None:
        # A NON-EMPTY id that does not resolve is a real client error (400); a param-less default with an
        # empty registry is just "nothing to plan yet" (projection degrades, never 400s the on-load read).
        if pattern_id or template_id:
            return 400, {"dry_run": True, "error": "unknown pattern_id or template_id",
                         "pattern_id": pattern_id, "template_id": template_id}
        return 200, {"dry_run": True, "available": False, "would_create": [],
                     "note": "no patterns/templates available to plan a default preview"}
    # resolve the ids we actually planned against (the defaults, when none were supplied)
    pattern_id = pattern_id or str(pat.get("pattern_id") or pat.get("id") or "")
    template_id = template_id or str(tmpl.get("template_id") or tmpl.get("id") or "")
