# AIDONERIGHT-UNIVERSE — how `shared-backend-components` interfaces with the rest of the AI Done Right org
...
## This repo — `shared-backend-components` (`aidoneright-shared-backend-components`)
- **Layer:** substrate · **Kind:** substrate
- **Role:** the SUBSTRATE every product consumes — the primitive/component registry + factory + catalog, codegraph, naming planes, eval, storage tiers, pipelines, credential plane. Reusable infrastructure, product-neutral.
- **How others interface with you:** Call the substrate registry seam same-origin at `/registry/` (cloud base `OH_SEAM_REGISTRY_BASE`, a local port in dev). Seam: `/registry/`.
...
| `shared-backend-components` **← you** | aidoneright-shared-backend-components | substrate | seam `/registry/` | the SUBSTRATE every product consumes — the primitive/component registry + factory + catalog, codegraph, naming planes… |
