...
| Live hubs (makeHub) | OpenContextHub · OpenSkillsHub · OpenToolsHub · OpenSkillToTool · OpenMCPHub · OpenCompressionHub · OpenBenchmarkHub · OpenReviewHub · OpenHubForAI |
...
> OpenHubForAI is the one bespoke `pt-*` site — chrome + account pages on the shared kit,
...
- **OpenHubForAI:** ~43 routes — catalog in `openhubforai/PAGES.md`; map in `proto-main.jsx` `App()`.
...
- Per-folder `README.md` — file map for each site. `openhubforai/PAGES.md` — OpenHubForAI routes.
