...
function PLanding() {
  ...
  const brand = { name: 'OpenHubForAI', realm: 'openhubforai', tld: '.io', glyph: '⎔', accent: 'var(--accent)' };
  const [hint, setHint] = React.useState('');
  const [exMod, setExMod] = React.useState('text');
  // Kept only for analytics tracking. The openhubforai_hero / openhubforai_subhead A/B
  // experiments were retired 2026-06-27 — OpenHubForAI leads with one message: the open store.
  const [, hero] = window.useExperiment('openhubforai_hero', ['A', 'B']);
  const submit = (t) => {
    const v = (t ?? task).trim();
    ...
    <div className="pt-mkt pt-view">
      ...
      <div className="pt-mkt-body">
        <section className="pt-hero">
          <div className="pt-hero-eyebrow">The open store of context, tools, skills &amp; harnesses</div>
          <h1>The open store<br />for AI.</h1>
          <p className="sub">Browse every building block your agents need — context, tools, skills, and harnesses, in one open store that both Teleon and Baltor consume. Describe a task below and OpenHubForAI assembles a governed harness from it.</p>
          ...
          <div className="pt-quiet">
            ...
            <a onClick={() => navigate('/registries')}>Browse the store</a><span className="dot">·</span>
...
function PPreview() {
  ...
  return (
    <div className="pt-mkt pt-view">
      <window.OhTopBar brand={{ name: 'OpenHubForAI', realm: 'openhubforai', tld: '.io', glyph: '⎔', accent: 'var(--accent)' }}
        cta={{ label: 'Start free', href: '/signup' }} signInHref="/signin" />
      <div className="pt-mkt-body">
        <div className="pt-pv">
