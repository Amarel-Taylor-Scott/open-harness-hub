...
OpenHubForAI) link to their sites; Teleon + the three new OpenHubForAI registries are portfolio cards
...
- Product cards use the product accents: `--cie-baltor` (teal), `--cie-openhubforai` (ember).
...
- Cross-links to the two product sites via relative paths (`../context-enrichment/…`,
  `../openhubforai/…`); swap for real subdomains in production.
