---
name: building-permit-amendment-regularization-route
description: Given a building case packet, detect whether the reported as-built deviates
  from the approved permit enough to require a revised/amended permit (regularization),
  and route the case to the jurisdiction-specific authority (PH OBO/DPWH, ID Dinas
  PUPR/SIMBG, NG state building-control) with citations; abstain to a human when the
  approved scope is not human-verified or retrieval is weak.
...
- **rule_packs**: `rule-pack/grep-municipal-permit-review-flags`, `rule-pack/hybrid-retrieval-policy`

## Success criteria

- rubric `rubric/municipal-permit-review-quality-v1` threshold 0.66
...
- Hub component: `pipeline/building-permit-amendment-regularization-route` v0.1.0
- License: `MIT`
