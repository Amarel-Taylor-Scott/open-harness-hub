---
name: guarded-image-generation
description: Given an image prompt (and optional style/rules), generate candidate
  images, block illicit prompts/outputs, reject anatomically distorted results, and
...
2. **generate** — `adapter` → `adapter/flux-1-schnell-local` (when `$.steps.screen_prompt.output.allow == true`)
...
- rubric `rubric/generated-image-safety-quality` threshold 0.8

## Provenance

- Hub component: `pipeline/guarded-image-generation` v0.1.0
- License: `MIT`
