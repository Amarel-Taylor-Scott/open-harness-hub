---
name: supplier-forced-labor-prescreen
description: 'Pre-screen a supplier (name, country, product, parent) for forced-labour
  risk signals and route it: standard-onboarding (no signal) | escalate-to-full-audit
  (signal found), citing the matched signal.'
when_to_use: 'Pipeline kind: route.'
---
...
- **knowledge_packs**: `knowledge-pack/csddd-and-forced-labor-indicators`, `knowledge-pack/sanctions-list-shape`
- **rule_packs**: `rule-pack/grep-apparel-forced-labor-trace-flags`, `rule-pack/sanctions-screening`

## Success criteria

- deterministic `$.steps.classify_route.output.result.routing_decision` in `['standard-onboarding', 'escalate-to-full-audit']`
- deterministic `$.steps.check_citations.output.result.pass` == `True`
- deterministic `$.steps.cost_gate.output.result.within_ceiling` == `True`

## Provenance

- Hub component: `pipeline/supplier-forced-labor-prescreen` v0.1.0
- License: `MIT`
