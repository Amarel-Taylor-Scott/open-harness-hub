---
name: worker-voice-ilo-indicator-extract
description: Extract which of the ILO 11 forced-labour indicators are evidenced in
  an anonymized worker-voice transcript (present | absent | insufficient-evidence),
  citing the testimony span and the ILO indicator definition, abstaining where unsupported.
...
10. **iterate_if_weak** — `loop` → `pattern/corrective-rag` (when `$.steps.grade_retrieval.output.result.coverage_weak == true`)
...
- **rule_packs**: `rule-pack/grep-human-trafficking-ugc-flags`, `rule-pack/hybrid-retrieval-policy`

## Success criteria

- rubric `rubric/apparel-forced-labor-trace-quality-v1` threshold 0.72
- deterministic `$.steps.check_citations.output.result.pass` == `True`
- deterministic `$.steps.redaction_audit.output.result.pass` == `True`

## Provenance

- Hub component: `pipeline/worker-voice-ilo-indicator-extract` v0.1.0
- License: `MIT`
