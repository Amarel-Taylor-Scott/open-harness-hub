...
Runnable goal for `/polish`. The job: take the OpenHubForAI product front-end (`_repos/openhubforai/frontend/`,
...
```bash
bash scripts/serve_showcase.sh            # serves _repos/openhubforai/frontend/ on :8000 + a persistent Cloudflare URL
cat dist/showcase-share-url.txt           # the live, token-gated share URL (verify it serves _repos/openhubforai/frontend/)
node --check _repos/openhubforai/frontend/app.js _repos/openhubforai/frontend/data.js       # core must parse
ls _repos/openhubforai/frontend/pages/                             # the self-registering screen modules
```

The front-end contract (do not break it): `_repos/openhubforai/frontend/app.js` defines `window.OHH`; screens live in
`_repos/openhubforai/frontend/pages/*.js` and self-register via `OHH.register(route, render, onMount, {theme})`; shared
data is `OHH.data` (never redeclare); styling is the ported design system in `_repos/openhubforai/frontend/styles/*.css`
(reuse classes, add none). Full map: `_repos/openhubforai/frontend/README.md`, `_repos/openhubforai/frontend/design/PAGES.md`, `_repos/openhubforai/frontend/design/HANDOFF.md`.
...
2. **Sell** — in the first screenful it makes the value proposition unmissable. The canonical
   props live in `_repos/_shared/design/value-propositions.md`. The spine on every surface:
...
2. Take the lowest score; make surgical edits to its `_repos/openhubforai/frontend/pages/*.js` (vanilla, no new CSS/deps).
...
No-build (`_repos/openhubforai/frontend/` stays vanilla + ported CSS). No PII, no `_reference/` republish, no faked
lift numbers (label samples). Don't edit `_repos/openhubforai/frontend/app.js`/`data.js`/`index.html` except for a real
router/nav fix. Keep the live URL serving the latest `_repos/openhubforai/frontend/` (static — no restart needed).
Cross-link: [[../design/value-propositions.md]], `_repos/openhubforai/frontend/README.md`, `archive/legacy/docs/strategy/acquihire-roadmap.md`.
