...
- **Teleon federation:** **103 registries** in `architecture/registry_ontology.json` (schema-conformant, grounded on
  ...
  `_repos/teleon/backend/src/teleon/registry/port.py`; population engines are `populate.py` / `enrich.py` (#84, the enrichment worker:
...
- **Global Software Knowledge Graph** (`_repos/teleon/backend/src/teleon/knowledge/`): `dependency_graph` (#101, transitive "this stack
...
- **Observer/Spotter** (`_repos/teleon/backend/src/teleon/observer/`): one **router** over an **11-module intervention taxonomy**
...
- **Baltor / Teleon split (architectural law):** Baltor → Teleon → OpenHubForAI, **never the reverse**
...
`/goal follow the instructions in _repos/shared-backend-components/context/goals/full-surface-enrichment-loop.md` should pick the **highest-leverage gap**:
