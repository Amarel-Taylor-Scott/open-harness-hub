...
| Spec binding | `taxonomy/SPEC.md` §18 (canonical; surfaced at `_repos/openhubforai/context/reference/spec.md`) | Names the targets, IRIs, and additive envelope fields. |
...
`ohh:` namespace use `openhubforai.io` (741 files), whereas the product
domain is `openhubforai.com` (3 files). That is internally consistent so it
...
- It does **not** treat the `openhubforai.io` vs `openhubforai.com` split
...
`_repos/shared-backend-components/context/codex/schema-extensibility.md` · `taxonomy/SPEC.md` §18.
