...
3. **Keep context current.** Bind volatile facts to tools, CDC feeds, freshness
   ...
   them into Actions or static prompts.
...
5. **Strengthen fragile components.** Promote stronger If Statements, tools,
   Knowledge Corpus entries, processors, harnesses, or verified global context
   feeds when a dependency is stale, low-lift, unsafe, or too expensive.
...
- an If Statement fires on examples but fails adversarial variants;
...
- higher-lift If Statements, processors, or harnesses from the catalog;
