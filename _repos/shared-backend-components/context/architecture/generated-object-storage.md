...
JSONL is cheap, append-friendly, easy to validate in CI, and easy for workers to stream. It is the right format for seed batches and partition files. Production should load those rows into Postgres, then treat the database as the authority.
...
Use `scripts/db/object_count_report.py` for local staged JSONL counts and `db/postgres/object_count_report.sql` for canonical database row counts. See `_repos/shared-backend-components/context/architecture/postgres-pgvector-bootstrap.md` (which absorbed the object-counts-and-database-bootstrap plan) for the bootstrap and reporting workflow.
