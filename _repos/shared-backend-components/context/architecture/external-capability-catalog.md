...
The single in-code reader is `_repos/baltor/backend/src/baltor/runtime/registry/capability_registry.py`
...
- `check_no_direct_external_imports` — a provider SDK may only be imported inside an approved adapter path (`_repos/baltor/backend/src/baltor/adapters`, the LLM gateways).
