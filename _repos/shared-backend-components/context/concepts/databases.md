# Database representations

Every component in the catalog persists in four representative database
shapes. The canonical store is **PostgreSQL** (`db/postgres/schema.sql`);
the other backends derive from it.

| Backend | Best at | File |
|---|---|---|
| PostgreSQL / SQLite | "Find every harness in industry X with capability Y." | [`db/postgres/schema.sql`](https://github.com/Amarel-Taylor-Scott/openhubforai/blob/main/db/postgres/schema.sql) |
| MongoDB / Couchbase / Firestore | "Give me the full manifest of one component." | [`db/mongodb/collections.md`](https://github.com/Amarel-Taylor-Scott/openhubforai/blob/main/db/mongodb/collections.md) |
| Redis / DynamoDB | Runtime cache + ephemeral run state. | [`db/redis/keys.md`](https://github.com/Amarel-Taylor-Scott/openhubforai/blob/main/db/redis/keys.md) |
| Pinecone / Weaviate / Qdrant / Chroma / pgvector | Catalog semantic search + RAG retrieval. | [`db/vector/spec.md`](https://github.com/Amarel-Taylor-Scott/openhubforai/blob/main/db/vector/spec.md) |

## The row shape every backend agrees on

```sql
-- One row per component.
component(id, type, version, name, description, license, lifecycle,
         trust_boundary, freshness, created, updated, attribution,
         links, body /* full manifest as JSONB */)

-- Join tables for cross-cutting axes.
component_industry(component_id, industry)
component_capability(component_id, capability)
component_modality(component_id, modality)
component_tag(component_id, tag)

-- Edges between components (consumes / emits / step_ref / rubric / ...).
component_ref(src_id, dst_id, role)

-- Leaves inside rule packs and knowledge packs.
rule(rule_id, pack_id, family, severity, category, pattern, body, enabled)
knowledge_leaf(leaf_id, pack_id, leaf_type, industry, language, body)

-- Pipeline / benchmark / harness runs.
run(run_id, component_id, started_at, finished_at, status, adapter_id,
    inputs, outputs, trace, cost_usd, trust_boundary)
```

Most deployments use **two** backends: PostgreSQL + pgvector
(or PostgreSQL + Pinecone/Weaviate). Redis is a nice-to-have for
high-throughput hosted catalogs.
