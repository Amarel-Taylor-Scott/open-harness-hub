...
- **Retrieve**: `_repos/teleon/backend/src/teleon/synthesis/component_search.py` (label + search + compose), `_repos/teleon/backend/src/teleon/retrieval/hybrid.py`
  ...
  `_repos/teleon/backend/src/teleon/storage/record_store.py` (config / operational[SQLite·Postgres+pgvector] / history tiers),
  `_repos/teleon/backend/src/teleon/runtime/access_policy.py` (who may use what).
- **Compose**: `scripts/compile_capability_live.py` (`candidate_pool`, `intelligent_compile`, validate + repair retry,
  type-aware grounding), `_repos/teleon/backend/src/teleon/synthesis/intent_to_dag.py`, `_repos/teleon/backend/src/teleon/synthesis/synthesis_tree.py` (backtracking)
  + `strategist.py` (escape strategies).
- **Type contracts**: `architecture/plane_io_contracts.json` + `_repos/teleon/backend/src/teleon/synthesis/io_contracts.py` (`edge_compatible`).
- **Bound & verify**: `_repos/teleon/backend/src/teleon/synthesis/dag_contract.py` (`verify_buildable_dag` → `verified_working`).
- **Descend**: `_repos/teleon/backend/src/teleon/dag/pipeline_dag.py` (`DAG.choose` cheapest-viable per choice-point),
  `architecture/optimization_passes.json`.
- **Execute**: `_repos/teleon/backend/src/teleon/components/` (`Component.invoke`, `lower_to_dag`, `run_compiled`, `assert_conforms`),
