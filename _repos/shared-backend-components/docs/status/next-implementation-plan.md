...
Then P1:
- **OPP-consume-ui** (ui_pages, M, risk low) — _repos/baltor/frontend/consume.html ingestion→consumption page. Done when: /consume renders all panels
...
- **OPP-admin-monolith-split** (api_runtime, L, risk medium) — Split the admin server monolith into _repos/baltor/backend/src/baltor/api routes. Done when: ≥1 route extracted to _repos/baltor/backend/src/baltor/api/routes
