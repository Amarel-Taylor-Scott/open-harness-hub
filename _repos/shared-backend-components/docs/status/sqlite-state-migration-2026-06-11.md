...
1. `check_events_beacon_wiring.py` — `2 FAILURES: ['A: index.html loads events.js', 'A: events.js
   loads after data.js']`. `_repos/openhubforai/frontend/index.html` was rebuilt by the full-design transplant
   (commit `5b17af2d`); the page the check describes lives on as `legacy.html`. HEAD's
   `index.html` contains zero `events.js` references (`git show HEAD:_repos/openhubforai/frontend/index.html |
   grep -c events.js` → 0). Sections C/D of the same check — the ones that exercise the events
   SERVICE live over HTTP — PASS.
2. `check_openhubforai_auth_wiring.py` — `2 FAILURES: ['A: index.html loads identity.js',
...
Fixing these requires edits to `_repos/openhubforai/frontend/index.html` or to the check scripts — both outside
