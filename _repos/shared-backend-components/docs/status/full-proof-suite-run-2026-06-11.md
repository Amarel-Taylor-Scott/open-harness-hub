...
- **`check_identity_local_service_runtime.py`, `check_openhubforai_auth_wiring.py`,
...
|---|---|---|---|
| `check_openhubforai_auth_wiring` | FAIL (exit 1) | `A: index.html loads identity.js`; `A: identity.js loads before app.js` | **Pre-existing committed design-drift (active-edit `web/` zone).** |
...
`_repos/openhubforai/frontend/index.html` and `_repos/baltor/frontend/index.html` are **GENERATED by
...
`kit/oh-registry.js`, `openhub-live.js`, `oh-artifacts.jsx` — and the new SPA root scope. The 4 checks
...
- **Root cause:** the design-port regeneration **superseded** the old per-file script wiring. The old
  ...
  **already failing at the committed HEAD** (confirmed: `git show HEAD:_repos/openhubforai/frontend/index.html` does
...
- **Why not fixed here:** (1) These are not the *drifted hardcoded-value* class TASK 1 owns — they
  ...
  design-system contract decision (re-point the assertions to `kit/oh-identity.js`/`openhub-live.js` and the
...
- **Proposed fix (for the design-port owner):**
  - `scripts/check_openhubforai_auth_wiring.py:69-78` (block A) — the identity client moved into the kit;
  ...
  - `scripts/check_events_beacon_wiring.py:52-58` (block A) — events now flow through `openhub-live.js` /
  ...
  - `scripts/check_baltor_design_system.py:37` — assert the new `_repos/baltor/frontend/index.html` root scope class
...
`check_auth_kit_realm_isolation`, `check_openhubforai_auth_wiring`*, `check_service_handshake_slice`,
...
- `web/`-design wiring drift in `check_openhubforai_auth_wiring`, `check_events_beacon_wiring`,
