...
|---|---|---|---|---|---|---|---|
| AI Done Right (_repos/aidoneright/frontend) | 01 + live prototype | ✓ (single-page + tower) | ✓ | ✓ (kit byte-identical) | ✓ (kit toggle) | ✓ | ✓ |
| Demo Control Tower (served from parent origin) | 02 + live prototype | ✓ | ✓ | ✓ | n/a (dark surface) | ✓ | ✓ |
| Baltor.ai (_repos/baltor/frontend) | 03 + live prototype | ✓ 12/12 sampled (marketing + console + kit pages) | ✓ | ✓ | ✓ (kit toggle) | ✓ | ✓ |
| OpenHubForAI (_repos/openhubforai/frontend) | 13 (broken — see D3) + live prototype | ✓ 8/8 sampled | ✓ | ✓ | ✓ (`ohp-mode` override, as prototype) | ✓ | ✓ |
...
- **D3 — `screens/13-openhubforai.png` is a broken reference** (blank dark frame with only the
  theme switcher; capture-time failure). Parity was gated against the LIVE prototype
  (`:9210/openhubforai/…`) instead — structures match 1:1.
...
- **D6 — falsy same-origin override (found ONLY by the public gate):** `oh-identity.js#base()`
  used `window.OPENHUBFORAI_IDENTITY_BASE || DEFAULT` — the injected `''` (same-origin) is falsy, so auth
...
- **Public investor gate green**: `e2e/ohh_public_gate.mjs` 10/10 and the full route audit
  ...
  `_repos/openhubforai/context/status/openhubforai-investor-demo.md`.
...
- Video: `journey-4-teleon.mp4` (88s · 25 chapters · recorded through the tunnel); brief:
  `_repos/teleon/context/status/teleon-demo.md`.
