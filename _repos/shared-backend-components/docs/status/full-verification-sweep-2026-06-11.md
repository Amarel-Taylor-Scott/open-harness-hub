...
All in the inference/model plane (`_repos/teleon/backend/src/teleon/inference`, `oips.py`, `model_compatibility`),
...
- **`KeyError: 'node_id'` × 13**: `check_model_compatibility`, `check_inference_api`,
  ...
  **Root trace:** `_repos/teleon/backend/src/teleon/digestion/model_compatibility.py:32` does
...
3. **`check_oh_states_kit` — `A: index.html loads oh-states.js` (P1).** A shipped front-end
   ...
   as MVP).
4. **`check_openhubforai_auth_wiring` + `check_adversarial_auth_all_realms` (P1).** Front-end:
...
2. **"OIPS receipts carry is_truth:false"** — TRUE. `_repos/teleon/backend/src/teleon/inference/oips.py:184` →
   `"llm_output_is_truth": False`.
3. **"supervisor records DECISIONS not spawns"** — TRUE.
   `_repos/teleon/backend/src/teleon/workers/execution_dispatch.py:73` → `store.record_decision(...)`.
...
- Re-run the 19 inference-plane gates after the live edit settles; if `node_id` persists in a
  CLEAN subprocess, then `_repos/teleon/backend/src/teleon/digestion/model_compatibility.py:32` + `oips.load_graph`
...
- `check_openhubforai_auth_wiring` / `check_adversarial_auth_all_realms` — restore
