...
**Shape:** 2 core products (Baltor, Teleon) consume content from **21 OpenHubForAI registries** (8 live + 13 private-bench), inside the OpenHubForAI open store.
...
**Dependency law:** Baltor -> Teleon -> OpenHubForAI, NEVER reverse (enforced by check_portfolio_dependency_law). Content flows hub -> core (the core CONSUMES from hubs); a hub never depends on the core.
...
flowchart LR
  ...
  subgraph live["Live OpenHubForAI registries (8) — dual-use"]
    OpenContextHub
    OpenSkillsHub
    OpenToolsHub
    OpenSkillToTool
