...
5. Run the gates against the public domains: `node _repos/dev-rules-context/e2e/ohh_public_gate.mjs`,
   `node _repos/dev-rules-context/e2e/teleon_gate.mjs`, then re-film journeys if surfaces changed.
...
every call receipted. Lanes and the route policy: `_repos/teleon/context/architecture/flexible-inference-lanes.md`.
