...
- **Shared KIT** — one set of flow shapes, object shapes, and primitives lives in
  `_repos/openhubforai/backend/src/openhubforai/auth_kit/` (the open, bottom layer). Same UI components, same backend elements.
...
The kit is in **OpenHubForAI** because it's the open bottom layer: Baltor → Teleon → OpenHubForAI. So
...
**Also built (2026-06-09): the first wired product UI.** `_repos/openhubforai/frontend/identity.js` (realm client:
drift-gated port, `OPENHUBFORAI_IDENTITY_BASE` deploy override, request-id correlation, session-handle-only storage)
...
(never fakes a login). Proof: `scripts/check_openhubforai_auth_wiring.py` (static contract + `node --check`).
