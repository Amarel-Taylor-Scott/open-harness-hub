...
- Vocabulary is the canonical product vocabulary — **components / subcomponents**,
  **Knowledge Corpus**, **If Statement**, **Action** (see
...
- **Lift is pipeline-level.** No single retrieval or prompt component carries a
  ...
  value-prop 1. Most of these steps are deterministic and **freezable** (If Statements,
...
### R3 — Rerank / Fuse *(Action: rerank, and/or If Statement fusion policy)*
...
#### P5 — Injection defense *(If Statement / Stop-End; a guard before the call)*
...
| **Heuristic/classifier injection screen** (If Statement) | Blocks "ignore previous instructions" / prompt-extraction patterns | False positives; pattern upkeep |
...
- **R0** Self-RAG / Rewrite-Retrieve-Read or FLARE-style on-demand retrieval ·
  **R1** hybrid BM25 + dense · **R2** recursive/parent-child · **R3** RRF →
  ...
  If Statement that grades retrieval quality** (corrective gate: if retrieved
...
- **Why these locks:** the corrective If Statement + iterate loop are what make it
...
> upgrade BM25→SPLADE, add a corrective If Statement) without rewiring the flow —
...
THEN) or **Knowledge Corpus** (the fact store) or **If Statement** (the IF / gate).
...
|---|---|---|---|
| **R0** Query transform | **Action** (Transform / Model-Call) — or **If Statement** for self-query filter extraction | `processor`, `harness`; `rule-pack`/`logic-pack` for self-query | pre — companion to *Build the system prompt* (query-side); decomposition/Self-RAG also drive the *retry loop* |
...
| **R3** Fuse | **If Statement** (retrieval/fusion policy) | `rule-pack` (e.g. `hybrid-retrieval-policy`) | pre — governs how the *Knowledge retrieval* legs merge (RRF/CC/DBSF) before *Knowledge ranking* |
| **R4** Summarize / Compress | **Action: Transform** | `processor` (extractive); `harness` (abstractive small-model) | pre — *Knowledge summarizing* |
| **R5** Select · order · de-conflict | **Action: Transform** (+ an **If Statement** for source-precedence/recency policy) | `processor`; `rule-pack` for the precedence policy | pre — *Token reduction — format · prioritize · compress* |
...
| **P5** Injection defense | **Stop / End** guard (+ **If Statement** screen) | `rule-pack`/`logic-pack`; `processor` to sanitize | pre — *Prompt-injection check* (short-circuits the call) |
...
| Output verify/recover | **If Statement** | (deterministic built-in) | post — *Check output*, *Verify JSON (recover if malformed)* |
...
| Trigger gate (admit input) | **If Statement** (qualify) + **Stop / End** (disqualify) | `rule-pack`/`logic-pack` | gate — *Trigger gate*, *Pattern packs — qualify*, *Anti-pattern packs — disqualify* |
