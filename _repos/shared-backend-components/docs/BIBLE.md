...
| **AIDevObserver** | aidevobserver.io | watches AI **usage** — post-session review + intra-session coaching (renamed from *Teleon Observer* 2026-06-25) | `web/aidevobserver` (engine: `_repos/teleon/backend/src/teleon/observer`) | the WEDGE |
| **Baltor** | baltor.ai | managed, verified, provable **context**, powered by Teleon (a tenant) | `web/baltor` | TRUTH |
| **OpenHubForAI** | **OpenHubForAI.io** (ONE site) | the open registry / faceted catalog Teleon, Baltor, and AIDevObserver consume + the open CapabilityTask spec | `web/openhubforai` | the COMMONS |
...
- **Architectural dependency law** (enforced by `scripts/check_portfolio_dependency_law.py` over
  ...
  never import Baltor; OpenHubForAI imports neither. PurposeTask is **Teleon**, not a Baltor subsystem. AIDevObserver
  is a registry-consuming usage layer: it may consume the shared Backend registry substrate and OpenHubForAI artifacts,
  but its findings remain candidate-only and it must never create a direct path into Baltor.
- **Component flow** (diagram: `dist/sites/aidoneright-design/diagrams/component-flow.html`): components are published
  ...
  context. AIDevObserver watches the usage and searches the same registries for reuse, primitive/template matches,
  cheaper paths, and candidate refreshes. Consumption flows up; dependency points down.
...
**FLEXIBLE FACETS, not rigid stores (owner 2026-06-25):** the browsable unit is the **RECORD** (a primitive,
primitive template, task/pipeline template, tool, model, prompt, skill, harness, dataset…). The hubs + registries become
**facets** of one faceted catalog — browse/filter by category (the old
...
category, not an exclusive home. Backed by the existing federated search (`_repos/teleon/backend/src/teleon/registry/search.py`) + `RegistryPort`
(`_repos/teleon/backend/src/teleon/registry/port.py`) — do NOT rebuild the search; build the browse UI over it.
...
ontology; `component`/`primitive`-shaped rows hold reusable building blocks and templates; `model` holds models).
Each registry entry carries a **`maps_to_hub`** field, so MANY registries roll up
...
**pointer + shape**; RECORDS come from POPULATION engines (`_repos/teleon/backend/src/teleon/registry/populate.py`), never hand-building.
Records may be primitives, primitive templates, component cards, task/pipeline templates, adapters, tools, skills,
harnesses, rubrics, datasets, fixtures, conformance packs, source links, provenance rows, embeddings, or registry
metadata rows. The shared consumers are Teleon, Baltor, AIDevObserver, and OpenHubForAI itself; each consumes the same
substrate for a different job.
All behind one universal `RegistryPort` (`_repos/teleon/backend/src/teleon/registry/port.py`), across 5 layers (Discovery + Capability /
...
- **BYO keys** — demos + runtime accept the user's key via a TRANSIENT scope (`_repos/teleon/backend/src/teleon/demos/byo_key_demo.py`,
  `_repos/teleon/backend/src/teleon/runtime/key_holder.py`): used for one call, never stored, never logged, only a redacted status returned.
...
8. **Thin flexible wrappers** — base/universal objects (`ObjectShell`, `RegistryObject`) are a STABLE envelope
   ...
   inherit thin-ness via `_repos/teleon/backend/src/teleon/io/governed_record.mint_record` (envelope + payload; version in `schema_version`).
   ...
   `primitive_record.v0` in `_repos/teleon/backend/src/teleon/synthesis/pipeline_templates.py`, recorded 2026-07-01 — migrate its version
...
| Federated search across registries | `_repos/teleon/backend/src/teleon/registry/search.py` |
| Faceted browse (OpenHubForAI.io — records as the unit, hubs/registries as facets) | `_repos/teleon/backend/src/teleon/registry/browse.py` + `scripts/openhubforai_browse_server.py` |
| Registry menu / populate / compose | `_repos/teleon/backend/src/teleon/registry/{port,populate,compose}.py` |
| Run a capability descent | `_repos/teleon/backend/src/teleon/synthesis/` + `_repos/teleon/backend/src/teleon/economics/` |
| BYO-key demos (all surfaces) | `scripts/byo_demo_server.py` + `_repos/teleon/backend/src/teleon/demos/byo_key_demo.py` |
| Durable work queue + flywheel | `scripts/work_queue.py` + `_repos/teleon/backend/src/teleon/monitoring/flywheel_queue.py` |
| Git-backed records + branch promotion | `_repos/teleon/backend/src/teleon/storage/git_record_store.py` |
| Multi-location sync (CDC + leaf-shard) | `_repos/teleon/backend/src/teleon/storage/sync_engine.py` |
| Self-tuning (ledger + contrastive tuner) | `_repos/teleon/backend/src/teleon/tuning/` |
| Infra scale ports (Temporal/PG/CH/Vespa/Redpanda) | `_repos/teleon/backend/src/teleon/infra/scale_ports.py` |
...
> The consolidated state + operations master is **`docs/OPERATIONS-BIBLE.md`** (created 2026-07-02): current
> measured state, lane commands, diagnosis playbook, doc-consolidation map. This section stays as the pointer index.
...
- **Matching engine + path dynamics** — hybrid search + fit classes live in `_repos/teleon/backend/src/teleon/registry/primitive_match.py`
  (contract-locked by `scripts/check_primitive_hybrid_search.py`); learned pair affinity in
  `_repos/teleon/backend/src/teleon/registry/composition_affinity.py` (boost-only law). Path portfolios (generation/search/planning/
