...
**Grounding:** every value below is extracted from the **real shipped CSS** in `_repos/baltor/frontend/`
...
parses `_repos/baltor/frontend/*` `<style>` blocks for `--token:` defs and lists every token with >1 distinct
...
`_repos/baltor/frontend/tokens.css` imported everywhere, then flip the reporter to `--strict` in CI.
