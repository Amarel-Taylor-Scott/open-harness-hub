...
Everything passes through the safety gate in `docs/sales/public-claim-and-engagement-policy.md` (single sources: `architecture/sales_public_claim_policy.json`, `architecture/sales_engagement_policy.json`; guard `_repos/baltor/backend/src/baltor/sales/claim_guard.py`, redteam `scripts/check_sales_guardrails.py`).
