...
- **No paying customer.** All three cite the pre-revenue state (`_repos/_shared/strategy/design-partner-pilot-program.md`:
...
- **Scaffolding + unwired backends are real, not stylistic.** Both external models independently cited the SAME
  files: empty `_repos/baltor/backend/src/baltor/{processors,llm_gateway,security}/*/__init__.py`, the `CONTRACT STUB` supermemory
  adapters, the `RE-EXPORT SHIM` workers/purpose_tasks, and `_UnwiredCloudStore` (`_repos/teleon/backend/src/teleon/storage/record_store.py`)
...
- **The cost-descent margin thesis has no per-run cost RECEIPT wired into the runtime** - all three discuss it
  abstractly, but none flag that `_repos/teleon/backend/src/teleon/economics/vertical_proof.py` computes savings from fixture constants,
