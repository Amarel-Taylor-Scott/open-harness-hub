...
> **Update 2026-07-03 (owner reconcile — CLAUDE.md is the authority; the rules below are unchanged):** the canonical
> BUILT-OUT, shipped surfaces are the **full apps in `web/{context-is-everything,teleon,baltor,openhubforai,aidevobserver}`
> served by the showcase** (`OH_PRODUCT=<brand> python3 -m scripts.showcase`) over the shared kit, wired to the
> service-plane through same-origin seams. **`scripts/surface_server.py` is FALLBACK-ONLY, NOT canonical** — a
> config-driven static renderer to lean on only when the built-out app cannot run; a generator must NEVER serve it as a
> replacement for the rich app, and NEVER build a new basic/skinny replacement server (the deleted `web_app_server.py`
> was exactly this mistake). The `web/` React apps and the `dist/sites/aidoneright-design/` bundle are the protected,
> richer references (rule 1 applies to them). Design reference: **`docs/DESIGN-BIBLE.md`**. `serves_truth = false`.
...
`scripts/codegraph.py --audit <area>`, `scripts/check_substrate_layers.py`. Search `_repos/teleon/backend/src/teleon/**` for an existing
...
`_repos/teleon/backend/src/teleon/retrieval/search_port.py` + `synthesis/component_search.py` + `registry/port.py`.)
