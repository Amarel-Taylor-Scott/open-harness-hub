...
## Factory State Snapshot (2026-07-02 — supersede in place; every number cites its computed source)

Verified primitive candidates (all candidate=true / serves_truth=false; recompute via
`report_multilane_primitive_generation.py`):

```text
grand total verified: 30,524  (sum over data/dev-intel/primitive_factory/verified_candidates/*/manifest.json)
2026-07-02 day total: 1,516 across 770 distinct areas, 0 saturated
  (data/dev-intel/primitive_factory/multilane_generation_report_2026-07-02.md +
   data/dev-intel/primitive_saturation/2026-07-02_saturation.md)
  - 2026-07-02-uc02 (Fable ultracode wave, incl. new s17 rendering lane): 702 verified / 0 dup / 0 rejected
  - 2026-07-02-uc03 (Fable ultracode second wave, 21 briefs, deeper member decomposition): 811 / 0 / 0
  - 2026-07-02-slowtier-glm (Ollama GLM, 3-card slow-tier recipe): 3 verified (169 raw rejects = json truncation)
  - 2026-07-02-slowtier-kimi: 0 (440 raw rejects = json truncation at max_tokens 4000)
2026-07-01 reference day (fast Ollama tier): 14,307 verified in the single 20k-fleet label
```

Working flows proven today (receipts in the named paths):

```text
Fable ultracode lanes: briefs -> batch_runs/uc0N/<shard>/extracted/extracted_candidates.jsonl ->
  run_primitive_verification_loop.py --source-root ... --max-ticks 1  (0-rejection schema fidelity both waves)
Ollama lanes: .env key swap applies PER CALL (no restarts; _llm_client._env falls back to .env);
  circuit breaker OLLAMA_PAUSE.json (session-cap 429 -> 1h pause; concurrency 429 -> 3 min);
  GLM lane timeout 480s (was 300 — healthy max measured 296s).
Gemma lane: GEMMA_PAUSE.json honored per call by RUNNING loops; SEVERE cross-process rate limiter
  (fcntl, max concurrency 1, owner-tunable GEMMA_RATE_LIMIT.json) + Cloudflare origin-5xx session-down
  breaker — in scripts/_llm_client.py, self-tested; new lanes pick it up on next process start.
```

Model-lane throughput (measured 2026-07-02, single clean stream, scripts/_llm_client.py chat probe):

```text
ollama glm-5.2:        4.3 tokens/sec   (959 completion tokens / 224.6s)
ollama kimi-k2.7-code: 6.5 tokens/sec   (843 / 129.0s)
=> current key tier is ~10-20x slower than the 2026-07-01 tier (median full calls 192s then; impossible now).
   Slow-tier lane recipe until the plan is upgraded: 2-3 candidate cards per call, max_tokens >= 6000,
   timeout >= 900s GLM / 700s Kimi, workers 1 per model (account concurrency cap is small — 429
   "too many concurrent requests" observed at ~4-6 in flight).
```

Efficacy / token-reduction (measured-only sources — never restate these numbers without re-running):

```text
context reduction (edge cards vs full source): scripts/primitive_lift_benchmark.py receipts,
  surfaced by track_primitive_saturation_and_savings.py (the 486x figure lives THERE, not here)
matching engine: src/teleon/registry/primitive_match.py (blocking lanes + FIT_EXACT/DETERMINISTIC_EDIT/
  NONDETERMINISTIC_EDIT classes) + composition_affinity.py (boost-only pair affinity)
path portfolios (10 generation / 15 search / 10 planning / 9 compilation / 18 proof paths etc.):
  catalog/knowledge-packs/data/compiled-primitive-route-benchmark-seeds/manifest.json (row_counts)
```

Known failure modes captured today (negative memory — do not re-derive):

```text
Ollama zero-row day = quota/credits/tier event FIRST (429 "session usage limit" -> held requests ->
  full-timeout burns logged as TimeoutError; later 402 NO CREDITS). Probe tokens/sec before debugging code.
Claude wave limits: Fable 5 usage + 5h session limits kill mid-wave agents; ALWAYS relaunch via
  Workflow resumeFromRunId (completed agents replay from cache free) with reduced effort on retries.
json_decode_error rejects on slow tiers = output truncation (max_tokens < cards x tokens/card), not model failure.
```
...
Recent seed packs and handoffs already exist. Reuse them instead of inventing
parallel schemas:
...
The generated seed pack for compiled primitive benchmarks now exists:
