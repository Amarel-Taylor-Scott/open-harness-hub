...
registry cards AIDevObserver actually searches: 108,762 after the verified→registry bridge (was 79,026)
  = edge_foundry live + implemented_code (2,000) + verified-factory bridge (29,736)
  recompute: load_edge_foundry_primitive_count() ; bridge subset = prim:vf:* count
raw pre-primitive corpus (owner-provided fuel): generated_primitive_packs/ — 13 dimension-driven MD table packs
  (api-contract, browser, cloud-guardrail, data-science, devops-iac, kaggle-adapters, marketplace, observability,
  package-api, public-company, rag, search-method, search-query) + million-row ZIPs (variation pack, naics 300k,
  atlas 1m+ seed, 99pct dev bundle). Format = structured tables, ready for table->row or LLM-interrogation lanes.
```
...
**THE BRIDGE — verified rows are NOT usable until loaded into the searchable registry (2026-07-02 fix).** Generation
+ verification only stage rows into the `verified_candidates/` silo; AIDevObserver's `registry_search` reads a fixed
file list, so verified rows were invisible to the product for days. Close it every run:
`PYTHONPATH=. python3 scripts/load_verified_candidates_into_registry.py --write` maps ALL verified candidates →
edge-foundry card schema at `data/dev-intel/aidevobserver_edge_foundry/verified_factory_primitive_cards.jsonl`,
registered in `src/teleon/observer/registry_search.py::_edge_foundry_paths()` (settings field
`verified_factory_cards_path`). Gate: `scripts/check_verified_candidates_registry_load.py --self-test` (registered
in the proof suite). Proof it worked: `load_edge_foundry_primitive_count()` rises (79,026 → 108,762 on the first
bridge run — the 29,736 mapped cards, deduped from 30,524, deterministic ids, no local-path leak,
`surface_visibility=private_internal_only`). Full generation→usable pipeline + the raw-corpus prompt-queue path +
the verification tool suite (deterministic/skill/contract/governance): `docs/handoff/primitive-generation-verification-and-upload-manual.md`.
...
## 6b. Parallel paths + path tracking (unlimited paths per step)

Every pipeline step (scrape/build/test/benchmark/search/verify/compile/deploy/repair) runs a PORTFOLIO of paths, not
one hardcoded choice. Spine (reuse, do not rebuild): engine `src.teleon.experiments.parallel_paths.run_parallel`
(modes baseline/candidate/shadow/canary/fallback) + promotion gate `src.teleon.experiments.path_promotion` + tracking
ledger `src.teleon.evolution.descent_attempt_store.DescentAttempt` (append-only; losers kept as negatives; baseline =
rollback_target). Machine-readable catalog of paths-per-step: `catalog/knowledge-packs/data/step-path-portfolios/`
(9 steps, 52 tracked paths; builder `scripts/build_step_path_portfolio_pack.py`, gate
`scripts/check_step_path_portfolio_pack.py`). Adding a path = one row + a Runner branch; the engine already runs any
number of candidates. Already wired: hub_freshness (scraper descent), search_enrich, document_extraction_cascade.
Full recipe to wire a scraper/builder/tester/benchmark + emit receipts:
`docs/handoff/parallel-paths-and-path-tracking-manual.md`. The full flexible multi-path PHILOSOPHY (portfolios at
every step; the black-box contract + nesting-doll; the columnar/dimensional/edge-type architecture for matching
without reading; deterministic + non-deterministic + tool-assisted mutation; globally-unique names for deterministic
graphing): `docs/codex/flexible-multi-path-primitive-architecture.md`.
...
single source; superseded scale docs (`million-object-goal.md`, `billion-component-goal.md`) once the northstar-docs build slice
lands their unified replacement (`primitive-throughput-scale-plan.md` has already been folded into
`primitive-factory-20k-daily-operating-plan.md` and archived under `archive/legacy/`).
