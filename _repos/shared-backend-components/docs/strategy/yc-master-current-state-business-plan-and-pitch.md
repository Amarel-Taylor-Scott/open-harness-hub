...
> **Canonical inputs** (read these for depth): `_repos/aidoneright/context/strategy/brand-architecture.md` (identity, LOCKED) ·
> `_repos/baltor/context/strategy/baltor-cloud-cost-pricing-pro-forma.md` (cost + pricing + pro forma, authoritative) ·
> `_repos/fundraising/context/baltor-gtm-fundraising-plan.md` (GTM) · `_repos/_shared/strategy/context-layer-pmf.md` (TAM/PMF) ·
> `archive/legacy/_repos/_shared/strategy/competitive-landscape-2026.md` + `competitive-positioning-deep-dive.md` (competition) ·
...
> `_repos/_shared/baltor-north-star-continuous-builder.md` (the engineering North Star).
...
**Deterministic proof count (computed, recompute with `python3 -c "from scripts.flywheel_proof_modules import PROOF_MODULES; print(len(PROOF_MODULES))"`):** **⟦computed: 553⟧ green** as of 2026-06-20. A stdlib-only Python 3.14 watchdog re-runs all ⟦computed: 553⟧ every ~10 min; any regression is caught within one tick.
...
`_repos/baltor/backend/src/baltor/ports/execution_provider.py` defines one `ExecutionProviderPort`. A `CapabilityTask` is submitted
...
> Source of truth: `_repos/baltor/context/strategy/baltor-cloud-cost-pricing-pro-forma.md`. Refresh competitor/pricing links
...
13. **Traction / milestones** — *be honest:* a working governed engine (⟦computed: 553⟧ deterministic proofs, full offline
...
work (⟦computed: 553⟧ green proofs as of 2026-06-20), and the capability catalog. Pricing, raise size, TAM math, and
