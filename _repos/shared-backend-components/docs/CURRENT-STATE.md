...
> **⚠ 2026-06-09 SNAPSHOT (the name "Current State" is stale).** This predates the 07-08/07-09 executed
> token-savings experiment workstream. For the current handoff read `HANDOFF-GPT-5.6.md`.
...
- **Open ecosystem:** the current design-family proof covers 21 OpenHubForAI registry prototype surfaces: 8 live open hubs
  plus 13 private-bench hubs, including the full Baltor method spine
  Reconcile → Harden → Enhance → Optimize → Verify.

**Dependency law (enforced):** Baltor → Teleon → OpenHubForAI/hubs, **never reverse**. None of the open hubs is
...
- **Generated status reports** (computed, not hand-written): `_repos/baltor/context/status/baltor-current-state-and-opportunities.md`
...
- **Shared I/O + Resource Spine** (`architecture/shared_io_spine.json`): 6 layers ratified + proven against their
  ...
  (incl. the boundary-expansion **HumanApprovalReceipt** gate). Code in `_repos/teleon/backend/src/teleon/{io,resources,inference,
  templates}` + `schemas/{shared,resources,io,inference,governance}`.
- **Inference Gateway + OIPS** (`_repos/teleon/backend/src/teleon/inference`): object-level model preferences → numeric provider graph →
  ...
  `_repos/baltor/frontend/inference-plane.html`, and a governed inference step fires inside `run_full_pipeline` (rendered on the
  Live Ops dashboard as candidate-not-truth). Adversarially proven by `scripts/check_inference_pipeline_redteam.py`.
- **Free/Limited LLM Endpoint Intelligence** (`_repos/teleon/backend/src/teleon/inference/free_endpoint_intel.py`): ClawLess=runtime not
...
- **Sales safety gate** (`_repos/baltor/backend/src/baltor/sales/claim_guard.py`) + a configurable Chatbot Guardrail Audit
  (`_repos/baltor/backend/src/baltor/sales/diagnostics/`) — appears-not-illegal, private-first, authorized-inputs-only, draft-only outreach.
...
- The sales **lead-funnel target maps + any outreach** + **real named-company seeds** + **live third-party
  ...
  `_repos/aidoneright/context/portfolio-sales-lead-funnel-and-problem-proof-tools.md` and `..._competitive-intelligence...md`.
- Owner-decision items: HoldCo name confirmation; domain/trademark clearance for the new `.io` hubs (marked
  proposed/unverified); the portfolio app/console identity model (see `_repos/aidoneright/context/portfolio-app-console-build-brief.md`).
...
```
It feeds `_repos/_shared/baltor-north-star-continuous-builder.md` each cycle, resumes from
