...
> The executable spec for the OpenHubForAI surface (the open registry storefront Teleon, Baltor, and AIDevObserver consume). Pair with `_repos/shared-backend-components/docs/DESIGN-BIBLE.md`
> (the kit), `_repos/_shared/design/aidoneright-claude-design/DESIGN-ASSETS.md` (the source), `_repos/shared-backend-components/docs/INTEGRATION-BIBLE.md` (the seams), and `_repos/_shared/design/aidoneright-claude-design/ARCHITECTURE-AND-PAGES.md`
...
A basic faceted browse already exists at `scripts/openhubforai_browse_server.py` (older, dark, superseded); rebuild it rich
...
Backed by `_repos/shared-backend-components/scripts/registry_api_server.py` and `_repos/teleon/backend/src/teleon/registry/browse.py`. See `_repos/shared-backend-components/docs/INTEGRATION-BIBLE.md` for the seam.
