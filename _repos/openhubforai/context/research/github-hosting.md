...
2. **Companion repo**: mirror `dist/agent-skills/` to a sibling repo
   (e.g. `openhubforai-skills`) on every push. Cleaner separation;
...
1. **Day 1**: create the repo at `github.com/Amarel-Taylor-Scott/openhubforai`,
...
2. **Day 1**: CI runs `validate.yml`, `pages.yml`, `emit.yml`. Pages
   deploy lands at `https://amarel-taylor-scott.github.io/openhubforai/`.
...
4. **Week 1**: tweet / post the Claude Code plugin marketplace command:
   `/plugin marketplace add Amarel-Taylor-Scott/openhubforai@dist-published`.
...
## Host-agnostic deploy targets

*(Merged from the former `howto/deploy.md`.)* The site is **host-agnostic**: the same `mkdocs build` output
deploys unchanged to any of the following.

- **GitHub Pages** — `.github/workflows/pages.yml` runs `mkdocs build` and publishes `site/` on every push to
  `main`. Set the repo's Pages source to "GitHub Actions" once, then every push to `main` deploys.
- **Hugging Face Spaces** — two flavors: **Static** (push the `site/` build to a Space with `sdk: static`;
  catalog is read-only; set up via `hf-space/README.md`) and **Gradio** (the same Space directory holds
  `app.py` exposing the live playground with `sdk: gradio`).
- **Vercel** — `vercel.json` runs `mkdocs build` and serves `site/`. Connect the repo; defaults work.
- **Netlify** — `netlify.toml` runs `mkdocs build` and serves `site/`. Connect the repo; defaults work.
- **Cloudflare Pages** — build command `mkdocs build`, publish directory `site/`; the `_headers` and
  `_redirects` files (if present) are respected automatically.
- **Other Docker hosts** — the Gradio playground runs in any Docker host; `hf-space/Dockerfile` is portable,
  build and push it anywhere.
