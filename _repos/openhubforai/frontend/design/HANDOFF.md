...
Run orchestrator (single-call core + map-over-list batch) · priority job queue (paid>trial, per-tenant concurrency = anti-scrape lever, failed-message quarantine queues, retries, circuit-breakers) · sandboxed worker fleet + internet workers · CDC pipeline · measurement engine (decay) · cache · pgvector · signing/key infra (C2PA) + canary registry · metering→billing (Stripe) + credits ledger · secrets vault · tenant isolation · SOC2/GDPR.
