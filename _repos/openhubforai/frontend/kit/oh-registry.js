/* shared/oh-registry.js — realm-aware REGISTRY client for the full-design kit.
   ...
   - Base: window.OPENHUBFORAI_REGISTRY_BASE override, else the local default (matches the service registry).
...
(function () {
  ...
  function base() { return String(window.OPENHUBFORAI_REGISTRY_BASE || DEFAULT_BASE).replace(/\/+$/, ""); }
  ...
  window.OHRegistry = {
    ...
    search: function (realm, q) {
      return call("GET", "/api/openhubforai/" + realm + "/search" + (q ? "?q=" + encodeURIComponent(q) : ""))
        .then(function (r) { return (r.body && r.body.entries) || null; }, function () { return null; });
    },
    entry: function (realm, id) {
      return call("GET", "/api/openhubforai/" + realm + "/entry/" + encodeURIComponent(id))
    ...
    install: function (realm, entry) {
      return call("POST", "/api/openhubforai/" + realm + "/install", { session_id: sid(realm), entry: entry })
        .then(function (r) { return { ok: r.status === 202, body: r.body }; }, function () { return { ok: false }; });
    },
    uninstall: function (realm, entryId) {
      return call("POST", "/api/openhubforai/" + realm + "/uninstall", { session_id: sid(realm), entry_id: entryId })
        .then(function (r) { return { ok: r.status === 202, body: r.body }; }, function () { return { ok: false }; });
    },
    publish: function (realm, entry) {
      return call("POST", "/api/openhubforai/" + realm + "/submit", { session_id: sid(realm), entry: entry })
        .then(function (r) { return { ok: r.status === 202, body: r.body }; }, function () { return { ok: false }; });
    },
    ...
    submissions: function (realm) {
      ...
      return call("GET", "/api/openhubforai/" + realm + "/submissions?session_id=" + encodeURIComponent(s))
    ...
    amReviewer: function (realm) {
      ...
      return call("GET", "/api/openhubforai/" + realm + "/review/status?session_id=" + encodeURIComponent(s))
        .then(function (r) { return !!(r.status === 200 && r.body && r.body.reviewer); }, function () { return false; });
    },
    reviewQueue: function (realm) {
      var s = sid(realm);
      if (!s) return Promise.resolve(null);
      return call("GET", "/api/openhubforai/" + realm + "/review/queue?session_id=" + encodeURIComponent(s))
    ...
    decide: function (realm, entryId, decision, reason, score) {
      return call("POST", "/api/openhubforai/" + realm + "/review/decide",
    ...
    amAdmin: function (realm) {
      var s = sid(realm);
      if (!s) return Promise.resolve(false);
      return call("GET", "/api/openhubforai/" + realm + "/admin/status?session_id=" + encodeURIComponent(s))
    ...
    audit: function (realm) {
      var s = sid(realm);
      if (!s) return Promise.resolve(null);
      return call("GET", "/api/openhubforai/" + realm + "/audit?session_id=" + encodeURIComponent(s))
    ...
    workspace: function (realm) {
      var s = sid(realm);
      if (!s) return Promise.resolve(null);
      return call("GET", "/api/openhubforai/" + realm + "/workspace?session_id=" + encodeURIComponent(s))
