...
const OPENHUBFORAI_BRAND = { name: 'OpenHubForAI', realm: 'openhubforai', tld: '.io', glyph: '⎔', accent: 'var(--accent)' };
const OPENHUBFORAI_GROUPS = NAV_GROUPS.map(([label, items]) => ({ label, items: items.map(([lb, p, ic]) => [p, ic, lb]), defaultOpen: ['Explore', 'Workspace'].includes(label) }));
...
function AppShell({ route, crumb, children, bare }) {
  ...
  return (
    <OhAppShell brand={OPENHUBFORAI_BRAND} groups={OPENHUBFORAI_GROUPS} isActive={ohhActive(route)} route={route}
...
function MarketingShell({ children }) {
  ...
  return (
    <div className="pt-mkt">
      <OhTopBar brand={OPENHUBFORAI_BRAND}
      ...
      <div className="pt-mkt-body">
        {children}
        <OhFooter brand={OPENHUBFORAI_BRAND} tagline="Open harness ecosystem · part of AI Done Right" cols={[
          ...
          ['Group', [['AI Done Right ↗', '../context-is-everything/Context is Everything.html'], ['Baltor.ai ↗', '../context-enrichment/Context Enrichment.html'], ['Teleon.dev ↗', '../teleon/Teleon.html']]],
        ]} />
      </div>
    </div>
  );
...
function App() {
  ...
  React.useEffect(() => {  // live seam: real realm session ⇒ app mode (reflects real state only)
    const sync = () => {
      try {
        const s = window.OHIdentity && OHIdentity.session(OHIdentity.realmOf(OPENHUBFORAI_BRAND));
  ...
  if (route === '/') body = <PLanding />;
  ...
  else if (route === '/about') body = <MarketingShell><window.OhAbout brand={OPENHUBFORAI_BRAND} /></MarketingShell>;
  else if (route === '/cases') body = <MarketingShell><window.OhCaseStudies brand={OPENHUBFORAI_BRAND} cases={(window.CASES && window.CASES.openhubforai) || []} /></MarketingShell>;
  else if (route.startsWith('/cases/')) body = <MarketingShell><window.OhCaseStudy brand={OPENHUBFORAI_BRAND} cases={(window.CASES && window.CASES.openhubforai) || []} id={route.slice(7)} cta={{ label: 'Start free →', href: '/signup' }} /></MarketingShell>;
    ...
    else { page = <window.OhNotFound brand={OPENHUBFORAI_BRAND} home="/" links={[['Explore', '/pipelines'], ['Build', '/build']]} />; crumb = 'Not found'; }
