/* OpenHubForAI — identity client (realm: openhubforai)
   ...
   - Base URL: window.OPENHUBFORAI_IDENTITY_BASE override (e.g. a TryCloudflare preview of port 9410), else the
     local default below. The default port must match architecture/identity_realm_registry.json
     defaults.port — drift-gated by scripts/check_openhubforai_auth_wiring.py.
...
(function () {
  ...
  // window.OPENHUBFORAI_IDENTITY_REALM); defaults to this product's own realm. Backend enforces realm
  // isolation regardless — no cross-realm session, no SSO.
  var DEFAULT_REALM = "openhubforai";
  var REALM = (function () {
    try { return new URLSearchParams(location.search).get("realm") || window.OPENHUBFORAI_IDENTITY_REALM || DEFAULT_REALM; }
    catch (e) { return DEFAULT_REALM; }
  })();
  var DEFAULT_BASE = "http://127.0.0.1:9410";
  var SESSION_KEY = "ohh-identity-session";

  function base() { return String(window.OPENHUBFORAI_IDENTITY_BASE || DEFAULT_BASE).replace(/\/+$/, ""); }
