...
lifecycle, parameterized per hub by a ``HubSpec`` (single-sourced from _repos/shared-backend-components/architecture/portfolio_connection_map.json):
...
hub never imports them (dependency law: OpenHubForAI imports neither Baltor nor Teleon; they read this feed).
...
_CONN = _resource("architecture") / "portfolio_connection_map.json"
