...
Single-sourced: method primitives come from _repos/shared-backend-components/architecture/descent_method_catalog.json (filtered by axis/keyword).
Generators that read Teleon/Baltor internals (e.g. ``descent_brain`` over the descent_attempt_store) are NOT imported
here — the dependency law forbids OpenHubForAI importing Teleon — they are INJECTED by the operator layer via
...
strategy (_repos/shared-backend-components/architecture/hub_population_strategy.json) names each hub's generator + optional generator_args.
...
_METHOD_CATALOG = _resource("architecture") / "descent_method_catalog.json"
_STRATEGY = _resource("architecture") / "hub_population_strategy.json"
...
#: OHH-NATIVE generators (law-clean: read only _repos/shared-backend-components/architecture/ JSON). Operator-layer generators (descent_brain, run
