# CLAUDE.md — `aidoneright-api-endpoint-wrappers` agent operating manual
...
- **Name:** `aidoneright-api-endpoint-wrappers`
- **Kind:** DEV TOOL (reusable infrastructure, product-neutral).
- **North star (one line):** Wrap every third-party / local / cloud API endpoint behind a stable,
  cost-ordered PORT so every product calls a **capability**, never a hardcoded host.
- **What this repo exposes (its published interface):** `port-adapters`, `provider-registry`,
  `cost-gates`, `retry/backoff`, `credential-port`. Keep these stable — the surfaces below depend on them.
- **Depth before breadth:** the ONE thing proven here is the provider-agnostic adapter port. Serve the
  surfaces that consume it — `teleon`, `baltor`, `aidevobserver`, `shared-backend-components`, `scraping`,
  `context-injection` — before adding a provider no consumer has asked for. Breadth of adapters without a
  consuming surface is the failure mode.
...
- **Who consumes you (keep these interfaces stable):** `aidevobserver`, `baltor`, `context-injection`,
  `scraping`, `shared-backend-components`, `teleon`.
...
- **Reuse-first.** Before building any adapter, port, or provider route, check it does not already exist in
  `../dev-rules-context/_shared/`, in this repo's `context/`, or in `shared-backend-components`. *"This
  already exists, don't rebuild it"* is the highest-ROI decision here.
- **Audit neighbors before and after a code change.** A green suite says the code runs; the dependency
  graph says what else the change can break. Six surfaces consume this repo — an adapter-port signature
  change is high blast radius.

## Adding work

- **A new provider adapter** = a new port implementation behind the existing port, registered in the
  `provider-registry`, cost-ordered (deterministic-first, then cheapest capable route), stamped
  `candidate = true` (§3) until proven. Never widen the published `exposes` without a warrant (§4).
- **A new decision point** (which provider, which retry policy) = a portfolio behind one selector (§1),
  not an `if`.
...
- **No real PII, secrets, confidential data, or proprietary dumps.** Synthetic or public metadata only.
  The `credential-port` handles **environment-variable names only, never secret values** — no key is ever
  read into a record, logged, or committed.
...
- **Insurance pipelines are restricted** — do not add or expand insurance-related adapters.

## When stuck

If one path is blocked, switch paths — do not stop. Add proofs, readiness/audit tooling, a new
deterministic-first adapter, cost-gate coverage, or documentation that prevents a repeated slow or wrong
path. Do not stop because one provider or full rebuild is slow. Every serious turn improves at least one
durable thing.
