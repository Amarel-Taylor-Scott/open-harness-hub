...
| [`parse_csv_rows`](parse_csv_rows/) | Parse CSV text into JSON-compatible row dictionaries |
...
The corresponding `_repos/shared-backend-components/catalog/processors/.../<slug>.yaml` manifest
...
python _repos/shared-backend-components/scripts/new.py processor my-template
$EDITOR _repos/shared-backend-components/catalog/processors/.../my-template.yaml
# (add: implementations: [{kind: callable, path: code_templates.my_template:run, language: python}])

# 6. Validate + run tests
python _repos/shared-backend-components/scripts/validate.py
