# AIDONERIGHT-UNIVERSE — how `aidevobserver` interfaces with the rest of the AI Done Right org
...
## This repo — `aidevobserver` (`aidoneright-aidevobserver`)
- **Layer:** product · **Kind:** product_usage_layer
- **Role:** the AI-USAGE layer — session/AI-usage coaching (not code review); MCP + CLI + extension + app + local service.
- **How others interface with you:** Call its HTTP seam same-origin (`/api/aidevobserver/…`); cloud base `OH_SEAM_AIDEVOBSERVER_BASE`, a local port in dev. Seam: `/api/observer/`.
...
|---|---|---|---|---|
| `aidevobserver` **← you** | aidoneright-aidevobserver | product | seam `/api/observer/` | the AI-USAGE layer — session/AI-usage coaching (not code review); MCP + CLI + extension + app + local service. |
