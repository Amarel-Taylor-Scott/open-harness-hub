# AIDONERIGHT-UNIVERSE — how `api-endpoint-wrappers` interfaces with the rest of the AI Done Right org
...
## This repo — `api-endpoint-wrappers` (`aidoneright-api-endpoint-wrappers`)
- **Layer:** library · **Kind:** dev_tool
- **Role:** DEV TOOL — reusable adapters that wrap third-party / local / cloud API endpoints behind stable PORTS (agnostic-adapter pattern), so every product calls a capability, never a hardcoded host. Cost-ordered, deterministic-first.
...
| `api-endpoint-wrappers` **← you** | aidoneright-api-endpoint-wrappers | library | port | DEV TOOL — reusable adapters that wrap third-party / local / cloud API endpoints behind stable PORTS (agnostic-adapter… |
