# AIDONERIGHT-UNIVERSE — how `yc-applications` interfaces with the rest of the AI Done Right org
...
## This repo — `yc-applications` (`aidoneright-yc-applications`)
- **Layer:** business · **Kind:** context
...
- **How others interface with you:** Read as grounding context/memory — never imported as code, never a running service.

**What you expose** (your published interface — keep it stable):
- `yc-application` — The Y Combinator / accelerator application draft: answers, founder story, video script.
- `yc-readiness` — YC-readiness scoring across the dimensions gate (computed by scripts/yc_readiness.py, not asserted).
...
### `aidoneright` (`aidoneright-parent`) — parent_brand
- **How to interface:** The portfolio site; it references other surfaces' PUBLIC interfaces for display/routing only.
- **Its role:** the PARENT BRAND / portfolio umbrella (aidoneright.dev). References every surface's PUBLIC interface for the portfolio site; owns no product logic.
- **You may use:**
...
| `yc-applications` **← you** | aidoneright-yc-applications | business | read | CONTEXT repo — Y Combinator (and other accelerator) application drafts, answers, founder/video scripts, readiness… |
