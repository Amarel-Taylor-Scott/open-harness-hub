# CLAUDE.md — `aidoneright-scraping` agent operating manual
...
- **Name:** `aidoneright-scraping`
- **Kind:** DEV TOOL (reusable infrastructure, product-neutral).
- **North star (one line):** A **consented, governed** fetch toolkit that climbs the descent ladder
  **API → browser → stealth → vision**, feeding intake and discovery with the least-intrusive method that
  works.
- **What this repo exposes (its published interface):** `fetch-ladder`, `browser-automation`,
  `extract-cascade`, `rate-governance`, `robots/consent-gate`. Keep these stable — the surfaces below
  depend on them.
- **Depth before breadth:** the ONE thing proven here is the governed fetch ladder that climbs only as far
  as it must. Serve the surfaces that consume it — `aidevobserver`, `baltor`,
  `shared-backend-components` — before adding a source class no consumer has asked for. Uncontrolled
  breadth of scrapers without a consuming surface (or without a consent gate) is the failure mode.
...
- **You may consume (via their published interface only):**
  ...
  - `api-endpoint-wrappers` (`aidoneright-api-endpoint-wrappers`) — port-adapters, provider-registry,
    cost-gates, retry/backoff, credential-port. (Reach every remote endpoint through these ports, never a
    hardcoded host.)
- **Who consumes you (keep these interfaces stable):** `aidevobserver`, `baltor`,
...
- **Reuse-first.** Before building any fetcher, extractor, or automation step, check it does not already
  exist in this repo's `context/`, behind an `api-endpoint-wrappers` port, or in
  `shared-backend-components`. *"This already exists, don't rebuild it"* is the highest-ROI decision.
- **Climb the ladder, do not skip it.** Try the cheapest, least-intrusive method first (API via a wrapper
  port), then browser, then stealth, then vision — and record which rung each source needed.
...
- **A new source or fetch method** = a new rung/strategy in the ladder behind the selector (§1), gated by
  `robots/consent-gate` and `rate-governance`, emitting rows born `candidate = true` (§3).
- **A new decision point** (which rung, which rate) = a portfolio behind one selector (§1), not an `if`.
- **A new generated id** = one call to the shared `canonical_id` (§2).

## Safety and scope

- **Consent + robots gate is mandatory, not optional.** Scrape only consented, public, and governed
  sources; honor robots and rate limits; never bypass a paywall, auth wall, or an explicit `disallow`.
- **No real PII, secrets, confidential data, or proprietary dumps.** Public metadata only. Do not scrape or
  store personal data. Do not republish `_reference/`.
...
- **Insurance pipelines are restricted** — do not add or expand insurance-related sources.

## When stuck

If one source, provider, or rung is blocked, **switch paths — do not stop**; climb the ladder (API →
browser → stealth → vision) or add readiness/audit tooling, source-surface seeds, or documentation that
prevents a repeated slow or wrong path. Every serious turn improves at least one durable thing.
