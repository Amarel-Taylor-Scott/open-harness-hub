...
> This is the missing piece. The other files give the design system (`_repos/shared-backend-components/docs/DESIGN-BIBLE.md`), the actual source
> (`_repos/_shared/design/aidoneright-claude-design/DESIGN-ASSETS.md`), and the backend contract (`_repos/shared-backend-components/docs/INTEGRATION-BIBLE.md`). This file tells you exactly WHAT to build
...
1. **Marketing home** (`/`): already exists at `_repos/aidevobserver/frontend/aidevobserver-main.jsx`; elevate it.
...
- **Render:** the **mode** selector (`silent_record`, `review_only`, `advisory`, `active`, `enforcing`), the
  **interruption budget** (a small integer, the max live interruptions per session), the **BYO key** field (governed
  ...
  _repos/shared-backend-components/scripts/aidevobserver_mcp_server.py`), the CLI (`python3 -m src.teleon.observer.cli review --latest`), and the
  live-coaching Claude Code PreToolUse hook (`python3 _repos/shared-backend-components/scripts/aidevobserver_hook.py --install`).
...
- **MCP server** — `claude mcp add aidevobserver -- python3 _repos/shared-backend-components/scripts/aidevobserver_mcp_server.py`.
...
- **PreToolUse hook** — live in-session coaching, non-blocking (`_repos/shared-backend-components/scripts/aidevobserver_hook.py --install`).
...
1. **Deep-link a finding to the live registry record.** A `reinvention` finding's `source_ref.existing` names a
   ...
   trustworthy.
2. **Show dollars and hours saved (the economics registry, `_repos/teleon/backend/src/teleon/economics`).** A reuse-vs-rebuild finding can
...
3. **The cheaper-path ladder (`_repos/shared-backend-components/architecture/capability_ladders.json`).** Each capability has a cost-ordered,
...
`AIDevObserver-app-STARTER.html` (in `_repos/shared-backend-components/docs/design/aidoneright-claude-design/`)** — it is the complete app ALREADY WORKING (the shared kit + all
...
2. **`INTEGRATION-HANDOFF.md`**: a README that explains how to merge your work into THIS codebase WITHOUT breaking it.
   ...
   - **The app lives at `_repos/aidevobserver/frontend/`** (React 18 rendered in-browser via Babel, no build step, the shared kit
     in `_repos/aidevobserver/frontend/kit/`). Served by the showcase (`OH_PRODUCT=aidevobserver python3 -m scripts.showcase`).
   ...
   - **The proof gate must stay green:** `PYTHONPATH=. python3 _repos/shared-backend-components/scripts/run_proofs.py`. Note which `check_*` proofs
