...
// ---------- shared hooks ----------
const OH_DEFAULT_ROUTES = { home: '/', dashboard: '/dashboard', settings: '/settings', billing: '/billing' };
...
function ohRoute(key) {
  const routes = window.OH_ROUTE_MAP || {};
  return routes[key] || OH_DEFAULT_ROUTES[key] || OH_DEFAULT_ROUTES.home;
}
...
// nav: [ [label, href], … ] or [ { label, href, strong? }, … ] · cta: { label, href }
function OhTopBar({ brand, nav, cta, signInHref, theme, onToggle, extraNav }) {
  const topNav = (nav || []).concat(extraNav || []);
  return (
    <header className="ohs-top">
      ...
      <nav className="ohs-top-nav">
        {topNav.map((item) => {
          ...
          const h = Array.isArray(item) ? item[1] : item.href;
          const strong = !Array.isArray(item) && item.strong;
          return <a key={l} style={strong ? { fontWeight: 600 } : undefined} onClick={() => navigate(h)}>{l}</a>;
        })}
      </nav>
          ...
          {foot || (
            <>
              ...
              <div className="ohs-side-foot-row">
                <a className="ohs-navlink" onClick={() => navigate(ohRoute('settings'))}>Settings</a>
                {onToggle && <OhThemeToggle theme={theme} onToggle={onToggle} />}
              </div>
            </>
          )}
...
function OhAuth({ brand, mode }) {
  ...
  async function submitAuth() {
    ...
    const enterPreview = () => { try { sessionStorage.setItem('oh-preview', '1'); } catch (e) {} navigate(ohRoute('dashboard')); };
    ...
    try {
      ...
      setBusy(false);
      if (r && r.ok) { try { sessionStorage.removeItem('oh-preview'); } catch (e) {} navigate(ohRoute('dashboard')); }  // real session — clear any stale preview flag
...
function OhSettings({ sections, eyebrow = 'Account', title = 'Settings', sub = 'Manage your profile, team and preferences.' }) {
  return (
    <div className="ohs-page">
        ...
        <div className="ohs-dash-side">
          {plan && (
            <div className="oh-card oh-card--pad ohs-planmini">
              ...
              <div className="ohs-meter-row"><span>{plan.usageLabel}</span><span className="mono">{plan.usagePct}%</span></div>
              <button className="oh-btn oh-btn--ghost oh-btn--sm" style={{ marginTop: 14, width: '100%', justifyContent: 'center' }} onClick={() => navigate(ohRoute('billing'))}>Manage plan</button>
            </div>
          )}
          <div className="oh-card oh-card--pad">
