# CLAUDE.md — `aidoneright-fundraising` agent operating manual
...
- **Name:** `aidoneright-fundraising`
- **Kind:** CONTEXT repo — **no product code**. The money/market plan, read by humans and agents for
  grounding, never imported as code.
- **North star (one line):** The money/market **plan** — go-to-market and fundraising strategy, investor
  outreach, round planning, and market sizing — kept separate from the deck itself and from product code.
- **What this repo exposes (its published interface):** `gtm-plan`, `fundraising-strategy`,
  `investor-outreach`, `market-sizing`. Keep these stable — a surface may reference them.
- **Depth before breadth:** the ONE thing proven here is a coherent plan tied to a single beachhead vertical
  and a sourced market size — every strategy claim carries a warrant. Serve that before broadening. A plan
  of unwarranted assertions and unsourced TAM is the failure mode.
...
- Add or update one plan / strategy / outreach / sizing note; validate only the changed paths; do not start
  with a full rebuild.
- **Reuse-first.** Before adding a plan element, check it is not already recorded — a contradicting
  duplicate is worse than a gap. *"This already exists, don't rebuild it"* applies to the plan too.
- When a new decision supersedes an old one, **update or archive the stale entry in the same change** (§4,
  §8) — no orphaned contradictions in the plan.

## Adding work

- **A new plan / strategy / outreach / sizing note** = added under `context/`, dated, with its source.
- **A strategy / pricing / round-shape decision** = carries its warrant (§4: owner-intent quote, ≥2
  corroborating sources, or a named principle). These are strategy — **never a unilateral single-agent
  call**; they need clear owner intent or strong corroboration.
- **A market-sizing number** = sourced and datable, built from the source (§6), never a bare assertion.
...
- **No confidential financials or real investor PII.** Public / synthetic only. Never commit a real cap
  table, term sheet, bank balance, burn/runway detail, or an investor's private contact/terms into a note.
...
If one source or number is blocked, switch paths — do not stop. Tighten the GTM motion, reconcile a stale
strategy claim, add a source for a TAM figure, or add documentation that prevents a repeated wrong path.
Every serious turn improves at least one durable piece of the plan.
