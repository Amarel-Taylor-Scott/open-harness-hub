/* Baltor — overview / landing (route "/"). Standalone Baltor app; see
   context/baltor/strategy/context-enrichment-service.md. Every link targets a WIRED route (no dead ends). */
(function () {
  "use strict";
  ...
  function render(ctx) {
    return '<div class="pt-mkt pt-view">' + ctx.header("/") +
      ...
      '<div style="margin-top:12px;display:flex;gap:8px;flex-wrap:wrap"><button class="oh-btn oh-btn--ghost oh-btn--sm" data-nav="/trust">See the trust policy →</button><a class="oh-btn oh-btn--ghost oh-btn--sm" href="https://openhubforai.com">Build governed workflows in OpenHubForAI →</a></div>' +
      "</div>" +
      "</div></div></div>";
  }
