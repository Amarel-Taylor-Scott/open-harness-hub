# AIDONERIGHT-UNIVERSE — how `baltor` interfaces with the rest of the AI Done Right org
...
## This repo — `baltor` (`aidoneright-baltor`)
- **Layer:** product · **Kind:** product_applied
...
- **How others interface with you:** Call its HTTP seam same-origin (`/api/baltor/…`); cloud base `OH_SEAM_BALTOR_BASE`, a local port in dev. Seam: `/api/baltor/`.
...
| `baltor` **← you** | aidoneright-baltor | product | seam `/api/baltor/` | the applied, customer-facing CONTEXT product, powered by Teleon (a TENANT). Governance / provenance / signed facts /… |
