...
a bounded backfill. It reads the durable SyncState (SyncState), emits an IngestionRun plan + a SyncCursor
(SyncCursor), and applies a cursor back onto state idempotently. It NEVER fetches and NEVER writes served
...
class SyncPlannerPort(Protocol):
    ...
    def plan_full_sync(self, *, tenant_id: str, source_id: str, scope: str, sync_state: dict[str, Any] | None,
        ...
        Returns an IngestionRun-shaped plan with sync_mode='full_sync'. `now` injected."""
        ...

    def plan_incremental_sync(self, *, tenant_id: str, source_id: str, scope: str,
                              sync_state: dict[str, Any], now: str) -> dict[str, Any]:
        """Plan a delta read from the current cursor in `sync_state`. Returns an IngestionRun-shaped plan
    ...
    def plan_backfill(self, *, tenant_id: str, source_id: str, scope: str, from_cursor: dict[str, Any],
        ...
        Returns an IngestionRun-shaped plan with sync_mode='backfill'. `now` injected."""
        ...

    def apply_cursor(self, *, sync_state: dict[str, Any], cursor: dict[str, Any], run_id: str,
                     now: str) -> dict[str, Any]:
        """Fold a completed run's SyncCursor into SyncState and return the new state. Idempotent: applying
