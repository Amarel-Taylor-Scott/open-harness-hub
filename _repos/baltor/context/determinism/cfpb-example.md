...
Engine: `_repos/baltor/backend/src/baltor/determinism/`. The existing reference authority it asserts-equivalence to:
`_repos/shared-backend-components/scripts/artifact_graph/reconciliation.py` (+ `conflict_detector.py`). Applied proofs:
`_repos/shared-backend-components/scripts/check_cfpb_reconciliation_rule_distillation.py` (against the real runtime) and
`_repos/shared-backend-components/scripts/check_determinism_full_stack.py` (against the engine, end-to-end).
...
- `_repos/shared-backend-components/scripts/check_cfpb_reconciliation_rule_distillation.py` — assert-equivalence to the REAL runtime reference.
- `_repos/shared-backend-components/scripts/check_determinism_full_stack.py` — the same slice driven through the ENGINE end-to-end (M0→M7).
- `_repos/shared-backend-components/scripts/check_determinism_redteam.py` — the "serve FAQ-30 as truth" attack FAILS SAFELY.
