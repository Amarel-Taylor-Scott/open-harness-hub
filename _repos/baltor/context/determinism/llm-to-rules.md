...
Engine: `_repos/baltor/backend/src/baltor/determinism/` (`pattern_miner` → `rule_candidate_generator` → `replay_engine` →
...
EXISTING Baltor reference it reproduces (e.g. `_repos/shared-backend-components/scripts/artifact_graph/reconciliation.py`). The replay engine
...
- `_repos/shared-backend-components/scripts/check_determinism_full_stack.py` — drives every stage above end-to-end and asserts both invariants.
- `_repos/shared-backend-components/scripts/check_determinism_pattern_miner.py` · `check_rule_candidate_generation.py` ·
...
PYTHONPATH=. python3 _repos/shared-backend-components/scripts/check_rule_promotion_gate.py --self-test
PYTHONPATH=. python3 _repos/shared-backend-components/scripts/check_deterministic_rule_fallback.py --self-test
```
