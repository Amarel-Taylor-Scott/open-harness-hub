...
Baltor reconciliation (`_repos/shared-backend-components/scripts/artifact_graph/reconciliation.py` + `conflict_detector.py`),
...
PART 1 — Contracts (_repos/shared-backend-components/schemas/determinism/*.v1.schema.json; register in _repos/shared-backend-components/architecture/contract_registry.json):
...
PART 2 — Trace store (_repos/baltor/backend/src/baltor/determinism/trace_store.py): append-only; tenant_private cannot train
global rules unless anonymized+approved; private prompts/responses → object store / private tenant store;
no secrets.

PART 3 — Consensus recorder (_repos/baltor/backend/src/baltor/determinism/consensus.py): record multi-model/multi-run outputs;
compute agreement score; record disagreement clusters; DOES NOT decide truth; emit ConsensusRun;
low agreement → human/review or deterministic validator. Proof: _repos/shared-backend-components/scripts/check_consensus_recorder.py
(multiple outputs recorded; agreement computed; disagreement does not promote truth; consensus alone
cannot serve a fact).

PART 4 — Pattern miner (_repos/baltor/backend/src/baltor/determinism/pattern_miner.py): find repeated non-deterministic
...
PatternCandidate. Proof: _repos/shared-backend-components/scripts/check_determinism_pattern_miner.py.

PART 5 — Rule candidate generator (_repos/baltor/backend/src/baltor/determinism/rule_candidate_generator.py): rule types —
...
_repos/shared-backend-components/scripts/check_rule_candidate_generation.py.

PART 6 — Replay engine (_repos/baltor/backend/src/baltor/determinism/replay_engine.py): run a RuleCandidate against historical
WorkflowTraces; compare to verified final decisions; compute precision, recall, FP, FN, abstention rate,
unsafe-promotion count, tenant-leakage risk → RuleReplayReport. Proof: _repos/shared-backend-components/scripts/check_rule_replay_engine.py.

PART 7 — Shadow mode (_repos/baltor/backend/src/baltor/determinism/shadow_runner.py): rule runs alongside current LLM/human path;
output recorded but NOT used; compare to final decision → ShadowRunReport. Proof: _repos/shared-backend-components/scripts/check_rule_shadow_mode.py.

PART 8 — Promotion gate (_repos/baltor/backend/src/baltor/determinism/rule_promotion_gate.py): schema valid; source handles
...
after RulePromotionReceipt. Proof: _repos/shared-backend-components/scripts/check_rule_promotion_gate.py.

PART 9 — Fallback router (_repos/baltor/backend/src/baltor/determinism/fallback_router.py): deterministic rule first if active;
low confidence / unknown / OOD → LLMGateway, human review, or hold-out; fallback recorded; fallback traces
feed future mining. Proof: _repos/shared-backend-components/scripts/check_deterministic_rule_fallback.py.
...
→ winner=higher_authority_source, loser=held_out. Proof: _repos/shared-backend-components/scripts/check_cfpb_reconciliation_rule_distillation.py
...
fact. Proof: _repos/shared-backend-components/scripts/check_source_classification_rule_distillation.py.
...
tenant boundary, or lacks promotion receipt. Proof: _repos/shared-backend-components/scripts/check_optimization_rejection_rule_distillation.py.
...
tenant-scope warnings). Proofs: _repos/shared-backend-components/scripts/check_determinism_api.py, _repos/shared-backend-components/scripts/check_determinism_ui.py.

PART 14 — Redteam (_repos/shared-backend-components/scripts/check_determinism_redteam.py): all must FAIL SAFELY — promote rule from consensus
...
PART 15 — Docs: _repos/shared-backend-components/docs/determinism/{overview, llm-to-rules, consensus-is-not-truth, replay-engine, shadow-mode,
