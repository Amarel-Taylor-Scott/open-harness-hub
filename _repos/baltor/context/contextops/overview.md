...
`_repos/shared-backend-components/scripts/artifact_graph/reconciliation.py` — the foundry never builds a second reconciliation authority.
...
| Stage | Module | Role |
|---|---|---|
| TRIAGE | `_repos/baltor/backend/src/baltor/contextops/triage.py` | detect — route a claim into the twelve lanes |
| RESEARCH | `_repos/baltor/backend/src/baltor/contextops/research_stub.py` | propose — discover candidate sources offline |
...
| RECONCILE | `_repos/shared-backend-components/scripts/artifact_graph/reconciliation.py` (existing) | decide the winner deterministically |
| COST_LADDER | `cost_tracking.py` | price the moat: research once, deterministic forever |

## Proofs

- `_repos/shared-backend-components/scripts/check_contextops_full_stack.py` — prints the stage table; every stage green + the invariant.
- `_repos/shared-backend-components/scripts/check_contextops_cfpb_reference.py` — the deep end-to-end thesis proof (see `cfpb-example.md`).
- `_repos/shared-backend-components/scripts/check_contextops_cost_reduction.py` — the M0→M7 cost ladder (see `cost-reduction.md`).
- `_repos/shared-backend-components/scripts/check_contextops_redteam.py` — every attack fails safely (see `redteam.md`).
...
```bash
PYTHONPATH=. python3 _repos/shared-backend-components/scripts/check_contextops_full_stack.py --self-test
PYTHONPATH=. python3 _repos/shared-backend-components/scripts/check_contextops_cfpb_reference.py --self-test
PYTHONPATH=. python3 _repos/shared-backend-components/scripts/check_contextops_cost_reduction.py --self-test
PYTHONPATH=. python3 _repos/shared-backend-components/scripts/check_contextops_redteam.py --self-test
```
