...
slot, every external provider → a local/emulator strategy. Outputs: `_repos/shared-backend-components/docs/research/{current-market-map,
...
assumptions labeled). *(Builds on the existing _repos/shared-backend-components/docs/research/*-landscape.md set — extend, don't duplicate.)*

## 6. Competitor lane — `architecture/competitor_matrix.json` + `_repos/shared-backend-components/docs/strategy/{competitor-analysis,positioning-map}.md`
...
## 8. Startup-strategy lane — `_repos/shared-backend-components/docs/strategy/{startup-strategy,customer-discovery-plan,icp,pricing-hypotheses,gtm-plan,yc-application-notes,pitch-deck-outline,demo-script}.md`
...
## 9. YC/fundraising lane — `_repos/shared-backend-components/docs/strategy/{yc-one-liner,yc-application-draft,yc-pitch-deck,yc-demo-plan,yc-metrics-plan,investor-faq}.md`
...
## 10. Architecture-strategy lane — `_repos/shared-backend-components/docs/architecture/{architecture-strategy,flexibility-matrix,parallel-paths-and-promotion,self-evolving-cost-reduction,provider-graph-strategy,cloud-agnostic-execution,local-emulators-first,no-bandaids}.md`
...
cost/reliability/quality, promote only when safe. Contracts `_repos/shared-backend-components/schemas/experiments/{PathDefinition,ParallelPathRun,
...
contract_validation·safety_validation·created_at. Code `_repos/baltor/backend/src/baltor/experiments/{parallel_paths,path_comparator,
...
`_repos/baltor/backend/src/baltor/self_evolution/{path_reviewer,improvement_planner,cost_reducer,skill_writer}.py`. Rules: agents
