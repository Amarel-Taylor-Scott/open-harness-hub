...
> **Relocation + ownership note (2026-07-03).** This is a **historical Baltor build-workflow** (staged spec,
> predates the portfolio split), relocated from `_repos/teleon/context/` to `_repos/baltor/context/`. **The
> PurposeTask dashboards it describes are now Teleon's, not Baltor's:** the staff surface is the **Teleon Control
> Tower** and the customer surface is the **Capability Assurance Portal** (naming LOCKED; PurposeTask is a Teleon
> object). Baltor consumes them as tenant `baltor-internal` — it does not own the control/trust plane. Read the
> "Baltor STAFF + CUSTOMER views" language below as historical intent; the locked naming +
> `architecture/portfolio_dependency_law.json` win where they differ.
...
safety core is **already built + proven** (`_repos/baltor/backend/src/baltor/purpose_tasks/projections.py` +
...
- **P3 — projections:** extend `_repos/baltor/backend/src/baltor/purpose_tasks/projections.py` (staff/customer redaction DONE) with
