...
The **single source of truth** is the registry `_repos/shared-backend-components/architecture/native_output_modes.json` — this doc explains
it; it does not re-define it (see [No Magic Values](../codex/no-magic-values.md)).

## Owner

- Registry: `_repos/shared-backend-components/architecture/native_output_modes.json` (Lane C).
- Enforcement: `_repos/shared-backend-components/scripts/api_native_handler.py` reads the registry to validate the requested mode and to gate
...
bool`, `best_for: [..]`. Three invariants are proven by `_repos/shared-backend-components/scripts/check_native_output_modes.py`:
...
- `_repos/shared-backend-components/scripts/check_native_output_modes.py --self-test` — registry shape + the three invariants above.
- `_repos/shared-backend-components/scripts/check_native_export_no_truth_bypass.py --self-test` — only the three mutating modes may change a
