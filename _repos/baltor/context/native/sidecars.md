...
- Builder (Lane B): `_repos/baltor/backend/src/baltor/native/sidecar_writer.py`.
- Projection (Lane C): `GET /api/native/sidecar/<source_id>` and the `sidecar` field of an export, served by
  `_repos/shared-backend-components/scripts/api_native_handler.py`.
...
- `_repos/shared-backend-components/scripts/check_native_sidecar_lineage.py --self-test` — the lineage invariants above.
- `_repos/shared-backend-components/scripts/check_native_export_rehydration.py --self-test` — export + sidecar pivot back to the original via
...
- Land `_repos/baltor/backend/src/baltor/native/sidecar_writer.py` (Lane B) so real conflicts/reconciliations/receipts flow through
