...
- **Port:** `_repos/baltor/backend/src/baltor/ports/memory_provider.py` (`MemoryProviderPort`, `MemoryProfileProviderPort`,
  `MemoryMCPProviderPort`, `UnavailableProvider`).
- **Artifact builder (single source of the artifact shape):** `_repos/baltor/backend/src/baltor/adapters/memory/__init__.py`
  (`make_memory_artifact`, `memory_content_hash`, `external_source_handle`).
- **Working providers:** `_repos/baltor/backend/src/baltor/adapters/memory/baltor_local.py` (active local store),
  `_repos/baltor/backend/src/baltor/adapters/memory/supermemory_emulator.py` (deterministic offline emulator).
- **Candidate contract stubs:** `_repos/baltor/backend/src/baltor/adapters/memory/supermemory_api.py`,
  `_repos/baltor/backend/src/baltor/adapters/memory/supermemory_mcp.py` (raise `UnavailableProvider` without creds — never import
  the SDK, never call out).
- **Projection (read-only):** `_repos/shared-backend-components/scripts/api_memory_handler.py` + the `/memory` page at
  `_repos/baltor/frontend/memory.html`.
...
- Seven schemas under `_repos/shared-backend-components/schemas/memory/` (`MemoryArtifact`, `MemoryWriteRequest`, `MemorySearchRequest`,
...
- `_repos/shared-backend-components/scripts/check_memory_provider_contract.py` — the seven schemas + governance boundary.
- `_repos/shared-backend-components/scripts/check_local_memory_provider.py`, `_repos/shared-backend-components/scripts/check_supermemory_emulator.py` — the working providers.
- `_repos/shared-backend-components/scripts/check_memory_results_become_artifacts.py` — every output is a candidate, stubs fail closed.
- `_repos/shared-backend-components/scripts/check_memory_tenant_project_scoping.py` — no cross-tenant leak.
- `_repos/shared-backend-components/scripts/check_supermemory_candidate_catalog.py` — Supermemory is cataloged **candidate** with a
  fallback/stub + contract_tests, never active.
- `_repos/shared-backend-components/scripts/check_memory_provider_full_stack.py` — the whole stack end-to-end, offline, no creds.
- `_repos/shared-backend-components/scripts/check_memory_api.py`, `_repos/shared-backend-components/scripts/check_memory_page_projection_only.py`,
  `_repos/shared-backend-components/scripts/check_no_direct_supermemory_imports.py`, `_repos/shared-backend-components/scripts/check_supermemory_no_truth_bypass.py`,
  `_repos/shared-backend-components/scripts/check_memory_trace_written.py`.

## Commands

```bash
python3 _repos/shared-backend-components/scripts/check_memory_provider_full_stack.py --self-test
python3 _repos/shared-backend-components/scripts/check_supermemory_candidate_catalog.py --self-test
python3 _repos/shared-backend-components/scripts/check_memory_provider_contract.py --self-test
```
