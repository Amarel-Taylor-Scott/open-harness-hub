...
Known visible debt to register (not hide): the large `_repos/shared-backend-components/scripts/baltor_admin_demo_server.py`; any duplicate registry.
...
- **C-CONSOLIDATE-2:** numeric provider graph + runtime config-DB object model + `check_no_brittle_provider_string_logic`
  ...
  _repos/_shared/archive/legacy/baltor/context/baltor-numeric-provider-selection-graph.md.)*
...
## Scoreboard + redteam
`_repos/shared-backend-components/docs/status/{technical-debt-scoreboard,architecture-integrity}.md` (open debts by severity · monoliths + split
