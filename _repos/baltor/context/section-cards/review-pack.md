...
`_repos/shared-backend-components/scripts/make_review_pack.py` — `build_report(...)`, `run_check(...)`; writes
...
`_repos/shared-backend-components/scripts/make_review_pack.py` (own `--self-test`) and `_repos/shared-backend-components/scripts/check_review_pack_recorders.py` (guards that
every listed review-pack screenshot has a real producer) — both registered in the flywheel.

## Commands

```bash
PYTHONPATH=. python3 _repos/shared-backend-components/scripts/make_review_pack.py --self-test
PYTHONPATH=. python3 _repos/shared-backend-components/scripts/check_review_pack_recorders.py --self-test
```
