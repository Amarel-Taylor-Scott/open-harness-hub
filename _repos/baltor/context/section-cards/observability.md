...
`_repos/shared-backend-components/scripts/runtime/logger.py` — `RuntimeLogger` (`info`, `warn`, `error`, `child`) with built-in redaction;
plus the `to_otel_span` seam that lifts events to OTel GenAI spans. Registered runtime owner `RuntimeLogger`
(`_repos/shared-backend-components/architecture/runtime_ownership.json#RuntimeLogger`).
...
`_repos/shared-backend-components/scripts/check_structured_runtime_logging.py` and `_repos/shared-backend-components/scripts/check_otel_spans.py` — both registered in the
flywheel; the OTel proof runs over a REAL pipeline run, not a synthetic event.

## Commands

```bash
PYTHONPATH=. python3 _repos/shared-backend-components/scripts/check_structured_runtime_logging.py --self-test
PYTHONPATH=. python3 _repos/shared-backend-components/scripts/check_otel_spans.py --self-test
```
