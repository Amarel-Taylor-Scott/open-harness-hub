...
`_repos/shared-backend-components/scripts/artifact_graph/conflict_detector.py` — `ConflictDetector.detect(...)`. Every `conflict_id` is a
...
(`_repos/shared-backend-components/architecture/runtime_ownership.json#ConflictDetector`).

## Proof scripts

`_repos/shared-backend-components/scripts/check_cfpb_conflict_detection.py` (registered in the flywheel) — asserts the Reg E vs FAQ deadline
conflict (and the other rule classes) are detected deterministically.

## Commands

```bash
PYTHONPATH=. python3 _repos/shared-backend-components/scripts/check_cfpb_conflict_detection.py --self-test
```
