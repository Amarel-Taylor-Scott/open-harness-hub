...
(`_repos/baltor/backend/src/baltor/experiments/*`, `_repos/shared-backend-components/schemas/experiments/*`) from `baltor-parallel-path-hard-mode`; this factory adds
...
`_repos/baltor/backend/src/baltor/paths/{path_registry,parallel_path_runner,path_comparator,path_scorecard,path_promotion_gate,
...
`_repos/baltor/backend/src/baltor/paths/{path_reviewer,path_improvement_planner,path_variant_generator}.py`. Agents MAY inspect a path
...
`_repos/baltor/backend/src/baltor/skills/skill_discovery.py` (sources: local folder · GitHub URL · GitHub search/watchlist candidate ·
MCP catalog · codex/agent skill folder · awesome-lists · manual; live GitHub optional, offline fixture
`fixtures/skills/sample_skill_repos.json` MUST work; discovery ≠ trust). Pipeline `_repos/baltor/backend/src/baltor/skills/{skill_intake,
...
`_repos/baltor/backend/src/baltor/skills/local_skill_sandbox.py` (reuse managed-venv/sandbox port if present): temp dir · copy files ·
