...
> predates the portfolio split), relocated from `_repos/teleon/context/` to `_repos/baltor/context/` because it
> is a Baltor spec. Naming has since been LOCKED and supersedes the framing below where they differ: the
> **CapabilityTask spec (CTS)** is stewarded by **OpenHubForAI** (neutral) and **Teleon** — not Baltor — is its
> reference implementation; **PurposeTask is a Teleon object**, and the PurposeTask control & trust surfaces are
> Teleon's (**Teleon Control Tower** for staff, **Capability Assurance Portal** for customer). Baltor consumes
> Teleon as tenant `baltor-internal`; it does not own the runtime, the spec impl, or those dashboards. Read the
> "Baltor implements CTS / Baltor = best conformant runtime" language as historical intent; the locked naming +
> `architecture/portfolio_dependency_law.json` win.
