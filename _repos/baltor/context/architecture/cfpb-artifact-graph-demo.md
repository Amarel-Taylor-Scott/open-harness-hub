...
Modules: `_repos/shared-backend-components/scripts/artifact_graph/{artifact_ledger,cfpb_artifacts,vector_store,graph_builder,conflict_detector,reconciliation}.py`
+ orchestrator `_repos/shared-backend-components/scripts/cfpb_artifact_graph_demo.py`. API `POST /api/demo/cfpb-artifact-graph`; page `_repos/baltor/frontend/cfpb-artifact-graph.html`.
...
`_repos/shared-backend-components/scripts/check_cfpb_artifact_ledger.py` · `check_cfpb_vectorization.py` · `check_cfpb_deterministic_graph.py`
· `check_cfpb_conflict_detection.py` · `check_cfpb_reconciliation.py` · `check_cfpb_artifact_graph_demo.py`
(all registered in the flywheel). Run: `PYTHONPATH=. python3 _repos/shared-backend-components/scripts/cfpb_artifact_graph_demo.py --self-test`.
