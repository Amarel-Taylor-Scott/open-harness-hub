...
2. Context worker / backend-agent CI/CD
...
to the context workers that operate the context supply chain.
...
## Context worker / Backend-Agent CI/CD

Context workers are the agents/workers that operate Baltor's backend:
...
This is a core CI/CD rule. A context worker should not appear because an agent
...
use that pattern for context worker creation, but generated agents still need
creation provenance.
...
Every context worker release should run:
...
identities for backend workers and context workers:
...
```text
1. Worker rollback
   Revert a context worker release, prompt version, tool policy, or model route.
