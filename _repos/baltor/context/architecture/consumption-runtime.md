...
`_repos/shared-backend-components/scripts/runtime/consumption.py` (declared runtime owner `ConsumptionService`):
...
`ContextResponse`, `ConsumptionReceipt`, `ConsumptionRequest` (`_repos/shared-backend-components/schemas/consumption/`), registered in
`_repos/shared-backend-components/architecture/contract_registry.json`. A served fact's schema **requires** `artifact_id` + `source_handle` +
`claim_status`; the response **requires** the three receipt ids + lineage + freshness.

## Proven reference result (offline, deterministic)

`python3 _repos/shared-backend-components/scripts/check_cfpb_to_consumption_end_to_end.py --self-test` produces a served `ContextResponse` where:
