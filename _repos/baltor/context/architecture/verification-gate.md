...
`_repos/shared-backend-components/scripts/runtime/verification_gate.py` — `class VerificationGate` (declared owner in
`_repos/shared-backend-components/architecture/runtime_ownership.json`). It is a **decision + receipt maker**, never a mutator: `verify()`
...
source — `_repos/shared-backend-components/scripts/pipeline_runtime/artifact_types.py` — and schema validity through the runtime
...
Proof: `_repos/shared-backend-components/scripts/check_verification_gate.py` (the seven owner scenarios + tenant isolation, model grounding,
