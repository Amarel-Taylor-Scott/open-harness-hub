...
`_repos/shared-backend-components/scripts/durable_store.py` (stdlib `sqlite3`, ACID, WAL, **thread-safe** under ThreadingHTTPServer; no
broker/cloud/pip):
...
- **C26 — event envelope contract (NEXT).** Lock one envelope for every bus event + queue command:
  ...
  payload, evidence{receipt_uri,input_hash,output_hash}`. Build as a validator (`_repos/shared-backend-components/schemas/runtime/*` +
