...
(`_repos/shared-backend-components/scripts/baltor_flywheel.py`). Product boundary: **Baltor owns the contract** (context objects,
...
1. **Warrant before change** — clear user intent / ≥2 agreeing sources / an established repo
   ...
   (no orphaned contradictions). See `_repos/shared-backend-components/docs/codex/change-verification-contract.md`.
...
3. **Verify-first** — web-confirm any tool/repo/library/fact before it enters the repo; never
   trust a list. Source of truth: `_repos/shared-backend-components/data/backend-tools.yaml` + `research/backend-tool-verification.md`.
...
5. **Real, or a labeled SEAM** — no faked results; absent network/creds → an interface + an offline
   `Canned*` fixture + a `--live` flag (e.g. `_repos/shared-backend-components/scripts/ingest/sanctions_feed_live.py`,
   `_repos/shared-backend-components/scripts/ingest/parser_provider.py`).
...
7. **Decomposition contract** — documents are recursive trees of addressable typed objects with
   ...
   minimal node; re-decompose→diff drives per-node rot. See `_repos/baltor/context/backend/document-decomposition.md`.
...
9. **Context-rot management** — TTL + content-hash CDC + ACL + supersession → typed signals →
   serve/refresh/block/human-review (`_repos/shared-backend-components/scripts/ingest/context_rot.py`). Freshness ≠ correctness.
