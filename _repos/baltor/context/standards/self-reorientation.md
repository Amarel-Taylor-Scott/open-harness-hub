...
2. **Inspect local state** — read `.claude/commands/baltor-goal-loop.md`,
   ...
   the relevant `_repos/shared-backend-components/docs/backend/*` + `schemas/*`. **Confirm what already exists before creating it**
...
6. **PROVE** — a runnable, deterministic, offline self-test (`--self-test`) green + `py_compile`;
   then `python3 _repos/shared-backend-components/scripts/baltor_flywheel.py --once` to confirm nothing regressed.
