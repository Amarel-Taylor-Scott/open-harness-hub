...
capability has a **primary + fallback** (verified, OSS-first — `_repos/shared-backend-components/data/backend-tools.yaml`), and the
...
- **Fetcher** (`_repos/shared-backend-components/scripts/foundry/scrapers.py`): `HttpFetcher` (live) / `CannedFetcher` (offline).
...
- **ParserProvider** (`_repos/shared-backend-components/scripts/ingest/parser_provider.py`): `DoclingParser` (live, lazy-imports
...
`RepoDirectoryProvider` · `ApiCatalogProvider`. Primary/fallback per port: `_repos/shared-backend-components/data/backend-tools.yaml`.
