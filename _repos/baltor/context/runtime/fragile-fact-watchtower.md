...
- `_repos/baltor/backend/src/baltor/facts/` — the watchtower home (section `fragile_fact_watchtower`).
- Logic owner: `_repos/baltor/backend/src/baltor/facts/refresh_planner.py` (planner) + `_repos/baltor/backend/src/baltor/facts/classifier.py` (classifier).
- Contracts owner: `_repos/baltor/backend/src/baltor/contracts/artifacts/` (the six frozen dataclasses).

## Contracts (six artifact types, all v1, content-addressed ids)

| Artifact | Module | Schema | Key fields |
|---|---|---|---|
| FactAssertion | `_repos/baltor/backend/src/baltor/contracts/artifacts/fact_assertion.py` | `_repos/shared-backend-components/schemas/artifacts/FactAssertion.schema.json` | `claim_type` ∈ CLAIM_SHAPED, `scope`, `asserted_at` |
| CanonicalFact | `_repos/baltor/backend/src/baltor/contracts/artifacts/canonical_fact.py` | `_repos/shared-backend-components/schemas/artifacts/CanonicalFact.schema.json` | `status` (9-value lifecycle), `scope`, `fragility_id` |
| FragilityMetadata | `_repos/baltor/backend/src/baltor/contracts/artifacts/fragility_metadata.py` | `_repos/shared-backend-components/schemas/artifacts/FragilityMetadata.schema.json` | `volatility_class`, `last_verified_at` + (`next_verify_at` OR `ttl_seconds`) + `watch_policy_id` |
| WatchPolicy | `_repos/baltor/backend/src/baltor/contracts/artifacts/watch_policy.py` | `_repos/shared-backend-components/schemas/artifacts/WatchPolicy.schema.json` | `refresh_interval_seconds`, `no_refresh`, `escalate_on_conflict` |
| VerificationTask | `_repos/baltor/backend/src/baltor/contracts/artifacts/verification_task.py` | `_repos/shared-backend-components/schemas/artifacts/VerificationTask.schema.json` | `due_at` (missed horizon), `scope`, `watch_policy_id`, `status` |
| VerificationResult | `_repos/baltor/backend/src/baltor/contracts/artifacts/verification_result.py` | `_repos/shared-backend-components/schemas/artifacts/VerificationResult.schema.json` | `new_status`, `applied`, `evidence`, `next_verify_at` |
...
seconds). The classifier (`_repos/baltor/backend/src/baltor/facts/classifier.py`) maps text cues (`current`, `deadline`, `rate`,
...
`_repos/shared-backend-components/scripts/check_watchtower_minimum_freshness.py` (`--self-test`, 31 checks). It asserts: Reg E "10 business
...
```bash
PYTHONPATH=. python3 _repos/shared-backend-components/scripts/check_watchtower_minimum_freshness.py --self-test
```
