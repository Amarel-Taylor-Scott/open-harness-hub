...
**Owner.** `_repos/shared-backend-components/scripts/api_context_handler.py` (pure handler) + dispatch hooks in `_repos/shared-backend-components/scripts/baltor_admin_demo_server.py`.
**Contract.** `ContextResponse` (`_repos/shared-backend-components/schemas/consumption/`). **Registry.** `_repos/shared-backend-components/architecture/contract_registry.json#api_routes`.
...
```
PYTHONPATH=. python3 _repos/shared-backend-components/scripts/check_consumption_api.py --self-test
PYTHONPATH=. python3 _repos/shared-backend-components/scripts/check_context_response_retrieval_api.py --self-test
PYTHONPATH=. python3 _repos/shared-backend-components/scripts/check_runtime_sections_api.py --self-test
PYTHONPATH=. python3 _repos/shared-backend-components/scripts/check_no_api_consumption_bypass.py --self-test
```
