...
`_repos/baltor/backend/src/baltor/distillation/rehydration.py` (uses `lineage.py` + `lossless_store.py`).
...
```bash
PYTHONPATH=. python3 _repos/shared-backend-components/scripts/check_distillation_rehydration.py --self-test
```
