...
Make Baltor's top-level **lossless distillation law** (`_repos/shared-backend-components/docs/codex/lossless-distillation.md`) a *checkable
...
`_repos/baltor/backend/src/baltor/distillation/` (engine) + `scripts/check_*lossless*` and `scripts/check_distillation_*`,
`_repos/shared-backend-components/scripts/check_lineage_bundle_complete.py`, `_repos/shared-backend-components/scripts/check_information_retention_report.py` (proofs).
Standing law owner: `_repos/shared-backend-components/docs/codex/lossless-distillation.md`.

## Contracts

Six v1 schemas under `_repos/shared-backend-components/schemas/distillation/` (each with a passing `valid` example and a rejected
`invalid` example):
...
```bash
PYTHONPATH=. python3 _repos/shared-backend-components/scripts/check_lossless_distillation_full_stack.py --self-test
PYTHONPATH=. python3 _repos/shared-backend-components/scripts/check_lossless_distillation_redteam.py --self-test
PYTHONPATH=. python3 _repos/shared-backend-components/scripts/check_lossless_distillation_contracts.py --self-test
```
