...
## Run (one command)
```bash
bash e2e/run.sh        # serves _repos/baltor/frontend on :8000 if needed, npm installs, drives, makes the gif
```
Or step by step:
```bash
cd _repos/baltor/frontend && python3 -m http.server 8000 &      # serve the demo
