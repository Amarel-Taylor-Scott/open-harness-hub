/* e2e/oh_experiments_beacon_probe.mjs — REAL-JS proof that the kit's experiment + analytics
   ...
   (the served web/openhubforai/kit/oh-identity.js + oh-experiments.js, passed in as OH_KIT_DIR by the
   Python gate) in a tiny browser stub, drives OHExp exactly as the React SPA does (useExperiment →
   define/variant/exposure, then
...
// The Python gate (scripts/check_events_beacon_wiring.py) passes the resolved shipped-kit dir
// (web/openhubforai/kit → _repos/openhubforai/frontend/kit after the _repos/ migration) via OH_KIT_DIR, so
// this probe loads the EXACT files the gate asserts against — no repo-layout assumption baked in here.
// Fallback: the shared kit that ships beside this e2e dir (_repos/baltor/frontend/kit) keeps a standalone
// `node e2e/oh_experiments_beacon_probe.mjs` run working.
const SHARED = process.env.OH_KIT_DIR || join(REPO, "frontend", "kit");
...
const win = {
  OPENHUBFORAI_EVENTS_BASE: "http://127.0.0.1:9420",       // same value the porter injects (window.OPENHUBFORAI_EVENTS_BASE='/analytics' at deploy)
...
try {
  ...
  OHExp.define("openhubforai_hero", ["A", "B"]);
  const v1 = OHExp.variant("openhubforai_hero");
  const v2 = OHExp.variant("openhubforai_hero");             // sticky: must equal v1
  OHExp.exposure("openhubforai_hero");                       // → an exposure beacon via the bridge
  OHExp.track("build_submit", { experiment: "openhubforai_hero", variant: v1 });  // → a conversion beacon

  const page = captured.find((c) => c.body.event === "page");
  const exposure = captured.find((c) => c.body.event === "exposure" && c.body.experiment === "openhubforai_hero");
  const conversion = captured.find((c) => c.body.event === "conversion" && c.body.experiment === "openhubforai_hero");
