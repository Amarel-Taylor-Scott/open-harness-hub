...
async def _run_cross_encoder_reranker(args: dict[str, Any]) -> Any:
    """Cross-encoder reranker — Jointly encode query+doc to rescore the fused top-50→100 and keep the top-5 — the single highest-precision lever in the pipeline (~40%+ recall@k / MRR lift); cost is bounded by candidate count, not corpus size. Open-weight (bge-reranker-v2-m3) is commercial-safe."""
