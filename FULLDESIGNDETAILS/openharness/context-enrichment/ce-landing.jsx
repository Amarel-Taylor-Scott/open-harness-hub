...
function Hero() {
  ...
  const subheads = BRANDCE.subheads || [];
  const ctas = BRANDCE.ctas || [];
  const [variant, { track, assign }] = window.useExperiment('baltor_hero', hooks.map((x) => x.id));
  const [subV, sub] = window.useExperiment('baltor_subhead', subheads.map((x) => x.id));
  const [ctaV, cta] = window.useExperiment('baltor_cta', ctas.map((x) => x.id));
  const h = hooks.find((x) => x.id === variant) || hooks[0];
  const subText = (subheads.find((x) => x.id === subV) || {}).text || BRANDCE.subhead;
  const ctaLabel = (ctas.find((x) => x.id === ctaV) || {}).label || 'Connect a source →';
  const startCta = () => { track('cta_click', { cta: 'primary', label: ctaLabel, where: 'hero' }); cta.track('cta_click', { label: ctaLabel }); navigate('/ingest'); };
  // inline A/B controls — one row per live test (prototype affordance; ids come from the arrays)
  const TESTS = [['Headline', hooks, variant, assign], ['Subhead', subheads, subV, sub.assign], ['CTA', ctas, ctaV, cta.assign]];
  return (
    <section className="ce-hero">
      <div className="ce-wrap ce-hero-grid">
        <div>
          <div className="ce-abset" role="group" aria-label="Preview landing variants">
            {TESTS.map(([label, opts, cur, set]) => (
              <div className="ce-ab" key={label}>
                <span className="ce-ab-lbl">{label}</span>
                {opts.map((x) => (
                  <button key={x.id} className={cur === x.id ? 'on' : ''} onClick={() => set(x.id)} aria-pressed={cur === x.id} title={x.text || x.label || (x.lead + ' ' + x.tint)}>{x.id}</button>
                ))}
              </div>
            ))}
          </div>
...
// Landing layout A/B — three narrative orderings of the same sections, run through the
// shared experiments engine. The Hero stays first; the body re-sequences to lead with a
// different argument. Force with ?exp=baltor_landing:engine (or problem / proof).
...
function Landing() {
  ...
  const [layout] = window.useExperiment('baltor_landing', Object.keys(LANDING_LAYOUTS));
  const plan = LANDING_LAYOUTS[layout] || LANDING_LAYOUTS.default;
  return (
