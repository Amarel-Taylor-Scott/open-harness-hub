...
``scripts/check_determinism_full_stack.py`` (that proof is the template for this build). It runs ONE CFPB
...
network), and it never mutates durable truth. The handler (``scripts/api_determinism_handler.py``) projects
this dict read-only; the page (``web/baltor/determinism.html``) renders it.
"""
