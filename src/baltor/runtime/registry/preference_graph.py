...
_ARCH = Path(__file__).resolve().parents[4] / "architecture"
...
#: vocabulary's own labels are loaded from architecture/provider_status_codes.json (single source).
...
def _status_label_to_code() -> dict:
    """label → numeric status code, sourced from architecture/provider_status_codes.json (the single source),
