...
runtime concepts being extracted out of `src/baltor/`.
...
reference implementation of the **open CapabilityTask spec** (the spec itself is stewarded by OpenHarnessHub
for neutrality — "Teleon implements the Open CapabilityTask Spec").

## Migration in progress (lossless, incremental — never big-bang)
Generic runtime code still lives under `src/baltor/` and moves here one layer at a time, updating imports +
proofs and keeping the flywheel green at each step. Planned order (see the law file's `migration_status`):
1. execution layer (`src/baltor/workers/execution_*`) → `src/teleon/runtime`
2. Parallel-Path Engine (`src/baltor/experiments`) → `src/teleon/experiments`
3. `src/baltor/purpose_tasks` → `src/teleon/purpose_tasks`
4. add `src/baltor/teleon_client` so Baltor calls Teleon as tenant `baltor-internal`

Canonical design: `docs/strategy/teleon-baltor-openharnesshub-portfolio.md`. Build kit for Teleon's control &
