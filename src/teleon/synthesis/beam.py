...
def rank_candidates(candidates: list, *, profile: PreferenceProfile | None = None, require_verified: bool = True) -> list:
    ...
    if not survivors:
        return []
    ranked = SIM.rank_candidates([c for c, _ in survivors], profile=profile)   # [(index, estimate, score)]
...
def beam_compose(candidates: list, *, profile: PreferenceProfile | None = None, beam_width: int = 3,
    ...
    profile = profile or cost_first()
    ranked = rank_candidates(candidates, profile=profile, require_verified=require_verified)
