...
reference implementation of the open CapabilityTask spec (stewarded by OpenHarnessHub).

ARCHITECTURAL LAW (architecture/portfolio_dependency_law.json, enforced by
scripts/check_portfolio_dependency_law.py): Teleon is reusable infrastructure. It MAY consume OpenHarnessHub
artifacts; it must NEVER import Baltor (`src.baltor.*`) — Baltor is a TENANT/consumer of Teleon, not the other
way around. Generic Teleon concepts are being extracted incrementally out of `src/baltor/` into this package
