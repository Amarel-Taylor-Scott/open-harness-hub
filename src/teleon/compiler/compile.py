...
``architecture/deploy_topology.json``. This module owns the PURE compile; ``emit.py`` renders the unit into the
...
NO MAGIC VALUES — every field is JOINED from an existing single source, never hand-typed:
  ...
  * resources{cpu,memory_mb,gpu_required} : ``architecture/worker_resource_classes.json`` keyed by the
    CapabilityTask ``required_resource_class``.
  * budgets.timeout_s : the SLA policy ``target_seconds`` (``architecture/worker_sla_policies.json`` keyed by
  ...
  * container.image : ``architecture/deploy_topology.json`` ``image.registry_hint`` (one source).
...
DEFAULT_MAX_OUTPUT_TOKENS = 300   # matches the runtime's per-call max_tokens=300 in scripts/teleon_local_runtime.py
...
# ── resource resolution (architecture/worker_resource_classes.json) ──────────────────────────────────────────
...
def compile_capability(
    ...
    *,
    ...
    # injected single-source data (defaults loaded from architecture/*.json) — overridable for testing/determinism
...
def validate_unit(unit: dict) -> list[str]:
    ...
    import jsonschema  # the repo's contract validator for union-typed schemas (scripts/validate.py uses it too)
