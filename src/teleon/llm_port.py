...
registries — the model index (architecture/model_index.json) + the provider lanes (scripts/_llm_client.PROVIDERS) —
so adding a model to a registry makes it selectable everywhere; routing reuses the Teleon inference plane
(src/teleon/inference) and never re-implements it. serves_truth=false (an LLM is never a source of truth).
...
_MODEL_INDEX = Path(__file__).resolve().parents[2] / "architecture" / "model_index.json"
...
class GatewayLLM:
    ...
    Never re-implements routing — it delegates to src/teleon/inference."""
...
def available_llms() -> dict:
    ...
    Adding a model to architecture/model_index.json (or a lane to _llm_client) makes it selectable here, no code change."""
