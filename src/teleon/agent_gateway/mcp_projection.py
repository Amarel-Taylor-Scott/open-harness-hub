...
TOOL_LIST_CAPABILITIES = "teleon_list_capabilities"
TOOL_DESCRIBE_CAPABILITY = "teleon_describe_capability"
TOOL_RUN_CAPABILITY = "teleon_run_capability"
TOOL_GET_RECEIPT = "teleon_get_receipt"
TOOL_REQUEST_BOUNDARY_EXPANSION = "teleon_request_boundary_expansion"
...
COMPACT_RESULT_FIELDS: tuple[str, ...] = (
    "request_id",
    "capability_id",
    "status",
    "output",
...
def _is_forbidden_key(key: str) -> bool:
    ...
    low = str(key).lower()
    return any(bad in low for bad in _FORBIDDEN_KEY_SUBSTRINGS)
...
def _scrub(value: Any) -> Any:
    ...
    if isinstance(value, dict):
        return {k: _scrub(v) for k, v in value.items() if not _is_forbidden_key(k)}
...
def _project(source: dict, fields: tuple[str, ...]) -> dict:
    ...
    return {f: _scrub(source[f]) for f in fields if f in source}
...
class TeleonMcpProjection:
    ...
    Projection-only invariants (asserted by ``scripts/check_teleon_agent_gateway_mcp_projection.py``):
    ...
    def __init__(self, gateway: AgentCapabilityGateway | None = None) -> None:
        ...
        self.gateway: AgentCapabilityGateway = gateway or AgentCapabilityGateway()
        ...
        self._dispatch: dict[str, Callable[[dict, str], dict]] = {
            TOOL_LIST_CAPABILITIES: self._tool_list_capabilities,
            TOOL_DESCRIBE_CAPABILITY: self._tool_describe_capability,
            TOOL_RUN_CAPABILITY: self._tool_run_capability,
            TOOL_GET_RECEIPT: self._tool_get_receipt,
            TOOL_REQUEST_BOUNDARY_EXPANSION: self._tool_request_boundary_expansion,
        }
        # the surfaced set is EXACTLY the five names — keep the descriptor list and the dispatch table in lock.
        assert tuple(self._dispatch) == TOOL_NAMES, "dispatch table must be exactly the five tool names"
    ...
    def tool_descriptors(self) -> list[dict]:
        ...
        return [
            ...
            {
                "name": TOOL_REQUEST_BOUNDARY_EXPANSION,
                "description": (
    ...
    def call_tool(self, name: str, input: dict | None = None, *, now: str) -> dict:
        ...
        handler = self._dispatch.get(str(name))
        if handler is None:
            return {
                "error": ERROR_UNKNOWN_TOOL,
                "message": f"unknown tool {name!r}; this projection exposes exactly {EXPOSED_TOOL_COUNT} tools",
        ...
        if not isinstance(input, dict):
            return {
                ...
                "message": f"tool {name!r} expects an object input, got {type(input).__name__}",
                "tool": str(name),
                "serves_truth": AGENT_GATEWAY_SERVES_TRUTH,
            }
    ...
    def _tool_list_capabilities(self, input: dict, now: str) -> dict:
        ...
        actions = {str(a) for a in allowed_actions} if isinstance(allowed_actions, (list, tuple, set)) else None

        cards = self.gateway.list_capabilities()
        out: list[dict] = []
        for card in cards:
            cid = str(card.get("capability_id", ""))
            if domain and not (cid == str(domain) or cid.startswith(f"{domain}.")):
                continue
            if actions is not None and not (set(card.get("allowed_use", [])) & actions):
                continue
            out.append(_project(card, COMPACT_CARD_FIELDS))
        return {
            "tool": TOOL_LIST_CAPABILITIES,
            "capabilities": out,
            "count": len(out),
    ...
    def _tool_describe_capability(self, input: dict, now: str) -> dict:
        ...
        if not isinstance(capability_id, str) or not capability_id:
            return {"error": ERROR_BAD_INPUT, "message": "capability_id (string) is required",
                    "tool": TOOL_DESCRIBE_CAPABILITY, "serves_truth": AGENT_GATEWAY_SERVES_TRUTH}
        card = self.gateway.describe(capability_id)
        if card is None:
            return {"tool": TOOL_DESCRIBE_CAPABILITY, "found": False, "capability_id": capability_id,
                    "capability": None, "serves_truth": AGENT_GATEWAY_SERVES_TRUTH}
        return {"tool": TOOL_DESCRIBE_CAPABILITY, "found": True, "capability_id": capability_id,
                "capability": _project(card, COMPACT_CARD_FIELDS), "serves_truth": AGENT_GATEWAY_SERVES_TRUTH}
    ...
    def _tool_run_capability(self, input: dict, now: str) -> dict:
        ...
        if not isinstance(capability_id, str) or not capability_id:
            return {"error": ERROR_BAD_INPUT, "message": "capability_id (string) is required",
                    "tool": TOOL_RUN_CAPABILITY, "serves_truth": AGENT_GATEWAY_SERVES_TRUTH}
        if not isinstance(consumer_id, str) or not consumer_id:
            return {"error": ERROR_BAD_INPUT, "message": "consumer_id (string) is required",
                    "tool": TOOL_RUN_CAPABILITY, "serves_truth": AGENT_GATEWAY_SERVES_TRUTH}
        ...
        max_cost = input.get("max_cost")
        if isinstance(max_cost, (int, float)) and not isinstance(max_cost, bool):
            quote = self.gateway.quote(capability_id)
            expected = quote.get("expected_cost") if isinstance(quote, dict) else None
            usd = expected.get("usd") if isinstance(expected, dict) else None
            if isinstance(usd, (int, float)) and not isinstance(usd, bool) and usd > max_cost:
                return {
                    "tool": TOOL_RUN_CAPABILITY,
                    "capability_id": capability_id,
                    "status": RUN_STATUS_UNAVAILABLE,
                    "output": {"refused": True,
                               "reason": f"expected cost {usd} exceeds max_cost {max_cost}"},
                    "source_handles": [],
                    "held_out": [],
                    "receipt_id": None,
                    "serves_truth": AGENT_GATEWAY_SERVES_TRUTH,
        ...
        if isinstance(input.get("allowed_use"), (list, tuple)):
            annotations["requested_use"] = [str(a) for a in input["allowed_use"]]
        if isinstance(input.get("freshness_requirement"), str):
            annotations["freshness_requirement"] = input["freshness_requirement"]

        request = {"capability_id": capability_id, "payload": payload}
        if annotations:
        ...
        result = self.gateway.run(request, now=now, consumer=consumer)
        compact = _project(result, COMPACT_RESULT_FIELDS)
        compact["tool"] = TOOL_RUN_CAPABILITY
        # serves_truth is pinned by the gateway; reassert it on the surfaced view so it can never be absent.
        compact["serves_truth"] = bool(result.get("serves_truth", AGENT_GATEWAY_SERVES_TRUTH))
        return compact
    ...
    def _tool_get_receipt(self, input: dict, now: str) -> dict:
        ...
        if not isinstance(receipt_id, str) or not receipt_id:
            return {"error": ERROR_BAD_INPUT, "message": "receipt_id (string) is required",
                    "tool": TOOL_GET_RECEIPT, "serves_truth": AGENT_GATEWAY_SERVES_TRUTH}
        receipt = self.gateway.get_receipt(receipt_id)
        if receipt is None:
            return {"tool": TOOL_GET_RECEIPT, "found": False, "receipt_id": receipt_id,
                    "receipt": None, "serves_truth": AGENT_GATEWAY_SERVES_TRUTH}
        return {"tool": TOOL_GET_RECEIPT, "found": True, "receipt_id": receipt_id,
                "receipt": _scrub(receipt), "serves_truth": AGENT_GATEWAY_SERVES_TRUTH}
    ...
    def _tool_request_boundary_expansion(self, input: dict, now: str) -> dict:
        ...
        req = {
            "consumer_id": input.get("consumer_id", ""),
            "capability_id": input.get("capability_id", ""),
            "requested_change": input.get("requested_change", {}),
            "justification": input.get("justification", ""),
        }
        result = self.gateway.request_boundary_expansion(req, now=now)
        # pass the gateway's queued request through (scrubbed); annotate the tool name for the MCP host.
        out = _scrub(result)
        out["tool"] = TOOL_REQUEST_BOUNDARY_EXPANSION
        out["serves_truth"] = AGENT_GATEWAY_SERVES_TRUTH
        return out
