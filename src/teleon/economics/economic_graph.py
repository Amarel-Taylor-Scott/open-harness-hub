...
@lru_cache(maxsize=1)
def _pricebook() -> dict:
    return json.loads((REPO / "architecture" / "execution_backend_pricebook.json").read_text()).get("backends", {})


@lru_cache(maxsize=1)
def _lanes() -> dict:
    return {ln["lane_id"]: ln for ln in
            json.loads((REPO / "architecture" / "inference_lane_profiles.json").read_text())["lanes"]}


@lru_cache(maxsize=1)
def _graph() -> dict:
    return json.loads((REPO / "architecture" / "model_provider_graph.json").read_text())
