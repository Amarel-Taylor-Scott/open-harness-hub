...
The work item itself IS the CapabilityTask record the FleetLedger already produces; these helpers name the
...
def project_worker_claim(task: dict) -> dict:
    ...
    return {"schema_version": "WorkerClaim", "task_id": task["task_id"], "worker_id": task["lease_owner"],
...
def project_ack_nack_receipt(task: dict, *, outcome: str, worker_id: str, retryable: bool | None = None) -> dict:
    ...
    return {"schema_version": "AckNackReceipt", "task_id": task["task_id"], "worker_id": worker_id,
...
def project_dead_letter(task: dict) -> dict:
    ...
    return {"schema_version": "DeadLetterEntry", "task_id": task["task_id"], "tenant_id": task["tenant_id"],
...
def project_idempotency_key(task: dict) -> dict:
    ...
    return {"schema_version": "IdempotencyKey", "idempotency_key": task["idempotency_key"],
