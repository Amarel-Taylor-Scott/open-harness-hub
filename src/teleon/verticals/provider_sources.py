...
_SOURCES = Path(__file__).resolve().parents[3] / "architecture" / "provider_directory_sources.json"
...
def fetch(source_id: str, query: str, *, network_allowed=None) -> dict:
    ...
    try:
        if r["plane"] == "search":
            ...
            return {"available": True, "source": source_id, "plane": "search", "results": select_search("auto").search(query, limit=3), "serves_truth": False}
