...
Why (the agnostic-adapter principle, docs/architecture/agnostic-adapters.md): more OCR engines keep arriving, so the
document-extraction cascade depends on an engine-AGNOSTIC port, never a specific engine. Selectable engines are
POPULATED FROM architecture/ocr_provider_registry.json, cheapest-first: a PDF text layer (no OCR) → local deterministic
...
_REGISTRY = Path(__file__).resolve().parents[3] / "architecture" / "ocr_provider_registry.json"
