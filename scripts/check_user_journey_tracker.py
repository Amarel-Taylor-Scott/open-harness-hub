...
emits a schema-valid, deterministic, REPLAYABLE UserJourneyTrace.v1 — the structured, governed complement to
...
def _self_test() -> int:
    ...
    print("\n" + ("PASS — check_user_journey_tracker: every registered journey runs the REAL demo backend into a "
                  "schema-valid, deterministic, REPLAYABLE UserJourneyTrace.v1 (same journey -> same trace_hash); "
