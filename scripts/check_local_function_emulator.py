...
CLI: PYTHONPATH=. python3 scripts/check_local_function_emulator.py --self-test
"""
...
from src.baltor.ports.execution_provider import ExecutionResult, ProviderUnavailableResult
...
def _self_test() -> int:
    ...
    chk("invoke returns ExecutionResult", isinstance(res, ExecutionResult))
    ...
    r = GcpCloudRunFunctionCandidate().invoke({"task_id": "y", "capability_id": CAP})
    chk("unconfigured cloud function → ProviderUnavailableResult (non-consumable)", isinstance(r, ProviderUnavailableResult) and r.consumable is False)
    chk("candidate adapter declares candidate status + no truth", GcpCloudRunFunctionCandidate().describe()["owns_truth"] is False)
