...
CLI:
    python3 scripts/check_experiments_isolation.py --self-test
"""
...
_TEMPLATE = _REPO / "experiments" / "_template" / "experiment.py"
...
def _self_test() -> int:
    ...
    check("experiments/README.md exists", (_REPO / "experiments" / "README.md").exists())
    check("experiments/_template/experiment.py exists", _TEMPLATE.exists())
    check("a seed experiment exists (masfactory_context_swarm)",
          (_REPO / "experiments" / "masfactory_context_swarm" / "run.py").exists())
