...
deterministic, and finds the on-topic doc. serves_truth=false (it ranks candidates, it does not verify them).
  ...
  python3 scripts/check_hybrid_retrieval.py --self-test
"""
from __future__ import annotations

from src.teleon.retrieval.hybrid import RRF_K, hybrid_search, reciprocal_rank_fusion
...
def _self_test() -> int:
    ...
    ck("RRF score = sum of 1/(k+rank) across lists", abs(dict(fused)["a"] - (1 / (RRF_K + 1) + 1 / (RRF_K + 1))) < 1e-9)
    # a doc present in only one list still contributes (recall from either signal)
    fused2 = reciprocal_rank_fusion([["x"], ["y"]])
    ck("RRF: a doc in only ONE list still scores (hybrid recall)", set(k for k, _ in fused2) == {"x", "y"})
    ck("RRF: empty input -> empty", reciprocal_rank_fusion([]) == [])
    ...
    r1 = hybrid_search("revenue growth report", docs)
    r2 = hybrid_search("revenue growth report", docs)
    ...
    ck("hybrid_search: top=2 truncates", len(hybrid_search("revenue", docs, top=2)) == 2)
    ck("hybrid_search: empty docs -> empty", hybrid_search("x", []) == [])
