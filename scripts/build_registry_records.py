...
tier, not git). serves_truth=false; candidates only (promotion boundary applies).
  ...
  python3 scripts/build_registry_records.py            # write manifest + pgvector plan + sample
  python3 scripts/build_registry_records.py --self-test
"""
...
from src.teleon.registry.records import EMBED_DIM, build_records, real_records, record_search  # noqa: E402

_TARGET = 1000
_SAMPLE_PER_REG = 4
_EMBED_PREVIEW = 6
_DIR = _REPO / "data" / "dev-intel"
...
def main() -> int:
    ...
    if not args.self_test:
        ...
        print(f"wired {m['registries_wired']} registries -> {m['totals']['records']} records "
              ...
              f"manifest + pgvector plan (vector({EMBED_DIM})) + sample written")
        return 0
    ...
    recs = build_records("component", target=_TARGET)
    ...
    ck("every record has embedding + description + metadata", all(
        len(r["embedding"]) == EMBED_DIM and r["description"] and r["metadata"]["keywords"] for r in recs[:50]))
    ...
    hits = record_search("pdf document parser", recs, limit=5)
    ...
    plan = _pgvector_plan()
    ck("pgvector DDL uses vector(EMBED_DIM) single-source", f"vector({EMBED_DIM})" in plan)
