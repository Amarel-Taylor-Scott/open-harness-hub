...
components per ladder rung (the search-driven option set the synthesizer branches over). serves_truth=false.
  ...
  python3 scripts/check_component_search.py --self-test
"""
...
from src.teleon.synthesis.component_search import INDEX_DIM, compose, embed, index_entry, search
...
def _self_test() -> int:
    ...
    v1, v2 = embed("fast cross-encoder reranker"), embed("fast cross-encoder reranker")
    ck("embedding deterministic", v1 == v2)
    ck(f"embedding fixed-dim ({INDEX_DIM}) + L2-normalized", len(v1) == INDEX_DIM and abs(math.sqrt(sum(x * x for x in v1)) - 1.0) < 1e-6)
    ck("different text -> different vector", embed("ocr scanned document") != v1)
    ...
    ck("every component labelled with text+keywords+vector", all(r["text"] and r["keywords"] and len(r["vector"]) == INDEX_DIM for r in idx))

    rerank = search("cross encoder reranker relevance", k=5, index=idx)
    ...
    fc = search("forecast a time series", k=5, index=idx)
    ...
    ocr = search("read text from a scanned image", k=5, index=idx)
    ...
    ck("kind filter works", all(h["kind"] == "tool" for h in search("reranker", k=5, index=idx, kind="tool")))

    # compose: candidate components per rung for a real capability
    comp = compose("document_extraction", k=3, index=idx)
