...
serves_truth=false.
  ...
  python3 scripts/check_registry_storage_tiers.py --self-test
"""
...
def _self_test() -> int:
    pol = json.loads((REPO / "architecture" / "storage_tier_policy.json").read_text(encoding="utf-8"))
    ...
    jsonl = REPO / streams["tool_registry_staged"]["jsonl"]
    if jsonl.exists():
