#!/usr/bin/env python3
"""scripts.check_standards_review_pack — proof: docs/status/standards-review-pack.md is grounded.
...
CLI: python3 scripts/check_standards_review_pack.py --self-test
"""
...
_PACK = _REPO / "docs" / "status" / "standards-review-pack.md"
...
def _self_test() -> int:
    ...
    catalogs_on_disk = [c for c in _REQUIRED_CATALOGS if not (_REPO / c).exists()]
    ...
    proofs_on_disk = [p for p in _REQUIRED_PROOFS if not (_REPO / p).exists()]
    ...
    missing_cited = sorted(p for p in cited if not (_REPO / p).exists())
    ...
    missing_ap = [p for p in ap_paths if not (_REPO / p).exists()]
