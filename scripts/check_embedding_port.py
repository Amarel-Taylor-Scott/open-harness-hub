...
register_embedder_adapter; adapter_layers marks embedding WIRED + drop-in-tested. serves_truth=false.
  ...
  python3 scripts/check_embedding_port.py --self-test
"""
...
from src.teleon.retrieval.embedding_port import (INDEX_DIM, LexicalEmbedder, NotWiredEmbedder, available_embedders,
                                                 register_embedder_adapter, select_embedder)
...
def _self_test() -> int:
    ...
    e = select_embedder("auto")
    ck("'auto' -> always-available lexical embedder", isinstance(e, LexicalEmbedder))
    v = e.embed("governed AI context")
    ck(f"fixed-dim ({INDEX_DIM}) + L2-normalized + deterministic",
       len(v) == INDEX_DIM and abs(math.sqrt(sum(x * x for x in v)) - 1.0) < 1e-6 and v == select_embedder("auto").embed("governed AI context"))
    ck("different text -> different vector", e.embed("ocr scanned doc") != v)
    m = select_embedder("openai_embed")
    ck("a model embedder is honest 'not wired'", isinstance(m, NotWiredEmbedder))
    try:
        m.embed("x"); ck("not-wired raises on use", False)
    except RuntimeError:
        ck("not-wired raises on use", True)
    av = available_embedders()
    ...
    register_embedder_adapter("my_future_embedder", LexicalEmbedder)
    ck("drop-in: a newly registered embedder is selectable", select_embedder("my_future_embedder").name == "lexical")
    layer = next((l for l in json.loads((REPO / "architecture" / "adapter_layers.json").read_text())["layers"] if l["layer"] == "embedding"), None)
