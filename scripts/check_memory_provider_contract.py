...
* MemoryArtifact.v1 — the boundary — REQUIRES external_source_handle + tenant_id + scope + container +
...
* MemoryProfile.v1 is {static:[...], dynamic:[...]} and every entry REFERENCES a MemoryArtifact/handle
  (artifact_id + external_source_handle), never an inline asserted fact.
* MemoryProviderStatus.v1.credential_ref is an env://… (or secret://…) REFERENCE, never an inline secret VALUE —
...
Deterministic, stdlib-only, offline. CLI: python3 scripts/check_memory_provider_contract.py --self-test
"""
...
_SCHEMA_DIR = _REPO / "schemas" / "memory"
...
#: the governance fields MemoryArtifact.v1 MUST require (no recall becomes an artifact without these).
...
def _self_test() -> int:
    ...
    # every schema declares $id memory/<Stem>.v1, type object, additionalProperties false, required + properties,
    ...
    for stem, sc in schemas.items():
        check(f"{stem}.v1 $id is memory/{stem}.v1", sc.get("$id") == f"memory/{stem}.v1", str(sc.get("$id")))
    ...
    for stem, ex in examples.items():
        ...
        check(f"{stem}.v1 has at least one 'valid' example", len(valids) >= 1)
        for k, v in valids:
            errs = _validate(v, sc)
        ...
        for k, v in invalids:
            errs = _validate(v, sc)
            check(f"{stem}.v1 {k} is correctly REJECTED", errs != [])

    # ---- MemoryArtifact.v1 governance boundary ----
    ma = schemas.get("MemoryArtifact", {})
    req = set(ma.get("required", []))
    check("MemoryArtifact.v1 requires external_source_handle/tenant_id/scope/container/claim_status/content/provider_id/lineage/created_at",
    ...
    if base and ma:
        ...
        for f in sorted(_MEMORY_ARTIFACT_REQUIRED):
            ...
            check(f"MemoryArtifact.v1 rejects an artifact missing '{f}'", _validate(broken, ma) != [])
        ...
        check("MemoryArtifact.v1 claim_status enum == {candidate,promoted,held_out}", cs_enum == _CLAIM_STATUS_ENUM, str(sorted(cs_enum)))
        for good in sorted(_CLAIM_STATUS_ENUM):
            ok_art = dict(base); ok_art["claim_status"] = good
            check(f"MemoryArtifact.v1 accepts claim_status='{good}'", _validate(ok_art, ma) == [])
        bad_claim = dict(base); bad_claim["claim_status"] = "served"
        check("MemoryArtifact.v1 rejects claim_status='served' (remembered != served)", _validate(bad_claim, ma) != [])
        ...
        check("MemoryArtifact.v1 rejects an out-of-enum scope", _validate(bad_scope, ma) != [])
        ...
        check("MemoryArtifact.v1 has NO field asserting it is canonical/served/verified by itself",
              present_forbidden == set(), str(sorted(present_forbidden)))

    # ---- MemorySearchResult.v1: a recall result can only be candidate|held_out (never mints 'promoted') ----
    ...
    if sr:
        ...
        check("MemorySearchResult.v1 result claim_status enum == {candidate,held_out} (recall != promotion)",
              entry_enum == {"candidate", "held_out"}, str(sorted(entry_enum)))

    # ---- MemoryProfile.v1: {static:[...], dynamic:[...]}, each entry REFERENCES an artifact/handle ----
    mp = schemas.get("MemoryProfile", {})
    if mp:
        props = mp.get("properties", {})
        mp_req = set(mp.get("required", []))
        check("MemoryProfile.v1 has static + dynamic arrays (required)",
        ...
        for side in ("static", "dynamic"):
            ...
            check(f"MemoryProfile.v1 {side} entries reference a MemoryArtifact/handle (artifact_id + external_source_handle required)",
                  {"artifact_id", "external_source_handle"} <= entry_req, str(sorted({"artifact_id", "external_source_handle"} - entry_req)))

    # ---- MemoryProviderStatus.v1: credential_ref is an env://… REFERENCE, never a VALUE ----
    ...
    if ps:
        ...
        check("MemoryProviderStatus.v1 status enum == {active,candidate,emulated,unavailable}",
    ...
    check("MemoryProviderStatus.v1 valid example credential_ref is an env:// reference (not a value)",
    ...
    check("MemoryProviderStatus.v1 emulator status: emulated + available=true + no credential_ref (offline correctness invariant)",
    ...
    check("MemoryProviderStatus.v1 credential_ref-as-a-value is DETECTED (the ref rule actually bites)",
