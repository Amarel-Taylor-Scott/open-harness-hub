...
class CFPBFixtureSource(Processor):
    ...
    def handle(self, command, ctx):
        ...
        arts = [_artifact(command, artifact_type="source_record", schema="SourceRecord.v1",
        ...
        for rec in records:
            ...
            for c in decompose_cfpb_complaint(rec, native_id=f"cfpb:{sid}")["components"]:
                if c["claim_status"] == CLAIM_FACT:
                    arts.append(_artifact(command, artifact_type="atomic_fact", schema="AtomicFact.v1", key=c["fact_id"],
                    ...
                    arts.append(_artifact(command, artifact_type="narrative_allegation", schema="NarrativeAllegation.v1",
                                          key=c["fact_id"],
                    ...
                    alleg += 1
        ev = EventEnvelope(type="baltor.pipeline.step.completed.v1", source="baltor.decompose.cfpb_structured",
...
class ContextPackBuilder(Processor):
    ...
    def handle(self, command, ctx):
        ...
        pack = _artifact(command, artifact_type="context_pack", schema="ContextPack.v1", key="final",
                         payload={"included": included, "held_out": held_out}, claim_status="context_pack", promo=False)
        receipt = _artifact(command, artifact_type="receipt", schema="Receipt.v1", key="final",
...
class DeterministicVectorizer(Processor):
    ...
    def handle(self, command, ctx):
        ...
        arts = [_artifact(command, artifact_type="embedding", schema="ArtifactEnvelope.v1", key=str(i),
...
class DeterministicGraphBuilder(Processor):
    ...
    def handle(self, command, ctx):
        ev = EventEnvelope(type="baltor.graph.edge.created.v1", source="baltor.graph.deterministic_edges",
