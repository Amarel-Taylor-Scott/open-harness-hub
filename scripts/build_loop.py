...
HONEST proof gate (scripts/run_proofs.py) as the objective arbiter — committing ONLY on green and reverting
...
CLI: PYTHONPATH=. python3 scripts/build_loop.py --run --harness opencode
"""
...
PROPOSALS = REPO / "data" / "dev-intel" / "proposals.jsonl"
STATE = REPO / "data" / "dev-intel" / "build_loop_state.json"
LEDGER = REPO / "data" / "dev-intel" / "build_loop.jsonl"
STOP = REPO / ".agent" / "BUILD_STOP_REQUESTED"
GOAL_DEFAULT = REPO / "docs" / "goals" / "ollama-build-loop.md"

DEFAULT_HARNESS = "opencode"
DEFAULT_MODEL = "kimi-k2.7-code"          # coding-specialized; --architect adds a GLM 5.2 plan pass (aider)
ARCHITECT_MODEL = "glm-5.2"
GATE_CMD = ["python3", "scripts/run_proofs.py"]   # exit code == number of red proofs (0 = green)
...
def run_gate(substr: str | None, timeout: int = 1200) -> tuple[bool, int, str]:
    cmd = ["python3", "scripts/run_proofs.py"] + ([substr] if substr else [])
