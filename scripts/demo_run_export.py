...
`graph` (nodes/edges for visualization), and the headline. `web/baltor/demo-console.html` loads this
...
`cfpb` is a LABELED SEAM — the live CFPB connector (`scripts/demo_cfpb_context_pack.py`) runs the
same motion over a public source, but it needs network and is recorded here as available-on-request,
never faked.

CLI:
    python3 scripts/demo_run_export.py --self-test        # offline, deterministic assertions
    python3 scripts/demo_run_export.py --write            # write web/baltor/demo-run.json
    python3 scripts/demo_run_export.py --corpus acme
"""
...
OUT_FILES: dict[str, Path] = {
    "acme": _REPO / "web" / "baltor" / "demo-run.json",
    "cfpb": _REPO / "web" / "baltor" / "demo-run.cfpb.json",
}
#: the human-in-the-loop review queue (across all corpora) the reviews.html page reads.
REVIEW_QUEUE_FILE = _REPO / "web" / "baltor" / "review-queue.json"
#: the append-only object version timelines (v1 stale → v2 proposed-pending-review).
VERSION_TIMELINE_FILE = _REPO / "web" / "baltor" / "version-timeline.json"
...
#: The LIVE CFPB API (scripts/demo_cfpb_context_pack.py) is a SEPARATE path (needs network), not this.
...
def _self_test() -> int:
    ...
    try:
        ...
        schema = json.loads((_REPO / "schemas/governance/steward-review-request.schema.json").read_text())
    ...
    try:
        from jsonschema import Draft202012Validator
        cv = Draft202012Validator(json.loads((_REPO / "schemas/context-version.schema.json").read_text()))
    ...
    # ── DRIFT-CHECK: the committed web/baltor/*.json must byte-match their builders ──
...
def _main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--write", action="store_true", help="(re)write web/baltor/demo-run*.json for ALL corpora")
