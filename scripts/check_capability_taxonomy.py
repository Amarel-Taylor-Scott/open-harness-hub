...
the runnable set (real unbounded AI uses queued for grids). serves_truth=false.
  ...
  python3 scripts/check_capability_taxonomy.py --self-test
"""
...
TAX = REPO / "architecture" / "capability_taxonomy.json"
PLANES = REPO / "architecture" / "tool_planes.json"
CATALOG = REPO / "architecture" / "tunable_task_catalog.json"
...
def _self_test() -> int:
    ...
    pmiss = [p["plane"] for p in planes["planes"] if p["status"] == "wired"
             and not (p.get("port_module") and (REPO / p["port_module"]).exists())]
