...
Proves the inference lanes the owner uses are wired, not just listed:
  * each lane's ``base_url`` exists as a governed node in ``architecture/model_provider_graph.json``,
...
CLI: python3 scripts/check_model_provider_lanes.py --self-test
"""
...
_GRAPH = _REPO / "architecture" / "model_provider_graph.json"
