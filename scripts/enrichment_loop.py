...
honest. The loop never crashes on a task error or a red gate; it records and continues for YOU to review.
  ...
  python3 scripts/enrichment_loop.py --supervise               # the 3-day loop (every 30 min); ./enrich start wraps this
  python3 scripts/enrichment_loop.py --run                     # one cycle (foreground)
  python3 scripts/enrichment_loop.py --status                  # read-only monitor
  python3 scripts/enrichment_loop.py --supervise --days 3 --interval-min 30 --offline
  python3 scripts/enrichment_loop.py --self-test               # offline wiring check (no subprocess)
"""
...
_STATE = _REPO / "data" / "dev-intel" / "enrichment_loop_state.json"
_LOG = _REPO / "data" / "dev-intel" / "enrichment_loop.log"
...
def _self_test() -> int:
    ...
    ck("all task scripts exist on disk", all((_REPO / t["argv"][0]).exists() for t in TASKS),
       str([t["argv"][0] for t in TASKS if not (_REPO / t["argv"][0]).exists()]))
