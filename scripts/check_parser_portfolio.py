...
"""check_parser_portfolio — proof for the swappable parser portfolio (the parser-portfolio gap, 2026-06-18):
  ...
  1. ``schemas/ParsedDocument.v1.schema.json`` is the canonical, engine-neutral parsed-document artifact — a
...
_SCHEMA_PATH = _REPO / "schemas" / "ParsedDocument.v1.schema.json"
...
def _self_test() -> int:
    ...
    ck("ParsedDocument.v1 is a valid JSON Schema", True)
    validator = Draft202012Validator(schema)

    out = adjudicate([{"engine": "docling", "parsed": _CLEAN}, {"engine": "mineru", "parsed": _DEGRADED}])
    ck("the adjudicator output conforms to ParsedDocument.v1",
    ...
    print("\n" + ("PASS — check_parser_portfolio: ParsedDocument.v1 is the canonical engine-neutral artifact "
