...
CLI: PYTHONPATH=. python3 scripts/check_parallel_dev_lanes.py --self-test
"""
...
def _self_test() -> int:
    ...
    ck("the proof registry + count token are declared shared-serialized (the real conflict files)",
       {"scripts/flywheel_proof_modules.py", "docs/strategy/yc-master-current-state-business-plan-and-pitch.md"} <= shared,
       str(shared))
