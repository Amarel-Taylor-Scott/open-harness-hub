...
from src.teleon.agents.agent_runtime_provider import (  # noqa: E402
    LOCAL_EMULATOR_RUNTIME_ID, AgentRuntimeRequest, dispatch_agent_request, load_catalog,
)
...
def find_violations(catalog: dict, outcomes: list[dict], *, module_src: str, generic: set) -> list[str]:
    ...
    if not (len(active) == 1 and active[0].get("runtime_id") == LOCAL_EMULATOR_RUNTIME_ID):
    ...
    for o in outcomes:
        ...
        if o.get("runtime_status") in ("unknown", "unavailable") and o.get("result_kind") != "AgentRuntimeUnavailableResult":
...
def _outcome(env: dict, *, worker_bucket: str, allow_generic_proof: bool = False, crashed: bool = False) -> dict:
    ...
    return {"runtime_id": env["runtime_id"], "runtime_status": env["runtime_status"],
            ...
            "result_kind": type(res).__name__, "crashed": crashed}


def main() -> int:
    gen = generic_functions()
    cat = load_catalog()
    module_src = (_REPO / "src" / "teleon" / "agents" / "agent_runtime_provider.py").read_text()

    # ── CONTROL: real catalog + real dispatch outcomes + real module source → MUST be clean ──
    e_local = dispatch_agent_request(
        AgentRuntimeRequest(task_id="T1", tenant_id="acme", runtime_id=LOCAL_EMULATOR_RUNTIME_ID, intent="triage"), now=NOW)
    e_hermes = dispatch_agent_request(
        AgentRuntimeRequest(task_id="T2", tenant_id="acme", runtime_id="hermes@candidate", intent="browse+act"), now=NOW)
    e_unknown = dispatch_agent_request(
        AgentRuntimeRequest(task_id="T3", tenant_id="acme", runtime_id="nope@x", intent="x"), now=NOW)
    ...
    attack("A7 unknown not degraded",
           outcomes=[{**control[2], "result_kind": "AgentRuntimeResult"}], expect="not_degraded")
