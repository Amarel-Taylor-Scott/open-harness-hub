...
that FAILS, and SourceArtifact.v1 — the governance boundary — REQUIRES tenant_id/source_id/source_version/
...
#: the governance fields SourceArtifact.v1 MUST require (no raw source becomes an artifact without these).
...
def _self_test() -> int:
    ...
    # every schema declares $id ingestion/<Stem>.v1, type object, additionalProperties false, required + properties
    for stem, sc in schemas.items():
        check(f"{stem}.v1 $id is ingestion/{stem}.v1", sc.get("$id") == f"ingestion/{stem}.v1", str(sc.get("$id")))
    ...
    # SourceArtifact.v1 governance: REQUIRES the full provenance/scope/authority field set
    sa = schemas.get("SourceArtifact", {})
    req = set(sa.get("required", []))
    check("SourceArtifact.v1 requires tenant/source/source_version/handle/content_hash/scope/authority/type/created_at",
    ...
    if base and sa:
        for f in sorted(_SOURCE_ARTIFACT_REQUIRED):
            ...
            check(f"SourceArtifact.v1 rejects an artifact missing '{f}'", _validate(broken, sa) != [])
        ...
        check("SourceArtifact.v1 rejects an out-of-enum scope", _validate(bad_scope, sa) != [])
        bad_auth = dict(base); bad_auth["authority"] = "vibes"
        check("SourceArtifact.v1 rejects an out-of-enum authority", _validate(bad_auth, sa) != [])
    ...
    print(f"\n{'PASS — check_ingestion_contracts: 7 ingestion schemas exist + validate; each has a passing valid example and a rejected invalid example; SourceArtifact.v1 requires the full tenant/source/scope/authority/handle/content_hash governance set (each field load-bearing); example matches the normalize() seam.' if not fails else f'{len(fails)} FAILURES: {fails}'}")
