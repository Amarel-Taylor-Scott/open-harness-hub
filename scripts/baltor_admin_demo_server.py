...
def context_object_schema_payload() -> dict:
    schema_path = REPO_ROOT / "schemas" / "context-object.schema.json"
    try:
...
def _object_governance_visibility(sql_text: str) -> dict:
    try:
        ...
        seed_coverage = {}
        concrete_coverage = {}
        concrete_profile_seed = REPO_ROOT / "db/seeds/object-governance/concrete_object_governance_profile.jsonl"
    else:
...
def _archive_candidate_visibility() -> dict:
    ledger_path = REPO_ROOT / "docs/archive/archive-candidate-ledger.md"
    text = ""
    try:
...
def operational_readiness_payload() -> dict:
    schema_path = REPO_ROOT / "db/postgres/schema.sql"
    try:
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _serve_web(self, name: str) -> None:
        ...
        try:
            self.send_bytes(200, (Path(__file__).resolve().parents[1] / "web" / "baltor" / name).read_bytes())
        except OSError:
            self.send_bytes(404, f"{name} not found".encode(), "text/plain")

    def _serve_pipeline_file(self, rel: str) -> None:
        """Serve a projection page or the shared css from web/baltor/pipeline/ (open GET).
        ...
        try:
            self.send_bytes(200, (Path(__file__).resolve().parents[1] / "web" / "baltor" / "pipeline" / rel).read_bytes(), ctype)
        except OSError:
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if path.startswith("/api/graph/temporal/"):
            ...
            self.send_bytes(code, json.dumps(payload, indent=2).encode(), "application/json")
            return
        # Shared LLM Plane (Inference Gateway / OIPS) — projection over src/teleon/inference; no raw key, output never truth.
        ...
        if path in {"/", "/admin-demo"}:
            ...
            try:
                body = (Path(__file__).resolve().parents[1] / "web" / "baltor" / "dashboard.html").read_bytes()
            ...
            try:
                body = (Path(__file__).resolve().parents[1] / "web" / "baltor" / "dev-dashboard.html").read_bytes()
                self.send_bytes(200, body)
            except OSError:
