...
CLI: PYTHONPATH=. python3 scripts/check_teleon_control_plane.py --self-test
"""
...
def _self_test() -> int:
    ...
    lanes = json.loads((REPO / "architecture" / "lowcost_llm_endpoint_registry.json").read_text())["entries"]
    ...
    ck("the final unit runs on the customer's account (compute_ownership=customer_account)",
       CloudflareWorkersProvider().compute_ownership == "customer_account")

    # the positioning is documented (single source, non-fragile)
    ck("the control-plane positioning is documented",
       (REPO / "docs" / "architecture" / "teleon-control-plane-vs-compute.md").exists())
