...
CLI: PYTHONPATH=. python3 scripts/check_default_brain_policy.py --self-test
"""
...
def _self_test() -> int:
    ...
    idx = {e["model_id"]: e for e in json.loads((_REPO / "architecture" / "model_index.json").read_text())["entries"]}
    cheaper = []
    for r in rs:
