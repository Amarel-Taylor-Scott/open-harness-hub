...
the repo has TWO promotion notions that never touch —
  (a) the Teleon runtime gate (`scripts/teleon_local_runtime.py`): train+holdout
  ...
  (b) `scripts/eval/measured_lift_headtohead.py`: the REAL two-axis discipline
...
`scripts/eval/reason_codes.py`, `scripts/eval/durable_gap_harness.py`) says the
...
# has scripts/eval/__init__.py so `-m` works without this). Stdlib only; no-op
...
# stated here (and in docs/status/p2-measured-lift-promotion.md) so the bridge can
