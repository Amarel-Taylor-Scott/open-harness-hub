...
Asserts:
  ...
  J. DEPENDENCY LAW: src/teleon/resources never imports src.baltor; no new top-level shared-platform/ tree.
...
def _self_test() -> int:
    ...
    dangling = [c["schema"] for L in spine["layers"] for c in L["contracts"]
                if c["status"] in ("exists", "new") and not (_REPO / c["schema"]).exists()]
    ...
    bad_imp = [str(p.relative_to(_REPO)) for p in (_REPO / "src" / "teleon" / "resources").rglob("*.py")
               for line in p.read_text().splitlines() if line.strip().startswith(("import src.baltor", "from src.baltor"))]
    check("J: src/teleon/resources never imports src.baltor", not bad_imp, "; ".join(bad_imp))
    check("J: no parallel shared-platform/ tree (reuse schemas/ + src/teleon)", not (_REPO / "shared-platform").exists())
