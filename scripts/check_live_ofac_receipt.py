...
B1 in docs/goals/yc-readiness-loop.md requires one real ``--live`` OFAC run to be
...
RECEIPT_DIR = REPO / "data" / "live-receipts"
...
def _self_test() -> int:
    ...
    ev_path = REPO / "docs" / "strategy" / "evidence" / f"ofac-live-run-{source.get('list_date')}.json"
