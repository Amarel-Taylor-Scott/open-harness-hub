...
Reads the SINGLE sources (no hand-typed roster):
  * architecture/portfolio_connection_map.json — parent / core (Baltor, Teleon) / open_ecosystem / 22 hubs (tiers) / dependency_law
  * architecture/surface_map.json              — the 7 top-level surfaces + wedge + communicates_with
  * architecture/teleon_demo_catalog.json      — the demo catalog
  * docs/use-cases/*.md                        — the use-cases

Emits:
  * dist/architecture/index.html        — a self-contained, layered VISUAL map (open in a browser to review)
  * docs/strategy/architecture-map.md   — the documented map (a Mermaid diagram + honest layer/surface notes) for the north star

Layers are DERIVED from the connection map's roles + the dependency law (Baltor → Teleon → OpenHarnessHub), so the
...
CLI: PYTHONPATH=. python3 scripts/build_architecture_map.py --build
"""
...
SURF = REPO / "architecture" / "surface_map.json"
DEMOS = REPO / "architecture" / "teleon_demo_catalog.json"
USECASES = REPO / "docs" / "use-cases"
HTML_OUT = REPO / "dist" / "architecture" / "index.html"
DOC_OUT = REPO / "docs" / "strategy" / "architecture-map.md"
...
def load() -> dict:
    ...
    hubs = conn.get("hubs", [])
    live = [h for h in hubs if h.get("tier") == "live"]
    private = [h for h in hubs if h.get("tier") != "live"]
...
def render_html(d: dict) -> str:
    ...
    return "".join([
        ...
        "<p class=sub>Top → bottom follows the enforced dependency law: <b>Baltor → Teleon → OpenHarnessHub</b>, never "
...
def _self_test() -> int:
    ...
    d = load()
    ck("roster derived from the single source = 22 hubs (9 live + 13 private)", len(d["live_hubs"]) == 9 and len(d["private_hubs"]) == 13)
