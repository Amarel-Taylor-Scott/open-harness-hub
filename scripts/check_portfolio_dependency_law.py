...
Portfolio (owner-directed 2026-06-06; architecture/portfolio_dependency_law.json): a holding company owns
OpenHarnessHub (open ecosystem + spec), Teleon (purpose-driven runtime SaaS), and Baltor (applied product, a
TENANT of Teleon). The law: Baltor → Teleon → OpenHarnessHub; never the reverse.
...
Asserts:
  ...
  B. NO FORBIDDEN IMPORT EDGE exists in code: scanning real import statements under each layer's package roots,
     no module imports a layer it must not (teleon↛baltor; openharnesshub↛baltor; openharnesshub↛teleon).
  C. SELF-CONSISTENCY: every may_depend_on target is a real layer; a layer never lists itself.
  D. MIGRATION DEBT is TRACKED (not silent): the generic modules still under src/baltor that must move into
...
_LAW = _REPO / "architecture" / "portfolio_dependency_law.json"


def _module_prefix(package_root: str) -> str:
    """'src/teleon' -> 'src.teleon' (the import path a forbidden edge would use)."""
...
def _self_test() -> int:
    ...
    check("A: three layers present", names == {"openharnesshub", "teleon", "baltor"}, str(sorted(names)))
    ...
    print("    migration debt (generic code still under src/baltor, to move into src/teleon):")
    for d in debt:
        print(f"      - {d}")

    print("\n" + ("PASS — check_portfolio_dependency_law: the portfolio law is well-formed and holds in code — "
                  "Teleon never imports Baltor, OpenHarnessHub imports neither; Baltor→Teleon→OpenHarnessHub is "
