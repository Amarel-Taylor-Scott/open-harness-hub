...
DEFAULT_OUT = REPO / "dist" / "catalog-manifest-bridge"
...
def _iter_manifest_paths(paths: list[Path] | None) -> Iterable[Path]:
    if paths:
        for path in paths:
            yield path if path.is_absolute() else (REPO / path)
        return
...
def _context_object_id(component_id: str) -> str:
    return "ctx://open-harness-hub/component/" + component_id
...
def _component_version_row(doc: dict[str, Any], path: Path, definition_hash: str) -> dict[str, Any]:
    return {
        "component_version_id": f"ctxv://open-harness-hub/component/{doc['id']}@{definition_hash}",
        "component_id": doc["id"],
        "version": doc["version"],
