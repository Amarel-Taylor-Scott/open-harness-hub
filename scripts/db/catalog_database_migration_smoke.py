...
DEFAULT_OUTPUT = REPO / "dist" / "catalog-migration-smoke" / "catalog-database-migration-smoke.json"
DEFAULT_MANIFEST_EXPORT_DIR = REPO / "dist" / "catalog-migration-smoke" / "manifest-export-from-seed-rows"
DEFAULT_PAGES_DIR = REPO / "dist" / "catalog-migration-smoke" / "pages-from-db-rows"
DEFAULT_SQLITE = REPO / "dist" / "catalog-migration-smoke" / "catalog-from-db-rows.sqlite"
DEFAULT_BROWSER_INDEX = REPO / "dist" / "catalog-migration-smoke" / "index-from-db-rows.json"
DEFAULT_VECTOR_INDEX_DIR = REPO / "dist" / "catalog-migration-smoke" / "vector-index-from-db-rows"
DEFAULT_VECTOR_STORE = REPO / "dist" / "catalog-migration-smoke" / "vector-store-from-db-rows.sqlite"
    ...
    *,
    ...
    if db_row_dir.exists():
        ...
        steps.append(step("capability_lift_gate_database_rows", lambda: {
            **{
                key: value
                for key, value in run_capability_lift_gate(REPO / "catalog", row_dir=db_row_dir).items()
                if key != "decisions"
            },
            "ok": True,
        }))
