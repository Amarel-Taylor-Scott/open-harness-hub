...
a cycle FAILS; an un-typed plane never false-fails; the dry-run executes through the real pipeline_dag engine.
  ...
  python3 scripts/check_dag_contract.py --self-test
"""
...
def _self_test() -> int:
    ...
    good_edges = [["x", "y"], ["y", "z"]]
    v = verify_buildable_dag(good_nodes, good_edges)
    ...
    bad = verify_buildable_dag([{"step": "a", "plane": "tts"}, {"step": "b", "plane": "field_parsing"}], [["a", "b"]])
    ...
    uns = verify_buildable_dag([{"step": "r", "plane": "reranker"}], [], graph_inputs=("document",))
    ...
    # cycle
    cyc = verify_buildable_dag([{"step": "a", "plane": "llm"}, {"step": "b", "plane": "llm"}], [["a", "b"], ["b", "a"]])
    ...
    unt = verify_buildable_dag([{"step": "a", "plane": "ocr"}, {"step": "b", "plane": "caching"}], [["a", "b"]])
