...
(docs/codex/no-magic-values.md) an actual SCANNER, not just a doc + ad-hoc per-check enforcement.
...
Usage:
  python3 scripts/audit_magic_numbers.py [PATH] [--top N]   # report (default PATH = src/teleon)
  python3 scripts/audit_magic_numbers.py --self-test         # embedded test; exit nonzero on failure
...
def main() -> int:
    ...
    ap.add_argument("path", nargs="?", default="src/teleon", help="file or dir to audit (default: src/teleon)")
    ...
    target = (_REPO / args.path) if not Path(args.path).is_absolute() else Path(args.path)
