...
one and FAILS a non-conforming one. serves_truth=false.
  ...
  python3 scripts/check_component_standardization.py --self-test
"""
from __future__ import annotations

import src.teleon.components.registry as REG
from src.teleon.components import (ComponentUnavailable, assert_conforms, make_component, register_component_invoker,
                                   run_compiled)
...
def _self_test() -> int:
    ...
    _orig_ocr = REG._PLANE_INVOKERS.get("ocr")
    ...
    try:
        ...
        REG._PLANE_INVOKERS.pop("ocr", None)
        if _orig_ocr:
    ...
    REG._PLANE_INVOKERS.pop("ner", None)
