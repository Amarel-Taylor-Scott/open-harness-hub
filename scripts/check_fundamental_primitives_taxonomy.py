...
architecture/fundamental_primitives_taxonomy.json is one single-source map of the system's fundamental primitives
...
CLI: PYTHONPATH=. python3 scripts/check_fundamental_primitives_taxonomy.py --build | --self-test
"""
...
_TAX = _REPO / "architecture" / "fundamental_primitives_taxonomy.json"
_MAP = _REPO / "docs" / "architecture" / "fundamental-primitives-map.md"
...
def _self_test() -> int:
    ...
    missing = sorted({a for f in fams for a in f["canonical_artifacts"] if not (_REPO / a).exists()})
