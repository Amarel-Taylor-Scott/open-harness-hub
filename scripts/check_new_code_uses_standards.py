...
scripts/check_no_direct_provider_bypass.py and scripts/check_file_layout_policy.py — those enforce the
...
of its pattern). Rules enforced:
  ...
  * source adapters (src/baltor/adapters/source/*.py): must declare ``source_type`` + ``parser_provider`` and
    a ``def ingest(`` — i.e. match the SourceAdapter shape (scripts/ingest/source_adapters.py#SourceAdapter);
  * proof scripts (scripts/check_*.py): must support ``--self-test``;
  * docs (docs/standards/*.md): must carry a top-level ``# `` heading (a titled document);
  * projection code (scripts/runtime/projections.py, src/baltor/api/projections/*.py): must NOT mutate truth
    (no INSERT/UPDATE/DELETE, no sqlite3 import) and must NOT embed secrets;
  * UI pages (web/baltor/*.html): must NOT write durable truth (projection-only);
...
actually bites. Honored exceptions are recorded in architecture/pattern_waivers.json (read here so a waived
deviation does not block — the waiver itself is validated by scripts/check_pattern_waivers.py).

CLI: python3 scripts/check_new_code_uses_standards.py --self-test
"""
...
_SOURCE_ADAPTER_DIR = _REPO / "src" / "baltor" / "adapters" / "source"
_PROOF_GLOB = "scripts/check_*.py"
_STANDARDS_DOCS_DIR = _REPO / "docs" / "standards"
_PROJECTION_FILES = ["scripts/runtime/projections.py"]
_PROJECTION_DIRS = ["src/baltor/api/projections"]
_UI_PAGES_DIR = _REPO / "web" / "baltor"
_CONFIG_TEMPLATES_DIR = _REPO / "templates" / "configs"
_PROCESSOR_GLOBS = ["scripts/runtime/builtin_processors.py", "src/baltor/processors/**/*.py"]
_VAGUE_SCAN_SCOPE = ["scripts", "src/baltor"]
...
def _waived_paths() -> set[str]:
    ...
    wf = _REPO / "architecture" / "pattern_waivers.json"
    if not wf.exists():
        return set()
...
def _self_test() -> int:
    ...
    check("proof scripts exist to lint", len(list((_REPO / "scripts").glob("check_*.py"))) >= 10)
    ...
    try:
        ...
        (tmp / "src" / "baltor" / "processors" / "bad").mkdir(parents=True)
        ...
        (tmp / "src" / "baltor" / "processors" / "bad" / "writer.py").write_text(
            "import sqlite3\nsqlite3.connect('x')\n", encoding="utf-8")
