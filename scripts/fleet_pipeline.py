...
CLI: PYTHONPATH=. python3 scripts/fleet_pipeline.py --self-test
"""
...
def _self_test() -> int:
    ...
    srv = (Path(__file__).resolve().parents[1] / "scripts" / "baltor_admin_demo_server.py").read_text()
    ...
    dash = (Path(__file__).resolve().parents[1] / "web" / "baltor" / "dashboard.html").read_text()
