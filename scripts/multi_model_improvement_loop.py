...
CLI: PYTHONPATH=. python3 scripts/multi_model_improvement_loop.py --run --limit 5
"""
...
LEDGER = REPO / "data" / "dev-intel" / "findings.jsonl"
CURSOR = REPO / "data" / "dev-intel" / "_cursor.json"
_CTX_CAP = 12000   # focused per-target context (compact — one unit, not the whole repo)


def _models() -> list[dict]:
    """The defer-to panel: GLM-5.2 + Kimi-2.7 on the Ollama lane (the working, near-zero-cost lane)."""
    return [{"name": "glm-5.2", "model": "glm-5.2"}, {"name": "kimi-k2.7-code", "model": "kimi-k2.7-code"}]
...
def enumerate_targets(kinds: tuple = _ALL_KINDS) -> list[dict]:
    ...
    if "plane" in kinds or "wedge" in kinds:
        try:
            for s in json.loads((REPO / "architecture" / "surface_map.json").read_text())["surfaces"]:
    ...
    if "presentation" in kinds:
        extra = ["architecture/teleon_pitch_deck.json"] + [str(p.relative_to(REPO)) for p in sorted((REPO / "docs" / "strategy").glob("yc-application*.md"))]
        for rel in extra:
            if (REPO / rel).exists():
                out.append({"id": f"presentation:{rel}", "kind": "presentation", "ref": rel})
    if "demo" in kinds:
        try:
            for dm in json.loads((REPO / "architecture" / "teleon_demo_catalog.json").read_text())["demos"]:
...
def _architecture_context(ref: str) -> str:
    ...
    if ref == "taxonomy":
        vocab = [p.name for p in (REPO / "vocabularies").glob("*.yaml")] if (REPO / "vocabularies").exists() else []
        regs = [p.name for p in (REPO / "architecture").glob("*registry*.json")]
    ...
    if ref == "coordination":
        ...
        try:
            surfaces = json.loads((REPO / "architecture" / "surface_map.json").read_text())["surfaces"]
        ...
        return ("COORDINATION & COHERENCE — do the components coordinate cleanly AND stay flexible? Assess: (a) the "
                "dependency law Baltor->Teleon->OpenHarnessHub holds (never reversed); (b) cross-surface communication "
...
def focused_context(target: dict) -> str:
    ...
    if kind in ("plane", "wedge"):
        try:
            surfaces = {s["id"]: s for s in json.loads((REPO / "architecture" / "surface_map.json").read_text())["surfaces"]}
            s = surfaces.get(ref, {})
...
def review_target(target: dict) -> list[dict]:
    ...
    out = []
    for m in _models():
        res = _chat(m["model"], IMPROVEMENT_SYSTEM, improvement_user(target, ctx))
        out.append({"target": target["id"], "kind": target["kind"], "model": m["name"],
...
def ask_models(question: str, *, context: str = "") -> list[dict]:
    ...
    out = []
    for m in _models():
        res = _chat(m["model"], ASK_SYSTEM, ask_user(question, context))
        out.append({"question": question, "model": m["name"], "insight": res.get("text", ""),
                    "error": res.get("error"), "status": "candidate", "serves_truth": False})
    return out


def run_loop(*, limit: int = 5, kinds: tuple = _SWEEP_KINDS) -> int:
    ...
    if not todo:
        ...
        return 0
    print(f"reviewing {len(todo)} of {len(targets) - len(reviewed)} remaining targets (Kimi + GLM-5.2) ...")
    n = 0
    for t in todo:
        try:
            recs = review_target(t)
        except Exception as e:  # noqa: BLE001 — a single bad target NEVER halts the sweep; record + advance
            recs = [{"target": t["id"], "kind": t["kind"], "model": m["name"], "opportunities": "",
...
def run_all(*, limit: int = 8, kinds: tuple = _ALL_KINDS, forever: bool = False) -> int:
    ...
    while True:
        ...
        try:
            run_loop(limit=limit, kinds=kinds)
...
def _main(argv=None):
    ...
    if "--no-compress" in argv:
        _COMPRESS = False
    limit, kinds, question, ctxfile = 5, _ALL_KINDS, None, None
    ...
    if "--all" in argv or "--forever" in argv:
        return run_all(limit=limit, kinds=kinds, forever="--forever" in argv)
    if "--run" in argv:
        return run_loop(limit=limit, kinds=kinds)
    print("usage: multi_model_improvement_loop.py --self-test | --run [--limit N] | --all | --forever | --kinds ... | --ask \"q\"")
    return 0
