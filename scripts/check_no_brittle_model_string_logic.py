...
Asserts (against the real src/teleon/inference/oips, via a process-local injected graph copy that is restored):
...
def _self_test() -> int:
    ...
    base = json.loads((_REPO / "architecture" / _GRAPH).read_text())
