...
def _seed(cost, latency_ms):
    ...
    OBS.record_observation(rid, source="test", cost=cost, latency_ms=latency_ms)
    return rid
...
def _self_test() -> int:
    ...
    lin = SIM.simulate_dag([{"step": "x", "resource_id": ra}, {"step": "y", "resource_id": rb}], [["x", "y"]])
    ...
    ranked = SIM.rank_candidates([{"nodes": [{"step": "n", "resource_id": exp_r}]},
                                  {"nodes": [{"step": "n", "resource_id": cheap_r}]}], profile=cost_first())
