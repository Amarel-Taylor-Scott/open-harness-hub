...
Drives a LIVE FleetLedger (in-memory, deterministic) through the full lifecycle and asserts:
  A. CONTRACTS: WorkerClaim/AckNackReceipt/DeadLetterEntry/IdempotencyKey.v1 registered; WorkItem ratified to
     CapabilityTask.v1 in the spine (no duplicate schema).
  B. WORK ITEM: enqueue → a work item carrying all WORK_ITEM_CORE_FIELDS, status queued, schema CapabilityTask.v1.
  ...
  I. DETERMINISM (projectors pure) + DEPENDENCY LAW (src/teleon/io never imports src.baltor).
...
def _required(schema_rel: str) -> list[str]:
    return json.loads((_REPO / "schemas" / "io" / schema_rel).read_text())["required"]
...
def _self_test() -> int:
    ...
    check("A: WorkerClaim/AckNackReceipt/DeadLetterEntry/IdempotencyKey.v1 registered",
          all(f"io/{s}.v1.schema.json" in contracts for s in ("WorkerClaim", "AckNackReceipt", "DeadLetterEntry", "IdempotencyKey")))
    spine = json.loads((_REPO / "architecture" / "shared_io_spine.json").read_text())
    wi = next((c for L in spine["layers"] if L["layer"] == "command_work_io" for c in L["contracts"] if c["name"] == "WorkItem.v1"), {})
    check("A: WorkItem ratified to CapabilityTask.v1 (no duplicate schema)", wi.get("status") == "ratified")
    ...
    check("B: work item carries core fields + queued + CapabilityTask.v1",
          all(f in wi_rec for f in WORK_ITEM_CORE_FIELDS) and wi_rec["status"] == "queued" and wi_rec["schema_version"] == "CapabilityTask.v1",
    ...
    check("C: re-enqueue same key → same task (dedup) + IdempotencyKey conforms",
          t_again["task_id"] == t["task_id"] and conforms(idk, "IdempotencyKey.v1.schema.json"))
    ...
    check("D: claim → WorkerClaim conforms (lease owner + lease_until + claimed)",
          ...
          and wc["lease_until"] and wc["claimed_at"] and conforms(wc, "WorkerClaim.v1.schema.json"))
    ...
    check("E: ack → succeeded + AckNackReceipt conforms (result ids)",
          ...
          and conforms(ar, "AckNackReceipt.v1.schema.json"))
    ...
    check("F: nack(retryable) with attempts left → requeued (status queued, lease cleared, attempt incremented)",
          ...
          and nacked["lease_owner"] == "" and conforms(nr, "AckNackReceipt.v1.schema.json"),
          json.dumps(nr))
    ...
    check("G: nack at max_attempts → dead + DeadLetterEntry conforms (error preserved)",
          ...
          and conforms(dl, "DeadLetterEntry.v1.schema.json") and nr3["resulting_status"] == "dead",
          json.dumps(dl))
    ...
    check("H: all work-I/O schemas declare required fields",
          all(_required(f"{s}.v1.schema.json") for s in ("WorkerClaim", "AckNackReceipt", "DeadLetterEntry", "IdempotencyKey")))
    ...
    print("\n" + ("PASS — check_shared_command_work_io: the durable runtime (FleetLedger) produces records that "
                  "conform to the typed work-I/O contracts — work item = CapabilityTask.v1, idempotent enqueue, "
