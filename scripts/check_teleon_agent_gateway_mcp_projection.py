"""check_teleon_agent_gateway_mcp_projection — proof for src/teleon/agent_gateway/mcp_projection.py.
...
Run: PYTHONPATH=. python3 scripts/check_teleon_agent_gateway_mcp_projection.py --self-test
"""
...
from src.teleon.agent_gateway.gateway import AgentCapabilityGateway  # noqa: E402
...
def main() -> int:
    ...
    proj = TeleonMcpProjection(gw)

    # ---- (1) exactly five descriptors, with the five fixed names + required keys --------------------------- #
    descriptors = proj.tool_descriptors()
    check("five_descriptors", len(descriptors) == EXPOSED_TOOL_COUNT == 5,
          f"got {len(descriptors)} (EXPOSED_TOOL_COUNT={EXPOSED_TOOL_COUNT})")
    names = [d.get("name") for d in descriptors]
    check("descriptor_names_exact", names == list(TOOL_NAMES), f"names={names}")
    expected_names = {
        TOOL_LIST_CAPABILITIES, TOOL_DESCRIBE_CAPABILITY, TOOL_RUN_CAPABILITY,
        TOOL_GET_RECEIPT, TOOL_REQUEST_BOUNDARY_EXPANSION,
    }
    ...
    listed = proj.call_tool(TOOL_LIST_CAPABILITIES, {}, now=_NOW)
    ...
    for card in listed.get("capabilities", []):
        ...
        extra = set(card) - set(COMPACT_CARD_FIELDS)
    ...
    cfpb_only = proj.call_tool(TOOL_LIST_CAPABILITIES, {"domain": "cfpb"}, now=_NOW)
    ...
    described = proj.call_tool(TOOL_DESCRIBE_CAPABILITY, {"capability_id": "utility.hash"}, now=_NOW)
    ...
    check("describe_compact", set(card) <= set(COMPACT_CARD_FIELDS) and "capability_id" in card,
          f"keys={sorted(card)}")
    check("describe_no_runtime_internals", not any(f in card for f in _FORBIDDEN_CARD_FIELDS))
    missing = proj.call_tool(TOOL_DESCRIBE_CAPABILITY, {"capability_id": "does.not.exist"}, now=_NOW)
    ...
    got = proj.call_tool(TOOL_GET_RECEIPT, {"receipt_id": receipt_id}, now=_NOW)
    ...
    miss_rcpt = proj.call_tool(TOOL_GET_RECEIPT, {"receipt_id": "agentcaprcpt-deadbeef"}, now=_NOW)
    ...
    if not crashed:
        ...
        check("unknown_tool_structured_error", unknown.get("error") == "unknown_tool",
              f"got {unknown!r}")
        check("unknown_tool_lists_five", unknown.get("available_tools") == list(TOOL_NAMES))
    ...
    bad_input = proj.call_tool(TOOL_DESCRIBE_CAPABILITY, ["not", "a", "dict"], now=_NOW)  # type: ignore[arg-type]
    ...
    src_path = _REPO / "src" / "teleon" / "agent_gateway" / "mcp_projection.py"
    ...
    print(
        f"PASS — check_teleon_agent_gateway_mcp_projection: projection exposes exactly {EXPOSED_TOOL_COUNT} "
        f"MCP tools {list(TOOL_NAMES)}; cards are compact (no runtime/secret/backend fields); "
