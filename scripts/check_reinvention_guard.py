"""check_reinvention_guard — proof for the reinvention guardrail (src/teleon/registry/reinvention_guard.py).
...
from src.teleon.registry.reinvention_guard import check, detect_intent  # noqa: E402
...
def main() -> int:
    ...
    pdf = check("Let me write a function to parse a PDF and extract text from scratch")
    ...
    addr = check("I'll implement my own address validation")
    ...
    novel = check("Let me build a novel quantum-resistant consensus protocol from scratch")
    ck("quiet on genuinely-novel work", novel["fire"] is False)

    # QUIET on non-build messages
    chat = check("what is the weather in Tokyo today")
    ck("quiet on non-build message", chat["fire"] is False and chat["tier"] == 0)

    # Tier 0 detection is sane
    t0 = detect_intent("let me build a CSV parser from scratch")
