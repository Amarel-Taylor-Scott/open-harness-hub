...
CLI: python3 scripts/check_import_boundaries_manifest.py --self-test
"""
...
def _self_test() -> int:
    ...
    m = json.loads((_REPO / "architecture" / "import_boundaries.json").read_text())
    ...
    for f in m["runtime_core_files"]:
        text = (_REPO / f).read_text(encoding="utf-8")
