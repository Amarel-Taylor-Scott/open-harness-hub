...
def find_violations(*, consumer: AgentConsumer, run_result: dict, receipt: dict | None,
    ...
    if boundary_request is not None:
        if boundary_request.get("auto_applied") is True \
                or boundary_request.get("status") != BOUNDARY_STATUS_PENDING:
    ...
    took_llm = rr.get("fallback_used") is True or rec.get("runtime_path") == RUNTIME_PATH_LLM_FALLBACK \
...
def _self_test() -> int:
    ...
    ctrl_card = caps.get_card("cfpb.deadline.verify")
    ...
    check("(c) the live gateway forces boundary expansion to pending/auto_applied False",
          real_b["status"] == BOUNDARY_STATUS_PENDING and real_b["auto_applied"] is False)
    ...
    e_rec["runtime_path"] = RUNTIME_PATH_LLM_FALLBACK
