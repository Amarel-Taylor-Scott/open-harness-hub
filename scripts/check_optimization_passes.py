...
along a descent AXIS. This proves the 10 canonical passes each name a real axis (architecture/descent_method_catalog.json),
...
is computed. serves_truth=false.
  ...
  python3 scripts/check_optimization_passes.py --self-test
"""
...
PASSES = REPO / "architecture" / "optimization_passes.json"


def _self_test() -> int:
    data = json.loads(PASSES.read_text(encoding="utf-8"))
    passes = data["passes"]
    axes = {d["axis"] for d in json.loads((REPO / "architecture" / "descent_method_catalog.json").read_text(encoding="utf-8"))["dimensions"]}
    fails = []
    def ck(n, ok, d=""):
    ...
    missing_mod = [p["pass"] for p in passes if p.get("status") == "have"
                   and not (p.get("module") and (REPO / p["module"]).exists())]
