...
CLI: PYTHONPATH=. python3 scripts/demo_readiness.py --report
"""
...
REPORT = REPO / "data" / "dev-intel" / "demo-readiness.md"
FLYWHEEL_STATE = REPO / "data" / "dev-intel" / "flywheel-state.json"


def _exists(*rel: str) -> bool:
    return all((REPO / r).exists() for r in rel)
...
def _probe_live_proof() -> tuple[bool, str, str]:
    ...
    receipts += list((REPO / "site" / "baltor-demos").glob("*/context-receipt.json")) if (REPO / "site" / "baltor-demos").exists() else []
    ok = bool(receipts)
...
def _probe_deploy() -> tuple[bool, str, str]:
    fly = list((REPO / "fly").glob("*.fly.toml")) if (REPO / "fly").exists() else []
