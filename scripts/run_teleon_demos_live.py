...
CLI: PYTHONPATH=. python3 scripts/run_teleon_demos_live.py --live
"""
...
_BRAIN = REPO / "data" / "descent-attempts" / "live-demos.jsonl"
_DOC = REPO / "demo-data" / "sample-employment-agency-licence.txt"
...
def live_enrichment(store: DescentAttemptStore, query: str, *, llm_fn=None) -> dict:
    ...
    store.append(DescentAttempt("enrichment:live_search_synthesis", "model_downgrade", before, after, "improved",
                                ...
                                raw_ref="src/teleon/dag/real_steps.py",
...
def _registered(pid: str) -> bool:
    reg = json.loads((REPO / "architecture" / "search_provider_registry.json").read_text())
