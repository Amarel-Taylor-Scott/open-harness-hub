...
Asserts:
  ...
  J. DEPENDENCY LAW: no module under src/teleon/digestion imports src.baltor.
...
def _self_test() -> int:
    ...
    adapter_src = (_REPO / "src" / "teleon" / "digestion" / "digester.py").read_text()
    ...
    # J dependency law
    badimp = [str(p.relative_to(_REPO)) for p in (_REPO / "src" / "teleon" / "digestion").rglob("*.py")
              for line in p.read_text().splitlines() if line.strip().startswith(("import src.baltor", "from src.baltor"))]
    check("J: no src/teleon/digestion module imports src.baltor", not badimp, "; ".join(badimp))
