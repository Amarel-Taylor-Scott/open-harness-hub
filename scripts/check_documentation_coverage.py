...
Every critical-path section in ``architecture/section_maturity_matrix.json`` MUST be documented to the
...
1. Every REFERENCE-PATH section resolves to at least one EXISTING doc — either a ``docs[]`` entry in the matrix
   that exists on disk (file OR directory), or a name-matched per-section card under ``docs/`` whose basename
...
2. Every REFERENCE-PATH section that the matrix leaves with an EMPTY ``docs[]`` MUST instead carry a per-section
   card (``docs/section-cards/<section-id>.md``) — so doc presence is provable WITHOUT a matrix edit (the
   matrix ``docs[]`` additions are REPORTED for the main agent to apply, not made here).
3. Every per-section card that exists (a file under ``docs/section-cards/`` named for a ``section_id``) MUST
...
5. A NON-reference section is allowed to have an empty ``docs[]`` ONLY IF it records a ``known_gap`` OR an
   opportunity in ``architecture/opportunities.json`` claims it via ``owner_section`` — otherwise its silence
   is unexplained and fails.

Deterministic: pure filesystem + JSON reads, no clock, no RNG, no network.

CLI: PYTHONPATH=. python3 scripts/check_documentation_coverage.py --self-test
"""
...
_OPPS = _REPO / "architecture" / "opportunities.json"
_DOCS = _REPO / "docs"
...
def _card_path(section_id: str) -> Path:
    """The canonical per-section card location for a section (hyphenated, under docs/section-cards/)."""
    return _CARDS / f"{section_id.replace('_', '-')}.md"


def _cards_by_section() -> dict[str, Path]:
    """Map normalised section_id -> its per-section card file (only files under docs/section-cards/)."""
    out: dict[str, Path] = {}
    if _CARDS.is_dir():
...
def _self_test() -> int:
    ...
    cards = _cards_by_section()  # normalised section_id -> docs/section-cards/<id>.md
    ...
    def _resolves(s: dict) -> bool:
        ...
        if any((_REPO / d).exists() for d in s.get("docs", [])):
            return True
