...
(package vetting). Config: architecture/research_radar.json. Feeds the GitHub Signal Flywheel +
architecture/intelligence_source_registry.json. Pure data in; offline-degrading; Teleon-layer; serves_truth=false.
"""
...
_CONFIG = _REPO / "architecture" / "research_radar.json"
LEDGER = _REPO / "data" / "dev-intel" / "research-radar.jsonl"
...
def _get_json(url: str, *, headers: dict | None = None, timeout: int = 20) -> dict:
    req = urllib.request.Request(url, headers={"User-Agent": "OpenHarnessHub-research-radar/0.1", **(headers or {})})
