...
CLI: python3 scripts/check_ingestion_connector_catalog.py --self-test
"""
...
_CATALOG = _REPO / "architecture" / "ingestion_connector_catalog.json"
_MATRIX = _REPO / "architecture" / "ingestion_maturity_matrix.json"
...
def _fixture_exists(ref: str) -> bool:
    """fallback_fixture may be a file path OR a 'path#anchor' / 'inline ...' / 'scripts/...#symbol' descriptor.
    ...
    return (_REPO / ref.split("#", 1)[0].strip()).exists()
...
def _self_test() -> int:
    ...
    for r in rows:
        ...
        if r.get("status") in _PROVEN_RUNGS:
            if not any((_REPO / ps).is_file() for ps in r.get("proof_scripts", [])):
