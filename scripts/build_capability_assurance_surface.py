...
WORKS with real numbers, and the surfaces flywheel rebuilds it. serves_truth=false.
  ...
  python3 scripts/build_capability_assurance_surface.py
  python3 scripts/build_capability_assurance_surface.py --self-test
"""
...
OUT = REPO / "dist" / "teleon-demos" / "assurance.html"


def _counts() -> dict:
    from scripts.flywheel_proof_modules import PROOF_MODULES
    L = lambda n: json.loads((REPO / "architecture" / n).read_text(encoding="utf-8"))
    nlines = lambda p: sum(1 for ln in (REPO / p).read_text().splitlines() if ln.strip()) if (REPO / p).exists() else 0
    return {
