...
CLI: PYTHONPATH=. python3 scripts/check_worker_k8s_keda_mapping_docs.py --self-test
"""
...
_DOC = _REPO / "docs" / "workers" / "k8s-keda-worker-mapping.md"
_TEMPLATES = [
...
def _self_test() -> int:
    ...
    so = (_REPO / "infra/k8s/capability-worker-scaledobject.yaml.template").read_text()
    ...
    dep = (_REPO / "infra/k8s/capability-worker-deployment.yaml.template").read_text()
