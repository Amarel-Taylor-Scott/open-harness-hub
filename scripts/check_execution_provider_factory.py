...
CLI: PYTHONPATH=. python3 scripts/check_execution_provider_factory.py --self-test
"""
...
from src.teleon.runtime.execution_provider import ExecutionProviderPort, ProviderUnavailableResult
from src.teleon.runtime.execution_providers.factory import get_execution_provider, supported_backends
from src.teleon.runtime.dispatch import dispatch
...
def _self_test() -> int:
    ...
    ck("factory supports Cloudflare + Kubernetes + serverless family + local",
       ...
       <= set(supported_backends()), str(supported_backends()))
    ck("an unknown backend raises (no silent default)", _raises(lambda: get_execution_provider("nope")))
    ...
    for backend, exp_owner in [("cloudflare_workers", "customer_account"), ("k8s_deployment_worker", "customer_managed"),
        ...
        p = get_execution_provider(backend)
        ck(f"{backend}: implements ExecutionProviderPort", isinstance(p, ExecutionProviderPort))
    ...
    k8s_no = get_execution_provider("k8s_deployment_worker", secret_refs={"kube_api_endpoint": "secret://t/ep", "kube_token": "secret://t/tok"}, resolve_secret=lambda r: None)
    ck("k8s without creds is NOT eligible + invoke returns a non-consumable unavailable (falls back)",
       k8s_no.eligible({}, {})["eligible"] is False and isinstance(k8s_no.invoke({}), ProviderUnavailableResult))
    k8s_yes = get_execution_provider("k8s_deployment_worker", secret_refs={"kube_api_endpoint": "secret://t/ep", "kube_token": "secret://t/tok"}, resolve_secret=lambda r: "K8S_TOKEN_SECRET_XYZ", network_ok=True)
    ...
    lam = get_execution_provider("aws_lambda", secret_refs={"aws_access_key_id": "secret://t/k", "aws_secret_access_key": "secret://t/s"}, resolve_secret=lambda r: None)
    ck("aws_lambda is eligible only with AWS creds", lam.eligible({}, {})["eligible"] is False)
    ck("unknown serverless flavor raises", _raises(lambda: get_execution_provider("totally_made_up_lambda")))

    # CONTROL-PLANE DISPATCH: routes to the configured compute, falls back to local when creds/network absent
    d = dispatch("demo", "extraction", {"task_id": "t1", "input_text": "x"}, resolve_secret=lambda r: None, network_ok=False)
