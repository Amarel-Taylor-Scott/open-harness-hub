#!/usr/bin/env python3
"""scripts.check_native_ui — proof (C-NATIVE-UI-1): web/baltor/native.html is a PROJECTION/PREVIEW ONLY.

Model: scripts/check_memory_page_projection_only.py. The native preview page lets a user paste a small
...
CLI: python3 scripts/check_native_ui.py --self-test
"""
...
_PAGE = _REPO / "web" / "baltor" / "native.html"

#: fetch() targets that are allowed (the page may read ONLY the native projection/preview API).
_ALLOWED_FETCH_PREFIX = "/api/native/"

#: the 8 canonical output_mode ids the picker must offer (single source: architecture/native_output_modes.json).
_REQUIRED_MODES = (
...
def _self_test() -> int:
    ...
    check("web/baltor/native.html exists", _PAGE.exists())
    if not _PAGE.exists():
