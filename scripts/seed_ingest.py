...
OUT = REPO / "data" / "dev-intel" / "ingested_candidates.jsonl"
...
def _get(url: str, timeout: int = 25) -> tuple[int, str]:
    req = urllib.request.Request(url, headers={"User-Agent": "OpenHarnessHub-seed-ingest/0.1 (public metadata only)"})
    try:
