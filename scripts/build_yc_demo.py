...
Renders two self-contained, offline artifacts from the canonical sources:
  * dist/yc-demo/deck.html   — the pitch deck rendered from architecture/teleon_pitch_deck.json (presentable slides)
...
CLI: PYTHONPATH=. python3 scripts/build_yc_demo.py --build
"""
...
DECK_JSON = REPO / "architecture" / "teleon_pitch_deck.json"
OUT_DIR = REPO / "dist" / "yc-demo"
...
def render_entry(deck: dict) -> str:
    ...
    receipts = list((REPO / "data" / "live-receipts").glob("*.json")) if (REPO / "data" / "live-receipts").exists() else []
    ...
    return "".join([
        ...
        'dist/ artifacts + yc_readiness. The open store (OpenHubForAI — 22 hubs incl. OpenHarnessHub) supplies components both products '
