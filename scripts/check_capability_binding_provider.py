...
CLI: PYTHONPATH=. python3 scripts/check_capability_binding_provider.py --self-test
"""
...
_POLICY = json.loads((_REPO / "architecture" / "execution_backend_policy_matrix.json").read_text(encoding="utf-8"))
_NOW = "2026-06-08T00:00:00Z"
_BINDING_MODULE = _REPO / "src" / "teleon" / "runtime" / "capability_binding.py"
...
def _self_test() -> int:
    ...
    r1 = bind_capability_task(util_task, target=LOCAL_FUNCTION_TARGET, now=_NOW)
    r2 = bind_capability_task(util_task, target=LOCAL_FUNCTION_TARGET, now=_NOW)
    ...
    r_other = bind_capability_task(util_task, target=LOCAL_FUNCTION_TARGET, now="2026-06-09T00:00:00Z")
    ...
    rk = bind_capability_task(util_task, target=LOCAL_FUNCTION_TARGET, now=_NOW,
    ...
    generic = generic_functions()
    open_task = {"capability_id": "agent.research", "worker_bucket": "open_ended_agent",
                 "estimated_runtime_ms": 9000}
    ro = bind_capability_task(open_task, target=LOCAL_FUNCTION_TARGET, now=_NOW,
    ...
    ro2 = bind_capability_task(open_task, target=LOCAL_FUNCTION_TARGET, now=_NOW,
    ...
    for tgt in _CANDIDATES:
        try:
            cr = bind_capability_task(open_task, target=tgt, now=_NOW)
    ...
    try:
        ur = bind_capability_task(util_task, target="made_up_target@v9", now=_NOW)
    ...
    truth_leak = []
    for tgt in TARGETS:
        out = bind_capability_task(util_task, target=tgt, now=_NOW)
