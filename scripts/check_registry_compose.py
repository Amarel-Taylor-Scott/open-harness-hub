"""check_registry_compose — proof for the playbook->DAG compiler (src/teleon/registry/compose.py).
...
from src.teleon.registry.compose import compile_playbook, compile_universal  # noqa: E402
...
def main() -> int:
    ...
    dag = compile_playbook("dental_clinic")
    ...
    ck("unknown playbook -> available False", compile_universal and compile_playbook("nope").get("available") is False)

    # universal/mixed tool: a 'weather' intent picks the weather portal
    uni = compile_universal("weather")
