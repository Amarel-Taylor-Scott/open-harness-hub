...
This drives the Stage-2 engine (src/baltor/experiments/) on the canonical CFPB reconciliation invariant —
the SAME one scripts/check_contextops_cfpb_reference.py + scripts/demo_offline_full_baltor.py ride on:
...
The baseline path runs the REAL reconciliation authority (scripts/artifact_graph/reconciliation.py) on the
...
CLI: PYTHONPATH=. python3 scripts/check_parallel_path_promotion_gate.py --self-test
"""
