...
--self-test : validate discipline (cloudflared presence, URL regex, no-fake) WITHOUT launching (flywheel-safe).
  ...
  PYTHONPATH=. python3 scripts/share_intro_site.py start
"""
...
def start() -> int:
    ...
    subprocess.run([sys.executable, "scripts/build_intro_site.py"], cwd=str(REPO),
