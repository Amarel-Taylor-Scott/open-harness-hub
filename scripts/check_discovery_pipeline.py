...
everything a license-classified, deduped CANDIDATE (discovery≠trust, promotion boundary). serves_truth=false.
  ...
  python3 scripts/check_discovery_pipeline.py --self-test
"""
...
def _self_test() -> int:
    spec = json.loads((REPO / "architecture" / "discovery_pipeline.json").read_text(encoding="utf-8"))
    cred = {s["id"] for s in json.loads((REPO / "architecture" / "credential_registry.json").read_text())["services"]}
    fails = []
    def ck(n, ok, d=""):
    ...
    st = HOLDER.status({"OH_GITHUB_TOKEN": secret}, {"github"})
    ...
    ck("rapidapi byo-gated: not held without a key", "rapidapi" not in HOLDER.held({}))
    ck("holder names the missing env var honestly", HOLDER.missing("rapidapi", {}) == ["RAPIDAPI_KEY"])
    ck("auth_headers empty without a key; shaped with one",
       HOLDER.auth_headers("rapidapi", {}) == {} and HOLDER.auth_headers("rapidapi", {"RAPIDAPI_KEY": "k"}) == {"X-RapidAPI-Key": "k"})
    ck("tenant byo key is honored (github with a token -> held byo)", "github" in HOLDER.held({"OH_GITHUB_TOKEN": "t"}, {"github"}))
