...
gateway for a bounded agent consumer and asserts, deterministically and offline:
  ...
  E. TELEON ↛ BALTOR (dependency law): ``src/teleon/agent_gateway/capabilities.py`` does NOT import
...
from src.teleon.agent_gateway.gateway import AgentCapabilityGateway, AgentConsumer

_NOW = "2026-06-08T00:00:00Z"  # injected — determinism
_CAP_ID = "context.governed_answer.evidence"
_CAPS_FILE = _REPO / "src" / "teleon" / "agent_gateway" / "capabilities.py"
...
def _self_test() -> int:
    ...
    check("D: governance marker equals the module's single-sourced GOVERNANCE_MARKER",
          gov == caps.GOVERNANCE_MARKER, f"{gov} vs {caps.GOVERNANCE_MARKER}")
