#!/usr/bin/env python3
"""scripts.check_identity_local_service_runtime — PROOF for scripts/identity_local_service.py.

The local Identity & Access service must run the owner-locked auth model END-TO-END over HTTP:
  A. REGISTRY-DRIVEN — served realms == architecture/identity_realm_registry.json; parent + Baltor +
...
def _self_test() -> int:
    ...
    try:
        ...
        products_js = (REPO_ROOT / registry["live_hub_realm_source"]).read_text(encoding="utf-8")
