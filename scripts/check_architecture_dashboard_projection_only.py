#!/usr/bin/env python3
"""scripts.check_architecture_dashboard_projection_only — proof: web/baltor pages are projections only —
they never write durable truth and never embed private memory/intel. Dashboard-truth drift fails.

CLI: python3 scripts/check_architecture_dashboard_projection_only.py --self-test
"""
...
def _self_test() -> int:
    ...
    check("web/baltor has pages to check", len(pages) >= 4, str(len(pages)))
