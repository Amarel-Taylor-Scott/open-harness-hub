...
Asserts:
  ...
  I. DEPENDENCY LAW: no module under src/teleon/agent_gateway imports ``src.baltor`` (line-anchored).
...
_PKG = _REPO / "src" / "teleon" / "agent_gateway"

#: the P0 card contract fields every card must expose.
_CARD_FIELDS = set(AgentCapabilityCard.__dataclass_fields__)
...
def _self_test() -> int:
    ...
    if rec:
        check("C: receipt runtime_path == 'deterministic'", rec["runtime_path"] == RUNTIME_PATH_DETERMINISTIC,
              rec["runtime_path"])
    ...
    # D. determinism ---------------------------------------------------------------------------------------
    hasher = AgentConsumer(consumer_id="agent.hasher", allowed_capabilities=frozenset({"utility.hash"}))
    ...
    denied_consumer = AgentConsumer(consumer_id="agent.denied",
    ...
    caps.CAPABILITIES[llm_only_id] = {
        "card": {**caps.get_card("utility.hash"), "capability_id": llm_only_id, "llm_fallback_allowed": True,
                 "deterministic_first": True},
        "handler": None,
    }
    try:
        gw2 = AgentCapabilityGateway()
        llm_consumer = AgentConsumer(consumer_id="agent.llm", allowed_capabilities=frozenset({llm_only_id}),
        ...
        no_perm = AgentConsumer(consumer_id="agent.noperm", allowed_capabilities=frozenset({llm_only_id}),
        ...
        del caps.CAPABILITIES[llm_only_id]  # leave the registry as we found it
    ...
    check("I: no src/teleon/agent_gateway module imports src.baltor (dependency law)",
