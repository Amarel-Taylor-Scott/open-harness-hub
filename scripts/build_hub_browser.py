...
candidate, from architecture/candidate_open_hubs.json) and (2) the REGISTRIES each hub owns, with entry counts COMPUTED
...
legible. Candidates are private_first (discovery != trust); serves_truth=false.
  ...
  python3 scripts/build_hub_browser.py            # -> dist/hubs/index.html
  python3 scripts/build_hub_browser.py --self-test
"""
...
REGISTRY_MAP = [
    ...
    ("architecture/capability_ladders.json", "ladders", "Capability ladders", "OpenHarnessHub"),
    ...
    ("architecture/plane_io_contracts.json", "planes", "Plane I/O contracts", "OpenHarnessHub"),
...
def _hubs() -> list:
    d = json.loads((REPO / "architecture" / "candidate_open_hubs.json").read_text())
...
def _self_test() -> int:
    ...
    html = build()
    ck("page lists the live + new candidate hubs (OpenHarnessHub, OpenLinkingHub, OpenSourcesHub)",
       all(h in html for h in ("OpenHarnessHub", "OpenLinkingHub", "OpenSourcesHub")))
