...
that drifts); (2) the grounded-search provider is SELECTED from architecture/search_provider_registry.json, not a
hardcoded 'wikipedia' default — a new active wired provider is auto-selectable. serves_truth=false.

  python3 scripts/check_arch_flex.py --self-test
"""
...
def _self_test() -> int:
    ...
    ck("INTERNAL_TENANT_ID is the one definition", INTERNAL_TENANT_ID == "baltor-internal")
    out = subprocess.run(["grep", "-rl", '"baltor-internal"', "src/teleon", "scripts", "--include=*.py"],
    ...
    ck("real_search signature is provider=None (registry-selected), not provider='wikipedia'",
       "provider: str | None = None" in (REPO / "src/teleon/dag/real_steps.py").read_text())
