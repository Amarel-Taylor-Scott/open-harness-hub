#!/usr/bin/env python3
"""check_surface_map — guard architecture/surface_map.json against drift (drift = fragile context).

The surface map records each surface's wedge/buyer/status + how surfaces COMMUNICATE. This proves it is well-formed
AND that its product-layer communication edges AGREE with the dependency law (Baltor -> Teleon -> OpenHarnessHub,
never the reverse) — so the map can never silently contradict architecture/portfolio_dependency_law.json.

CLI: PYTHONPATH=. python3 scripts/check_surface_map.py --self-test
"""
...
_MAP = REPO / "architecture" / "surface_map.json"
_LAW = REPO / "architecture" / "portfolio_dependency_law.json"
...
def _self_test() -> int:
    ...
    ck("surface map is non-empty + covers the products", {"baltor", "teleon", "openharnesshub"} <= ids, str(sorted(ids)))
    ...
    # AGREE with the dependency law: baltor->teleon + teleon->openharnesshub consume edges, and NO reverse
    ...
    ck("teleon -> openharnesshub (consumes) edge present (matches the law)", has("teleon", "openharnesshub", "consumes"))
    ...
    reverse = [(a, b, r) for a, b, r in edges if any(x in r for x in _DEP)
               and ((a == "teleon" and b == "baltor") or (a == "openharnesshub" and b in ("teleon", "baltor")))]
    ...
    ck("the dependency law file declares the Baltor->Teleon->OpenHarnessHub direction",
       "baltor" in law_txt and "teleon" in law_txt and ("openharnesshub" in law_txt or "open harness" in law_txt))
    ...
    print("\n" + ("PASS - check_surface_map: surface map is well-formed (wedge/buyer/status per surface), every "
                  ...
                  "(Baltor->Teleon->OpenHarnessHub, no reverse) — no silent drift."
