#!/usr/bin/env python3
"""scripts.check_pipeline_decomposition_ui — proof (C-PIPE-DECOMP-1): web/baltor/pipeline/decomposition.html
...
deterministic, offline. Models on scripts/check_consumption_ui.py + scripts/check_memory_page_projection_only.py.

CLI: python3 scripts/check_pipeline_decomposition_ui.py --self-test
"""
...
_PAGE = _REPO / "web" / "baltor" / "pipeline" / "decomposition.html"
...
def _self_test() -> int:
    ...
    check("web/baltor/pipeline/decomposition.html exists", _PAGE.exists())
    if not _PAGE.exists():
