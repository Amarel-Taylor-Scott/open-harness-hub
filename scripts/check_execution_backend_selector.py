...
CLI: PYTHONPATH=. python3 scripts/check_execution_backend_selector.py --self-test
"""
...
def _self_test() -> int:
    ...
    dk = select_backend(util, policy_override={"preferred_backends": ["k8s_deployment_worker@candidate"]},
    ...
    dc = select_backend(util, policy_override={"preferred_backends": ["gcp_cloud_run_function@candidate"]},
    ...
    d1 = select_backend(util, policy_override=elig, available_creds=creds, pricebook=pb_k8s_cheap)
    d2 = select_backend(util, policy_override=elig, available_creds=creds, pricebook=pb_fn_cheap)
    ...
    db = select_backend(br, available_creds=GENERIC_FUNCTIONS | {"browser_pool@candidate"})
    chk("browser task NOT routed to a generic cloud function", db["backend"] not in GENERIC_FUNCTIONS, db["backend"])
    ...
    dg = select_backend(gpu, available_creds=GENERIC_FUNCTIONS | {"gpu_pool@candidate"})
    chk("GPU task NOT routed to a generic cloud function", dg["backend"] not in GENERIC_FUNCTIONS, dg["backend"])

    # MISSING CREDENTIALS → fall back to local correctness invariant
    dm = select_backend(util, policy_override={"preferred_backends": ["gcp_cloud_run_function@candidate"]}, available_creds=set())
    ...
    dh = select_backend(util, policy_override={"preferred_backends": ["k8s_deployment_worker@candidate"]},
                        available_creds={"k8s_deployment_worker@candidate"},
    ...
    dh2 = select_backend(util, policy_override={"eligible": ["k8s_deployment_worker@candidate"]},
                         available_creds={"k8s_deployment_worker@candidate"},
                         provider_health={"k8s_deployment_worker@candidate": False})
    ...
    # determinism
    chk("deterministic", select_backend(util) == select_backend(util))
