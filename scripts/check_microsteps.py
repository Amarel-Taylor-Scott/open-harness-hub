...
prompt names the 7 primitives (so more rungs can be decomposed); coverage is computed. serves_truth=false.
  ...
  python3 scripts/check_microsteps.py --self-test
"""
...
def _self_test() -> int:
    reg = json.loads((REPO / "architecture" / "rung_microsteps.json").read_text(encoding="utf-8"))
    decomps = reg["decompositions"]
    planes = {p["plane"] for p in json.loads((REPO / "architecture" / "tool_planes.json").read_text())["planes"]}
    ladder_rungs = {(l["capability"], r["tier"]) for l in json.loads((REPO / "architecture" / "capability_ladders.json").read_text())["ladders"] for r in l["rungs"]}
    fails = []
    def ck(n, ok, d=""):
    ...
    rung_dag, micro_dag = I.build_dag(intent), M.expand_to_micro_dag(intent)
    ...
    mdp = M.micro_decision_points(intent)
    ...
    ck("a known rung decomposes (document_extraction/text_layer -> open/extract/segment)",
       {m["id"] for m in M.microsteps_for("document_extraction", "text_layer")} >= {"open_document", "extract_text_layer", "segment_pages"})

    prompt = M.generate_microstep_prompt("ocr", "enhanced_ocr", "preprocess + stronger OCR")
