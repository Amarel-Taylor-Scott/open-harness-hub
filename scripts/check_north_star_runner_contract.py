...
CLI: PYTHONPATH=. python3 scripts/check_north_star_runner_contract.py --self-test
"""
...
_PROMPT = _REPO / "prompts" / "baltor-north-star-continuous-builder.md"
...
def _self_test() -> int:
    ...
    for d in ("current-state.md", "opportunities.md", "risks-and-gaps.md"):
        chk(f"docs/status/{d} present (startup reads it)", (_REPO / "docs" / "status" / d).exists())
