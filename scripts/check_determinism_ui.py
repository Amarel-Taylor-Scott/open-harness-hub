#!/usr/bin/env python3
"""scripts.check_determinism_ui — proof: web/baltor/determinism.html is a PROJECTION ONLY over the Determinism
...
CLI: PYTHONPATH=. python3 scripts/check_determinism_ui.py --self-test
"""
...
_PAGE = _REPO / "web" / "baltor" / "determinism.html"
...
def _self_test() -> int:
    ...
    check("web/baltor/determinism.html exists", _PAGE.exists())
    if not _PAGE.exists():
