...
CLI: PYTHONPATH=. python3 scripts/check_execution_backend_redteam.py --self-test
"""
...
from src.teleon.runtime.execution_backend_selector import GENERIC_FUNCTIONS, select_backend  # canonical Teleon home (Baltor re-exports)
...
def _self_test() -> int:
    ...
    d = select_backend(br, policy_override={"preferred_backends": list(GENERIC_FUNCTIONS)}, available_creds=GENERIC_FUNCTIONS)
    chk("1 browser task not on a generic function (guard beats preference)", d["backend"] not in GENERIC_FUNCTIONS, d["backend"])
    ...
    dg = select_backend(gpu, policy_override={"preferred_backends": list(GENERIC_FUNCTIONS)}, available_creds=GENERIC_FUNCTIONS)
    chk("2 GPU task not on a generic function (guard)", dg["backend"] not in GENERIC_FUNCTIONS)

    # 3 — missing credentials never crash; a decision is still returned
    dm = select_backend({"capability_id": "u", "worker_bucket": "utility"},
                        policy_override={"preferred_backends": ["gcp_cloud_run_function@candidate"]}, available_creds=set())
    chk("3 missing creds → safe decision (no crash), not the function", "backend" in dm and dm["backend"] not in GENERIC_FUNCTIONS)

    # 4 — the selector hardcodes NO provider prices (reads the pricebook); no literal cost numbers in logic
    sel_src = (_REPO / "src/teleon/runtime/execution_backend_selector.py").read_text()  # selector now lives in Teleon
    ...
    adp_src = (_REPO / "src/teleon/runtime/execution_providers/gcp_cloud_run_function_candidate.py").read_text()  # canonical Teleon home (Baltor re-exports)
    ...
    chk("5 unconfigured candidate → ProviderUnavailableResult (no crash, no truth)", isinstance(r, ProviderUnavailableResult))
    ...
    a = select_backend({"capability_id": "u", "worker_bucket": "utility"})
    b = select_backend({"capability_id": "u", "worker_bucket": "utility"}, policy_override={"preferred_backends": ["k8s_deployment_worker@candidate"]}, available_creds={"k8s_deployment_worker@candidate"})
    ...
    chk("7 candidate adapter owns_truth=False", GcpCloudRunFunctionCandidate().describe()["owns_truth"] is False)
