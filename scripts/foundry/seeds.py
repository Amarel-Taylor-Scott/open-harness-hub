...
Cycle fuel for the build loop (`docs/codex/foundry-build-loop.md`):
...
AREAS_PATH = REPO / "data" / "research-queue" / "areas.jsonl"
LEDGER_PATH = REPO / "data" / "foundry-ledger.jsonl"
    ...
    def find(self, gap: dict) -> dict | None:
        ...
        for entry in self.registry:
            ...
            if not is_permissive(entry.get("license", "")):
                continue
            path = (REPO / entry["path"]) if not Path(entry["path"]).is_absolute() else Path(entry["path"])
            if not path.exists():
                continue
            facts = []
