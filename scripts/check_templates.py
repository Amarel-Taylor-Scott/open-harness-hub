...
serves_truth=false.
  ...
  python3 scripts/check_templates.py --self-test
"""
...
def _self_test() -> int:
    ...
    ck("retrieve: an extraction intent matches the document_extraction template",
       (T.retrieve_template("extract fields from these invoice pdfs into our schema") or {}).get("template_id") == "document_extraction")
    ck("retrieve: a Q&A intent matches rag_qa",
       (T.retrieve_template("answer questions grounded in these documents") or {}).get("template_id") == "rag_qa")
    ck("retrieve: an unrelated intent matches nothing (honest None)", T.retrieve_template("xyzzy nonsense plugh") is None)

    doc = T.retrieve_template("extract invoice fields")
    # mutate: fill the required slots, leave the optional ocr_fallback unfilled -> dropped + edges rerouted
    m = T.mutate(doc, {"parse": "pdfplumber", "fields": "llm", "validate": "jsonschema"})
    ...
    v = verify_buildable_dag(m["nodes"], m["edges"])
    ...
    m2 = T.mutate(doc, {"parse": "pdfplumber"})
    ...
    mined = T.mine_template([{"step": "a", "plane": "ocr"}, {"step": "b", "plane": "llm"}], [["a", "b"]],
