...
count manifest + a bounded sample. 1M is the floor, not a fabricated claim of 1M verified rows.
  ...
  python3 scripts/build_million_records.py            # write the manifest + sample
  python3 scripts/build_million_records.py --self-test
"""
...
from src.teleon.registry.variations import AXES, mutate_stream, mutation_space, reachable_count  # noqa: E402
...
def _build(write: bool) -> dict:
    seeds = _seed_records()
    per_record = mutation_space(AXES_USED)
    reachable = reachable_count(len(seeds), AXES_USED)
    sample = list(islice(mutate_stream(seeds[:3], AXES_USED), _SAMPLE))
    manifest = {
        "version": "0.1.0",
        ...
        "serves_truth": False,
        "axes_used": AXES_USED,
        "axis_sizes": {a: len(AXES.get(a, [])) for a in AXES_USED},
...
def main() -> int:
    ...
    sample = list(islice(mutate_stream(seeds, AXES_USED), 100))
