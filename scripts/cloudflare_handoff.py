...
_CT_URL = P.REPO / "dist" / "demo-control-tower-url.txt"
_DIST = P.REPO / "dist"
...
def build_inventory(now: str) -> dict:
    ...
    for s in reg:
        ...
        if pub:  # reviewable surfaces (have a public URL) get a review order
            order += 1
...
def _checklist_section(s: dict) -> list[str]:
    ...
    if sid == "site.openharnesshub":
        lines += ["### OpenHarnessHub specifics", "- [ ] Harnesses/evals separated from skills/tools/context.", "- [ ] '.io replaces .org' noted.", ""]
