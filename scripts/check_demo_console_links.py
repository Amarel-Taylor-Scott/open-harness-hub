...
CLI:
    python3 scripts/check_demo_console_links.py --self-test
    python3 scripts/check_demo_console_links.py            # report; exit 1 on any dangling link
"""
...
def check() -> list[str]:
    ...
    for rel in REQUIRED_FILES:
        if not (_WEB / rel).exists():
            problems.append(f"missing file: web/baltor/{rel}")
    for src, needle in REFERENCES:
        p = _WEB / src
        if not p.exists():
            problems.append(f"missing referrer: web/baltor/{src}")
            continue
        if needle not in p.read_text(encoding="utf-8"):
            problems.append(f"web/baltor/{src} does not reference {needle!r} (dangling/unwired)")
    return problems
...
def _self_test() -> int:
    ...
    chk("operator guide exists", (_REPO / "docs/deployment/demo-console.md").exists())
    chk("repo-root quickstart exists", (_REPO / "docs/baltor-full-app-quickstart.md").exists())
    chk("operator guide links the quickstart (not orphaned)",
        "baltor-full-app-quickstart.md" in (_REPO / "docs/deployment/demo-console.md").read_text(encoding="utf-8"))
