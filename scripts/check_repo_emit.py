...
CLI: PYTHONPATH=. python3 scripts/check_repo_emit.py --self-test
"""
...
def _main(argv=None):
    ...
    if "--emit" in argv:
        i = argv.index("--emit")
        dest = Path(argv[i + 1]) if i + 1 < len(argv) and not argv[i + 1].startswith("-") else REPO / "dist" / "teleon-capability-repo"
        return _emit(dest)
