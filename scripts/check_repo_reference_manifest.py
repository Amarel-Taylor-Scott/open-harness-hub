...
architecture/repo_reference_manifest.json records exactly what to re-clone (slug + commit SHA) and how we govern
...
CLI: PYTHONPATH=. python3 scripts/check_repo_reference_manifest.py --self-test
"""
...
_MANIFEST = _REPO / "architecture" / "repo_reference_manifest.json"
_SIGNAL_FEED = _REPO / "data" / "capability-candidates" / "discovered-feed-github-signal-2026-06-20.json"
...
def _self_test() -> int:
    ...
    template_artifacts = {"entity-intelligence-ingestion": _REPO / "scripts" / "ingest_entity_intelligence_catalog.py",
                          "profession-capability-seeder": _REPO / "src" / "teleon" / "seeds" / "profession_capability_seeder.py"}
    bad_seed = []
    for r in repos:
    ...
    ck("the README is the only tracked file inside repo_reference/", (_REPO / "repo_reference" / "README.md").exists())
