...
PROOF_MODULES: list[tuple[str, str]] = [
    ...
    # ── Command/work I/O: the durable FleetLedger's records conform to the typed work-I/O contracts (WorkItem=CapabilityTask, idempotent enqueue, atomic WorkerClaim, AckNackReceipt, DeadLetterEntry); projectors in src/teleon/io are pure ──
    ("scripts/check_shared_command_work_io.py", "check_shared_command_work_io"),
    # ── Event I/O: internal EventBus events project to CloudEvents 1.0 (EventEnvelope / CloudEventProjection) — type in EVENT_KINDS, correlation_id required, deterministic (source,type,seq) id, secrets redacted; pure projector in src/teleon/io ──
    ...
    # ── Shared LLM Plane SERVED end-to-end: scripts/api_inference_handler answers every /api/inference route (errors=ErrorEnvelope); structured-local output is a CANDIDATE never truth (ModelInvocationReceipt); admin server delegates GET+POST; read-only UI (web/baltor/inference-plane.html) consumes it; no secret-value leak in any response ──
    ...
    # ── Harness-hub auth WIRED to the local identity service (web/openhubforai/identity.js + pages/auth.js): real register→onboard→login→session + /account/keys console against /api/identity/openharnesshub/* — registry drift-gated default port, OHH_IDENTITY_BASE deploy override, X-AIDR-Request-Id, only the opaque session handle in localStorage (passphrase memory-only, raw key shown once never persisted), SSO/Google rendered as DISABLED owner-gated seams, honest service-down message, no hardcoded tunnel URLs; node --check syntax-gates both files when node exists. ──
    ...
    # ── Shared empty/loading/error UX kit (UX-BACKLOG P1 #3): web/openhubforai/oh-states.js — three token-only primitives with correct ARIA roles (status/aria-busy/alert), escaped interpolation, loaded in the shell; node-syntax-gated. ──
    ...
    # ── FULL-DESIGN bundle WIRED to real backends (owner: "bring the full design into the working apps"): dist/sites/aidoneright-design/shared/oh-identity.js — realm-aware identity client (drift-gated port, OHH_IDENTITY_BASE override, per-realm sessions, passphrase never stored) loaded across the surface HTMLs; the kit's OhAuth (oh-site.jsx) does REAL register/login against the identity service with honest preview fallback (never fakes a session) + Google/GitHub as disabled owner-gated seams; a page_view beacon fires to the events plane. The hundreds-of-hours design is now functional, not mock. ──
