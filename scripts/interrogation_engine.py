...
Reads architecture/interrogation_taxonomy.json (object_types x ~30 dimensions x spintax templates x substitution
...
(serves_truth=false) that feed the multi-model panel + the kickstart/rehydration injector.
  ...
  --run [--limit N] [--target PATH] [--dimensions a,b] [--cap N]
                              generate against real objects (default: scan src/teleon) -> interrogation_candidates.jsonl
CLI: PYTHONPATH=. python3 scripts/interrogation_engine.py --run --limit 5
"""
...
TAXO = REPO / "architecture" / "interrogation_taxonomy.json"
OUT = REPO / "data" / "dev-intel" / "interrogation_candidates.jsonl"
...
def code_targets(limit: int, target: str | None) -> list[dict]:
    ...
    if target:
        ...
        files = sorted((REPO / "src" / "teleon").rglob("*.py"))[:limit]
    for f in files:
