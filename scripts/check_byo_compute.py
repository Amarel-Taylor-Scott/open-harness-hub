...
CLI: PYTHONPATH=. python3 scripts/check_byo_compute.py --self-test
"""
...
from src.teleon.runtime.execution_providers.cloudflare_workers import CloudflareWorkersProvider, OFFLINE_EQUIVALENT

_MATRIX = Path(__file__).resolve().parents[1] / "architecture" / "execution_backend_policy_matrix.json"
...
def _self_test() -> int:
    ...
    prov_none = CloudflareWorkersProvider(token_secret_ref="secret://tenant/acme/cloudflare_api_token",
                                          resolve_secret=lambda r: None)
    ck("CloudflareWorkersProvider implements ExecutionProviderPort", isinstance(prov_none, ExecutionProviderPort))
    ...
    ck("invoke without a token returns ProviderUnavailableResult naming the SecretRef (graceful fallback)",
       isinstance(r, ProviderUnavailableResult) and "token" in r.reason.lower() and not r.consumable)
    ck("an offline equivalent is declared so dev needs no customer key", prov_none.describe()["offline_equivalent"] == OFFLINE_EQUIVALENT)

    # WITH a customer token -> eligible; the key is resolved via the injected resolver (never embedded/logged)
    prov_key = CloudflareWorkersProvider(account_id="acct123", token_secret_ref="secret://tenant/acme/cloudflare_api_token",
    ...
    ck("token-present-but-offline is honest — a non-consumable unavailable result, never a fake remote run",
       isinstance(r2, ProviderUnavailableResult) and not r2.consumable)
