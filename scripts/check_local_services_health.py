...
Asserts:
  ...
  B. PORT DRIFT GATES — portfolio site ports mirror scripts/portfolio_lib.py (PORTS + HUB_PORT)
     and the identity service port mirrors architecture/identity_realm_registry.json defaults.port;
...
def _self_test() -> int:
    ...
    site_map = {"parent_site": "aidoneright", "teleon_site": "teleon.dev", "baltor_site": "baltor",
                "openharnesshub_site": "openharnesshub", "opencontexthub_site": "opencontexthub",
    ...
    identity_registry = json.loads((REPO_ROOT / "architecture" / "identity_realm_registry.json")
    ...
    ck("D: no invented tunnel URLs (cells only from the recorded manifest)",
       "trycloudflare.com" not in text or (REPO_ROOT / "dist" / "portfolio-share-urls.json").exists())
