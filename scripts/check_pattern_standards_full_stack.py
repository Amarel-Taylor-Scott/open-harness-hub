...
over-claim:
  ...
  docs/examples/standardized-patterns.md -> the canonical example per shape
...
CLI: python3 scripts/check_pattern_standards_full_stack.py --self-test
"""
...
_STANDARDS = _REPO / "architecture" / "standard_catalog.json"
_TEMPLATES = _REPO / "architecture" / "template_catalog.json"
_ROUTINES = _REPO / "architecture" / "routine_library.json"
...
_EXAMPLES_DOC = _REPO / "docs" / "examples" / "standardized-patterns.md"
...
def _exists(rel: str) -> bool:
    ...
    return (_REPO / rel.split("#")[0]).exists()
...
def analyze() -> dict:
    ...
    for p in reg.get("patterns", []):
        ...
        card_doc = any((_REPO / e).exists() and e.endswith(".md")
        ...
        examples_real = any((_REPO / e).exists() for e in p.get("detected_examples", []))
