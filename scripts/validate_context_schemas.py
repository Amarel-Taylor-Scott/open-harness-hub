...
context schemas + fixtures land. Uses the same Draft202012Validator scripts/validate.py uses; the
self-test includes negative cases so enforcement is proven, not assumed.

CLI:
    python3 scripts/validate_context_schemas.py --self-test
    python3 scripts/validate_context_schemas.py            # validate + print results
"""
...
def _self_test() -> int:
    ...
    bad = json.loads((_REPO / "fixtures/parser-run.example.json").read_text(encoding="utf-8"))
    ...
    bad2 = json.loads((_REPO / "fixtures/parser-run.example.json").read_text(encoding="utf-8"))
