...
For every loop in architecture/agentic_loop_catalog.json: render the perceive→reason→act→verify→iterate steps, then an
...
All figures COMPUTED from the catalog (no magic). Writes dist/intro/loops/index.html (served by the live tunnel).
  ...
  PYTHONPATH=. python3 scripts/build_agentic_loop_viz.py [--self-test]
"""
...
OUT = REPO / "dist" / "intro" / "loops" / "index.html"
