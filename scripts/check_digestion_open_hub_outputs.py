...
Asserts:
  ...
  H. DEPENDENCY LAW: src/teleon/digestion never imports src.baltor.
...
def _self_test() -> int:
    ...
    check("F: composes Template Registry shell + digester", (_REPO / "src" / "teleon" / "templates" / "instantiator.py").exists() and (_REPO / "src" / "teleon" / "digestion" / "digester.py").exists())
    ...
    check("H: no src/teleon/digestion module imports src.baltor", not bad_imp, "; ".join(bad_imp))
