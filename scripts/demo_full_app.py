...
CLI:
    python3 scripts/demo_full_app.py --self-test      # offline, deterministic end-to-end assertions
    python3 scripts/demo_full_app.py                  # run the demo, print the headline + each step
    python3 scripts/demo_full_app.py --live           # allow a model route to phrase the answer
"""
...
CORPORA: dict[str, dict[str, Any]] = {
    "acme": {
        "seed": _REPO / "demo-data" / "acme-billing" / "seed-graph.json",
    ...
    "cfpb": {
        "seed": _REPO / "demo-data" / "cfpb-sample" / "seed-graph.json",
...
def _self_test() -> int:
    ...
    missing = [l["file_path"] for l in out["source_locators"] if not (_REPO / l["file_path"]).exists()]
    ...
    cf_missing = [l["file_path"] for l in cf["source_locators"] if not (_REPO / l["file_path"]).exists()]
    ...
    try:
        ...
        for fx, schema in (("lineage_manifest", "schemas/context/lineage-manifest.schema.json"),
                           ("receipt", None)):
            if schema:
                v = Draft202012Validator(json.loads((_REPO / schema).read_text()))
                errs = list(v.iter_errors(out[fx]))
                check(f"{fx} validates against its schema", not errs, "; ".join(e.message for e in errs[:2]))
        sl = Draft202012Validator(json.loads((_REPO / "schemas/context/source-locator.schema.json").read_text()))
