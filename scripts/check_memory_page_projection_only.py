#!/usr/bin/env python3
"""scripts.check_memory_page_projection_only — proof (C-MEM-1): web/baltor/memory.html is a PROJECTION ONLY.
...
CLI: python3 scripts/check_memory_page_projection_only.py --self-test
"""
...
_PAGE = _REPO / "web" / "baltor" / "memory.html"
...
def _self_test() -> int:
    ...
    check("web/baltor/memory.html exists", _PAGE.exists())
    if not _PAGE.exists():
