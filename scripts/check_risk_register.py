#!/usr/bin/env python3
"""scripts.check_risk_register — proof: architecture/risk_register.json covers the required risk categories
...
CLI: python3 scripts/check_risk_register.py --self-test
"""
...
def _self_test() -> int:
    ...
    reg = json.loads((_REPO / "architecture" / "risk_register.json").read_text())
    risks = reg["risks"]
    ...
    for r in risks:
        ...
        if not (dp.startswith("planned:") or (_REPO / dp).is_file()):
