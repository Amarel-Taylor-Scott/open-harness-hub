...
_RUBRICS = P.REPO / "rubrics" / "portfolio"
_STATE = P.REPO / ".agent" / "portfolio-sites"
_REVIEW = P.REPO / "review-pack" / "portfolio"
_DOCS = P.REPO / "docs" / "portfolio"
_built = False
...
def discovery() -> int:
    ...
    return _emit("portfolio discovery", [
        ...
        ("core seed brands searched", set(d.get("brand_names_searched", [])) >= {"ContextIsEverything", "Teleon", "Baltor", "OpenHarnessHub"}),
...
def reuse_no_reinvention() -> int:
    ...
    existing_serve = [P.REPO / "scripts" / "serve_all_sites.sh", P.REPO / "scripts" / "serve_showcase.sh"]
    return _emit("portfolio reuse / no-reinvention", [
        ...
        ("single render source reused (portfolio_lib) — not a second generator", (P.REPO / "scripts" / "portfolio_lib.py").exists()),
        ("architecture boundary configs intact", (P.REPO / "architecture" / "portfolio_dependency_law.json").exists()
         and (P.REPO / "architecture" / "company_portfolio_map.json").exists()),
    ])
...
def brand_boundaries() -> int:
    ...
    th = _html("teleon.dev"); bh = _html("baltor"); oh = _html("openharnesshub"); ah = _html("aidoneright")
    ...
    checks.append(("openharnesshub explicitly NOT a hosted runtime", "Not a hosted runtime" in oh, ""))
...
def customer_readiness_rubric() -> int:
    ...
    checks.append(("openharnesshub: discovery-is-not-trust", "Discovery is not trust." in _html("openharnesshub"), ""))
...
def technical_launch_rubric() -> int:
    ...
    return _emit("portfolio technical launch rubric", [
        ("static build manifest present", (P.REPO / "dist" / "portfolio-sites-build.json").exists(), ""),
        ...
        ("full-stack proof script exists", (P.REPO / "scripts" / "check_portfolio_launch_full_stack.py").exists(), ""),
    ])
...
def trycloudflare_rubric() -> int:
    ...
    manifests = [P.REPO / "dist" / f"portfolio-share-urls.{e}" for e in ("json", "md", "txt")]
