...
the shared LLM plane, BY REFERENCE (model_preference_ref -> an InferencePreference.v1), never inline; and a declared
...
Asserts:
  A. SHELL FIELD: ObjectShell.v1 recognizes an optional `model_preference_ref` (string|null), NOT required.
  ...
  E. CONTRACT: InferencePreference.v1 (the referenced object) is registered.
  F. DETERMINISM.
...
def _self_test() -> int:
    ...
    shell = json.loads((_REPO / "schemas" / "shared" / "ObjectShell.v1.schema.json").read_text())
    mp = shell["properties"].get("model_preference_ref", {})
    check("A: ObjectShell.v1 has optional model_preference_ref (string|null, not required)",
    ...
    check("C: a declared model_preference_ref resolves through the gateway",
          ...
          and resolved["schema_version"] == "ResolvedInferencePreference.v1", json.dumps(resolved.get("preference_id")))
    ...
    check("E: InferencePreference.v1 (the referenced contract) is registered", "inference/InferencePreference.v1.schema.json" in contracts)
    ...
    print("\n" + ("PASS — check_objects_have_inference_preferences: key object families declare a model preference by "
                  "reference (model_preference_ref -> InferencePreference.v1), never inline; the ref resolves through "
