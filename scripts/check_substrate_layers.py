#!/usr/bin/env python3
"""check_substrate_layers — proof for architecture/substrate_layers.json (the systems-layer reconciliation map).
...
status/priority vocab must stay closed. Deterministic, offline, stdlib only. serves_truth=false.
  ...
  PYTHONPATH=. python3 scripts/check_substrate_layers.py --self-test
"""
...
_MAP = _REPO / "architecture" / "substrate_layers.json"
...
def _self_test() -> int:
    ...
    for anchor in _DISCIPLINE_ANCHORS:
        ck(f"discipline anchor exists: {anchor}", (_REPO / anchor).exists())
    ...
    for l in layers:
        ...
        for a in l["existing_assets"]:
            if not (_REPO / a).exists():                       # file OR directory must really exist
