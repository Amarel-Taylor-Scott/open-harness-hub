/* global React, OHH_BRAND, OhLayout, OhTable, OhPageHead */
...
const REG_BASE = (window.OHH_REGISTRY_BASE || '/registry');
...
function RecordList({ id }) {
  ...
  React.useEffect(() => {
    let live = true;
    fetch(REG_BASE + '/api/openhub/registries/' + encodeURIComponent(id) + '/records')
...
function PRegistryBrowser() {
  ...
  React.useEffect(() => {
    ...
    fetch(REG_BASE + '/api/openhub/registries' + (query ? '?' + query : ''))
      .then((r) => (r.ok ? r.json() : Promise.reject(r.status)))
  ...
  return (
    <div className="oh-site ohb" style={{ '--accent': OPENHUB_ACCENT }}>
      <OhLayout variant="no-sidebar" brand={OHH_BRAND} nav={nav}
