---
name: cfr-amendment-tracking-drift
description: Detect Federal Register amendments to tracked CFR sections, snapshot
  and content-hash the authoritative text, and emit CDC events plus review tickets
  for every downstream component that cites a changed section.
...
4. **diff_against_known** — `loop` → `pattern/source-document-to-persistent-knowledge-layer` (when `$.steps.fetch_federal_register.output.count > 0`)
...
- deterministic `$.steps.verify_official_source.output.summary.all_primary` == `True`
- deterministic `$.change_events` != `None`

## Provenance

- Hub component: `pipeline/cfr-amendment-tracking-drift` v0.1.0
- License: `MIT`
